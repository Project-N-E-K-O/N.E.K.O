...
1. React 侧紧凑工具扇形菜单有导出按钮：
   - `frontend/react-neko-chat/src/App.tsx`
   - `.compact-input-tool-item-export`
   - 改造前点击调用 `onExportConversationClick`
2. 宿主层把导出回调接到：
   - `static/app-react-chat-window.js`
   - `handleExportConversationClick()`
3. 当前 `handleExportConversationClick()` 优先调用：
   - `window.appChatExport.open()`
4. 导出模块位置：
   - `static/app-chat-export.js`
5. 当前导出模块 `open()` 会走 `openExportPreviewWindow()`：
   - 打开 `neko-chat-export-preview` 新窗口。
   - 在新窗口中创建 preview modal。
6. 导出模块读取消息的来源已经是 React host：
...
4. 当前 `MessageBubble.tsx` 内部有消息 block 渲染能力，但不是独立公共导出：
   - Inline history 如果需要渲染 text / image / link / status / buttonGroup，不应复制一套 block 解析和渲染。
   - 合理做法是把 block 渲染抽成共享组件，或让 `MessageBubble` 支持 compact history variant。
   - 不要在 inline history 里用临时字符串拼接替代现有 block 语义。
5. 当前导出模块 `static/app-chat-export.js`：
   ...
   - Compact inline 只能复用导出能力，不能把 full preview DOM 和复选框搬进紧凑层。
6. 当前 compact geometry 采集在 `static/app-react-chat-window.js`：
   ...
   - 如果 history 需要“外层视觉区域”和“内部真实命中区域”不同，必须扩展 geometry 采集逻辑支持子 hit rect 或显式 payload，不能只靠 CSS `pointer-events`。
7. 当前 NEKO-PC 桌面壳：
...
3. 最大高度受控。
4. 不预留空透明区域。
5. 历史层宽高必须跟随紧凑聊天框本体尺寸变量等比例变化；后续紧凑聊天框支持拉长 / 拉宽时，history 不能使用固定 430px、510px 等硬编码尺寸锁死。
...
新增状态应只表达导出历史层、inline 导出预览和选中集合，不把 full 导出窗口状态塞进 compact UI。
...
1. React 侧：
   ...
   - `compactExportPointerIntent`
   - `compactExportFormat`
   - `compactExportImageStyle`
   - `compactExportImageFormat`
2. 宿主或导出模块侧只提供导出能力，不持久化紧凑态 UI 开关。
...
7. “导出对话”按钮在 compact 下是开关控件，不是一次性 action。打开 history 时必须和现有 GalGame / 翻译开关一致地提供：
   ...
   关闭、无消息、离开 compact、消息清空时必须恢复未选中态。
8. 切换到 full / minimized 时关闭历史导出层。
9. 消息清空后如果历史导出层已打开，应保持历史层打开并显示空状态，同时清空选择并关闭 inline 导出预览。
10. Compact 无消息时点击“历史对话”仍打开 inline history 空状态：不调用旧导出入口、不弹旧导出提示；Full 导出入口的空对话提示仍由原导出模块处理。
...
21. `compactExportPointerIntent` 只用于一次 pointer interaction 的临时意图判定，不持久化到业务状态。
22. Compact 专属派生状态不能泄漏到 full / minimized 表面。向 DOM、geometry、NEKO-PC 或测试暴露的 `data-compact-export-*` 在非 compact 下必须回到关闭 / 空 / false 语义。
23. 不要提前添加没有 UI 或交互消费点的空状态。`pointer intent` 和导出 options 应在 history UI、滚动、选择或 preview 真实接入时落地，避免“状态存在但无人消费”的伪完成。

## 数据与导出能力复用

不应在 React 里重新写一套导出文本提取和文件生成逻辑。

应从 `static/app-chat-export.js` 拆出无窗口能力，供 compact inline 面板调用。

建议能力边界：

1. `getReactMessages()`
   - 继续从 `reactChatWindowHost.getState().messages` 读取。
2. `buildExportEntriesFromMessages(messages)`
   - 复用现有导出 entry 结构。
3. `getExportFormats()`
   - 返回 Markdown / Image。
4. `getImageExportStyles()`
   - 返回现有图片样式。
5. `getImageExportFormats()`
   - 返回 PNG / JPEG / WebP。
6. `buildExportDocument(entries, formatId)`
   - 继续负责 Markdown / Image 内容构建。
7. `copyTextToClipboard()` / `copyImageToClipboard()`
   - 继续负责复制。
8. `downloadExportFile()`
   - 继续负责下载。

需要保留：

1. full 模式继续调用 `appChatExport.open()`。
2. compact 模式先打开 inline history panel；点击 panel 内的“导出”后，再调用新的 inline API 或通过宿主回调拿导出预览能力。
...
6. 如需渲染消息 block，应优先共享 React chat 已有 block renderer；禁止在导出历史层重写一套 text/image/link/status/buttonGroup 解析。
...
9. 如果当前消息只有临时 id，且状态处于 `sending` 或其他未来新增的未确认阶段，第一版应禁用该气泡选择并给出轻量不可选状态。
10. 如果后续确认 NEKO 消息体系存在稳定 `clientMessageId` / `localId`，可用该稳定 key 作为选择 key；否则只有 `sent` 且有永久 id 的消息默认可被选择。
11. 按当前 schema，第一版实际要处理的是 `sending`、`sent`、`failed`、`streaming`；不要在实现里硬写 schema 不存在的 `pending` / `retrying`，除非先扩展 schema 和测试。
12. 如果 `streaming` 消息 id 是当前稳定 id，则允许选择；如果存在 streaming 完成后换 id 的实证，则按不稳定 key 处理，禁用选择直到进入 `sent`。
...
新增面板必须纳入 compact geometry。

新增 item：

1. `kind: history`
2. `owner: surface`
3. `interactive: composite`：`history:native` 只用于窗口 bounds union，不进入 hit；scroll / controls / preview 子区域才进入真实 hit。
4. `nativeRect`: 历史层真实可见区域、历史选择操作区、inline 导出预览区的 union。
5. `hitRect`: 可滚动区、历史选择操作区、inline 导出预览区的真实命中区域。
6. `measuredRect`: React 侧实际测量并通过 data attribute 或 geometry payload 提供的物理尺寸，禁止宿主按 CSS 设想值猜测。

规则：

1. history item 只在 `compactExportHistoryOpen === true` 时存在；无消息时也输出空状态对应的真实可见区域，不能退回旧导出提示或让桌面壳猜测空层尺寸。
...
12. 当前 geometry 采集会把普通 item 的元素 rect 同时作为 `nativeRect` 和 `hitRect`；history 如果存在透明外层和内部交互区，必须扩展采集逻辑，不能只给外层打 `data-compact-geometry-owner="surface"`。
13. 宿主 geometry 只能使用真实可见和真实可交互 rect；不能因为 wrapper 使用 `height: 100%`、`max-height` 或透明占位，就把整块透明区域纳入 native hit region。
14. 历史层高度动态变化时，React 必须把实际测量尺寸同步给宿主，例如通过 data attribute、ResizeObserver 或现有 geometry payload；关闭或收起后要同步移除对应 rect。
...
实际实施步骤、修改顺序、检查项和运行时回归方式已拆到：

`home-compact-chat-inline-export-history-implementation-plan.md`

本文只保留功能目标、设计结构、状态规则、交互约束、geometry 合同、桌面端要求、禁止方案和参考方案；实施时必须先读本文，再按实施文档执行。
