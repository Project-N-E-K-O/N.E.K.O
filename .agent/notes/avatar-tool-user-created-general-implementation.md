...
> 状态：阶段 1、2、3 已收口；阶段 4 开工准备已完成，尚未开始功能实施
>
> 编写日期：2026-09-02  
...
flowchart LR
    ...
    F --> G[React Flow 画布]
    F --> H[互动设置与图片引用保护]
    D --> I[阶段 3 保存边界<br/>不降级写回 v2]
    B --> J[现有 v2 API / Store]
    J --> K[公开列表 DTO<br/>defaultUrl + changeUrls]
    K --> L[Web / PC v2 runtime<br/>press-swap 或 click-advance]
    L --> M[changeIndex + revision]
    M --> N[Host / Python 权威处理]
    N --> O[模型反馈]
```

阶段 1～3 已完成编辑入口、同级图片和完整互动图；持久化与运行仍保持 v2。当前事实与最终目标之间只剩后续阶段负责的差异：

| 当前实现 | 目标实现需要 |
|---|---|
| 编辑器已经能建立通用互动图，但阶段 3 保存会明确停在边界 | 阶段 4 用 v3 记录完整保存并重新打开图片、节点、位置、连接和用户选择的连接边 |
| `recordVersion: 2`、Router 和 Store 仍只接受默认图、变化图及两种固定模式 | 阶段 4 增加严格的 v3 manifest、DTO、限额和原子写入，同时继续读取 v2 |
...
- 阶段 3 新建连接已经具有明确选边；旧 v2 投影目前没有选边数据。转换保存前必须按现有投影布局为旧连接一次性补出确定的两端边界，之后作为 v3 正式字段保存；不得从渲染后的 SVG 路径反推，也不得让已保存的 v3 连接继续缺省选边；
...
- [ ] 故意制造空道具名称、初始图片未连接互动、非法延时和不可达节点，分别得到可定位错误且页面内容不丢失；悬空引用、非法 ID/选边/坐标和未知字段由严格 manifest/record 自动化覆盖；
