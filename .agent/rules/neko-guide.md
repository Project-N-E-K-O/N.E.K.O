...
- **辅助 LLM 调用约定（memory/ + utils/）**：
  ...
  - **每个 LLM 调用必须有 budget + timeout（硬性，全仓生效）**：
    - **输出**：每个 `create_chat_llm()` / `ChatOpenAI()` 构造都必须带 **token budget**（`max_completion_tokens=` 或 `max_tokens=`）**和** `timeout=`。没有 budget 输出会失控（成本 / 延迟 / 上下文爆炸），没有 timeout 上游卡死会拖垮整条 async 管线——两者在 `utils/llm_client.py` 都**没有安全默认值**。若 budget/timeout 改在 **调用时** per-call 设（`invoke/ainvoke/astream/..(**overrides)`、或裸 `_client.chat.completions.create()`、或 `asyncio.timeout()/wait_for()` 包裹），在构造行加 `# noqa: LLM_OUTPUT_BUDGET` 并注明理由。
