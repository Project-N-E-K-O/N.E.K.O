...
def _get_config_directory() -> Path:
    ...
    存储路径优先级：
    1. Windows: 我的文档/N.E.K.O/config/
    2. macOS: ~/Documents/N.E.K.O/config/
    3. Linux: ~/Documents/N.E.K.O/config/
    4. 回退: exe所在目录/config/ 或 当前目录/config/
    """
    app_name = "N.E.K.O"
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            reg_path = os.path.expandvars(winreg.QueryValueEx(key, "Personal")[0])
            winreg.CloseKey(key)
        ...
        for docs_dir in candidates:
            try:
                if docs_dir.exists() and os.access(str(docs_dir), os.W_OK):
                    return docs_dir / app_name / "config"
            except Exception:
                continue
        ...
        if xdg_docs:
            return Path(xdg_docs) / app_name / "config"
        docs_dir = Path.home() / "Documents"
        if docs_dir.exists():
            return docs_dir / app_name / "config"
