...
def _should_preserve_write_blocking_mode(config_manager, root_state: dict[str, Any]) -> bool:
    ...
    if last_migration_result.startswith("restart_rebind:"):
        # Selecting the already-active root has no migration checkpoint.  The
        # launcher consumes this marker after services stop and asks the desktop
        # owner to start the next generation, so cloud-save shutdown must leave
        # it intact until then.
        return True

    # 真相源是 storage_migration.json 的 pending 状态：迁移真在跑就保留 readonly，
    # 否则视为孤儿态自愈。``restart_pending:`` 只是描述上一次操作意图，不能
    # 被当作"还在进行中"的硬证据——marker
