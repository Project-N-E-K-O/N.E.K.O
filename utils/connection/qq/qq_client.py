...
class QQClient(QQConnectionBase):
    ...
    def __init__(self, *, onebot_url: str, token: str = "", logger: Any = None,
        ...
        # Optional: async callback (image_url: str) -> str; VLM-describes images in quoted messages
        self._image_describer = image_describer
        # Optional: async callback (audio_base64: str) -> str; transcribes voice to text
    ...
    def _transcribe_record_segments(self, message: Dict[str, Any]) -> list[str]:
        ...
        # Format 1: OneBot array segments
        ...
        # Format 2: CQ-code string (message field)
        if isinstance(segments, str):
            for m in re.finditer(r'\[CQ:record,\s*file=([^,\]]+)', segments):
                f = m.group(1).strip()
                if f:
                    record_files.append(f)
        # Format 3: raw_message string (some OneBot private-message impls)
    ...
    def _collect_file_segments(self, message: Dict[str, Any]) -> list[dict]:
        ...
        Supports both OneBot array segments and CQ-code strings (raw_message);
        when NapCat's ``file_id`` is missing it falls back to the file name, so
        ``_fetch_file_content`` can still pull the content in the background.
        """
        import re
        segments = message.get("message")
        files: list[dict] = []
        if isinstance(segments, list):
            for seg in segments:
                ...
                if seg.get("type") != "file":
                    continue
                # External OneBot clients are untrusted: data may not be a dict and
                # busid may not be an int -- tolerate at the boundary; a single segment
                # parse failure must not take down the whole message.
                d = seg.get("data")
        ...
        if not files:
            ...
            for m in re.finditer(r"\[CQ:file,[^\]]*\]", raw):
                ...
                files.append({
                    ...
                    "url": "",
                    # Keep the CQ segment's busid: get_group_file_url needs it to resolve
                    # file URLs on some OneBot backends (matches the array-segment branch).
    ...
    def _expand_forward_segments(message: Dict[str, Any]) -> list[str]:
        """Expand the forward segments in a message (forward/merged-forward),
        extracting the nested message text.

        Format each sub-message as "[转发] 发送者: 内容" and append to raw_message.
        Returns the forward_ids that could not be parsed from inline data and must
        ...
        for seg in segments:
            ...
            # Some OneBot implementations inline sub-messages in data.messages
        ...
        if forward_texts:
            ...
            # Count forwarded sub-messages toward the buffer count
    ...
    def _expand_reply_segments(message: Dict[str, Any]) -> list[str]:
        """Collect the quoted-reply message IDs, for async full-content fetch (KiraAI-compatible)."""
        segments = message.get("message")
        if not isinstance(segments, list):
            return []
        reply_ids: list[str] = []
    ...
    async def _fetch_forward_content(self, message: Dict[str, Any], unresolved_ids: list[str]) -> None:
        """Fetch forwarded message content via API, build MessageChain and append to raw_message.

        Each sub-message carries the `[时间] 发送者: 内容` format (aligned with KiraAI)."""
    ...
    async def _build_message_chain(self, msg_data: dict[str, Any], *, depth: int = 0, seen: set[str] | None = None) -> "MessageChain":
        """Convert a OneBot message dict into a MessageChain, recursively expanding quote/forward."""
        ...
        for seg in segments:
            ...
            if st == "text":
                ...
                nickname = ""
                if qq != "all":
                    # Group messages prefer the group member's card; if the card is empty
                    # or it's a private chat, fall back to get_stranger_info; on failure
                    # return "" (At.repr falls back to [@用户{qq}]), so the message won't crash.
    ...
    async def _build_file_element(self, data: dict[str, Any], msg: dict[str, Any]) -> "File":
        """Turn an OneBot ``file`` segment into a File element, handling the way each
        backend resolves the real URL.

        - Lagrange-style backends: ``data.url`` is already an http direct link → use it;
        - NapCat / go-cqhttp style: only ``file_id`` → call ``get_group_file_url`` /
          ``get_private_file_url`` to swap for a URL;
        - Other/failed: fully fall back to the current ``data.file``.

        Any API exception is swallowed and never re-raised -- a single file segment
        failing must not break the whole chain.
        """
        from .message_chain import File
        raw_file = str(data.get("file") or "")
        url = str(data.get("url") or "").strip()
        # 1) Lagrange / direct-link backend: data.url starts with http → take it directly
        ...
        # 2) NapCat path: has file_id and a group/private context → call API to swap for a URL
        ...
        if file_id:
            ...
            try:
                if msg_type == "group" and group_id:
                    # Group file: only go through get_group_file_url when there's a real
                    # busid (NapCat requires a real busid; 0 is rejected); with only file_id use the generic get_file fallback.
                    try:
                    ...
                    # NapCat's get_private_file_url needs the sender's user_id; without it we
                    # can't get the URL and fall through to the fallback (bare [文件 name]).
                ...
                pass  # ActionFailed / timeout / not connected → degrade
        # 3) Fallback: keep the existing data.file behavior
    ...
    async def _resolve_at_nickname(self, qq: str, msg: dict[str, Any]) -> str:
        ...
        Group messages: prefer ``get_group_member_info``'s card, then
        ``get_stranger_info`` if the card is empty; private chat (no group_id) goes
        straight to stranger. Any API exception is swallowed and returns ""
        (At.repr falls back to ``[@用户{qq}]``).
        """
        group_id = str(msg.get("group_id") or "").strip()
        if group_id:
            try:
    ...
    async def _build_forward_chains(self, forward_data: dict[str, Any], *, depth: int = 0, seen: set[str] | None = None) -> "list[MessageChain]":
        """Build a list of MessageChains from a get_forward_msg return; each sub-message carries a timestamp."""
    ...
    async def _fetch_reply_content(self, message: Dict[str, Any], reply_ids: list[str]) -> None:
        ...
        if chains:
            # Build a readable quote context with MessageChain.repr
            lines: list[str] = []
            for chain in chains:
                sender = chain.sender_name or chain.sender_id or "未知用户"
                ts = chain.timestamp
    ...
    async def _fetch_record_content(self, message: Dict[str, Any], record_files: list[str]) -> None:
        ...
        Transcribe if a transcriber is set, otherwise mark [语音]."""
        ...
        for file_id in record_files:
            try:
                ...
                # get_record returns a URL or local path; fetch the file content then base64-encode it
                ...
                # Prefer passing the URL (Qwen DashScope only accepts URLs); fall back to base64
                transcript = ""
                ...
                if transcript:
                    ...
                    continue
                # Fall back: keep the existing text and only mark [语音]
    ...
    async def _fetch_file_content(self, message: Dict[str, Any], files: list[dict]) -> None:
        """Background-fetch file content: images go through VLM, decode text, mark
        binary/failed; replace CQ codes.

        Routing:
        - Image extension → existing ``_image_describer(url)`` (VLM path, consumes URL);
        - Other → download bytes; NUL in the first 512 bytes → binary, else decode as
          utf-8 (capped at ``_FILE_TEXT_MAX_BYTES``; truncate and annotate beyond that);
        - Failed → bare ``[文件 {name}]`` fallback.

        Finally replace the raw ``[CQ:file,...]`` in ``content``/``raw_message`` with
        readable text so the LLM sees the file content directly this turn (same as
        voice transcription).
        """
        import re
        from pathlib import Path as _Path
        ...
        for f in files:
            ...
            try:
                if not url and file_id:
                    if msg_type == "group" and group_id:
                        # Only go through get_group_file_url when there's a real busid,
                        # otherwise use the generic get_file fallback (group file messages often only carry file_id).
                        try:
                        ...
                        # NapCat's get_private_file_url needs the sender's user_id; without
                        # it we can't get the URL and fall through to the fallback.
                ...
                if not url:
                    ...
                    async with httpx.AsyncClient(
                        ...
                        # Stream download, stop after _FILE_TEXT_MAX_BYTES+1 bytes: group
                        # files can be hundreds of MB, and resp.content would read the whole
                        # body into memory and blow up the process. The +1 is only to detect
                        # the cap; the memory limit is still ~_FILE_TEXT_MAX_BYTES.
        ...
        if not replaced[0]:
            # content has no CQ file code (array-format only, raw_message empty) → append directly
            extra = " ".join(renders)
            new_raw = (
    ...
    async def enrich_message(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        Consumes the ``_pending_*`` markers ``_process_incoming`` stamped on
        ``message``, called after the dispatcher's eligibility filter and before
        the message lands in backlog / the recheck blacklist. Returns the (possibly
        rewritten) message dict.
        """
        ...
        # Main-message image: call VLM for the description and inject into content.
        if self._image_describer:
    ...
    async def _inject_image_descriptions(self, message: dict[str, Any]) -> None:
        ...
        For pure-image messages raw_message is often empty and the image lives in the
        message array, so replacing [CQ:image] in the text won't inject it (the
        backfilled summary has empty image content). After collecting all descriptions:
        - content is empty → set directly to a "[Image 描述]" list
        - content is non-empty → replace the [CQ:image] placeholders in order, append
          the leftover descriptions to the end if any placeholders are missing
        """
        ...
        if base:
            # Non-empty content: replace the [CQ:image] placeholders in order; when the
            # CQ codes run out, append the leftover descriptions to the end (multi-image
            # messages may only have partial placeholders in raw_message).
            ...
            # Pure image: join the descriptions directly
    ...
    async def _process_incoming(self, raw_message: str):
        ...
        if message.get("post_type") == "message":
            ...
            if msg_type in {"private", "group"}:
                # Mark segments that need background content fetch (do not await in the WS handler; avoid deadlock)
    ...
    async def receive_message(self, timeout: float = 1.0) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Pass through the background-fetch marker fields (set by _process_incoming, consumed by message_dispatcher)
            ...
            if msg_type == "group":
                ...
                # Reuse the result already cached by _fetch_reply_content to avoid a duplicate get_msg API call
