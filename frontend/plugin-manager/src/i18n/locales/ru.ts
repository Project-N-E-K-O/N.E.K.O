...
export default {
  ...
  mirrorSource: {
    title: 'Источник зеркала', description: 'Выберите способ загрузки пакетов GitHub Release из Plugin Market.', close: 'Закрыть источник зеркала', proxyOptional: 'GitHub Proxy (необязательно)', direct: 'GitHub напрямую', auto: 'Автоматически выбрать самый быстрый', specified: 'Выбрать прокси', source: 'Источник', testing: 'Проверка…', autoFastest: 'Самый быстрый: {source}', autoUnavailable: 'В последней проверке не было доступных источников; используется GitHub напрямую.', autoPending: 'Проверка ещё не выполнена; временно используется GitHub напрямую.', testAndSelect: 'Проверить и выбрать самый быстрый источник', recentResults: 'Последние результаты проверки', notice: 'Проксируются только ссылки загрузки плагинов github.com; пакет всё равно проверяется SHA-256 из Market.', activeSource: 'Текущий источник: {source}', unavailable: 'Недоступен или истекло время ожидания', noAvailable: 'Нет доступных источников загрузки GitHub; используется GitHub напрямую.', selectedFastest: 'Выбран самый быстрый источник: {source}', testFailed: 'Не удалось проверить источник зеркала', installFallback: 'Не удалось проверить источник зеркала; используется GitHub напрямую.'
  },
  plugins: {
