...
export default {
  ...
  market: {
    ...
    marketInstalledVersion: 'Versión del mercado v{version}',
    localInstalledVersion: 'Local v{version} · {source}',
    useMarketVersion: 'Usar versión del mercado',
    localVersionNewer: 'La versión local es más reciente',
    ...
    externalInstallConfirmationTitle: 'Confirmar instalación del Market',
    externalInstallConfirmationBody: 'El mercado solicitó {mode} para {id} v{version}. Canal: {channel}. Host de descarga: {host}. SHA256: {sha256}. Estos datos proceden de la página solicitante; N.E.K.O verificará el paquete antes de cambiar archivos. ¿Continuar?',
  ...
  plugins: {
    ...
    runtimeVersion: {
      title: 'Versión en ejecución',
      description: 'Hay varias instalaciones de código disponibles. Elige cuál debe ejecutar N.E.K.O.',
      current: 'En uso',
      useThisVersion: 'Usar esta versión',
      builtinHint: 'Código integrado incluido con N.E.K.O',
      marketHint: 'Código instalado desde el mercado de plugins',
      importedHint: 'Código importado desde un paquete local',
      localHint: 'Código de plugin local o heredado',
      sharedDataHint: 'El cambio solo afecta al código en ejecución. La configuración, los datos y la caché siguen compartidos por el mismo ID.',
      confirmTitle: 'Cambiar versión en ejecución',
      confirmBody: 'Se usará {source} v{version}. El plugin se detendrá y reiniciará con esta selección; los datos del usuario no se eliminarán.',
      confirmDowngradeBody: 'Esto cambia de v{currentVersion} a la versión anterior {source} v{version}. Es posible que el código anterior no pueda leer los datos más recientes. ¿Continuar?',
      confirmAction: 'Cambiar versión',
      switchSucceeded: 'Se cambió la versión en ejecución',
      switchFailed: 'El cambio falló. N.E.K.O intentó restaurar la versión anterior.',
      unavailable: 'No se puede cambiar la versión en ejecución',
      unavailableHint: 'Los registros de origen están incompletos o en conflicto. Actualiza e inténtalo de nuevo.',
      blockedReason: 'No se puede seleccionar: {reason}',
    },
    ...
    batchDeleteConfirm: '¿Quitar u ocultar los {count} plugins seleccionados? Se eliminará el código instalado por el usuario, se conservará el código integrado y no se borrarán los datos del plugin.',
    ...
    importSuccessRestartRequired: 'Se importó {name}. Reinicia N.E.K.O para cambiar el plugin en ejecución a la nueva versión.',
    ...
    dangerDialog: {
      ...
      deleteMessage: 'Al eliminar "{pluginName}" se quitará de la aplicación. Sus ajustes y datos se conservarán para una futura reinstalación.',
  ...
  package: {
    install: {
      ...
      upgradeUnmanagedBody: 'Esta carpeta no es una instalación administrada y puede contener cambios locales del desarrollador. Continuar sustituirá {current} por {target} y sobrescribirá el código, las dependencias y los recursos del paquete. La instalación se detendrá si algún archivo cambia después de confirmar.',
      ...
      restartRequired: 'Instalación completada. Reinicia N.E.K.O para usar la nueva versión.',
      activationBlocked: 'El plugin no se activó de forma segura, por lo que la instalación no se considera correcta. Motivo: {reason}',
      ...
      error: {
        ...
        manifestInvalid: 'El manifest.toml del paquete no es válido. Pide al autor que lo corrija y vuelva a empaquetarlo.',
        ...
        packageTypeMismatch: 'El tipo de paquete declarado no coincide con la cantidad de plugins incluidos. Pide al autor que vuelva a empaquetarlo.',
        ...
        hashMismatchRolledBack: 'El contenido del paquete no coincide con los datos de verificación. Se rechazó la importación y se restauró el estado previo.',
        packageStateConflict: 'Los datos del plugin que modificaste entran en conflicto con la nueva versión. Se restauró el plugin actual; guarda una copia del archivo modificado y vuelve a intentarlo.',
