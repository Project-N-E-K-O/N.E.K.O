...
export default {
  ...
  market: {
    ...
    install: 'Instalar',
    installed: 'Instalado',
    marketVersion: 'Mercado v{version}',
    marketInstalledVersion: 'Versão do mercado v{version}',
    localInstalledVersion: 'Local v{version} · {source}',
    useMarketVersion: 'Usar versão do mercado',
    localVersionNewer: 'A versão local é mais recente',
    ...
    externalInstallConfirmationTitle: 'Confirmar instalação do Market',
    externalInstallConfirmationBody: 'O mercado solicitou {mode} para {id} v{version}. Canal: {channel}. Host de download: {host}. SHA256: {sha256}. Estes dados vieram da página solicitante; o N.E.K.O verificará o pacote antes de alterar arquivos. Continuar?',
  ...
  plugins: {
    ...
    runtimeVersion: {
      ...
      description: 'Há várias instalações de código disponíveis. Escolha qual o N.E.K.O deve executar.',
      current: 'Em uso',
      useThisVersion: 'Usar esta versão',
      builtinHint: 'Código integrado fornecido com o N.E.K.O',
      marketHint: 'Código instalado pelo mercado de plugins',
      importedHint: 'Código importado de um pacote local',
      localHint: 'Código de plugin local ou legado',
      sharedDataHint: 'A troca altera apenas o código em execução. Configuração, dados e cache continuam compartilhados pelo mesmo ID.',
      confirmTitle: 'Trocar versão em execução',
      confirmBody: 'Será usada {source} v{version}. O plugin em execução será parado e reiniciado com esta seleção; os dados do usuário não serão apagados.',
      confirmDowngradeBody: 'Isso troca de v{currentVersion} para a versão mais antiga {source} v{version}. O código antigo pode não conseguir ler dados mais recentes. Continuar?',
      confirmAction: 'Trocar versão',
      switchSucceeded: 'A versão em execução foi alterada',
      switchFailed: 'A troca falhou. O N.E.K.O tentou restaurar a versão anterior.',
      unavailable: 'Não é possível trocar a versão em execução',
      unavailableHint: 'Os registros de origem estão incompletos ou em conflito. Atualize e tente novamente.',
      blockedReason: 'Não é possível selecionar: {reason}',
    },
    ...
    batchDeleteConfirm: 'Remover ou ocultar os {count} plugins selecionados? O código instalado pelo usuário será removido, o código integrado será mantido e os dados do plugin serão preservados.',
    ...
    importSuccessRestartRequired: '{name} foi importado. Reinicie o N.E.K.O para trocar o plugin em execução pela nova versão.',
    ...
    dangerDialog: {
      ...
      deleteMessage: 'Excluir "{pluginName}" o removerá do aplicativo. As configurações e os dados serão mantidos para uma futura reinstalação.',
  ...
  package: {
    install: {
      ...
      upgradeUnmanagedBody: 'Esta pasta não é uma instalação gerenciada e pode conter alterações locais do desenvolvedor. Continuar substituirá {current} por {target}, sobrescrevendo código, dependências e recursos do pacote. A instalação será interrompida se qualquer arquivo mudar após a confirmação.',
      upgradeConfirm: 'Atualizar plugin',
      upgradeCancelled: 'Atualização cancelada.',
      upgradeSucceeded: '{plugin} foi atualizado com sucesso.',
      restartRequired: 'Instalação concluída. Reinicie o N.E.K.O para usar a nova versão.',
      activationBlocked: 'O plugin não foi ativado com segurança, então a instalação não foi considerada bem-sucedida. Motivo: {reason}',
      ...
      error: {
        ...
        manifestInvalid: 'O manifest.toml do pacote é inválido. Peça ao autor para corrigi-lo e recriar o pacote.',
        ...
        packageTypeMismatch: 'O tipo declarado do pacote não corresponde à quantidade de plugins incluídos. Peça ao autor para recriá-lo.',
        ...
        hashMismatchRolledBack: 'O conteúdo do pacote não corresponde aos dados de verificação. A importação foi recusada e o estado anterior foi restaurado.',
        packageStateConflict: 'Os dados do plugin que você modificou entram em conflito com a nova versão. O plugin atual foi restaurado; faça backup do arquivo alterado e tente novamente.',
  ...
  messages: {
    ...
    pluginRevertedToBuiltin: 'A versão instalada pelo usuário foi removida e a versão integrada foi restaurada',
    pluginRevertedToBuiltinNotStarted: 'A versão instalada pelo usuário foi removida; a versão integrada foi restaurada, mas não pôde ser iniciada',
