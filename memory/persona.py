...
_NEGATIVE_PATTERNS = (
    ...
    (re.compile(r"\b(?P<topic>[a-z0-9][\w\s\-]{0,40}?)\s+sucks\b", re.IGNORECASE), False),
)
_NEGATIVE_KEYWORDS = (
...
_REFERENCE_TOPIC_PATTERNS = (
    ...
    re.compile(r'(?:不过|但是|但|只是)?(?P<topic>[^，。！？!?]{1,24}?)(?:你应该不会喜欢|你应该不喜欢|你不会喜欢|你不喜欢|应该不会喜欢|不适合你)'),
    re.compile(
    ...
    re.compile(
        r"\b(?:but|though|however)?\s*(?P<topic>[a-z0-9][\w\s\-]{0,40}?)\s+(?:you\s+(?:probably\s+)?(?:won't|will not|dont|don't)\s+like|may not suit you)\b",
        re.IGNORECASE,
    ),
)
...
def _is_emotion_state(topic: str) -> bool:
    topic = _clean_topic_candidate(topic)
    if not topic:
        return False
    return _EMOTION_STATE_RE.fullmatch(topic) is not None
...
def _extract_negative_topic(text: str, referenced_topic: str = "") -> tuple[str, bool]:
    ...
    for pattern, explicit in _NEGATIVE_PATTERNS:
        ...
        if match:
            topic = _clean_topic_candidate(match.group("topic"))
            if pattern is _GENERIC_AVOID_SIGNAL_RE and _is_emotion_state(topic):
                return "", False
            if _is_specific_topic(topic):
                return topic, explicit

    explicit = any(token in raw_lower for token in _EXPLICIT_AVOID_TOKENS)
    referenced = _extract_topic_from_reference_text(referenced_topic)
...
class PersonaManager:
    ...
    def _apply_topic_guidance_update(
        self,
        persona: dict,
        topic: str,
        *,
        ...
        now_str = now_str or datetime.now().isoformat()
    ...
    def apply_negative_preference_review(
        ...
        *,
        ...
        return {
            ...
            'response_instruction': self._build_negative_response_instruction(normalized_topic, applied_policy),
        }
