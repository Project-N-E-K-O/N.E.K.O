(function () {
    ...
    class YuiGuideGhostCursor {
        ...
        hasPosition() {
            if (this.overlay && typeof this.overlay.hasCursorPosition === 'function') {
        ...
        isVisible() {
            if (this.overlay && typeof this.overlay.isCursorVisible === 'function') {
                return this.overlay.isCursorVisible();
            }
    ...
    class YuiGuideDirector {
        constructor(options) {
            ...
            if (!window.TutorialHighlightController || typeof window.TutorialHighlightController.createController !== 'function') {
                throw new Error('TutorialHighlightController must be loaded before YuiGuideDirector');
            }
            ...
            this.interruptController = this.createInterruptController();
        ...
        createInterruptController() {
            if (!window.TutorialInterruptController || typeof window.TutorialInterruptController.createController !== 'function') {
                console.warn('[YuiGuide] TutorialInterruptController 未加载，打断分支降级为空操作');
                return {
                    playLightResistance: () => Promise.resolve(),
            ...
            return window.TutorialInterruptController.createController({
                overlay: this.overlay,
                cursor: this.cursor,
                callbacks: {
                    isDestroyed: () => this.destroyed,
                    ...
                    isAngryExitTriggered: () => this.angryExitTriggered,
                    setAngryExitTriggered: (active) => {
                        this.angryExitTriggered = active === true;
                    },
                    getInterruptCount: () => this.interruptCount,
                    getCurrentSceneId: () => this.currentSceneId,
                    getLastPointerPoint: () => this.lastPointerPoint,
                    getStep: (stepId) => this.getStep(stepId),
                    resolveResistanceVoices: (performance) => this.resolvePerformanceResistanceVoices(performance),
                    resolveBubbleText: (performance) => this.resolvePerformanceBubbleText(performance),
                    capturePresentationSnapshot: () => this.captureCurrentGuidePresentationSnapshot(),
                    pauseCurrentSceneForResistance: () => this.pauseCurrentSceneForResistance(),
                    interruptNarrationForResistance: () => this.interruptNarrationForResistance(),
                    resumeCurrentSceneAfterResistance: () => this.resumeCurrentSceneAfterResistance(),
                    restoreGuidePresentationSnapshot: (snapshot) => this.restoreGuidePresentationSnapshot(snapshot),
                    getActiveNarration: () => this.activeNarration,
                    scheduleNarrationResume: (options) => this.scheduleNarrationResume(options),
                    restoreCurrentScenePresentation: (options) => this.restoreCurrentScenePresentation(options),
                    prepareResistanceCursorReveal: (options) => this.prepareResistanceCursorReveal(options),
                    ...
                    runInterruptResistPerformance: (options) => this.runInterruptResistPerformance(options),
                    runAngryExitPerformance: (options) => this.runAngryExitPerformance(options),
                    speakResistanceLine: (message, options) => this.voiceQueue.speak(message, options),
                    speakGuideLine: (message, options) => this.speakGuideLine(message, options),
                    recordExperienceMetric: (eventName, payload) => this.recordExperienceMetric(eventName, payload),
                    clearSceneTimers: () => this.clearSceneTimers(),
                    disableInterrupts: () => this.disableInterrupts(),
                    cancelActiveNarration: () => this.cancelActiveNarration(),
                    beginGuideInterruptPresentation: () => this.beginGuideInterruptPresentation(),
                    setTutorialTakingOver: (active) => this.setTutorialTakingOver(active),
                    notifyPluginDashboardNarrationFinished: () => this.notifyPluginDashboardNarrationFinished(),
        ...
        setGuideChatInputLocked(locked, reason) {
            ...
            if (host && typeof host.setHomeTutorialInputLocked === 'function') {
                try {
                    host.setHomeTutorialInputLocked(isLocked, lockReason);
                } catch (error) {
            ...
            if (
                ...
                && typeof this.interactionTakeover.setExternalizedChatInputLocked === 'function'
            ) {
                this.interactionTakeover.setExternalizedChatInputLocked(isLocked, lockReason);
            }
            try {
        ...
        applyGuideHighlights(config) {
            const highlights = this.highlightController.applyGuideHighlights(config);
        ...
        getExternalizedChatCursorAnchorPoint(maxAgeMs) {
            ...
            if (
                this.latestExternalizedChatCursorAnchorPoint
                && Number.isFinite(this.latestExternalizedChatCursorAnchorPoint.x)
                && Number.isFinite(this.latestExternalizedChatCursorAnchorPoint.y)
            ) {
                const latestAt = Number(this.latestExternalizedChatCursorAnchorPoint.at);
                const ageLimit = Number.isFinite(maxAgeMs) ? maxAgeMs : 30000;
                if (!Number.isFinite(latestAt) || Date.now() - latestAt <= ageLimit) {
                    return {
                        x: this.latestExternalizedChatCursorAnchorPoint.x,
                        y: this.latestExternalizedChatCursorAnchorPoint.y
                    };
                }
            }
            const screenPoint = this.getRecentExternalizedChatCursorScreenPoint(maxAgeMs);
        ...
        postExternalChatGuideMessage(message) {
            ...
            if (
                ...
                || outgoingMessage.action === 'yui_guide_clear_chat_messages'
            ) {
        ...
        async playManagedScene(stepId, meta) {
            ...
            try {
                await this.playScene(stepId, Object.assign({}, meta || {}, {
        ...
        setCompactToolWheelIndexForGuide(index, reason) {
            ...
            if (this.isHomeChatExternalized()) {
                if (
                    this.interactionTakeover
                    && typeof this.interactionTakeover.setExternalizedChatCompactToolWheelIndex === 'function'
                ) {
                    this.interactionTakeover.setExternalizedChatCompactToolWheelIndex(
                        normalizedIndex,
                        reason || 'avatar-floating-guide'
                    );
                    return true;
        ...
        async playDay4PrivacyModeScene(scene, sceneRunId, previousSceneId, index, total) {
            ...
            let characterSettingsPanel = this.getCharacterSettingsSidePanel();
            const characterSettingsButton = this.getDay5CharacterSettingsButtonTarget()
                || this.getSettingsMenuElement('character');
            if (characterSettingsButton) {
                this.applyGuideHighlights({
	            ...
	            if (characterSettingsPanel) {
	                ...
	                await this.moveCursorToElement(characterSettingsPanel, 620);
	                const motionRect = this.getElementRect(characterSettingsPanel);
	                if (motionRect) {
	                    const centerX = motionRect.left + motionRect.width / 2;
	                    const centerY = motionRect.top + motionRect.height / 2;
        ...
        async runAvatarFloatingSceneOperation(scene, primaryTarget, narrationStartedAt) {
            ...
            if (operation === 'show-avatar-tools-then-hide-after-narration') {
                ...
                if (this.isHomeChatExternalized()) {
                    this.clickChatAvatarToolButton('avatar-floating-guide-open-avatar-tool-menu');
                    ...
                    this.clickChatAvatarToolButton('avatar-floating-guide-close-avatar-tool-menu-after-narration');
            ...
            if (operation === 'day2-open-settings-personalization') {
                return await this.openSettingsPanel();
            }
            if (operation === 'day2-settings-detail') {
            ...
            if (operation === 'settings-peek-panic') {
                const fullDurationMs = this.getGuideVoiceDurationMs(scene.voiceKey || '', resolveGuideLocale())
                    || estimateSpeechDurationMs(scene.text || '');
                const elapsedMs = Number.isFinite(narrationStartedAt)
                    ? Math.max(0, Date.now() - narrationStartedAt)
                    : 0;
                await this.runSettingsPeekPanicPerformance({
                    targetRect: this.getElementRect(primaryTarget),
            ...
            if (operation === 'open-agent') {
                const opened = await this.openAgentPanel();
                if (opened) {
                    this.applyGuideHighlights({
        ...
        getAvatarFloatingNarrationDurationMs(voiceKey, text) {
            ...
            return estimateSpeechDurationMs(text || '') || 2200;
        }
        ...
        rememberAvatarFloatingSceneCursorAnchorPoint(sceneId, point) {
            ...
            this.avatarFloatingSceneCursorAnchorPoints[normalizedSceneId] = {
                x: point.x,
                y: point.y
            };
        }
        ...
        getAvatarFloatingSceneCursorAnchor(sceneIds) {
            ...
            const anchors = this.avatarFloatingSceneCursorAnchorPoints || {};
            for (let index = 0; index < candidates.length; index += 1) {
                const sceneId = typeof candidates[index] === 'string' ? candidates[index].trim() : '';
                const point = sceneId ? anchors[sceneId] : null;
                if (point && Number.isFinite(point.x) && Number.isFinite(point.y)) {
                    return {
                        x: point.x,
                        y: point.y
        ...
        async playDay1AvatarFloatingScene(scene, sceneRunId, previousSceneId) {
            const sceneId = scene && typeof scene.id === 'string' ? scene.id : '';
        ...
        async playAvatarFloatingScene(scene, day, index, total) {
            ...
            if (
	            ...
	            if (scene && scene.id === 'day2_personalization_detail') {
	                return await this.playDay2PersonalizationDetailScene(
	                    scene,
	                    sceneRunId,
	                    previousSceneId,
	                    index,
            ...
            let secondaryTarget = null;
                ...
                if (introRect) {
                    this.rememberAvatarFloatingSceneCursorAnchor(scene.id, introChatSpotlightTarget);
                    this.cursor.showAt(
                        introRect.left + introRect.width / 2,
                        introRect.top + introRect.height / 2
                    );
                ...
                const externalizedCursorMoveDurationMs = this.getExternalizedChatCursorMoveDurationMs(scene, 760);
                ...
                if (
                    ...
                    this.setExternalizedChatCursorEffect(
                        externalizedSceneCursorKind,
                ...
                const day2CharacterSettingsPersistentTarget = this.getDay2CharacterSettingsPersistenceTarget(scene.id);
            ...
            const narrationPromise = shouldPlayNarration ? this.speakGuideLine(text, {
                voiceKey: voiceKey,
                minDurationMs: 1800
            }).catch((error) => {
                console.warn('[YuiGuide] 悬浮窗教程旁白失败，继续流程:', scene.id, error);
                ...
                if (!sceneOperationStarted) {
                    ...
                    try {
                        sceneOperationPromise = Promise.resolve(
                            this.runAvatarFloatingSceneOperation(scene, primaryTarget, narrationStartedAt)
                        );
                    } catch (error) {
            ...
            if (externalizedSceneTargetKind && scene.cursorAction === 'click') {
                ...
                const cursorTarget = scene.cursorTarget
                    ...
                    : null;
                await this.moveAvatarFloatingCursor(scene, cursorTarget || primaryTarget, secondaryTarget, previousSceneId, {
                    onClickStart: startSceneOperation
                });
            }
            if (sceneRunId !== this.sceneRunId || this.isStopping()) {
        ...
        async runSettingsPeekScene(step, performance, runId) {
            ...
            this.highlightChatWindow();
                ...
                const settingsButtonMovePromise = settingsSpotlightTarget
                    ? this.moveCursorToElement(
                        settingsSpotlightTarget,
                        clamp(Math.round(introVoiceDurationMs * 0.28), 520, 1100)
                    ).then((moved) => {
                        if (moved && runId === this.sceneRunId && !this.isStopping()) {
                            this.cursor.wobble();
                        }
                        return moved;
                    }).catch(() => false)
                ...
                await settingsButtonMovePromise;
                if (runId !== this.sceneRunId || this.isStopping()) {
                    return;
                }
                ...
                if (runId !== this.sceneRunId || this.isStopping()) {
	                ...
	                const cycleMs = 5600;
	                const ellipseAbortCheck = () => this.destroyed || this.angryExitTriggered || settingsPeekHighlightsCleared || stopSettingsDetailTour;
        ...
        beginTerminationVisualCleanup() {
            ...
            if (this.interruptController && typeof this.interruptController.destroy === 'function') {
        ...
        async playScene(stepId, meta) {
            ...
            const previousSceneId = meta && typeof meta.previousSceneId === 'string'
                ? meta.previousSceneId
                : '';
            ...
            const cursorTargetRect = returnControlChatTarget && typeof returnControlChatTarget.getBoundingClientRect === 'function'
                ...
                : this.resolveRect(performance.cursorTarget || step.anchor);
            ...
            const dedupedActionSpotlightTarget = actionSpotlightTarget === persistentSpotlightTarget
                ? null
                : actionSpotlightTarget;
            if (dedupedActionSpotlightTarget) {
                this.applyCircularFloatingButtonSpotlightHint(dedupedActionSpotlightTarget);
            ...
            if (shouldIntroduceCursor) {
                if (!this.restoreCursorFromExternalizedChatAnchor(30000)) {
                    const origin = this.resolveManagedSceneCursorAnchorPoint(previousSceneId)
                        || this.getDefaultCursorOrigin();
                    this.cursor.showAt(origin.x, origin.y);
                }
                await this.waitForSceneDelay(260);
            ...
            if (shouldMoveCursor && (performance.cursorAction === 'move' || performance.cursorAction === 'click' || performance.cursorAction === 'wobble')) {
                if (cursorTargetRect) {
                    ...
                    let movedToCursorTarget = false;
                    if (narrationPromise) {
                        await Promise.all([
                            movePromise.then((result) => {
                                movedToCursorTarget = result !== false;
                            }),
                            narrationPromise
                        ]);
                    } else {
                        movedToCursorTarget = await movePromise;
                    }
                    if (runId !== this.sceneRunId || this.destroyed) {
                        return;
                    }
                    if (movedToCursorTarget) {
                        this.rememberAvatarFloatingSceneCursorAnchorPoint(stepId, {
                            x: cursorTargetRect.left + cursorTargetRect.width / 2,
                            y: cursorTargetRect.top + cursorTargetRect.height / 2
                        });
                    }
                }
            ...
            let navigationOpenPromise = null;
            const startNavigationOpen = () => {
                if (!navigationOpenPromise && step && step.navigation && step.navigation.openUrl) {
                    try {
                        navigationOpenPromise = Promise.resolve(this.openPageWithHandoff(stepId, step));
                    } catch (error) {
                        navigationOpenPromise = Promise.reject(error);
                    }
                    navigationOpenPromise.catch(() => {});
                }
                return navigationOpenPromise;
            };
            ...
            if (step && step.navigation && step.navigation.openUrl) {
                const opened = await (navigationOpenPromise || startNavigationOpen());
                if (runId !== this.sceneRunId || this.destroyed) {
                    return;
                }
        ...
        handleInterrupt(event) {
            ...
            const shouldRequireDocumentFocus = !(
                ...
                && this.platformCapabilities.windowBoundsSource === 'electron-window-bounds'
            );
            if (shouldRequireDocumentFocus && this.page === 'home' && typeof document.hasFocus === 'function' && !document.hasFocus()) {
                return;
            }
            ...
            if (!previousPoint || !Number.isFinite(previousPoint.t)) {
                ...
                if (sampleDx !== null || sampleDy !== null) {
                    ...
                    this.noteUserCursorRevealAttempt(initialDistance, now);
                    this.playCursorResistanceToUserMotion(x, y, initialDistance, initialDx, initialDy);
                }
                this.interruptQualifyingMoveStreak = 0;
                return;
            }
            ...
            this.playCursorResistanceToUserMotion(x, y, distance, dx, dy);
            ...
            this.interruptQualifyingMoveStreak += 1;
            if (this.interruptQualifyingMoveStreak < DEFAULT_INTERRUPT_QUALIFYING_MOVE_STREAK) {
                return;
            }
            this.interruptQualifyingMoveStreak = 0;
        ...
        requestTermination(reason, tutorialReason) {
            ...
            this.setGuideChatInputLocked(false, 'avatar-floating-guide-' + finalReason);
