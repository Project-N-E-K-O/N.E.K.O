(function () {
        ...
        try {
            ...
            if (data && data.should_prompt) {
                try {
                    ...
                    // 多数情况下 onboarding 早已 settled，此处 await 立即过；
    ...
    async function waitForForegroundTrackingGate() {
        const AUTOSTART_FOREGROUND_GATE_FALLBACK_MS = 15000;
        if (typeof window.waitForStorageLocationStartupBarrier === 'function') {
            await window.waitForStorageLocationStartupBarrier();
        }
        ...
        const settled = await Promise.race([
            onboarding.whenSettled().then(() => true),
            new Promise((resolve) => setTimeout(() => resolve(false), AUTOSTART_FOREGROUND_GATE_FALLBACK_MS)),
        ]);
        if (settled) {
            return;
        }
    ...
    function startForegroundTrackingGate() {
        ...
        void waitForForegroundTrackingGate()
            .catch(function (error) {
