...
function openCatgirlPanel(card, originEl) {
    ...
    cardImage.addEventListener('click', () => {
        const makerUrl = `/card_export?name=${encodeURIComponent(name)}&mode=maker`;
        window.open(makerUrl, '_blank', 'width=1200,height=800');
    });

    // 监听角色卡制作页面的保存消息
    const onCardFaceMessage = (event) => {
        if (event.data && event.data.type === 'card-face-updated' && event.data.name === name) {
            const ts = event.data.timestamp;
            const newSrc = cardFaceUrl + '?t=' + ts;
