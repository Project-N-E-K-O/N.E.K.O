(function () {
    ...
    function cacheElements() {
        for (const [name, id] of Object.entries({
            ...
            profileToggle: 'voice-identity-profile-toggle', profileDetails: 'voice-identity-profile-details',
    ...
    function reasonMessage() {
        if (!state.profileAvailable) {
            ...
            return translate(EFFECTIVE_REASON_KEYS[state.effectiveReason] || 'voiceIdentity.reasonRuntimeDegraded', '声纹激活暂时不可用，待机音频不会上传');
        }
        if (state.effectiveEnabled) return translate('voiceIdentity.profileReady', 'Owner 声纹已保存，声纹激活已开启');
        if (!state.requestedEnabled || state.effectiveReason === 'disabled') {
            return translate('voiceIdentity.profileSavedDisabled', 'Owner 声纹已保存，声纹激活已关闭');
        }
        return translate(EFFECTIVE_REASON_KEYS[state.effectiveReason] || 'voiceIdentity.reasonRuntimeDegraded', '声纹激活暂时不可用，待机音频不会上传');
    }
    function enrollmentCompleteMessage() {
        if (state.effectiveEnabled) return translate('voiceIdentity.enrollmentComplete', 'Owner 声纹已保存，声纹激活已开启。');
        if (!state.requestedEnabled) return translate('voiceIdentity.profileSavedDisabled', 'Owner 声纹已保存，声纹激活已关闭');
