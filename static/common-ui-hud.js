...
window.AgentHUD._createAgentPopupContent = function (popup) {
    ...
    const agentToggles = [
        ...
        {
            id: 'agent-copaw',
            label: window.t ? window.t('settings.toggles.copawConnect') : 'OpenClaw',
            labelKey: 'settings.toggles.copawConnect',
            initialDisabled: true,
            initialTitle: window.t ? window.t('settings.toggles.checking') : '查询中...'
        }
    ];
    ...
    agentToggles.forEach(toggle => {
        ...
        // OpenClaw 侧边面板
        if (toggle.id === 'agent-copaw' && typeof this._createSidePanelContainer === 'function') {
            const sidePanel = this._createSidePanelContainer();
            sidePanel.style.flexDirection = 'column';
            sidePanel.style.alignItems = 'stretch';
            sidePanel.style.gap = '4px';
            ...
            const CHAT_KEY = 'settings.toggles.copawChat';
            const CHAT_FALLBACK = '开始对话';
            ...
            chatLabel.textContent = window.t ? window.t(CHAT_KEY) : CHAT_FALLBACK;
            chatLabel.setAttribute('data-i18n', CHAT_KEY);
            chatLabel.style.userSelect = 'none';
            const chatArrow = document.createElement('span');
            chatArrow.textContent = '↗';
            chatArrow.style.marginLeft = 'auto';
            chatArrow.style.opacity = '0.5';
            chatArrow.style.fontSize = '11px';
            chatBtn.appendChild(chatIcon);
            chatBtn.appendChild(chatLabel);
            chatBtn.appendChild(chatArrow);

            chatBtn.addEventListener('mouseenter', () => {
                chatBtn.style.background = 'var(--neko-popup-hover, rgba(68,183,254,0.1))';
            });
...
// CoPaw 聊天对话框（可拖动、可调整大小）
let _copawChatDialog = null;

window.toggleCopawChatDialog = function() {
    if (_copawChatDialog && _copawChatDialog.style.display !== 'none') {
        _copawChatDialog.style.display = 'none';
        return;
    }
    window.openCopawChatDialog();
};

window.openCopawChatDialog = function() {
    if (_copawChatDialog) {
        _copawChatDialog.style.display = 'flex';
        return _copawChatDialog;
    }

    const dialog = document.createElement('div');
    dialog.id = 'copaw-chat-dialog';
    let dialogX = window.innerWidth - 400;
    let dialogY = window.innerHeight - 540;
    let dialogW = 380;
    let dialogH = 500;
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        dialog.style.left = Math.max(0, Math.min(window.innerWidth - dialog.offsetWidth, dialogStartX + dx)) + 'px';
        dialog.style.top = Math.max(0, Math.min(window.innerHeight - dialog.offsetHeight, dialogStartY + dy)) + 'px';
    });
    ...
    title.textContent = 'OpenClaw Chat';
    title.style.cssText = 'font-weight: 600; color: var(--neko-text, #fff); font-size: 14px;';
    const closeBtn = document.createElement('button');
    closeBtn.innerHTML = '✕';
    closeBtn.style.cssText = 'background: none; border: none; color: var(--neko-text, #fff); cursor: pointer; font-size: 16px; opacity: 0.7;';
    closeBtn.onmouseenter = () => closeBtn.style.opacity = '1';
    closeBtn.onmouseleave = () => closeBtn.style.opacity = '0.7';
    closeBtn.onclick = () => { window.closeCopawChatDialog(); };
    ...
    messagesContainer.id = 'copaw-messages';
    ...
    welcomeMsg.style.cssText = 'color: var(--neko-text-secondary, #888); text-align: center; font-size: 12px; padding: 20px;';
    welcomeMsg.textContent = window.t ? window.t('settings.toggles.copawWelcome') : '已连接到 CoPaw，开始对话吧！';
    ...
    Object.assign(inputArea.style, {
        ...
        gap: '8px',
        padding: '12px 16px',
        borderTop: '1px solid var(--neko-border, rgba(255,255,255,0.1))',
        backgroundColor: 'var(--neko-header-bg, rgba(0,0,0,0.2))'
    });

    const input = document.createElement('input');
    input.type = 'text';
    input.placeholder = window.t ? window.t('settings.toggles.copawInputPlaceholder') : '输入消息...';
    ...
    input.onfocus = () => input.style.borderColor = '#44b7fe';
    input.onblur = () => input.style.borderColor = 'rgba(255,255,255,0.2)';

    const sendBtn = document.createElement('button');
    sendBtn.textContent = window.t ? window.t('settings.toggles.copawSend') : '发送';
    Object.assign(sendBtn.style, {
        padding: '10px 16px',
        borderRadius: '8px',
        border: 'none',
        backgroundColor: 'var(--neko-primary, #44b7fe)',
        color: '#fff',
        fontSize: '14px',
        cursor: 'pointer',
        fontWeight: '500'
    ...
    async function sendMessage() {
        ...
        userMsg.style.cssText = 'align-self: flex-end; max-width: 80%; padding: 10px 14px; border-radius: 12px 12px 4px 12px; background: var(--neko-primary, #44b7fe); color: #fff; font-size: 14px;';
        ...
        loadingMsg.style.cssText = 'align-self: flex-start; max-width: 80%; padding: 10px 14px; border-radius: 12px 12px 12px 4px; background: var(--neko-msg-bg, rgba(255,255,255,0.1)); color: var(--neko-text, #fff); font-size: 14px; opacity: 0.7;';
        loadingMsg.textContent = '...';
        messagesContainer.appendChild(loadingMsg);
        messagesContainer.scrollTop = messagesContainer.scrollHeight;

        try {
            const resp = await fetch('/api/agent/copaw/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ text, session_id: 'neko:copaw_dialog' })
            });
            const data = await resp.json();
            loadingMsg.textContent = data.reply || data.error || '无响应';
            loadingMsg.style.opacity = '1';
        } catch (err) {
            loadingMsg.textContent = '发送失败: ' + err.message;
    ...
    _copawChatDialog = dialog;
    return dialog;
};

window.closeCopawChatDialog = function() {
    if (_copawChatDialog) {
        _copawChatDialog.style.display = 'none';
    }
};
