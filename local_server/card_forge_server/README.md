...
Windows 上要一次拉起「主服务 + 铸造机服务 + `frontend/card-forge` 前端」三个窗口，用 `scripts/card_forge/start-card-forge.bat`，停止用同目录的 `stop-card-forge.bat`。启动器会在本地 Vite 缺失时先执行一次 `npm ci`；依赖安装失败时不会继续拉起三个服务。
