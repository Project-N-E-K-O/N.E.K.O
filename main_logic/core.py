...
class LLMSessionManager:
    ...
    def _flush_ai_turn_text_to_tracker(self) -> None:
        """把当前轮累积的 AI 文本交给 activity tracker，然后清空 buffer。

        三条路径都需要此操作：
        - ``handle_proactive_complete``（agent_callback 主动搭话）
        - ``_emit_turn_end``（普通用户回复轮结束）
        - ``finish_proactive_delivery``（/proactive_chat 流式完成路径）

        集中在此处避免遗漏：任何路径漏调都会让 proactive 文本残留在 buffer
        里，下一轮 user 消息触发 flush 时被误判为"AI 刚说的"，导致问号检测
        在错误时机触发、unfinished_thread 错误开启。
        """
    ...
    async def finish_proactive_delivery(self, full_text: str, expected_speech_id: str | None = None) -> bool:
        ...
        async with self._proactive_write_lock:
            ...
            try:
                ...
                pass
            # Activity tracker：flush proactive 文本 buffer，触发问号检测 /
            # unfinished_thread 逻辑。必须在 lock 内紧跟 send_lanlan_response，
            # 否则 _current_ai_turn_text 残留到下一轮 user 消息才被 flush，
            # 导致问号检测在错误时机触发，unfinished_thread 完全破功。
            self._flush_ai_turn_text_to_tracker()
