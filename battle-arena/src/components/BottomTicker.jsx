...
export default function BottomTicker({ items = MEME_ITEMS }) {
  ...
  return (
    <div className="w-full h-10 bg-white border-t border-gray-200 flex items-center overflow-hidden relative">
      {/* 滚动内容 — 去掉左右渐变遮罩 */}
      <div className="flex items-center animate-ticker whitespace-nowrap will-change-transform pl-4">
        {doubled.map((item, i) => (
          <span key={i} className="inline-flex items-center">
            <span className="text-xs text-black font-bold px-3">{item}</span>
