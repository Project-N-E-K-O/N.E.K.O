...
const tutorialSections = [
  ...
  ['筛选区', '搜索框、费用、类型、属性和喜爱筛选都会先缩小卡池范围，再决定当前显示哪些卡。筛选只影响浏览，不会删除已经放入卡组的卡。'],
  ['已选卡组区', '已选卡组区展示准备带进对局的卡。这里用于核对数量、同名张数和费用结构，移除卡牌后卡池仍可重新选择。'],
  ['卡组数量', '卡组需要正好 18 张卡。同名卡最多 3 张，左侧会显示当前数量和平均费用。'],
  ['费用与能量曲线', '费用决定对局里确认出牌时消耗的能量。能量曲线按费用档位统计卡组分布，用于快速观察卡组是否过重、过轻，或缺少前期能打出的牌。'],
  ...
  ['Forged卡', '奇遇制造机产出的卡会带有(Forged)后缀。它们的主效果、费用、名字、主属性跟随基础卡，Combo属性会随机，故事当前使用临时事件文本。'],
