...
import { ArrowLeft, Clock3, FileText, Flame, Heart, RotateCcw, Snowflake, Sparkles, Star, Swords, Wind, X, Zap } from 'lucide-react'
import { AnimatePresence, motion } from 'framer-motion'
import NekoAvatar from './NekoAvatar'

const BATTLE_BACKGROUND_SRC = '/@fs/F:/NEKO_bugfix/NekoBrawl/Gif_source/Background_forest.png?v=forest-1'
  ...
  return (
    <aside className={`absolute bottom-0 top-0 z-20 flex w-[18rem] flex-col items-start ${sideClass} border-neutral-800/70 bg-[#171814]/95 px-7 pb-6 pt-24 shadow-[0_0_48px_rgba(0,0,0,0.34)]`}>
      ...
      <section className="mt-6 flex w-full flex-col items-start">
        ...
        <div className={`mt-4 w-56 rounded-sm bg-neutral-950/88 px-6 py-5 shadow-[0_16px_36px_rgba(0,0,0,0.42)] ${textAlign}`}>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2">
            <p className="text-sm font-black text-neutral-300">剩余卡牌数：{deckCount}</p>
            {typeof handCount === 'number' && (
              <p className="text-sm font-black text-neutral-300">手牌数：{handCount}</p>
            )}
...
export default function NewBattleDuelUI({
  ...
  const bossHpPercent = bossBreakGoal > 0
    ...
    : 0
  const bossImageTone = {
    attack: 'border-rose-300 bg-rose-50 shadow-[0_16px_48px_rgba(244,63,94,0.18)]',
    damageTaken: 'border-amber-300 bg-amber-50 shadow-[0_16px_48px_rgba(245,158,11,0.14)]',
    weakDamageTaken: 'border-sky-300 bg-sky-50 shadow-[0_16px_48px_rgba(14,165,233,0.18)]',
    normal: 'border-neutral-200 bg-neutral-100 shadow-sm',
  }[bossImageState] || 'border-neutral-200 bg-neutral-100 shadow-sm'
  ...
  const getCardEffectTarget = (card) => {
    ...
    return targetsBoss ? 'boss' : 'player'
  }
  ...
  const handleConfirmPlay = () => {
    ...
    const effectTarget = getCardEffectTarget(selectedCards[0])
    const targetRect = effectTarget === 'player'
      ? playerAvatarRef.current?.getBoundingClientRect()
      : bossStageRef.current?.getBoundingClientRect()
    ...
    const targetX = targetRect
      ? targetRect.left + targetRect.width * (effectTarget === 'player' ? 0.5 : 0.56)
      : window.innerWidth * (effectTarget === 'player' ? 0.16 : 0.56)
    const targetY = targetRect
      ? targetRect.top + targetRect.height * (effectTarget === 'player' ? 0.42 : 0.43)
      : window.innerHeight * (effectTarget === 'player' ? 0.64 : 0.42)

    // 临时特效：攻击牌飞向Boss；支援牌飞向玩家猫娘头像，后续可替换为正式动画素材。
    setStarStrike({
      id: `star-strike-${Date.now()}`,
      kind: effectTarget === 'player' ? 'support' : 'attack',
      fromX: sourceX - 32,
      fromY: sourceY - 32,
      midX: (sourceX + targetX) / 2 - 32,
      midY: effectTarget === 'player'
  ...
  const inspectedCardPlacement = inspectedCard?.source === 'player-zone'
    ? 'absolute left-[calc(18rem+1.4rem)] top-32 z-[240] origin-left'
    : inspectedCard?.source === 'ally-zone'
      ? 'absolute right-[calc(18rem+1.4rem)] top-32 z-[240] origin-right'
      : 'absolute bottom-[7.4rem] left-1/2 z-[240] -translate-x-1/2 origin-bottom'
  ...
  return (
    ...
    >
      ...
      <main className="relative z-10 h-[calc(100vh-64px)]">
        ...
        <section
          ref={bossStageRef}
          className={`pointer-events-none absolute bottom-0 left-[18rem] right-[18rem] top-0 z-0 overflow-hidden border-x border-b border-neutral-300/60 bg-white/10 transition-all duration-200 ${
        ...
        <AnimatePresence>
          {inspectedCard && (
            <>
              <motion.div
                key="inspect-card-dismiss-layer"
                className="absolute inset-0 z-[235] bg-transparent"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              ...
              />
              <motion.div
                key={`inspect-${inspectedCard.card.id}-${inspectedCard.source}`}
                className={inspectedCardPlacement}
                initial={{ opacity: 0, y: 24, scale: 1.85, rotate: -2 }}
                animate={{ opacity: 1, y: 0, scale: 2.5, rotate: 0 }}
                exit={{ opacity: 0, y: 18, scale: 1.9, rotate: 2 }}
        ...
        {gameOver && (
          <div className="absolute inset-0 flex items-center justify-center bg-white/70 backdrop-blur-sm">
            <div className="rounded-sm border-2 border-neutral-900 bg-white px-10 py-8 text-center shadow-2xl">
              <Swords className="mx-auto mb-3 h-8 w-8" />
              <p className="text-3xl font-black">对局结束</p>
              <button
                type="button"
                onClick={onRestart}
                className="mt-5 rounded-sm bg-neutral-900 px-6 py-3 text-sm font-bold text-white"
              >
                再来一局
              </button>
            </div>
        ...
        {starStrike && (
          <motion.div
            ...
            initial={{
              ...
              y: starStrike.fromY,
              rotate: 0,
              scale: starStrike.kind === 'support' ? 0.54 : 0.72,
              opacity: 0,
            }}
            animate={{
              x: [starStrike.fromX, starStrike.midX, starStrike.toX],
              y: [starStrike.fromY, starStrike.midY, starStrike.toY],
              rotate: starStrike.kind === 'support' ? [0, 90, 180] : [0, 520, 920],
              scale: starStrike.kind === 'support' ? [0.54, 1.05, 1.18] : [0.72, 1.25, 0.82],
              opacity: starStrike.kind === 'support' ? [0, 0.92, 0.96] : [0, 1, 1],
            }}
            transition={{
              duration: starStrike.kind === 'support' ? 0.86 : 0.62,
              times: [0, 0.55, 1],
              ease: 'easeInOut',
            }}
