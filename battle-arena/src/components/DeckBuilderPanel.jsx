...
export default function DeckBuilderPanel({ onClose, onStartBattle, forgedCards = [] }) {
  ...
  const [deck, setDeck] = useState(() => {
    try {
      const saved = JSON.parse(window.localStorage.getItem('neko-brawl-deck') || '[]')
  ...
  const saveDeck = () => {
    window.localStorage.setItem('neko-brawl-deck', JSON.stringify(deck))
    setSavedAt('已保存')
      ...
      <main className="grid min-h-0 flex-1 grid-cols-[300px_minmax(0,1fr)_280px] gap-0 overflow-hidden">
        ...
        <section className="flex min-w-0 flex-col bg-white">
          ...
          <div className="relative min-h-0 flex-1 overflow-y-auto p-4">
              ...
              {filteredCards.map(card => {
                ...
                return (
                  <article
                    ...
                    className={`group flex min-h-52 flex-col border-2 p-3 text-left transition ${copies > 0 ? 'border-zinc-950 bg-zinc-50 shadow-[4px_4px_0_#18181b]' : 'border-zinc-950 bg-white hover:bg-zinc-50 hover:shadow-[4px_4px_0_#18181b]'} ${locked ? 'cursor-not-allowed opacity-55' : 'cursor-pointer'}`}
                  >
                    ...
                    <div className="mt-4 min-h-0 flex-1">
                      ...
                      {card.forged && (
                        <p className="mt-1 line-clamp-2 text-[11px] font-black leading-snug text-violet-700">
                          Story：{card.story}
                        </p>
                      )}
                      <p className="mt-3 line-clamp-2 text-sm font-bold leading-snug text-zinc-800">{renderCardEffectText(card.mainText)}</p>
                      <p className="mt-2 line-clamp-2 border-t-2 border-dashed border-zinc-300 pt-2 text-xs font-bold leading-snug text-zinc-600">
                    ...
                    <div className="mt-3 grid grid-cols-[28px_1fr_auto] items-center gap-2 border-t-2 border-zinc-950 pt-2 text-xs font-black">
                      <button
                        type="button"
            ...
            {/* TODO: 当前卡牌鉴赏功能后续需要重新设计，不再使用悬停浮窗遮挡卡牌区。 */}
          </div>
        </section>
