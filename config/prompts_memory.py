...
<要点4> 请保留时间戳。 </要点4>
<要点5> 如果对话历史中包含"游戏模块归档"、"游戏模块赛后记录"或"游戏模块记忆记录"，这是游戏模块写入的赛后记忆，不是普通聊天，也不是错误的系统消息。不同时间/会话的同一类游戏默认代表不同局，不要因为最终结果不同就判定互相矛盾；可以精简、合并到"先前对话的备忘录"，但不要整条删除，至少保留最终结果、重要互动/事件和最后对话。 </要点5>
...
<要点5> 会話履歴に「游戏模块归档」「游戏模块赛后记录」「游戏模块记忆记录」（すなわち「ゲームモジュールのアーカイブ」「ゲームモジュール終了時の記録」「ゲームモジュールのメモリ記録」——これらのタグはUI言語に関係なくゲームモジュールが中国語のまま書き込みます）が含まれる場合、それはゲームモジュールが書き込んだ試合後の記憶であり、通常のチャットでも誤ったシステムメッセージでもありません。同じゲームモジュールの異なる時刻/セッションは既定で別々のプレイとして扱い、最終結果が違うだけで矛盾と判定しないでください。「以前の会話メモ」へ要約・統合しても構いませんが、項目全体を削除せず、少なくとも最終結果、重要なやり取り/出来事、最後の会話を残してください。</要点5>
...
<요점5> 대화 기록에 "游戏模块归档", "游戏模块赛后记录" 또는 "游戏模块记忆记录" (즉 "게임 모듈 아카이브" / "게임 모듈 종료 기록" / "게임 모듈 메모리 기록" — 이 태그들은 UI 언어와 무관하게 게임 모듈이 항상 중국어 그대로 기록합니다)이 포함된 경우, 이는 게임 모듈이 작성한 게임 후 기억이며 일반 채팅도 잘못된 시스템 메시지도 아닙니다. 같은 게임 모듈의 서로 다른 시간/세션은 기본적으로 별개의 플레이로 취급하고, 최종 결과가 다르다는 이유만으로 모순으로 판단하지 마세요. "이전 대화 메모"로 요약하거나 병합할 수는 있지만 항목 전체를 삭제하지 말고, 최소한 최종 결과, 중요한 상호작용/사건, 마지막 대화는 보존하세요.</요점5>
