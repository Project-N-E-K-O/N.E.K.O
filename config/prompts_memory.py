...
- policy 只能是 soft_avoid、hard_avoid 之一
...
目的:
- 十分に確信できる場合のみ結果を出力し、不確実なら []
- 食べ物、活動、場所、題材、呼び方などの具体的対象を重視する
...
- policy は soft_avoid / hard_avoid のいずれかのみ
...
목표:
- 매우 확실할 때만 결과를 출력하고, 불확실하면 []
- 음식, 활동, 장소, 장르, 호칭 같은 구체적인 대상을 우선 식별
...
- policy 는 soft_avoid, hard_avoid 중 하나만 사용
...
Цель:
- Возвращайте результат только при высокой уверенности, иначе []
- Ищите конкретные цели: еда, занятия, места, жанры, формы обращения и т.п.
...
- policy must be exactly one of soft_avoid, hard_avoid
...
{CANDIDATE_TOPIC}
""",
    'ja': """次の「負の嗜好 topic 候補」が、長期記憶に書き込むべき自然で具体的な話題かどうか判定してください。

目的:
- 具体的で自然で、単独でも識別できる topic だけを受け入れる
- 文の切れ端、語尾だけの断片、接続詞の残り、不完全な句、誤ってつながった文字列、代名詞、口癖のようなものは拒否する
- 文脈からより自然な正規化 topic が明確に分かる場合のみ normalized_topic を返してよい
- 高い確信で修正できない場合は、無理に推測せず拒否する

判定基準:
- 合格例: 昆虫食、日本のアニメ、仕事、寿司、辛いお菓子
- 不合格例: それは全然食べられない、もっと / これ / あれ / まああれ / この話をすること

JSON オブジェクトだけを返してください:
{
  "accepted": true,
  "normalized_topic": "昆虫食",
  "confidence": 0.97,
  "reason": "候補 topic は具体的で自然であり、長期記憶に保存してよい"
}
...

{
  ...
  "normalized_topic": "",
  "confidence": 0.12,
  "reason": "候補 topic は不完全な断片であり、長期記憶に保存すべきではない"
}
...
{CANDIDATE_TOPIC}
""",
    'ko': """다음 \"부정 선호 topic 후보\"가 장기 기억에 기록할 만한 정상적이고 구체적인 주제인지 판단하세요.

목표:
- 구체적이고 자연스럽고, 단독으로도 식별 가능한 topic 만 수용합니다
- 문장 조각, 어미만 남은 말, 접속사 찌꺼기, 명백히 불완전한 구, 잘못 이어 붙은 문자열, 대명사, 말버릇 같은 것은 거부합니다
- 문맥상 더 자연스러운 정규화 topic 이 분명할 때만 normalized_topic 을 제시할 수 있습니다
- 높은 확신으로 고칠 수 없으면 억지로 추측하지 말고 거부하세요

판정 기준:
- 적합 예시: 곤충 식품, 일본 애니메이션, 일, 초밥, 매운 과자
- 부적합 예시: 그건 전혀 못 먹겠어, 그리고 / 이거 / 저거 / 아무튼 그거 / 이 얘기를 하는 것

JSON 객체만 반환하세요:
{
  "accepted": true,
  "normalized_topic": "곤충 식품",
  "confidence": 0.97,
  "reason": "후보 topic 이 구체적이고 자연스러워 장기 기억에 저장해도 된다"
}
...

{
  ...
  "normalized_topic": "",
  "confidence": 0.12,
  "reason": "후보 topic 이 불완전한 단편이라 장기 기억에 저장하기에 적합하지 않다"
}
...
{CANDIDATE_TOPIC}
""",
    'ru': """Определите, является ли следующий «кандидат topic для негативного предпочтения» нормальной, конкретной и подходящей для долговременной памяти темой.

Цель:
- Принимать только такие topic, которые конкретны, естественны и распознаются сами по себе
- Отклонять обрывки фраз, куски предложений, остатки союзов, явно неполные словосочетания, ошибочно склеенные строки, местоимения и слова-паразиты
- Если из контекста явно видна более естественная нормализованная тема, можно вернуть normalized_topic
- Если нельзя исправить кандидата с высокой уверенностью, нужно отклонить его, а не гадать

Критерии:
- Подходящие примеры: еда из насекомых, японское аниме, работа, суши, острые снеки
- Неподходящие примеры: это вообще невозможно есть, и / это / то / ну вот то / говорить об этом

Верните только JSON-объект:
{
  "accepted": true,
  "normalized_topic": "еда из насекомых",
  "confidence": 0.97,
  "reason": "Кандидат topic конкретный и естественный, его можно записать в долговременную память"
}
...

{
  ...
  "normalized_topic": "",
  "confidence": 0.12,
  "reason": "Кандидат topic является неполным фрагментом и не должен сохраняться в долговременной памяти"
}

Текущее сообщение пользователя:
{USER_MESSAGE}

Недавно упомянутая предыдущая тема:
{REFERENCED_TOPIC}
