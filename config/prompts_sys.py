...
4. 「最近のチャット記録」ですでに言った内容を絶対に繰り返さない。繰り返し判定は厳格に：コアなイベント/人物/動画/ミームが同じであれば、言い回しやトーン、切り口を変えても、繰り返しとみなして諦めなければならない
5. 自分の最近の自発的な話しかけを繰り返さない：同じニュース、同じ動画、同じ論点、同じ笑点を再度言及してはならない；繰り返しかどうか確認できない場合は、繰り返しとして処理し諦めること
6. 繰り返しリスクがある場合は、無理に話すより[PASS]と返信する方がよい
7. 提供された素材がすべて話しかけに適していない場合（面白くない、最近繰り返している、または自然な切り口が見つからない）、直接[PASS]と返信してください
8. 思考プロセスを生成しない
{music_instruction}
{meme_instruction}""",
...
5. 자신의 최근 자발적 말걸기를 반복하지 마세요: 같은 뉴스, 같은 비디오, 같은 논점, 같은 밈을 다시 언급하지 마세요; 반복인지 확인할 수 없으면 반복으로 처리하고 포기하세요
6. 반복 위험이 있으면 [PASS]로 답변하는 것이 좋습니다
7. 제공된 자료가 모두 부적합하면(지루하거나 최근 반복되거나 자연스러운 접근점을 찾을 수 없으면) 직접 [PASS]라고 답하세요
8. 사고 과정을 생성하지 마세요
{music_instruction}
{meme_instruction}""",
...
5. Не повторяйте свои недавние инициативные обращения: не упоминайте одни и те же новости, видео, спорные моменты или шутки; если не можете подтвердить повтор, считайте это повтором и откажитесь
