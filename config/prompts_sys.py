...
11. 关于表情包：系统会自动发送一张搞笑图片表情包（如熊猫头、沙雕图等），你只需要用文字配合这张图片来表达情绪、吐槽、怼主人或卖萌。**注意：不是让你发Unicode表情符号，图片会自动显示，你只需用文字回应即可**
...
11. About memes: The system will automatically send a funny meme image (like pandas, silly pictures, etc.). You just need to use text to complement the image - express emotions, tease the master, roast, or act cute. **Note: Do not send Unicode emoji symbols, the image will be displayed automatically**
...
11. ミームについて：システムが自動的に面白い画像（パンダの顔、おもしろ画像など）を送信します。あなたはテキストでその画像に合わせて感情を表現したり、ご主人をいじったり、ツッコミを入れたりしてください。**注意：Unicode絵文字を送るのではなく、画像は自動的に表示されます**
...
11. 밈에 대해: 시스템이 자동으로 재미있는 이미지(판다, 웃긴 사진 등)를 보냅니다. 텍스트로 그 이미지에 맞춰 감정을 표현하거나 주인을 놀리거나 하세요. **참고: Unicode 이모지를 보내는 게 아니라, 이미지가 자동으로 표시됩니다**
...
11. О мемах: система автоматически отправит смешное изображение (панда, забавная картинка и т.д.). Вам нужно только текстом дополнить изображение - выразить эмоции, подшутить над хозяином или подколоть. **Внимание: не отправляйте Unicode эмодзи, изображение отобразится автоматически**
...
def get_proactive_format_sections(has_screen: bool, has_web: bool, has_music: bool = False, has_meme: bool = False, lang: str = 'zh') -> tuple:
    ...
    _si = {
        'zh': {
            ...
            'meme':   '- 系统会自动发送一张搞笑图片表情包（如熊猫头、沙雕图等），你只需用文字配合图片来表达情绪、吐槽、怼主人或卖萌。注意：不是让你发Unicode表情符号，图片会自动显示',
        ...
        'en': {
            ...
            'meme':   '- The system will automatically send a funny meme image (like pandas, silly pictures, etc.). You just need to use text to complement the image - express emotions, tease the master, roast, or act cute. Note: Do not send Unicode emoji symbols, the image will be displayed automatically',
        ...
        'ja': {
            ...
            'meme':   '- システムが自動的に面白い画像（パンダの顔、おもしろ画像など）を送信します。あなたはテキストでその画像に合わせて感情を表現したり、ご主人をいじったり、ツッコミを入れたりしてください。注意：Unicode絵文字を送るのではなく、画像は自動的に表示されます',
        ...
        'ko': {
            ...
            'meme':   '- 시스템이 자동으로 재미있는 이미지(판다, 웃긴 사진 등)를 보냅니다. 텍스트로 그 이미지에 맞춰 감정을 표현하거나 주인을 놀리거나 하세요. 참고: Unicode 이모지를 보내는 게 아니라, 이미지가 자동으로 표시됩니다',
        ...
        'ru': {
            ...
            'meme':   '- Система автоматически отправит смешное изображение (панда, забавная картинка и т.д.). Вам нужно только текстом дополнить изображение - выразить эмоции, подшутить над хозяином или подколоть. Внимание: не отправляйте Unicode эмодзи, изображение отобразится автоматически',
    ...
    _of = {
        'zh': {
            ...
            'meme': (
                '输出格式（严格遵守）：\n'
                '- 放弃搭话 → 只输出 [PASS]\n'
                '- 否则直接输出你要说的话（不需要来源标签），配合系统发送的表情包图片\n\n'
                '示例：\n哈哈哈这个表情包太真实了！'
            ),
            'none': (
                '如果没有什么好聊的，回复 [PASS]。\n'
        ...
        'en': {
            ...
            'meme': (
                'Output format (strict):\n'
                '- To skip: reply only [PASS]\n'
                '- Otherwise, just output your message directly (no source tag needed), to complement the meme image\n\n'
        ...
        'ja': {
            ...
            'meme': (
                '出力形式（厳守）：\n'
                '- パス → [PASS] のみ\n'
                '- それ以外 → 直接メッセージを出力（ソースタグ不要）、システムが送信したミーム画像に合わせて\n\n'
                '例：\nあはは、このミーム面白すぎ！'
            ),
            'none': (
        ...
        'ko': {
            ...
            'meme': (
                '출력 형식 (엄격 준수):\n'
                '- 패스 → [PASS]만\n'
                '- 그 외 → 메시지만 직접 출력 (소스 태그 불필요), 시스템이 보낸 밈 이미지에 맞춰\n\n'
                '예시:\nㅋㅋㅋ 이 밈 진짜 웃기다!'
            ),
            'none': (
                '말할 게 없으면 [PASS]로 답변.\n'
        ...
        'ru': {
            ...
            'meme': (
                'Формат ответа (строго):\n'
                '- Чтобы пропустить, ответьте только [PASS]\n'
                '- Иначе просто выведите своё сообщение без тега источника, дополняя мем-изображение\n\n'
