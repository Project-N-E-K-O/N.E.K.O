...
_ZH_IDENT_PUNCT = (
    ...
    # 提过），所以判的是**缩写词的形状**——词首大写、总共 1~3 个字母：
    ...
    r"|(?<![A-Za-z](?-i:[A-Z]))(?<=(?-i:[A-Z]))\.(?=" + _ZH_HSPACE_ONE + r"(?-i:[A-Z]))"
    r"|(?<![A-Za-z](?-i:[A-Z])(?-i:[a-z]))(?<=(?-i:[A-Z])(?-i:[a-z]))\.(?=" + _ZH_HSPACE_ONE + r"(?-i:[A-Z]))"
    r"|(?<![A-Za-z](?-i:[A-Z])(?-i:[a-z]{2}))(?<=(?-i:[A-Z])(?-i:[a-z]{2}))\.(?=" + _ZH_HSPACE_ONE + r"(?-i:[A-Z]))"
    r"|(?<=[^\s，。！？；,.!?;]),(?=" + _ZH_HSPACE_ONE + r"?[^\s，。！？；,.!?;])"
...
_JA_GRAMMAR_RE = re.compile(
    "|".join((
        ...
        f"(?<=[一-鿿])(?:する|した|して|したい|しろ|せよ)(?={_JA_SENTENCE_END})",
        ...
        f"(?<=[一-鿿])(?:か|も|ね|よ|わ|な|さ|ぞ|ぜ|じゃん|かな|かも|だろ)"
        f"(?={_JA_SENTENCE_END})",
    ))
)
