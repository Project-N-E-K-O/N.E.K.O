...
_LOCAL_AVATAR_TOOL_MEMORY_TEMPLATES = {
    "zh": "[{master}用自定义道具“{name}”与你互动]",
    "zh-TW": "[{master}用自訂道具「{name}」與你互動]",
    "en": "[{master} interacted with you using the custom tool “{name}”]",
    "ja": "[{master}がカスタム道具「{name}」であなたと交流した]",
    "ko": "[{master}이 사용자 지정 도구 “{name}”로 너와 상호작용했다]",
    "ru": "[{master} взаимодействовал с тобой пользовательским предметом «{name}»]",
    "es": "[{master} interactuó contigo usando la herramienta personalizada «{name}»]",
    "pt": "[{master} interagiu com você usando a ferramenta personalizada “{name}”]",
}
...
def _build_avatar_interaction_instruction(
    ...
    """Build one interaction instruction; local tools use the selected user text."""
    tool_id = payload["tool_id"]
    if _LOCAL_AVATAR_TOOL_ID_PATTERN.fullmatch(tool_id):
        if not local_record:
            raise ValueError("Missing local avatar tool record")
        _require_avatar_interaction_facts(tool_id, "interact", payload)
        return local_record["meaning"]
