...
[重要]保留用户的负面反馈（高价值信号）：
- 用户明确表达"别再提 X / 不要做 Y / 不想聊 Z"这类**祈使句**时，必须原样写入摘要
...
[Important] Preserve user negative feedback verbatim (high-value signal):
- When the user explicitly says "don't mention X / stop talking about Y / I don't want Z" (imperative form), record it as-is in the summary
- Do NOT compress, paraphrase, or merge these — keep them literal (e.g., "User explicitly asked: don't bring up overtime")
...
[重要] ユーザーのネガティブフィードバック（高価値シグナル）は原文どおり保持してください：
- ユーザーが「その話はやめて / もう聞きたくない / 〇〇しないで」のような**命令形**で明示した場合、要約にそのまま書き留めること
- 圧縮・言い換え・統合は禁止——逐語で記録（例：「ユーザーは明確に要求：残業の話はもうしないで」）
...
[중요] 사용자의 부정적 피드백(고가치 신호)을 원문 그대로 보존하세요:
- 사용자가 "그 얘기는 그만 / 다시는 말하지 마 / X 하지 마"와 같은 **명령형**으로 명확히 표현하면, 요약에 그대로 기록하세요
- 압축, 의역, 병합 금지 — 문자 그대로 기록(예: "사용자가 명시적으로 요청: 야근 이야기는 더 이상 꺼내지 마세요")
...
[Важно] Сохраняйте негативную обратную связь пользователя дословно (высокоценный сигнал):
- Когда пользователь явно говорит "не упоминай X / хватит об Y / я не хочу Z" (повелительная форма), запишите это как есть в резюме
- НЕ сжимайте, не перефразируйте и не объединяйте — фиксируйте буквально (например, «Пользователь явно попросил: не поднимать тему переработок»)
...
[Importante] Preserva la retroalimentación negativa del usuario textualmente (señal de alto valor):
- Cuando el usuario diga explícitamente "no menciones X / deja de hablar de Y / no quiero Z" (forma imperativa), registra esto tal cual en el resumen
- NO comprimas, parafrasees ni fusiones — mantén la literalidad (p. ej., "El usuario pidió explícitamente: no traer a colación las horas extra")
...
[Importante] Preserve o feedback negativo do usuário literalmente (sinal de alto valor):
- Quando o usuário disser explicitamente "não mencione X / pare de falar de Y / não quero Z" (forma imperativa), registre isso como está no resumo
- NÃO comprima, parafraseie nem mescle — mantenha o texto literal (p. ex., "O usuário pediu explicitamente: não trazer à tona horas extras")
...
[重要]不要删除或合并用户的负面反馈（"别再提 X / 不要再做 Y / 不想聊 Z" 等祈使句）——这些是高价值信号，下游记忆系统据此避免再次触雷。即使在你看来"冗余"或"重复"，也必须原样保留。
...
[重要] ユーザーのネガティブフィードバック（「その話はやめて／〇〇しないで／もう聞きたくない」のような命令文）を削除・統合しないでください——これらは高価値シグナルで、後続の記憶システムはこれを頼りに再度の地雷踏みを避けます。あなたから見て「冗長」「重複」に見えても、原文どおり保持してください。
...
[중요] 사용자의 부정적 피드백("그 얘기는 그만 / 다시는 X 하지 마 / Y 듣고 싶지 않아" 같은 명령형)을 삭제하거나 병합하지 마세요 — 이는 고가치 신호로, 다운스트림 메모리 시스템이 이를 통해 재차 지뢰를 피합니다. 당신이 보기에 "중복" 또는 "반복"으로 보이더라도 원문 그대로 보존하세요.
...
[Importante] NO elimines ni fusiones la retroalimentación negativa del usuario (declaraciones imperativas como "no menciones X / deja de hacer Y / no quiero oír Z") — son señales de alto valor; el sistema de memoria aguas abajo depende de ellas para evitar volver a tropezar. Manténlas textualmente aunque te parezcan "redundantes" o "repetitivas".
