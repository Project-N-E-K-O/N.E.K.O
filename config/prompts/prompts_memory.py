...
- 若事实里没有任何时间线索，整段 event_when 写 null（系统会兜底为创建时刻）
...
Based on these facts, distill one higher-level reflective insight. Follow these four steps:
...
- If facts contain no time cue at all, write event_when as null (system falls back to creation time)
...
これらの事実に基づき、より高次元の反省的洞察を 1 つ抽出してください。次の 4 ステップで進めてください：
...
- 事実に時間の手掛かりが一切ない場合は event_when 全体を null にする（システムが作成時刻でフォールバック）
...
이 사실들을 바탕으로 더 높은 차원의 반성적 통찰 하나를 도출해 주세요. 다음 네 단계를 따르세요:
...
- 사실에 시간 단서가 전혀 없으면 event_when 전체를 null로 (시스템이 생성 시각으로 폴백)
...
На основе этих фактов выведите одно рефлексивное наблюдение более высокого уровня. Выполните четыре шага:
...
- Если фактах нет временных меток, всё event_when = null (система подставит время создания)
...
Con base en estos hechos, destila una sola reflexión de nivel superior. Sigue estos cuatro pasos:
...
- Si los hechos no contienen ninguna pista temporal, escribe event_when como null (el sistema usa el tiempo de creación)
...
Com base nesses fatos, extraia uma única reflexão de nível superior. Siga estes quatro passos:
...
- Se os fatos não tiverem nenhuma pista temporal, escreva event_when como null (o sistema usa o horário de criação)
