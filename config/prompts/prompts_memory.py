...
RECALL_MEMORY_TOOL_TIME_DESCRIPTION = {
    "zh": "可选。按时间回溯：填入日期就返回离那段时间最近的若干条记忆（事实和印象都包含），忽略 query 的语义匹配，也无需填 query（适合\"那天/那周发生了什么\"）。支持单日 2026-05-01、整月 2026-05、整年 2026，或日期区间 2026-05-01/2026-05-07。若同时填了 query，则 time 优先、query 被忽略（不会做\"语义+时间\"联合检索）。不填则按 query 做语义检索。",
    "en": "Optional. Time lookup: pass a date to return the memories closest to that period (both facts and impressions), ignoring semantic matching and not requiring query (good for \"what happened that day/week\"). Accepts a single day 2026-05-01, a month 2026-05, a year 2026, or a range 2026-05-01/2026-05-07. If query is also given, time takes precedence and query is ignored (no combined \"semantic + time\" search). Leave empty for normal semantic recall by query.",
    "ja": "任意。時間でさかのぼる：日付を入れると、その期間に最も近い記憶（事実も印象も含む）を返します。queryの意味的一致は無視し、queryも不要です（「その日/その週に何があったか」向け）。単日 2026-05-01、月 2026-05、年 2026、または期間 2026-05-01/2026-05-07 に対応。queryも同時に指定した場合は time が優先され query は無視されます（「意味＋時間」の複合検索にはなりません）。空欄なら query による通常の意味検索になります。",
    "ko": "선택. 시간 거슬러보기: 날짜를 넣으면 그 기간에 가장 가까운 기억(사실과 인상 모두 포함)을 반환합니다. query 의미 매칭은 무시하며 query도 필요 없습니다(\"그날/그 주에 무슨 일이 있었나\"에 적합). 단일 날짜 2026-05-01, 월 2026-05, 연 2026, 또는 기간 2026-05-01/2026-05-07 지원. query도 함께 주면 time이 우선하고 query는 무시됩니다(\"의미+시간\" 결합 검색은 하지 않음). 비워두면 query 기반 일반 의미 검색.",
    "ru": "Необязательно. Поиск по времени: укажите дату, чтобы вернуть воспоминания, ближайшие к этому периоду (и факты, и впечатления), игнорируя семантическое совпадение и не требуя query (удобно для «что было в тот день/неделю»). Принимает один день 2026-05-01, месяц 2026-05, год 2026 или диапазон 2026-05-01/2026-05-07. Если query тоже задан, приоритет у time, а query игнорируется (совмещённого поиска «семантика + время» нет). Оставьте пустым для обычного семантического поиска по query.",
    "es": "Opcional. Búsqueda por tiempo: indica una fecha para devolver los recuerdos más cercanos a ese periodo (tanto hechos como impresiones), ignorando la coincidencia semántica y sin requerir query (útil para \"qué pasó ese día/semana\"). Acepta un día 2026-05-01, un mes 2026-05, un año 2026 o un rango 2026-05-01/2026-05-07. Si también se indica query, time tiene prioridad y query se ignora (no hay búsqueda combinada \"semántica + tiempo\"). Déjalo vacío para la búsqueda semántica normal por query.",
    "pt": "Opcional. Busca por tempo: informe uma data para retornar as memórias mais próximas daquele período (tanto fatos quanto impressões), ignorando a correspondência semântica e sem exigir query (útil para \"o que aconteceu naquele dia/semana\"). Aceita um dia 2026-05-01, um mês 2026-05, um ano 2026 ou um intervalo 2026-05-01/2026-05-07. Se query também for informado, time tem prioridade e query é ignorado (não há busca combinada \"semântica + tempo\"). Deixe vazio para a busca semântica normal por query.",
}
