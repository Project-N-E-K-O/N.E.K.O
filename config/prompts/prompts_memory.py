...
RECALL_MEMORY_TOOL_TIME_DESCRIPTION = {
    "zh": "可选。把检索限定在某个时间段。只填 time（不填 query）就返回离那段时间最近的若干条记忆（事实和印象都包含，适合\"那天/那周发生了什么\"）；同时填 query 则做\"语义+时间\"联合检索——在该时间段内按 query 语义找相关记忆（适合\"五月聊过的旅行计划\"）。支持单日 2026-05-01、整月 2026-05、整年 2026，或日期区间 2026-05-01/2026-05-07。不填则按 query 做全量语义检索。",
    "en": "Optional. Restrict recall to a time period. With time only (no query) it returns the memories closest to that period (both facts and impressions — good for \"what happened that day/week\"). With both query and time it runs a combined \"semantic + time\" search — finds memories relevant to query within that period (good for \"the travel plan discussed in May\"). Accepts a single day 2026-05-01, a month 2026-05, a year 2026, or a range 2026-05-01/2026-05-07. Leave empty for full semantic recall by query.",
    "ja": "任意。検索をある期間に限定します。time だけ（query なし）なら、その期間に最も近い記憶（事実も印象も含む。「その日/その週に何があったか」向け）を返します。query と time の両方を指定すると「意味＋時間」の複合検索になり、その期間内で query に関連する記憶を探します（「5月に話した旅行計画」向け）。単日 2026-05-01、月 2026-05、年 2026、または期間 2026-05-01/2026-05-07 に対応。空欄なら query による全件の意味検索になります。",
    "ko": "선택. 검색을 특정 기간으로 제한합니다. time만 주면(query 없이) 그 기간에 가장 가까운 기억(사실과 인상 모두 포함, \"그날/그 주에 무슨 일이 있었나\"에 적합)을 반환합니다. query와 time을 함께 주면 \"의미+시간\" 결합 검색으로, 그 기간 안에서 query에 관련된 기억을 찾습니다(\"5월에 얘기한 여행 계획\"에 적합). 단일 날짜 2026-05-01, 월 2026-05, 연 2026, 또는 기간 2026-05-01/2026-05-07 지원. 비워두면 query 기반 전체 의미 검색.",
    "ru": "Необязательно. Ограничивает поиск периодом времени. Если задан только time (без query), возвращает воспоминания, ближайшие к этому периоду (и факты, и впечатления — удобно для «что было в тот день/неделю»). Если заданы и query, и time, выполняется совмещённый поиск «семантика + время» — ищет воспоминания, релевантные query, в пределах периода (удобно для «план поездки, обсуждавшийся в мае»). Принимает один день 2026-05-01, месяц 2026-05, год 2026 или диапазон 2026-05-01/2026-05-07. Оставьте пустым для полного семантического поиска по query.",
    "es": "Opcional. Limita la búsqueda a un periodo. Con solo time (sin query) devuelve los recuerdos más cercanos a ese periodo (hechos e impresiones — útil para \"qué pasó ese día/semana\"). Con query y time juntos hace una búsqueda combinada \"semántica + tiempo\": encuentra recuerdos relevantes a query dentro de ese periodo (útil para \"el plan de viaje hablado en mayo\"). Acepta un día 2026-05-01, un mes 2026-05, un año 2026 o un rango 2026-05-01/2026-05-07. Déjalo vacío para la búsqueda semántica completa por query.",
    "pt": "Opcional. Restringe a busca a um período. Com apenas time (sem query) retorna as memórias mais próximas daquele período (fatos e impressões — útil para \"o que aconteceu naquele dia/semana\"). Com query e time juntos faz uma busca combinada \"semântica + tempo\": encontra memórias relevantes para query dentro do período (útil para \"o plano de viagem conversado em maio\"). Aceita um dia 2026-05-01, um mês 2026-05, um ano 2026 ou um intervalo 2026-05-01/2026-05-07. Deixe vazio para a busca semântica completa por query.",
}
