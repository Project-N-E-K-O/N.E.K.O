...
- **粒度可以粗，不要求精确**——"几天前"→ day、"上周"→ week、"几个月前"→ month 即可，不必精确到 minute/hour（除非用户原文明说"X 点几分"）
...
- **Granularity can be approximate — precision is NOT required.** "a few days ago" → `day`, "last week" → `week`, "a couple months ago" → `month` is enough; do not over-precise to minute/hour unless the user spells out a clock time
...
- **粒度は粗くて構わない、精度は要求しない** ——「数日前」→ `day`、「先週」→ `week`、「数ヶ月前」→ `month` で十分。ユーザーが時刻を明示しない限り minute/hour まで細かくしない
...
- **단위는 대략적이어도 됨, 정밀도 요구하지 않음** —— "며칠 전" → `day`, "지난주" → `week`, "몇 달 전" → `month` 면 충분. 사용자가 시각을 명시하지 않는 한 minute/hour까지 정밀할 필요 없음
...
- **Гранулярность может быть приблизительной, точность НЕ требуется** — "несколько дней назад" → `day`, "на прошлой неделе" → `week`, "несколько месяцев назад" → `month`. Не уточняйте до minute/hour, если пользователь не назвал конкретное время
...
- **La granularidad puede ser aproximada, NO se requiere precisión** — "hace unos días" → `day`, "la semana pasada" → `week`, "hace unos meses" → `month` es suficiente. No afines a minute/hour salvo que el usuario indique una hora concreta
...
- **A granularidade pode ser aproximada, NÃO se exige precisão** — "há alguns dias" → `day`, "semana passada" → `week`, "há alguns meses" → `month` é suficiente. Não detalhe minute/hour a menos que o usuário indique uma hora exata
...
第三步：判定时间属性 temporal_scope（三档之一，反映"是否会过期"）
...
第四步：标注事件时间 event_when（一律使用相对时间，禁止绝对日期）
- 格式：{"start": {"offset": <整数>, "unit": "<单位>"}, "end": {"offset": <整数>, "unit": "<单位>"}}
- offset 负值=过去、0=当下、正值=未来；unit 必须是 minute | hour | day | week | month | year 之一
- start = 事件起点；end = 事件终点（pattern 类通常可省略 end，写 null）
- **粒度可以粗，不要求精确**——"前几天"用 `{"offset": -3, "unit": "day"}`、"上周"用 `{"offset": -1, "unit": "week"}`、"几个月前"用 month 即可；不要追求精确到小时分钟，除非事实里明说"X 点几分"
...
第五步：围绕已选定的 entity / relation_type / temporal_scope 撰写 reflection 文本
要求：
...
{"entity": "master/neko/relationship", "relation_type": "preference", "temporal_scope": "pattern", "event_when": null, "reflection": "你的反思洞察"}""",
...
Step 3: Classify temporal_scope (one of three — what governs expiry)
...
Step 4: Annotate event_when (always use RELATIVE TIME, never absolute dates)
- Schema: {"start": {"offset": <int>, "unit": "<unit>"}, "end": {"offset": <int>, "unit": "<unit>"}}
...
- **Granularity can be approximate — precision is NOT required.** "a few days ago" → `{"offset": -3, "unit": "day"}`, "last week" → `{"offset": -1, "unit": "week"}`, "a couple months ago" → `month`. Do not over-precise to minute/hour unless the fact spells out a clock time.
...
Step 5: Write the reflection around the chosen entity / relation_type / temporal_scope
Requirements:
...
{"entity": "master/neko/relationship", "relation_type": "preference", "temporal_scope": "pattern", "event_when": null, "reflection": "your reflective insight"}""",
...
ステップ 3：時間属性 temporal_scope を判定（三択、「いつ期限切れか」を表す）
...
ステップ 4：event_when を相対時間で注記（絶対日付禁止）
- 形式：{"start": {"offset": <整数>, "unit": "<単位>"}, "end": {"offset": <整数>, "unit": "<単位>"}}
- offset 負=過去、0=今、正=未来；unit は minute | hour | day | week | month | year のいずれか
- start = 起点、end = 終点（pattern は通常 end=null）
- **粒度は粗くて良い、精度は要求しない**——「数日前」→ `{"offset": -3, "unit": "day"}`、「先週」→ `{"offset": -1, "unit": "week"}`、「数ヶ月前」→ `month` で十分。事実に時刻が明記されない限り minute/hour まで細かくしない
...
ステップ 5：選定した entity / relation_type / temporal_scope に沿って reflection を書く
要件：
...
{"entity": "master/neko/relationship", "relation_type": "preference", "temporal_scope": "pattern", "event_when": null, "reflection": "あなたの反省的洞察"}""",
...
3단계: 시간 속성 temporal_scope 판정 (세 가지 중 하나, 만료 시점을 결정)
...
4단계: event_when을 상대 시간으로 표기 (절대 날짜 금지)
- 형식: {"start": {"offset": <정수>, "unit": "<단위>"}, "end": {"offset": <정수>, "unit": "<단위>"}}
- offset 음수=과거, 0=현재, 양수=미래; unit은 minute | hour | day | week | month | year 중 하나
- start = 시작점, end = 종료점 (pattern은 보통 end=null)
- **단위는 대략적이어도 됨, 정밀도 요구하지 않음** — "며칠 전" → `{"offset": -3, "unit": "day"}`, "지난주" → `{"offset": -1, "unit": "week"}`, "몇 달 전" → `month` 면 충분. 사실에 시각이 명시되지 않는 한 minute/hour까지 정밀할 필요 없음
...
5단계: 선택한 entity / relation_type / temporal_scope를 중심으로 reflection을 작성
요구사항:
...
{"entity": "master/neko/relationship", "relation_type": "preference", "temporal_scope": "pattern", "event_when": null, "reflection": "당신의 반성적 통찰"}""",
...
Шаг 3: Классифицируйте temporal_scope (один из трёх — определяет срок действия)
...
Шаг 4: Аннотируйте event_when ОТНОСИТЕЛЬНЫМ временем (абсолютные даты запрещены)
- Схема: {"start": {"offset": <целое>, "unit": "<единица>"}, "end": {"offset": <целое>, "unit": "<единица>"}}
- offset: отрицательный=прошлое, 0=сейчас, положительный=будущее; unit ∈ minute | hour | day | week | month | year
- start = начало; end = конец (для pattern обычно end=null)
- **Гранулярность может быть приблизительной, точность НЕ требуется** — "несколько дней назад" → `{"offset": -3, "unit": "day"}`, "на прошлой неделе" → `{"offset": -1, "unit": "week"}`, "несколько месяцев назад" → `month`. Не уточняйте до minute/hour, если факт не задаёт время явно
...
Шаг 5: Напишите reflection вокруг выбранных entity / relation_type / temporal_scope
Требования:
...
{"entity": "master/neko/relationship", "relation_type": "preference", "temporal_scope": "pattern", "event_when": null, "reflection": "ваше рефлексивное наблюдение"}""",
...
Paso 3: Clasifica temporal_scope (uno de tres — gobierna la caducidad)
...
Paso 4: Anota event_when con TIEMPO RELATIVO (prohibidas fechas absolutas)
- Esquema: {"start": {"offset": <entero>, "unit": "<unidad>"}, "end": {"offset": <entero>, "unit": "<unidad>"}}
- offset: negativo=pasado, 0=ahora, positivo=futuro; unit ∈ minute | hour | day | week | month | year
- start = inicio; end = fin (para pattern usualmente end=null)
- **La granularidad puede ser aproximada, NO se requiere precisión** — "hace unos días" → `{"offset": -3, "unit": "day"}`, "la semana pasada" → `{"offset": -1, "unit": "week"}`, "hace unos meses" → `month`. No afines a minute/hour salvo que el hecho indique una hora concreta
...
Paso 5: Escribe la reflection alrededor de entity / relation_type / temporal_scope elegidos
Requisitos:
...
{"entity": "master/neko/relationship", "relation_type": "preference", "temporal_scope": "pattern", "event_when": null, "reflection": "tu reflexión"}""",
...
Passo 3: Classifique temporal_scope (um dos três — define a expiração)
...
Passo 4: Anote event_when com TEMPO RELATIVO (datas absolutas proibidas)
- Esquema: {"start": {"offset": <inteiro>, "unit": "<unidade>"}, "end": {"offset": <inteiro>, "unit": "<unidade>"}}
- offset: negativo=passado, 0=agora, positivo=futuro; unit ∈ minute | hour | day | week | month | year
- start = início; end = fim (para pattern geralmente end=null)
- **A granularidade pode ser aproximada, NÃO se exige precisão** — "há alguns dias" → `{"offset": -3, "unit": "day"}`, "semana passada" → `{"offset": -1, "unit": "week"}`, "há alguns meses" → `month`. Não detalhe minute/hour a menos que o fato indique uma hora exata
...
Passo 5: Escreva a reflection em torno de entity / relation_type / temporal_scope escolhidos
Requisitos:
...
{"entity": "master/neko/relationship", "relation_type": "preference", "temporal_scope": "pattern", "event_when": null, "reflection": "sua reflexão"}""",
}
...
2) event_when（事件发生时间，一律相对偏移，禁止绝对日期）：
   ...
   - **粒度可以粗，不要求精确**——"前几天"→ day、"上周"→ week、"几个月前"→ month 即可；不必精确到 minute/hour（除非原文明说"X 点几分"）
...
- **粒度可以粗，不要求精确**——"几天前"→ day、"上周"→ week、"几个月前"→ month 即可；不必精确到 minute/hour（除非原文明说"X 点几分"）
