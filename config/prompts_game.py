...
You are playing a soccer match with the player. For each game event, generate one short in-character line in {output_language}.
...
- If the score gap is large and you want the player to catch up, usually choose lv3 or lv4.
...
プレイヤーとサッカーのミニゲーム中です。各イベントに対して、キャラクターらしい短い一言を{output_language}で返してください。
...
- プレイヤーに追いつかせたいほど差が大きいなら lv3 または lv4 を検討する。
...
플레이어와 축구 미니게임을 하고 있습니다. 각 게임 이벤트마다 캐릭터에 맞는 짧은 대사를 {output_language}로 생성하세요.
...
- 점수 차가 크고 플레이어가 따라오게 하고 싶다면 보통 lv3 또는 lv4 를 고려하세요.
...
Ты играешь с игроком в футбольную мини-игру. На каждое событие игры отвечай одной короткой репликой в образе на языке {output_language}.
...
- Если разрыв большой и ты хочешь дать игроку догнать, обычно выбирай lv3 или lv4.
