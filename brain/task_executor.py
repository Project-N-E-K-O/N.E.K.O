...
class DirectTaskExecutor:
    ...
    @staticmethod
    def _normalize_galgame_control_text(value: Any) -> str:
    ...
    @classmethod
    def _match_galgame_continue_command(cls, value: Any) -> bool:
        return cls._normalize_galgame_control_text(value) in {
            "继续推动剧情",
            "继续推进剧情",
            "继续推进",
            "继续剧情",
    ...
    @classmethod
    def _match_galgame_auto_mode_command(cls, value: Any) -> bool:
        return cls._normalize_galgame_control_text(value) in {
            "开启自动推进",
            "开启自动推进模式",
            "切到自动推进模式",
    ...
    @classmethod
    def _match_galgame_companion_mode_command(cls, value: Any) -> bool:
        return cls._normalize_galgame_control_text(value) in {
            "停止自动推进",
            "不要自动点",
            "只陪我看",
    ...
    @staticmethod
    def _plugin_has_entry(plugins: Any, plugin_id: str, entry_id: str) -> bool:
        try:
            iterable = plugins.values() if isinstance(plugins, dict) else plugins
            for plugin in iterable or []:
                pid = plugin.get("id") if isinstance(plugin, dict) else getattr(plugin, "id", None)
                if pid != plugin_id:
                    continue
                entries = plugin.get("entries", []) if isinstance(plugin, dict) else getattr(plugin, "entries", []) or []
    ...
    def _route_galgame_control_task(
        self,
        *,
        task_id: str,
        plugins: Any,
        ...
        if (
            self._match_galgame_continue_command(latest_user_request)
            and self._plugin_has_entry(plugins, plugin_id, "galgame_continue_auto_advance")
        ):
            logger.info(
                "[TaskExecutor] Using deterministic galgame control route: plugin_id=%s, entry_id=%s, reason=%s",
                plugin_id,
                "galgame_continue_auto_advance",
                "galgame_control_continue_auto_advance_rule",
            )
            return TaskResult(
                task_id=task_id,
                has_task=True,
                ...
                tool_args={"message": latest_user_request.strip()},
                reason="galgame_control_continue_auto_advance_rule",
                latest_user_request=latest_user_request,
                normalized_intent=normalized_intent,
                recent_context=recent_context,
            )
        if not self._plugin_has_entry(plugins, plugin_id, "galgame_set_mode"):
            return None
        if self._match_galgame_auto_mode_command(latest_user_request):
            logger.info(
                "[TaskExecutor] Using deterministic galgame control route: plugin_id=%s, entry_id=%s, reason=%s",
                plugin_id,
                "galgame_set_mode",
                "galgame_control_auto_mode_rule",
            )
            return TaskResult(
                task_id=task_id,
                has_task=True,
                ...
                tool_args={"mode": "choice_advisor", "push_notifications": True},
                reason="galgame_control_auto_mode_rule",
                latest_user_request=latest_user_request,
                normalized_intent=normalized_intent,
                recent_context=recent_context,
            )
        if self._match_galgame_companion_mode_command(latest_user_request):
            logger.info(
                "[TaskExecutor] Using deterministic galgame control route: plugin_id=%s, entry_id=%s, reason=%s",
                plugin_id,
                "galgame_set_mode",
                "galgame_control_companion_mode_rule",
            )
            return TaskResult(
                task_id=task_id,
                has_task=True,
                ...
                tool_name=plugin_id,
                entry_id="galgame_set_mode",
                tool_args={"mode": "companion", "push_notifications": True},
                reason="galgame_control_companion_mode_rule",
                latest_user_request=latest_user_request,
                normalized_intent=normalized_intent,
                recent_context=recent_context,
            )
    ...
    async def analyze_and_execute(
        ...
        if user_plugin_enabled and plugins:
            galgame_control_result = self._route_galgame_control_task(
                task_id=task_id,
                plugins=plugins,
                latest_user_request=latest_user_request,
                normalized_intent=normalized_intent,
            ...
            if galgame_control_result is not None:
                return galgame_control_result
        if user_plugin_enabled and plugins:
