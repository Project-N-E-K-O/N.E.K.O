...
| モデル名、呼び出し種別（`conversation` / `memory` 等） | ユーザー名・ユーザー ID・API キー・GitHub ID |
| 呼び出し回数 / エラー回数 | IP アドレス・位置情報・MAC・ハードウェアシリアル |
| 仮名化デバイス識別子 `SHA256(machine_id + インストールパス)`（一方向ハッシュ、不可逆） | 個人を特定できる PII 全般 |

> **仮名化デバイス識別子について**：OS の machine-id とインストールパスを連結した一方向 SHA-256 です。不可逆かつユーザー情報を含まず、個人特定には使えません。ただし同一マシン + 同一インストールでは同じ識別子が再生成されるため、GDPR / PIPL 等の分類では「完全な匿名データ」ではなく**仮名化識別子**に該当します。重複排除した DAU 集計とバージョン互換性の帰属分析のみに使用します。
