...
| 匿名デバイス指紋 `SHA256(machine_id + salt)`（不可逆） | 個人を特定できる PII 全般 |

実装とプロトコルの詳細は [`utils/token_tracker.py`](https://github.com/Project-N-E-K-O/N.E.K.O/blob/main/utils/token_tracker.py) および [`local_server/telemetry_server/README.md`](https://github.com/Project-N-E-K-O/N.E.K.O/blob/main/local_server/telemetry_server/README.md) を参照：HMAC-SHA256 署名・±5 分のリプレイ防止ウィンドウ・スライディングウィンドウ制限（120 req/h/device）・追記専用ストレージ。各サーバープロセスは最大 10 分に 1 回の送信で、メイン処理には影響しません。
