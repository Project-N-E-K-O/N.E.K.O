...
The session manager (`main_logic/core.py`) obtains this service via `_get_translation_service()` and translates text with the service's `translate_text_robust()` method when the user's language differs from the character's language.
