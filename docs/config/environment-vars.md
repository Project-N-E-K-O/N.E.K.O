...
| `NEKO_RUNTIME_STATE_DIR` | per-user runtime dir | Overrides where `launcher.lock` and `launcher.json` live. Defaults to `%LOCALAPPDATA%\N.E.K.O\runtime` on Windows, `~/Library/Application Support/N.E.K.O/runtime` on macOS, and `$XDG_RUNTIME_DIR/neko` on Linux. The override is used verbatim — no per-user suffix is appended and permissions are not checked — so it must point somewhere private to one user. A directory shared between users breaks the single-instance proof: on Windows two users contend for one lock, and on POSIX the second user cannot open the first user's lock file and starts with no uniqueness proof at all. |
