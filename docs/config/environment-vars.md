...
| `NEKO_PLUGIN_HOST_API_TOKEN` | Internal host API credential; generated by the launcher. Diagnostic split mode must provide the same non-empty value to `main_server` and `agent_server`. |
