# RNNoise / Silero 语音存在检测预备报告（2026-07-19，07-20 补充独立 holdout）
...
- RNNoise 100ms 结果说明“空闲阶段只用 RNNoise 做预热证据”值得继续 shadow；它不构成删除 Silero 的生产证据。07-20 补充的 source-group holdout 没有复用 calibration 原始语音源，结果良好，但仍未测试真实房间、电视、回声、远场麦克风和重叠说话。
...
- 替代或覆盖当前路由的 endpoint authority；
- 让分段式 Provider 的物理响应绕过 SmartTurn，或让流式 Provider 额外加载 SmartTurn；
- 把 PCM 送入 Core、Omni、小游戏或外部 consumer；
- 绕过 MicLease、ingress token、abort barrier 或有序 Activate/Seal；
- 在分段式路径 SmartTurn 未 READY 时产生 Provider wire audio。

独立 ASR 启用后的所有路径仍要求 `omni_mic_audio_bytes == 0`；用户关闭独立 ASR 时保留 Core/Omni 原生语音路径。
...
- 初始探索点使用同一份语料选阈值并汇报结果；07-20 已补 source-group holdout，但真实设备 holdout 仍为空。
...
### 07-20 独立 calibration / holdout

拆分以“原始来源”为最小单元：同一个教程 TTS 生成的 clean、20/10/5/0/-5dB 六个变体必须全部位于同一集合，避免同源语音泄漏。负样本按独立合成片段或独立音效来源分组；拆分种子仍为 `2398`。
...
两组原始来源交集为 0。阈值只在 calibration 上按“balanced accuracy → recall → specificity → 较低阈值”顺序选择，最终仍选择 RNNoise 严格连续 100ms、阈值 0.7。

| 集合 | Speech recall | 负样本误触发 | Balanced accuracy | onset median / p95 |
|---|---:|---:|---:|---:|
| Calibration | 97.22%（175/180） | 0%（0/50） | 98.61% | 未作为选点条件 |
| Holdout | 100%（60/60） | 0%（0/17） | 100% | 130 / 233ms |

Holdout 的 en/ja/ko/ru/zh 各 12 个正样本均为 100% 召回，clean 与 20/10/5/0/-5dB 各 10 个正样本也均为 100%。负样本按场景分层后包含 fan/game_sfx/impulse/pink/silence/white 各 3/2/3/3/3/3 个片段，均无误触发。这个结果消除了同源增强泄漏并覆盖每类已有负场景，但 holdout 仍来自仓库 TTS 与合成噪声，样本量较小，不能替代真实设备门槛。

### 真实设备补测状态

当前仓库和本机任务 worktree 中没有带正负标签、设备标识和场景标识的真实麦克风录音，因此本次报告明确记录：`real_device_metrics.available=false`，原因为 `no_real_device_manifest_supplied`。没有用教程 TTS 或游戏音效冒充真实设备数据。

评测工具现支持 `--real-device-manifest`。manifest 只把匿名 `id`、`device_id`、场景、标签和聚合指标写入报告，不写入本地录音路径或 PCM。最小结构如下：
...
```json
{
  ...
  "clips": [
    {
      "id": "quiet-room-speech-01",
      "path": "recordings/quiet-room-speech-01.wav",
      "label": true,
      "device_id": "desktop-usb-a",
      "scenario": "real_quiet_near_field",
      "locale": "zh",
      "speech_start_ms": 850
    },
    {
      "id": "fan-idle-01",
      "path": "recordings/fan-idle-01.wav",
      "label": false,
      "device_id": "desktop-usb-a",
...
环境：Windows 10.0.26200、Python 3.11.14、24 logical CPUs。以下数字是 07-20 source-group holdout 两次连续完整 runtime 基准的观察区间，不是跨设备保证值。

| 路径 | CPU时间 / 1秒音频 | Wall时间 / 1秒音频 | 近似 RSS 增量 |
|---|---:|---:|---:|
| RNNoise + AGC + limiter + 48k→16k 完整桌面管线 | 34.51–41.33ms | 34.73–41.66ms | 0.45–0.46 MiB |
| 首次 import ONNX Runtime共享层 | 不计流式推理 | 不计流式推理 | 10.34–10.87 MiB |
| Silero raw 16k session + warm inference | 2.61–2.89ms | 2.43–3.27ms | 21.45–21.53 MiB |
| RNNoise后额外 Silero推理 | 2.40–3.52ms | 2.23–3.07ms | 与上行同一session |

解释：

- 如果 RNNoise 本来就因“降噪”开启，复用其 probability 的增量成本接近零；此时空闲阶段不加载 Silero，理论上可再省约 2.6–2.9ms CPU/每秒音频和约 21.5MiB Silero session RSS。
- 如果当前进程尚未加载任何 ONNX 模型，首次使用 Silero 还会带入约 10.3–10.9MiB 共享 ONNX Runtime，总增量约 31.8–32.4MiB；如果 SmartTurn 已加载该 runtime，这部分不能再次算给 Silero。
- 如果 RNNoise 原本关闭，仅为了节流而启动完整 RNNoise 管线，在这台机器上它的 CPU 时间约为 Silero-only 的 13.2–14.3 倍；因此“RNNoise-only 一定更省 CPU”不成立。
...
2. 流式 ASR：逻辑 endpoint 来自 Provider，不需要也不得加载 SmartTurn；Silero/RNNoise 只决定本地节流和 transport 启停。
3. 分段式 ASR：SmartTurn 仍负责 COMPLETE。没有 Silero 时可周期性请求 SmartTurn，但本报告没有测该 fallback 的 CPU/队列压力、长语音误判、evaluation 合并率和完成延迟。
4. 未验证部分：还没有证明 RNNoise silence 能同时满足首字保护、pause 可靠性与资源目标，因此现在仍不能把“VAD完全不加载”设为默认。
...
→ 流式：Provider endpoint
→ 分段式：SmartTurn endpoint

阶段2（真实报告通过后）
LOCAL_LISTEN: RNNoise-only
候选打开后: Silero shadow；仅分段式路径评估 periodic SmartTurn
→ 流式：Provider endpoint，不运行 SmartTurn
→ 分段式：SmartTurn endpoint
...
- 在分段式路径单独验证 RNNoise silence 或 periodic evaluation 是否能可靠请求 SmartTurn，不能用 presence 报告代替 pause 报告；
- Electron 打包环境和移动 16k 路径分别测 CPU/RSS。当前 RNNoise evidence 来自桌面 48k 路径，不能外推到移动端。

若未达到门槛，继续使用 RNNoise 预热 + Silero confirm/pause；流式 ASR 保持 Provider 原生 endpoint，分段式 ASR 保持 SmartTurn endpoint。不得退回 Omni，也不得让 RNNoise/Silero 自行发布 logical final。
...
uv run python tools/voice_eval/evaluate_speech_presence.py `
  ...
  --output speech-presence-real-device.json
```
