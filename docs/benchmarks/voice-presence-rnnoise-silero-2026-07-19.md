...
| Holdout | 100%（60/60） | 0%（0/17） | 100% | 120 / 190ms |

Holdout 的 en/ja/ko/ru/zh 各 12 个正样本均为 100% 召回，clean 与 20/10/5/0/-5dB 各 10 个正样本也均为 100%。这个结果消除了同源增强泄漏，但 holdout 仍来自仓库 TTS 与合成噪声，样本量较小，不能替代真实设备门槛。
...
环境：Windows 10.0.26200、Python 3.11.14、24 logical CPUs。以下数字来自 07-20 source-group holdout 同一次完整 runtime 基准，不是跨设备保证值。

| 路径 | CPU时间 / 1秒音频 | Wall时间 / 1秒音频 | 近似 RSS 增量 |
|---|---:|---:|---:|
| RNNoise + AGC + limiter + 48k→16k 完整桌面管线 | 41.33ms | 41.66ms | 0.46 MiB |
| 首次 import ONNX Runtime共享层 | 不计流式推理 | 不计流式推理 | 10.87 MiB |
| Silero raw 16k session + warm inference | 2.89ms | 3.27ms | 21.53 MiB |
| RNNoise后额外 Silero推理 | 3.52ms | 3.07ms | 与上行同一session |

解释：

- 如果 RNNoise 本来就因“降噪”开启，复用其 probability 的增量成本接近零；此时空闲阶段不加载 Silero，理论上可再省约 2.9ms CPU/每秒音频和约 21.5MiB Silero session RSS。
- 如果当前进程尚未加载任何 ONNX 模型，首次使用 Silero 还会带入约 10.9MiB 共享 ONNX Runtime，总增量约 32.4MiB；如果 SmartTurn 已加载该 runtime，这部分不能再次算给 Silero。
- 如果 RNNoise 原本关闭，仅为了节流而启动完整 RNNoise 管线，在这台机器上它的 CPU 时间约为 Silero-only 的 14.3 倍；因此“RNNoise-only 一定更省 CPU”不成立。
