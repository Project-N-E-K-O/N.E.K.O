...
`host` 接受 ASCII 主机名或 IPv4 地址；`port` 范围为 `1..65535`；`send_rate_hz` 范围为 `1..120`。
...
- **144 Hz 显示器上只有约 48 Hz：**同时更新 `vrm-manager.js` 与 `vrm-vmc-sender.js`；当前版本使用累计调度，长期平均值应接近配置频率。
