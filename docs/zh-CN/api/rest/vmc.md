...
单帧最多发送 64 根骨骼和 256 个表情。人形骨架只有 55 根，因此只有自定义表情数以百计的模型才可能触顶；超出部分会被丢弃，并记录一次警告。第一方采样器在发送前就已截断，因此浏览器发布端的这条警告出现在浏览器控制台；后端自身的警告用于覆盖直接向 `/api/vmc/ws` 投递超限帧的第三方发布端。
