...
N.E.K.O. 插件系统是一个基于 Python 的插件框架，建立在**进程隔离**和**异步 IPC** 之上。新开发只采用两种范式：产品功能使用 **Plugin（插件）**，外部协议桥接使用 **Adapter（适配器）**。旧 **Extension（扩展）** 类型已经弃用；只有符合 loader contract 的 `PluginRouter` + `@plugin_entry` 旧包仍可加载，且不得新建。
...
| **Extension（已弃用）** | `plugin.sdk.extension` | 仅兼容已经存在的 Extension | 注入到宿主插件进程中 |
...
> 从 **Plugin** 开始。不要创建新的 Extension；把可加载的旧 Router 迁入普通 Plugin 或原宿主，历史 facade 形态需先转换。

## 主要特性

- **进程隔离** — 普通 Plugin 与 Adapter 独立运行；弃用兼容的 Extension 与宿主插件同进程，其崩溃或副作用可能影响宿主
- **异步支持** — 同时支持同步和异步入口点
...
- **总线系统** — `self.bus` 通过 `messages`、`events`、`lifecycle`、`conversations`、`memory` 读取宿主状态；只有前三者支持 `watch()`，`conversations` 与 `memory` 是只读快照。可重放 watcher 链使用 `get()` → 结构化 `filter(field=value, ...)` → `sort(by=...)` → `limit()` → `watch()`，且只订阅 `add`、`del`、`change` 增量。它没有 publish/emit 接口。最近记忆记录使用 `self.bus.memory.get(...)`，语义检索使用 `await self.ctx.query_memory(...)`。
- **动态入口** — 在运行时注册/注销入口点
...
- [高级主题](./advanced) — Extension 兼容、适配器、跨插件调用、钩子
