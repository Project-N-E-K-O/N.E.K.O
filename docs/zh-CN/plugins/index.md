...
- **总线系统** — `self.bus` 用于读取宿主状态并订阅其变化：`self.bus.messages` / `self.bus.events` / `self.bus.lifecycle` / `self.bus.conversations` / `self.bus.memory`，每个命名空间都通过 `.get(...)` 读取一份快照。若要响应变化，可对快照列表进行 watch 并订阅增量：`self.bus.events.get(...).watch(...).subscribe(on=...)`。它没有 publish/emit 接口 —— 该总线是只读/响应式的，并非通用的发布/订阅总线。
- **动态入口** — 在运行时注册/注销入口点
