...
`source` 为空时清空全部；指定 source 则只清那个来源注册的。
...
`output` 可以是任意 JSON（dict / list / 字符串 / 数字）；main_server 会
把整个 body 作为 `ToolResult.output` 喂回 LLM。`is_error: true` 时 LLM
会感知到工具调用失败，会选择跳过或换工具。
