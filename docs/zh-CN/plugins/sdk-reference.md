...
#### `export_push_image(**kwargs) -> object`（异步）

向宿主推送图片导出。可传 `image_url`，或传 `image_data` + `mime`。当插件由 Agent 运行时，非 JSON export 会被汇总到 `plugin_result["media"]`，用于向上游展示补充图片、链接或文本。
...
读取 Agent 从最新用户消息转发给本次插件运行的图片附件。每项通常形如 `{"type": "image_url", "url": "..."}`，`url` 可能是普通 URL 或 data URL。
...
读取当前用户语言。`get_user_language()` 使用本次 Agent 运行下发的 `_ctx.lang` 或插件手动覆盖值；`fetch_user_language()` 会通过 `SystemInfo` 查询主系统配置，适合 startup、timer 等没有用户上下文的入口。
