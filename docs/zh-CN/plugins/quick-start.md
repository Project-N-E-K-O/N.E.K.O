...
已有符合插件规范的源码时，无需克隆 N.E.K.O 主仓库或运行源码版后端：

开发目录接口只对本机桌面端和通过回环地址访问的网页开放；暂不支持局域网远程网页或反向代理接入。

1. 打开安装版 N.E.K.O 的插件管理器，点击左侧「服务器日志」下方的「开发模式」，在该页面开启开关。
2. 点击「加载未打包插件」，选择包含 `plugin.toml` 的插件源码目录。网页端填写**运行后端的机器**上的绝对路径；不是上传浏览器电脑上的文件夹。
3. 点击「校验」，核对名称、ID、版本、入口和路径，再点击「加载未打包插件」。启动失败会保留登记及错误，可修复后重试。
4. 在自己的编辑器中修改文件，点击开发插件卡片上的「重载」。入口和插件子模块会重新读取源码；依赖修改也需要重启插件。
5. 点击「打包」，下载 `.neko-plugin` 文件，在另一套 N.E.K.O 的普通导入入口安装验证。停止状态也能打包。

源码目录名必须匹配入口包名；插件 ID 不能与其他来源冲突。依赖继续采用现有声明和 `vendor/` 规则，本入口不自动安装依赖。源码移动后用「重新选择目录」修复关联；更改插件 ID 后需要移除原关联再重新登记。

关闭开发模式会停止开发插件、保留目录关联；应用重启后需要手动启动。卡片的「移除关联」停止插件并解除登记，保留源码与运行数据。停止失败不会报告移除成功。

打包期间请勿修改源码；检测到变化时需要重试。打包沿用现有排除规则，不保证自动清除所有密钥。构建成功仅表示安装包已生成，功能测试和市场审核需要另行完成。

开发产物单独保存，通过本机开发页面下载，不出现在普通安装包列表中。移除关联或关闭开发模式不会取消已有产物的本机访问限制；下载后可通过普通导入入口安装。

分发前源码目录名还必须与插件 ID 一致，因为现有安装包按 ID 创建安装目录。不一致时请修改目录名及入口/导入，再重新绑定后打包；打包器不会自动改写源码。

以下 CLI 教程适合需要创建模板、命令行检查和自动化发布的源码工作流。
