...
这里指的是 N.E.K.O **源码 checkout 中的内置插件源码根目录**，Core 维护者可以在这里开发随程序发布的插件。第三方插件应把独立仓库或独立源码目录作为权威工作副本，再构建 `.neko-plugin` 做安装测试。不要把已安装应用的用户插件目录当作唯一源码目录：手工复制进去的目录属于未受管开发副本；存在同 ID 内置插件时它不会仅靠“刷新”自动激活，而且在导入同 ID 包并明确警告、确认后仍会被替换。
