...
| **Только в Steam-сборке при выполненном входе в Steam-клиент**: Steam64 user ID (публичный числовой ID, который виден в URL вашего профиля Steam) | Любые другие ID учётных систем (GitHub / Google / OpenAI / …) |
...
>
> **О Steam64**: это публичный числовой ID, который Steam-клиент после входа выдаёт любому стороннему SDK (число в конце URL вашего профиля Steam — это он). Он не содержит email, телефона или настоящего имени, но стабилен между сессиями, поэтому мы честно вынесли его в колонку «Собираем». **Если вы не хотите, чтобы он отправлялся, просто запускайте из исходников (у source-сборки канал дистрибуции — `source`, и Steam SDK вообще не вызывается), либо используйте `DO_NOT_TRACK=1` для полного отключения.**
