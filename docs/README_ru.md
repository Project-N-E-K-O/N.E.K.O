...
| Имя модели, тип вызова (`conversation` / `memory` и т.д.) | Имя пользователя, user ID, API-ключ, GitHub ID |
| Количество вызовов / количество ошибок | IP-адрес, геолокацию, MAC, серийные номера железа |
| Псевдонимизированный идентификатор устройства `SHA256(machine_id + путь_установки)` (односторонний хэш, необратим) | Любые PII, по которым можно восстановить личность |

> **О псевдонимизированном идентификаторе устройства**: это односторонний SHA-256 от machine-id ОС и пути установки. Он необратим и не содержит пользовательских данных — по нему нельзя установить конкретного человека. Однако одна и та же машина + одна и та же установка дают тот же идентификатор, поэтому в классификации GDPR / PIPL это **псевдонимизированный идентификатор**, а не полностью анонимные данные. Мы используем его только для подсчёта DAU с дедупликацией и атрибуции по версиям.
