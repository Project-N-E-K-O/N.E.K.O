...
Each record carries `image_base64` (one copy — there is no raw-bytes twin to read), `mime`, `source`, `captured_at`, `turn_id`, `generation`, and `frame_id`. `source` is the channel the host attributed the frame to: `screen`, `camera`, `plugin`, `callback`, `proactive`, `user`, or `unknown` once a turn was reshaped and the host could no longer say. `screen` and `camera` separate the two live channels only on the voice path; in text mode every frame the user shares — a shared screen, a camera still, a dragged-in photo — arrives through one queue that does not keep them apart, so it is reported as `user`. Filter for `user` if you want "something the user showed the character" in both modes. Dedupe on `frame_id`: `generation` orders ambient frames but does not advance for one-shot cue images, so two records can share one.
