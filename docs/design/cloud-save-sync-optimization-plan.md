...
- 记忆数据已基本按 `memory/<name>/...` 组织，包含 `recent.json`、`settings.json`、`facts.json`、`persona.json`、`reflections.json`、`surfaced.json` 与 `time_indexed.db`。
...
- `bindings/<character_name>.json` 负责模型类型、资源来源、资源状态和回退信息。
...
#### 6.1.2 本地 `cloudsave_local_state`
...
- 导出前必须先做 checkpoint。
- 生成 shadow copy 后再写入 `cloudsave/memory/<character_name>/time_indexed.db`。
- 不允许让 Steam 直接观察运行中的主库、WAL 或 SHM。

#### 6.5.2 v1 文件级冲突策略

| 数据类型 | v1 策略 |
| --- | --- |
| `profiles/characters.json` | last-writer-wins，并保留冲突备份 |
...
- 同名角色导入时，云端版本会直接覆盖本地同名角色，并保留冲突备份。
...
- `time_indexed.db` 导出使用 shadow copy，不直接暴露运行中的主库。
