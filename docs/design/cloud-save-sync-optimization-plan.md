...
- 当前代码中的全局入口优先以独立窗口模式打开 `/cloudsave_manager`（`window.openOrFocusWindow` / `window.open`），并在可用时把当前角色名附带到 `lanlan_name` query 中；若独立窗口创建失败，则回退为使用 `window.location.href` 页面跳转到 `/cloudsave_manager`。
