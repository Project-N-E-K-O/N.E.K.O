...
- 已有本地 `cloudsave/` 导出层、`meta/` / `overrides/` 等子目录、staging 目录、备份目录和 `manifest.json`。
...
- 云存档摘要接口与页面展示已经开始区分“绑定资源来源”“绑定资源状态”“角色来源 origin”“Workshop 当前设备恢复状态”几层概念，不再把所有 `steam_workshop + 缺资源` 一律视为“必然可恢复”。
...
- 已经存在 cloudsave runtime / router / pages / i18n / config_manager 相关回归测试，因此文档不应再把这些内容写成“纯设计、未验证”。

### 2.3 当前仍未完成的部分

- 角色设置中的云存档入口已经在代码层接入，但当前只保留了角色管理页操作区里的一个全局入口，不再保留角色面板中的第二入口。
- 前端独立云存档管理页面已经有独立路由、模板、脚本和样式文件，也已补齐正式 `cloudsave.*` i18n 键、空页面渲染、本地时间显示、分组渲染与默认折叠详情交互；当前剩余的是不同启动方式和真实浏览器环境下的端到端验证，而不是页面本身仍未实现。
...
| 页面时间显示本地化 | 已实现 | 前端优先按当前 UI 语言 / 浏览器 locale 做本地化显示；若 Intl 格式化失败，再 fallback 为 `YYYY-MM-DD HH:MM:SS` |
| `origin` 来源字段合并展示 | 已实现 | `item` 已补充 `local_origin_*` / `cloud_origin_*` 字段，用于表达“角色原始来源”和当前绑定资源来源的差异 |
| 页面分组与详情折叠 | 已实现 | 页面按是否关联 Workshop 分成“我的角色”与“Workshop 角色”，详情默认折叠 |
...
5. 页面展示本地 / 云端关系：
   ...
   - `matched`
   - `diverged`
6. 页面按“非 Workshop 关联角色 / Workshop 关联角色”分组渲染，并在条目详情中展示本地与云端两侧元数据。
7. 用户对单只猫娘执行上传或下载。
8. 执行前给出覆盖提示，执行中做备份与校验，执行后刷新页面状态。
...
- 当前代码中的全局入口优先以独立窗口模式打开 `/cloudsave_manager`（`window.openOrFocusWindow` / `window.open`），并在可用时把当前角色名附带到 `lanlan_name` query 中，同时把当前 UI 语言附带到 `ui_lang` query 中；若独立窗口创建失败，则回退为使用 `window.location.href` 页面跳转到 `/cloudsave_manager`。
- 页面在“无角色”“无 `lanlan_name` query 参数”或当前角色不可用时也应能正常打开并显示空状态，而不是因为缺少角色名而不渲染。
...
- 页面应提供返回角色管理页的明确入口；当前实现会优先关闭由角色管理页打开的独立窗口，无法关闭时再回退到 `/chara_manager`。
  ...
  "items": [
    {
      ...
      "local_origin_source": "steam_workshop",
      "local_origin_source_id": "123456",
      "local_origin_display_name": "示例角色来源",
      "cloud_origin_source": "steam_workshop",
      "cloud_origin_source_id": "123456",
      "cloud_origin_display_name": "示例角色来源",
      ...
      "local_origin_workshop_status": "installed_and_subscribed",
      "cloud_origin_workshop_status": "available_needs_resubscribe",
      "local_workshop_title": "示例工坊物品",
      "cloud_workshop_title": "示例工坊物品",
      "local_origin_workshop_title": "示例工坊角色",
      "cloud_origin_workshop_title": "示例工坊角色",
...
- `local_origin_source` / `cloud_origin_source` 与对应的 `*_origin_source_id` 必须保留，用于表达“角色原始来源”和“当前绑定资源来源”并不总是同一件事。
- `asset_source`、`local_asset_source`、`cloud_asset_source` 当前应优先反映“实际命中的文件路径推断结果”，而不只是旧配置里记录的来源字符串。
- 当对应来源是 `steam_workshop` 时，接口还应补充 `local_workshop_status` / `cloud_workshop_status` 等当前设备视角的恢复状态字段；`local_origin_source` / `cloud_origin_source` 为 `steam_workshop` 时也同样需要补充对应的 `*_origin_workshop_status` 字段。
...
- `local_updated_at_utc` / `cloud_updated_at_utc` 是后端返回的 UTC 时间；当前页面显示层会优先按当前 UI 语言 / 浏览器 locale 转为本地时间，只有本地化格式化失败时才 fallback 为 `YYYY-MM-DD HH:MM:SS`。
- `asset_state = downloadable` 只表示“绑定来源来自 Workshop 且本地未命中文件”，页面必须继续结合 `*_workshop_status` 才能决定是“仍可恢复”“需要重新订阅”“条目已不可用”还是“当前未确认”。
- 当前页面还会把条目分成“我的角色”和“Workshop 角色”两组：
  - “我的角色”组按本地更新时间倒序排序，并优先把 query / 当前偏好角色排前。
  - “Workshop 角色”组优先把 query / 当前偏好角色排前，其余按角色名排序。
- 当前页面详情默认折叠，用户通过展开按钮查看本地 / 云端两侧的 meta、warning 和恢复提示。
...
- `provider_available` 当前实际由 `is_cloudsave_provider_available(config_manager)` 汇总；仓库内现阶段主要通过 `config_manager.cloudsave_provider_available` override 模拟 provider 不可用，而不是真实远端探测。
...
- `matched`
  - 当前默认展示“已同步”或“无差异”。
  - 当前 `available_actions = []`，页面不提供上传 / 下载按钮。
- `diverged`
  - 同时提供上传和下载。
  - 两个操作都必须带覆盖确认。
...
- 本地整包快照导出 / 导入。
- 已补一批 runtime / router / pages / i18n / config_manager 回归测试。
...
- 展示资源来源、资源状态、角色原始来源和 Workshop 当前设备恢复状态。
- 对单只猫娘显示上传 / 下载按钮。
- 对“改过模型的角色”“需要手动导入模型”“已取消订阅但本地仍有缓存”“条目已不可访问”这类情况给出不同提示。
- 页面在无角色时也能显示空状态。
- 时间显示已按用户本地时区格式化，不直接暴露原始 UTC ISO 字符串；格式优先跟随当前 UI 语言 / 浏览器 locale。
- 页面文案已接入正式 `cloudsave.*` 语言键。
- 页面当前已按“我的角色 / Workshop 角色”分组，并默认折叠详情区域。
...
- 当前页面主要依赖 `GET /summary` 与上传 / 下载接口；在真正执行上传 / 下载前，会额外调用 `GET /character/{name}` 刷新单角色最新摘要。当前详情展示仍以内联折叠区为主，尚未演进为单独的详情页 / 二级路由交互。
...
- 页面中的时间应按用户本地时区显示，并优先遵循当前 UI 语言 / 浏览器 locale，而不是直接显示原始 UTC ISO 字符串。
- 单只猫娘可以单独上传、单独下载。
