...
这个形态是后续小游戏入口的前置阶段，不是普通 idle 动作，也不是 10 秒临时演出。进入后，猫 gif 和当前关联目标先掉到屏幕底部；之后停留在 playground 形态里，直到用户点击猫 gif 触发“变回模型”才结束。
...
- 模块内有独立拖拽和重力物理表现。
- 点击猫 gif 是正常结束路径；页面销毁、切出 CAT1 等只作为兜底清理。
...
- 统一字段：`id`、`element`、`desktop`、`mass`、`inverseMass`、`visibleInsetRatios`、`x/y`、`vx/vy`、`dragging`、`grounded`、`floorY`、`wallLeft`、`wallRight`
- 统一更新：重力、边界、拖拽、碰撞和位置写回都走 playground 物理循环。
...
- `desktop-yarn`: 桌面最小化聊天框在 Pet 页面层的镜像 body，`mass = 0.65`；桌面端只接收 bounds。
- `question-block`: 从点击瞬间的问号视觉矩形创建的新 DOM 元素，`mass = 5`，大于猫 `2`。
...
- 点击事件为 `neko:idle-cat1-playground-question-block-click`，不触发原入口事件，也不触发猫退出。
...
- 非拖拽点击问号方块不触发原入口事件，也不触发猫退出；只触发新的独立事件 `neko:idle-cat1-playground-question-block-click`。
...
- playground drop 状态下，非拖拽点击猫 gif 是唯一正常退出交互。
...
- 桌面最小化聊天框的速度、重力和碰撞由主项目页面层物理体计算，桌面端只接收 bounds。
