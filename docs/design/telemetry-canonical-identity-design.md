...
FROM devices
WHERE steam_user_id != ''
  -- denylist 是所有产边路径的硬约束（§7），源 1 也不例外，否则删号后跑这条会复活被删 ID
  AND steam_user_id NOT IN (SELECT steam_user_id FROM steam_id_denylist);
```
...
def _normalize_steam_id(raw) -> str:
    """与 server.py ingest 同一套规则；不合法返回 ''（不产边）。

    raw 不标注 str：本 job 扫的是原始 events.payload JSON，伪造/异常行的
    steam_user_id 可能是 number / null / 其它类型，直接 .isdigit() 会抛异常
    中断回填。先做类型守卫再走字符串规则。ingest 侧传进来的是 Pydantic str，
    这层守卫对它无害。
    """
