...
高亮去重按导演通用规则执行：记忆入口和毕业收尾都只创建当前 scene 需要的一套 spotlight；设置类 scene 不再用整张设置弹窗做 persistent 高亮，避免把圆角矩形范围撑到整窗。普通 scene 不做 operation 后 `settled` 二次高亮刷新，只有收尾 `cleanup` 重新高亮聊天窗。
...
PC 端记忆入口、可选记忆页列表区域和终极长留收尾高光都由全局 overlay 渲染。主线仍不高亮存储位置或云存档入口，不登录、不上传、不下载、不展示账号或路径细节。最终收尾台词期间重新高亮聊天窗，并在最终花瓣 cue 同步隐藏 Ghost Cursor、清理高光和播放花瓣。
...
| `day7_graduation_wrap` | `chat-window` | `move` + `cleanup` + `petalTransition` | 最终毕业收尾，写入 Day 7 完成态。 |
...
- 动作：收尾台词开始前清理 handoff、临时弹窗和跨页入口高亮；随后完全复用 Day 1 `takeover_return_control` 的收尾动作：收尾台词播放期间 primary 重新回到聊天窗，Ghost Cursor 移到聊天窗或输入区附近 move 并停留；外置聊天窗模式同步高亮独立聊天窗；台词约 70% 处触发与 Day 1 相同的最终花瓣转场 cue，触发瞬间同步隐藏 Ghost Cursor、清理内置/外置聊天窗高亮和所有 highlight；转场期间恢复用户原有模型位置、按钮组和交互权限，当前至少保存 Day 7 完成态。
