...
- `static/tutorial/core/app-prompt.js`
- `static/tutorial/yui-guide/steps.js`
- `static/tutorial/yui-guide/overlay.js`
- `static/tutorial/yui-guide/page-handoff.js`
- `static/tutorial/yui-guide/wakeup.js`
...
- `static/tutorial/core/universal-manager.js`：教程生命周期、首页步骤推进、Yui Director 创建、教程期间模型覆盖、模型位置和大小自适应、结束清理。
- `static/tutorial/yui-guide/steps.js`：Yui 场景注册表、首页场景顺序、文案 key、voice key、interrupt 策略。
- `static/tutorial/yui-guide/director.js`：首页主演出控制器，负责旁白、聊天消息、表情、Ghost Cursor、spotlight、真实 UI 点击、打断处理、插件 Dashboard 握手、清理。
- `static/tutorial/yui-guide/overlay.js`：首页演出 DOM 层，负责 spotlight、圆形高亮、Ghost Cursor、预览层等。
- `static/tutorial/yui-guide/page-handoff.js`：跨页 handoff token、目标页打开包装、插件 Dashboard 打开辅助能力。
- `frontend/plugin-manager/src/yui-guide-runtime.ts`：插件 Dashboard 页面内的本地演出、spotlight、Ghost Cursor、本地打断检测和与首页通信。
- `static/tutorial/core/app-prompt.js`：首次引导提示、用户决策、教程开始/完成状态上报、统一首页教程交互锁。
...
首页提示逻辑在 `static/tutorial/core/app-prompt.js`。它会维护前台停留、弱交互、聊天轮次、语音使用等状态，并通过后端决定是否提示用户开始教程。
...
首页新手引导不只依赖演出层遮罩来挡点击。当前实现以 `static/tutorial/core/app-prompt.js` 暴露的统一锁作为业务判断入口：
...
`static/tutorial/yui-guide/director.js` 暴露：
...
普通跨页接力使用 `static/tutorial/yui-guide/page-handoff.js` 和后端 token：
...
```powershell
node --check static\tutorial/core/app-prompt.js
...
node --check static\tutorial/yui-guide/director.js
npm.cmd test
