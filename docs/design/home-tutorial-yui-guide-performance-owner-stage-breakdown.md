...
- 在教程期间临时切换到 `yui_default` Live2D 模型，不写入用户配置。
...
首页教程开始时，`UniversalTutorialManager.beginTutorialAvatarOverride()` 会临时切换当前角色为 Live2D `yui_default`：
...
- 首次首页提示能出现，接受后能启动教程。
- 手动启动教程能触发同一条 Yui 流程。
- 教程期间模型切到 `yui_default`，结束后恢复用户原模型。
