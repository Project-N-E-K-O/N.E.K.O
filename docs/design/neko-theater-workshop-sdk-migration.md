...
仅保存 `changes.editor` 时，项目 revision 仍递增，但原本有效的支线 `ending_review/preview` 草稿同步更新 `base_revision` 与上下文指纹，继续允许生成路径和显式应用。已经过期、指纹不匹配或版本不符的草稿不复活；已应用及失败状态保留。混合正文、设定或标题编辑继续使未应用草稿过期。此规则同步原 InkAI 项目存储，不改变生成中的 revision 冲突保护。
