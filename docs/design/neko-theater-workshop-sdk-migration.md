...
状态：`theater_workshop.sdk` 0.1.0 的无界面核心、公开接口、本体宿主、作者项目导入和发行检查入口已落地；真实模型生成、安装与演绎已有证据。全平台冻结发行物仍未验收，存量作者数据未整体切换，剧本质量按具体版本和路线判断。当前模块职责以[小剧场架构第2节](./neko-theater-architecture.md#2-模块与权限)为唯一速查表；本文只维护工坊的调用合同、写入边界和迁移状态。
...
| 旧作者数据是否迁完 | 完整导入接口和8份真实旧项目的隔离验证已完成；不等于正式目录已批量切换。旧包不做运行时兼容，必须重新编译到当前 v2.2 字段 |
| InkAI是否冻结 | 尚未冻结；按用户后续要求，共享创作与评改规则继续同步两端，见4.1 |
| 是否整体交付验收通过 | 源码／宿主与真实模型链路有证据；发行二进制、各平台及全路线质量不能据此推定 |
| 框架冗余与公共结构审查 | [SDK框架审查](./neko-theater-workshop-framework-review.md)、[公共数据结构审查](./neko-theater-shared-data-structures-review.md)；WS-01—02、DS-01—02已处理，原发现保留为历史证据，结果见问题2.125 |
| 更强模型是否成为工坊默认 | 没有。问题2.139是运行侧争议复核实验，接入已回退；工坊仍由调用方选择模型 |

运行权限、模型链和当前任务边界以[小剧场架构](./neko-theater-architecture.md)为准；作者协议见InkAI的 `docs/superpowers/specs/2026-08-06-neko-theater-numeric-v2-generator-design.md`。第1—8节为当前接入合同与验收状态，第9节按原编号保留迁移证据和问题索引，不再重复维护全部演绎实验。
...
职责速查：`host.py` 只做本体适配和写入保护；`sdk/workshop.py` 做作者工作流编排；`sdk/contracts.py` 定义无 HTTP 的输入合同；`sdk/generation/` 负责生成、事实/证据/文学检查和限定修订；`sdk/numeric_v2*.py` 负责作者字段投影、静态分析、支线合同和项目 revision；`sdk/packages.py` 只交付不可变编译候选。以上模块不读取玩家 Session，不参与实际演绎回合；正式包仍由 `services/theater/numeric_v2.py` 统一裁定。
...
- 问题2.125：各仓合并目标投影与元数据说明，保留作者字段而不恢复目标执行器；两端导入均改为组装后一次提交，JSON语法修复保护字符串原文。SDK纯hash核对不再重复生成无人消费的软诊断，公开编译仍返回作者警告。
- 问题2.126、2.128：续写携带实际输出合同、守住主线三次总调用上限，支线固定原文容量与复制合同、背景及关系标记的评改输入得到修正；不放宽运行时玩家授权或正式包校验。
- 问题2.130：事实初检不先替证据复核预设修复资格，证据输入移除临时`repairable`；复核后再统一计算允许修订的字段，原白名单及方案复核继续生效。

具体反例和验证范围以[问题文档2.125—2.130](./neko-theater-issues-and-solutions.md)为准。零号日志`draft_56`是2.131的具体作者稿修订，不是SDK schema升级；2.133—2.139的运行日志、历史计数、解析、结局Prompt和模型实验也不构成新的工坊协议。框架可用、编译通过和模型质量通过须分开判断。
...
仅保存 `changes.editor`、`changes.stage` 或两者组合（包括空更改），以及再次保存相同的规范化主线顺序时，项目 revision 仍递增，但原本有效的支线 `ending_review/preview` 草稿同步更新 `base_revision` 与上下文指纹，继续允许生成路径和显式应用。已经过期、指纹不匹配或版本不符的草稿不复活；已应用及失败状态保留。主线实际改变，或混合正文、设定、标题编辑时，继续使未应用草稿过期。这些不改变内容的保存共用草稿更新规则，并同步原 InkAI 项目存储；不改变生成中的 revision 冲突保护。
...
仅修改数值定义或初始值也会使质量报告过期，并清除旧节奏诊断；主线顺序实际改变时，无论是否已有质量报告，都清除旧路径的节奏诊断。提交相同的规范化 setup 或相同主线顺序时保留对应报告。这些项目存储规则同步维护于 InkAI，不增加模型调用。
...
问题2.126修正了第三次初始请求后仍发第四次续写的越界；额度耗尽时保留原候选和问题，等待显式恢复。续写按请求路径附带首轮已有结构/枚举示例，首轮完整Prompt不变。支线结局单次输出上限调整为8192，为合法固定原文、JSON转义和结局结构预留空间；这不保证模型逐字复制长文，真实失败另记在[问题文档](./neko-theater-issues-and-solutions.md)。评改输入同步包含正式背景与关系指标标记，背景只读，指标数量建议仍严格限定0—4整数；两仓保持同源合同。
...
当前 SDK 不为缺少显式姓名的旧包提供运行时投影兼容。旧作者工程或生成检查点需要先导入当前 SDK、补齐姓名和状态主体字段，再重新编译、复验和发布；已安装旧包不在 Runtime 中原地改写，也不与新字段并行读取。
...
- 正式编译器验证显式姓名及状态主体；运行投影只消费当前包的完整姓名字段。
...
| 迁出基线一致性（历史验证） | 普通合同样例和显式姓名样例各一份，经旧工坊桥接编译与新进程内编译，canonical bytes、hash、warnings 一致；这是迁出期间的字节一致性证据，不是旧包运行时兼容证明 |
...
SDK与InkAI已同步来源邀请、桥段、目标开场的去向／时段／活动一致性规则。晴川镇draft_2通过SDK编译复验并在用户授权删除旧包后安装；SDK仍禁止同ID直接覆盖。2.114单条七成推荐轨迹22轮到安静结局、节奏可接受，但复核长等待和其他路线边界仍在；当时暂停后，用户另行发起了2.125以后的审查与迭代，不再将旧停止要求当作当前禁令。完整证据见[问题2.110—2.114](./neko-theater-issues-and-solutions.md)，现行共享维护边界见4.1。
