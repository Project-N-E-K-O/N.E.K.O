...
仅保存 `changes.editor` 或再次保存相同的规范化主线顺序时，项目 revision 仍递增，但原本有效的支线 `ending_review/preview` 草稿同步更新 `base_revision` 与上下文指纹，继续允许生成路径和显式应用。已经过期、指纹不匹配或版本不符的草稿不复活；已应用及失败状态保留。主线实际改变，或混合正文、设定、标题编辑时，继续使未应用草稿过期。两种不改变内容的保存共用草稿更新规则，并同步原 InkAI 项目存储；不改变生成中的 revision 冲突保护。
