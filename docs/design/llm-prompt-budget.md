...
**Owner**: Wave-of-Budget 重构（PR #969 起）
...
本文档**只管输入侧**：每条会被拼进 LLM `messages` 的字符串都应该有明确的 budget（或显式列入"咎由自取"清单），并在 [config/__init__.py:843](../../config/__init__.py) §3.7 集中维护。

输出侧 `max_tokens` / `max_completion_tokens` 也在 §3.7 里维护，但路由由 [utils/llm_client.py](../../utils/llm_client.py) `ChatOpenAI._params()` 自动按 base_url 选字段名（Anthropic → `max_tokens`，其他 → `max_completion_tokens`），caller 无需关心。
...
- **OmniOfflineClient**：累计 token > `SESSION_ARCHIVE_TRIGGER_TOKENS` (5000) 触发新会话准备 + 记忆压缩 + 历史归档。
- **OmniRealtimeClient**：不维护 server 侧 history，走轮次/时间触发（`_session_turn_count >= 10` 或 `_elapsed >= 40s`），由 Gemini Live API 自己管 session 上限。
...
| 一轮往返 | ~550 | 用户+AI |
| 触发归档 | ~9 轮 | 5000 / 550 ≈ 9，与 `_session_turn_count >= 10` 对齐 |
...
| Component | 常量 | 默认 | 说明 |
|---|---|---|---|
| 单轮 user/assistant 历史归档触发 | `SESSION_ARCHIVE_TRIGGER_TOKENS` | 5000 | 仅 offline 路径 |
...
| surfacing top-K | `REFLECTION_SURFACE_TOP_K` | 2 |
| 单次 synthesis 最多带入的 unabsorbed fact 数 | `REFLECTION_SYNTHESIS_FACTS_MAX` | 30 |
...
| 渲染主对话 prompt 的 reflection 预算 | `REFLECTION_RENDER_TOKEN_BUDGET` | 1000 |
...
| candidates 总和兜底 | `RECALL_CANDIDATES_TOTAL_MAX_TOKENS` | 25000 |

#### 4.2.5 Evidence signal detection

| Component | 常量 | 默认 |
|---|---|---|
| Stage-2 候选 observation 条数上限 | `EVIDENCE_DETECT_SIGNALS_MAX_OBSERVATIONS` | 200 |
| 单条 observation 截断 | `EVIDENCE_PER_OBSERVATION_MAX_TOKENS` | 200 |
| observations 总和兜底 | `EVIDENCE_OBSERVATIONS_TOTAL_MAX_TOKENS` | 25000 |
...
> 路由：caller 传 `max_completion_tokens=N`，[utils/llm_client.py](../../utils/llm_client.py) `ChatOpenAI._params()` 按 `base_url` 决定写进请求体的字段名（Anthropic → `max_tokens`，其他 → `max_completion_tokens`）。
...
| Proactive retry 调用 | `PROACTIVE_LLM_RETRY_MAX_TOKENS` | 1024 |
| Proactive 默认 | `PROACTIVE_LLM_DEFAULT_MAX_TOKENS` | 1536 |
| 翻译输出 | `TRANSLATION_OUTPUT_MAX_TOKENS` | 2000 |
...
|---|---|---|
| 用户原话直拼 HumanMessage | [main_logic/omni_offline_client.py:413](../../main_logic/omni_offline_client.py) | 用户故意贴 1MB 文本攻击自己的会话——咎由自取；session-level 5000 token 触发归档兜底 |
| OpenClaw `_classify_magic_intent_with_llm` user_text | [brain/openclaw_adapter.py:363](../../brain/openclaw_adapter.py) | 同上；用 1MB 文本做 80-token 分类是用户的事 |
| Emotion 分析 user text | [main_routers/system_router.py:1757](../../main_routers/system_router.py) | 同上 |
| Bilibili `knowledge_context` | [plugin/plugins/bilibili_danmaku/llm_client.py:130](../../plugin/plugins/bilibili_danmaku/llm_client.py) | 用户配置的知识库；用户自己写多大就多大 |
| sts2_autoplay `strategy_prompt` | [plugin/plugins/sts2_autoplay/service.py](../../plugin/plugins/sts2_autoplay/service.py) | 用户写的 `.md` 策略文件，配置项 |
...
实现：[utils/llm_prompt_audit.py](../../utils/llm_prompt_audit.py) + [utils/llm_client.py](../../utils/llm_client.py) `ChatOpenAI._params()` 末尾 hook。
...
1. 在 [config/__init__.py](../../config/__init__.py) §3.7 改常量值，**同步更新 docstring**。
2. 如果是新加的 component：
