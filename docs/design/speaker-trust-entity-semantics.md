...
- **bind 的把关必须在临界区里，UI 侧那次只是为了给人话错误。** 两个页签并发绑同一个未绑的源，两次前置检查可以都答「没绑」，第二次就走进 merge 分支融合两个候选。所以 `abind_account` 有一个 `require_unbound`（默认关，不动既有调用方的语义），在 `_bind_locked` 里判断——那是唯一不会失效的地方。
