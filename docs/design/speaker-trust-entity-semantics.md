...
**unbind 必须和 bind 在同一个界面上。** bind 会立刻把两个账号的信赖度合到一起，操作者选错一项就得当场退得回来，而不是去翻文档找一个内部端点。`/internal/identity/accounts/unbind` 早就实现了，缺的只是入口。对没绑过的 ID 点它是无害的（`changed=false`）。`ledger_delta` 与 `effective_delta` 两个数原样透传给 UI——它们通常不相等，这不是 bug，只给一个数操作者就没法把它和分数对上。
