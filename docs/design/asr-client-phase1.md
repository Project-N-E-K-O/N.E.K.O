...
玩家麦克风 PCM
  ...
  -> Core 对应的 ASR worker
  -> final 文本
  -> on_transcript_event(AsrTranscriptEvent)  # optional partial/final metadata
  -> on_input_transcript(text)                # legacy final-only callback
```
...
- 只有首个有效、非空 `final` 调用兼容接口 `on_input_transcript()`。可选的 `on_transcript_event()` 接收通过过期过滤后的 partial/final 及 generation、buffer epoch、utterance ID；重复 final、冲突 final 以及 clear/close 后到达的旧结果都不会进入业务回调。
