...
5. 显示时长为 `5000ms`，之后自动移除 active class。
...
8. `CAT2` 每次显示有 `1/3` 概率把背景 GIF 换成 `thought-items/sleeping-zzz.gif`，`CAT3` 概率为 `2/3`。选中 ZZZ 背景时隐藏内容图，显示时长改为 `8000ms`；普通气泡仍为 `5000ms`。
...
5. 如果气泡 5s 生命周期内 tier 已变化，会清理旧气泡状态，避免从旧 tier 串到新 tier。
...
| CAT2 / CAT3 随机睡眠声播放成功 | 显示思考气泡，5s 后渐隐 |
| 长按猫但还未超过拖拽阈值 | 不显示思考气泡，不扩大容器影响猫位置 |
...
8. 思考气泡在长按 pending / 拖拽 / CAT1 子动作期间隐藏，并且只由随机声音播放成功触发 5s 渐显渐隐。
