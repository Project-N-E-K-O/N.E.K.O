...
CAT1 本地文字另有固定 `3%` 的哈气回复彩蛋。它不是 observation 到动作的映射，也不进入 selector；回复执行时只通过窄表现入口请求既有独立伸懒腰 runner。runner 仍执行 CAT1、可见性、贴边、拖拽、return、transition、playground 和其他动作等全部限制，拒绝时回退普通猫叫。该条文字仍只结算原有一次互动反馈，彩蛋不另写 cooldown、严格 action result、return episode 或 near-chat 伸展完成 observation。
...
同一个独立伸懒腰 runner 也可由 CAT1 本地回复的 `3%` 哈气彩蛋请求。该入口不要求 near-chat；因此它的完成不冒充 journey 的 `cat1_stretch_done_near_chat`，本次互动数值已经由对应的 `cat_local_text_received` 结算。runner 拒绝时只回退普通猫叫。
