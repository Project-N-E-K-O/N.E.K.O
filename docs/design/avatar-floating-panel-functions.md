...
- Day 2-4 的 scene 配置目前集中在 `static/yui-guide-director.js` 的 `AVATAR_FLOATING_GUIDE_ROUNDS` 中，保留稳定 `textKey` 和 `voiceKey`。
- 首页重置按钮由 `static/avatar-floating-guide-reset.js` 绑定。Day 2-4 重启优先走 Manager/Director 正式流程，旧的临时播放器只作为兜底。
...
- `main_routers/pages_router.py` 必须把 `static/universal-tutorial-manager.js`、`static/yui-guide-director.js`、`static/avatar-floating-guide-reset.js` 纳入 `static_asset_version` 计算，否则刷新首页可能继续加载旧脚本。
...
Day 2-4 的台词目前集中在 `static/yui-guide-director.js` 的 `AVATAR_FLOATING_GUIDE_ROUNDS` 中。文本 key 必须稳定，便于后续只替换文案和音频；如果后续多语言文案膨胀，再拆到类似 `static/yui-guide-steps.js` 或新增 `static/avatar-floating-guide-steps.js`。
...
- `static/yui-guide-director.js`：首页教程场景编排、spotlight、ghost cursor 和真实 UI 操作；Day 2-4 的 `AVATAR_FLOATING_GUIDE_ROUNDS` 和 `playAvatarFloatingRound()` 也在这里。
- `static/universal-tutorial-manager.js`：教程启动、完成、跳过和页面级调度；负责 Day 2-4 状态持久化、自动排期、手动重启、临时切模校验和 `yui-origin` 兜底。
- `static/avatar-floating-guide-reset.js`：首页 Day 1-4 重置按钮绑定。Day 2-4 优先委托 `UniversalTutorialManager.startAvatarFloatingGuideRound()`；兜底播放器在 N.E.K.O.-PC 环境必须把 Ghost Cursor show/move/click/wobble/hide 发送到全局透明教程 overlay，并隐藏本地 fallback cursor。
...
22. `main_routers/pages_router.py` 的 `_YUI_GUIDE_ASSET_VERSION_PATHS` 必须包含 `static/universal-tutorial-manager.js`、`static/yui-guide-director.js`、`static/avatar-floating-guide-reset.js`。修改这些文件后需要重启后端/应用进程，让新的 `static_asset_version` 生效。
