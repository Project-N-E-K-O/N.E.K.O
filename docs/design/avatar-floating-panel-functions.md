...
- `static/avatar-floating-guide-reset.js`：首页 Day 1-4 重置按钮绑定。Day 2-4 优先委托 `UniversalTutorialManager.startAvatarFloatingGuideRound()`；兜底播放器在 N.E.K.O.-PC 环境必须把 Ghost Cursor show/move/click/hide 发送到全局透明教程 overlay，并隐藏本地 fallback cursor。
