...
5. 收尾 `day4_wrap` 先关闭设置弹窗和侧边栏，重新高亮胶囊输入框，并在约 70% cue 同步清理高光、Ghost Cursor 和外置聊天窗状态。
6. round 开场不得预热或等待聊天窗 surface ready；临时切到 `yui-origin` 并确认模型可见后，先显示等待 1500ms 再开始播放本日流程。`day4_intro_companion` 在本 scene 内按需打开聊天窗，并在聊天窗打开后通过 `NekoHomeTutorialFeatureController.enforce()` 再次禁用 proactive/Galgame，然后建立首句聊天窗高光；教程期间胶囊输入框和聊天窗各功能按钮都禁止用户点击。
...
| 8 | `day4_wrap` | `chat-input` | `move` | `cleanup` | 收尾重新高亮胶囊输入框并播放花瓣转场。 |
