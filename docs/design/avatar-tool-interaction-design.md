...
但目前这套交互还停留在“前端彩蛋 / 光标反馈”层，没有真正进入猫娘会话链路。现在要做的是：

1. `composer-tool-btn composer-emoji-btn` 内的图标切换为当前选中道具图片
...
| 道具 | 命中范围内动作 | 范围外动作 |
|------|----------------|-----------|
| `lollipop` | `offer`, `tease`, `tap_soft` | `wave_item` |
| `fist` | `poke`, `bonk_light`, `threaten_playful` | `shadow_box` |
| `hammer` | `bonk`, `slam_warning`, `fake_out` | `swing_air` |
...
- 现有宿主并不会自动消费这个新事件
- 因此必须同时补一条 host callback 或显式 listener 接线
...
| fist | poke | angry |
| fist | threaten_playful | surprised |
| hammer | bonk | surprised |
| hammer | slam_warning | angry |
| hammer | fake_out | surprised |
...
| `fist` | `reward_drop` | `normal` | 互动中意外掉落奖励，需要开心提醒用户去捡 | `happy` | 主反应仍然围绕拍一下，奖励提示只占一句里的后半部分 |
...
| `hammer` | `slam_warning` | `easter_egg` | 放大彩蛋触发，重击远超平时，彻底砸晕或彻底破防 | `angry` | 戏剧化、夸张、强烈，允许惨叫或爆发式反应 |
...
- 在 `static/app-buttons.js` 或等效模块注册 `reactChatWindowHost.setOnAvatarInteraction`
...
- Host 注册存在脚本时序依赖：
  - 模板里当前 `app-buttons.js` 早于 `app-react-chat-window.js` 加载
  - `app-buttons.js` 里对 host setter 的注册是一次性判断
  - 如果未来初始化顺序变化，`setOnAvatarInteraction()` 可能会漏注册
  - 因此实现时最好补一次延后重试，或在 host ready 后统一注册
...
def normalize_avatar_interaction_payload(payload: dict) -> dict | None:
    ...
    if tool_id not in {"lollipop", "fist", "hammer"}:
        return None
    if not action_id:
        return None
    if target not in {"avatar", "outside"}:
        return None
    ...
    reward_drop = bool(payload.get("reward_drop", False))
    easter_egg = bool(payload.get("easter_egg", False))
...
def resolve_interaction_intensity(payload: dict) -> str:
    ...
    if tool_id == "hammer" and action_id == "slam_warning":
        return "easter_egg"
