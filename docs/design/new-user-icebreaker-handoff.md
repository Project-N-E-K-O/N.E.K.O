...
- `static/icebreaker/new-user-icebreaker.js`：监听教程结束、gate、runner、聊天投递、选项推进。
- `static/icebreaker/icebreaker_scripts.json`：只放 day、node、option、next、handoff、TTS voiceKey 和文案 key。
- `static/icebreaker/locales/*.json`：8 个 locale 的台词和选项文案。
...
- `static/avatar-floating-guide-reset.js`
...
- 首页教程已经在 `static/yui-guide-director.js` 里复用这条链路发教程聊天消息，破冰可以照这个思路走。
...
- 破冰长台词不要塞进 `static/locales/*.json` 主翻译包，单独放 `static/icebreaker/locales/*.json`，避免污染全局 UI 文案。

缓存版本：

- 如果新增 `static/icebreaker/*.js` 或 `static/icebreaker/*.json`，需要把关键运行时文件加进 `main_routers/pages_router.py` 的静态资源版本计算，避免刷新后仍加载旧脚本。
- `templates/index.html` 里把破冰 runtime 放在 `universal-tutorial-manager.js` 和 `avatar-floating-guide-reset.js` 后面加载。
...
- `static/icebreaker/icebreaker_scripts.json`

多语言文案：

- `static/icebreaker/locales/zh-CN.json`
- `static/icebreaker/locales/zh-TW.json`
- `static/icebreaker/locales/en.json`
- `static/icebreaker/locales/ja.json`
- `static/icebreaker/locales/ko.json`
- `static/icebreaker/locales/ru.json`
- `static/icebreaker/locales/es.json`
- `static/icebreaker/locales/pt.json`
