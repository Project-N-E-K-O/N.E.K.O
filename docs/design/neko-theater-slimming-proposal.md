...
本文记录已经确认的产品目标、瘦身决策和当前实施结果。单猫娘轻量主链已经完成第一轮替换；最终版的 World Contract、受约束剧情补丁和多猫娘人格仍是未来能力。

提案的目的，是把当前版本需要的 1v1 互动小说能力，与最终版本需要的受约束动态叙事和多猫娘人格能力分开，避免为了最终愿景一次性建设全部通用引擎。

当前实现细节参见：

- [`neko-theater-v2.3-implementation-architecture.md`](./neko-theater-v2.3-implementation-architecture.md)
...
3. 自由输入必须得到猫娘回应；未推进时保持当前节点，而不是显示“GM 拉回”。
...
- 行动与对白选项分区；
- 自由输入的场景内角色回应，以及现有 Choice 的非权威上下文化文案；
- 非结局节点始终存在可用选项；
...
自由输入让猫娘回应并记录有上限的非权威场景笔记，不匹配按钮文案、不猜测推进意图，也不改变关键剧情状态。同一次模型调用可以为当前 `choice_id` 返回承接对话的新标签；服务端只采用标签，作者声明的模式、目标节点和 callback 保持不变。只有前端提交稳定 `choice_id` 时才推进作者剧情。

### 10.3 当前版模型输出边界

当前版本的一次模型调用只允许返回旁白、猫娘对白和当前 Choice 的显示文案改写。模型不得返回或控制：
...
使用原创短篇《留给明天的那盘磁带》和二十八回合都市爱情剧本《在晚风重逢以前》确认不同长度的正式 JSON 剧本都可以完整通关：
...
当前 `services/theater` 已从 30 个模块、约 12,689 行缩减为九个 Python 文件（含 `__init__.py`）、共 1,328 行。
...
- 新增原创正式短篇《留给明天的那盘磁带》，用校园记忆中的未完成告别验证“作者主线可通关 + 猫娘自由演绎”；
- 新增二十八回合原创都市爱情剧本《在晚风重逢以前》，验证长主线持续供给选项、完整 Runtime 提交与正式落幕；
...
- 删除全部自由输入 Choice 匹配与澄清链，只有按钮提交的 `choice_id` 才能推进作者节点；
- 删除代码内置测试故事，正式故事统一从 `config/theater/stories/*.json` 加载；
- 增加轻量 Session `schema_version=1`，拒绝迁移未知旧状态并自动清除旧 active 索引；
- 通过两份正式剧本的 70/30 混合压力测试，并把对话上下文、幂等响应与场景笔记限制为固定窗口；
- 删除公开 Trace 中前端不读取的提交结果、线索变化和道具变化字段，以及规则层对应的差异计算；
- 使用真实模型完成两轮共 232 个 70/30 演绎回合，修复自由输入照抄作者固定台词、最近上下文缺少旁白动作和故障回退误接越界请求的问题；
- 修复自由互动与静态选项脱节：模型可在同一次调用中上下文化现有 Choice 标签，节点推进承接最近对话，不再否认刚说出的内容；
- 两份正式剧本补齐 `scenario_card.catgirl_role`，前端在棕色舞台内常驻展示背景、双方身份和目标，避免玩家在不知道自身人设时突兀开演；
- 将前端脚本从约 900 行缩减到 417 行；
- 将 Prompt 配置从多 Engine 提示词缩减为一个 79 行的演绎提示词；
- 用轻量测试替换只验证已删除能力的旧测试。

当前版本仍坚持：自由输入自然回应并保持当前节点；模型只能改写现有 Choice 的显示文案，不能直接写入权威事实。最终版的真相改写能力将在用户反馈验证后通过 World Contract 剧情补丁单独实现。
