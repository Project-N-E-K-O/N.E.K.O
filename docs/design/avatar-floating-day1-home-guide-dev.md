...
| `TutorialInterruptController` | takeover 场景中轻微打断走 `interrupt_resist_light`，生气退出走 `interrupt_angry_exit`；有效打断由位移和加速度共同判断；抵抗结束后恢复当前 presentation。 | 不把 angry exit 当成正常完成；触发瞬间必须清理当前高亮和 Ghost Cursor，语音后走统一 skip。 |
...
| 轻微打断/生气退出 | `angry` | 保留现有抵抗/angry exit 自定义动作，随机 angry motion 不抢锁；生气退出触发时 angry exit 必须覆盖正在播放的教程动作 session。 |
