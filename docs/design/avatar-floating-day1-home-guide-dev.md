...
| 设置一瞥第一句：“在这个只属于我们的空间里……” | 先关闭 Agent 面板；settings 按钮 `#${p}-btn-settings` 作为圆形 retained spotlight；到 `openSettingsPanel` cue 时 action spotlight 指向齿轮。 | 等 cue 到达后移动到齿轮并 click，真实打开设置弹窗。 | 打开后等待设置弹窗稳定，不自动展开角色设置侧边栏。 |
