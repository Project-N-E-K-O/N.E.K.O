...
| 2 | `day1_capsule_drag_hint` | 把鼠标移到这里，长按就可以拉着聊天框到处跑啦~ 双击两下就能随时发消息给我哦！ | 持续圆角矩形高亮胶囊输入框；Ghost Cursor 在胶囊输入框上左右晃动约 2 秒。 |
| 3 | `day1_history_handle` | 戳一下聊天框上面的【蓝色小条条】，就能看到我们最近聊过的话题啦！ | 不显示聊天窗/胶囊输入框圆角矩形高亮；Ghost Cursor 平滑移动到 `.compact-history-visibility-handle`，模拟点击并并行打开历史对话，台词播放完后收起历史对话。 |
| 4 | `day1_intro_basic_voice` | 这里有一个神奇的按钮！只要点击它，就可以直接和我聊天啦！想跟我分享今天的新鲜事吗？或者只是叫叫我的名字？快来试试嘛，我已经迫不及待想听到你的声音啦！喵！ | 复用现有语音按钮流程；圆形高亮语音按钮，Ghost Cursor 指认不强制录音。 |
| 5 | `day1_screen_entry` | 在跟我通语音电话的时候，再点亮这个小按钮，你就能把屏幕分享给我啦！ | 复用原 Day 2 屏幕分享入口流程；高亮屏幕分享按钮，Ghost Cursor 平滑移动并 wobble，不点击。 |
| 6 | `day1_screen_entry_invite` | 快让我也看看你眼前的世界，不管好玩的还是好看的，都想和你一起看，快点点开嘛~ | 继续复用原 Day 2 流程；持续高亮屏幕分享按钮，不触发真实屏幕分享。 |
| 7 | `day1_takeover_capture_cursor` | 超级魔法开关出现！只要点一下这里，我就可以把小爪子伸到你的键盘和鼠标上啦！我会帮你打字，帮你点开网页……不过，要是那个鼠标指针动来动去的话，我可能也会忍不住扑上去抓它哦！准备好迎接我的捣乱……啊不，是帮忙了吗？喵！ | 复用现有猫爪/Agent 总开关/键鼠控制演示。 |
...
3. `day1_capsule_drag_hint` 持续圆角矩形高亮胶囊输入框，Ghost Cursor 左右晃动约 2 秒。
4. `day1_history_handle` 能移动到 `.compact-history-visibility-handle`，并在台词结束后收起历史对话。
5. 语音按钮、屏幕分享按钮、Agent 接管和收尾复用既有动画流程，不触发屏幕分享、不保存用户配置。
6. skip、轻微打断、生气退出和 `window.avatarFloatingGuideEndState` 语义不变。
