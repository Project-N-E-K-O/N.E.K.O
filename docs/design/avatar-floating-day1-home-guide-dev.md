...
本文把 `avatar-floating-guide-feature-tree.md` 中 Day 1 的“初次唤醒、聊天与基础入口”落到当前首页 Yui 教程实现上，并以 `avatar-floating-7day-complete-guide-dev.md` 作为逐句导演、生命周期和验收基准。Day 1 不新增 `AVATAR_FLOATING_GUIDE_ROUNDS[1]`，继续复用现有首页教程 `HOME_SCENE_ORDER`。
...
Day 1 的主线内容仍以本文的首页实现细节为准；凡涉及高光、Ghost Cursor、emotion、真实点击、skip、angry exit、花瓣收尾和外置聊天窗等通用行为，必须同时满足 `avatar-floating-7day-complete-guide-dev.md` 的以下硬约束：

1. `TutorialInteractionTakeover`、`TutorialHighlightController`、`TutorialInterruptController`、`TutorialSkipController`、`TutorialAvatarReloadController` 必须完整接入，不在 Day 1 专属逻辑里复制生命周期。
2. `#neko-tutorial-skip-btn` 全程可见、可点、白名单放行；skip、destroy、pagehide、angry exit 都要清理高光和 Ghost Cursor，并恢复用户原模型。
3. 同一时刻最多保留一套 primary spotlight；聊天窗 composer、语音按钮、Agent 面板、设置弹窗和主动搭话开关之间必须先清理再切换，不创建后再隐藏重复高光。
4. 所有目标选择器优先使用完整指南“目标选择器字典”；不可见时只允许降级到同组真实容器，不伪造 DOM。
5. 收尾 `takeover_return_control` 是 Day 2-7 复用的收尾基准：收尾台词期间重新高亮聊天窗，约 70% cue 同步隐藏 Ghost Cursor、清理内置/外置高光并播放花瓣。
6. Day 1 managed scene 和 Day 2-7 daily scene 一样，必须记录上一段 Ghost Cursor 成功到达的目标锚点；若下一段开始时 cursor position 丢失，应从上一段锚点恢复并平滑移动到新目标，不能闪现到页面中心或当前目标。
...
| `TutorialInterruptController` | takeover 场景中 Ghost Cursor 始终监听真实鼠标移动；真实鼠标有有效移动时先以当前可见停止位置为原点做轻微反方向对抗位移，并回到该停止位置。单次位移约 56px 以上或加速度约 0.16px/ms^2 以上连续累计 3 次后触发一次 `interrupt_resist_light`；第 3 次轻微打断升级为 `interrupt_angry_exit`；抵抗结束后恢复当前 presentation。 | 不把 angry exit 当成正常完成；触发瞬间必须清理当前高亮和 Ghost Cursor，语音后走统一 skip。 |
...
Day 1 迁移到 N.E.K.O.-PC 全局透明 overlay 时，必须严格保留现有首页教程时序：输入框激活、首句问候、语音按钮、猫爪/Agent、插件预览、设置一瞥、主动搭话和 `takeover_return_control` 收尾都不改 scene 顺序。迁移只替换视觉演出层，Ghost Cursor、高光和花瓣由 PC 全局 overlay 统一渲染；网页端继续使用当前 DOM overlay。

输入激活和收尾是 Day 1 的基准能力：Ghost Cursor 初始位置必须来自聊天输入区的 screen 坐标，后续移动到语音、猫爪、设置、主动搭话按钮时都从上一个 cursor 位置平滑过渡；收尾台词期间重新高亮聊天窗，并在约 70% cue 同步隐藏 Ghost Cursor、清理所有高光和播放花瓣。PC bridge 不可用时自动回退 DOM overlay，不阻塞首日教程。

外置聊天窗模式下，聊天窗只负责回传输入区或窗口中心的 screen 锚点，不直接驱动 PC 全局 overlay 的 Ghost Cursor；首页 Director 收到 `yui_guide_chat_cursor_anchor` 后负责显示、移动和隐藏全局 Ghost Cursor。首句问候期间 cursor 必须停在聊天输入区或聊天窗内，不得由外置聊天窗和首页同时渲染两套 cursor；skip、destroy、pagehide 和 angry exit 进入终止清理时必须立即清空外置聊天窗 spotlight/cursor 状态。
...
Day 1 现有自定义动作必须保留并优先：苏醒 pose、输入激活/语音按钮 LookAt、猫爪接管期间 Ghost Cursor LookAt、插件 dashboard handoff、设置一瞥慌乱动作、轻微抵抗、生气退出、归还控制权挥手和花瓣转场。随机动作只在没有抢占这些自定义 motion/params/lookAt/expression 锁时播放。

| 台词段落 | 情绪分类 | 随机动作规则 |
| --- | --- | --- |
| 初见问候：“微风、阳光……” | `happy` | 可随机 happy motion；苏醒 pose 和爱心/拥抱演出优先。 |
| 语音入口：“这里有一个神奇的按钮……” | `happy` | 可随机 happy motion；不得打断语音按钮 LookAt。 |
| 猫爪粗略介绍：“超级魔法开关出现……” | `happy` | 当前 `static/yui-guide-day1-home-guide.js` 配置为 `happy`；真实授权边界由台词和 Agent 总开关演出表达。 |
| 插件预览：“这里还有超多好玩的插件呢……” | `happy` | 从 happy 池随机；插件 dashboard 页面内 runtime 自己负责页面内动作。 |
| 设置一瞥：“在这个只属于我们的小空间里……” | `surprised` | 当前代码配置为 `surprised`，用于进入设置一瞥时的发现感；后续设置细节仍按低打扰说明处理。 |
| 主动搭话：“这个小按钮也很重要哦……” | `happy` | 从 happy 池随机。 |
| 归还控制权：“好啦好啦……” | `happy` | 开头可 happy；70% cue 后归还控制权挥手和花瓣转场独占。 |
| 轻微打断/生气退出 | `angry` | 运行时强制切到 angry。轻微打断触发时暂停当前 scene 和 Ghost Cursor 动画，但不主动清理当前高光；Ghost Cursor 只围绕触发前记录的可见停止位置做轻微反方向局部位移，并回到该停止位置，抵抗语音结束后恢复原 scene 的 cursor 演出和 emotion。生气退出触发时 angry exit 必须覆盖正在播放的教程动作 session，并在台词/演出结束后走统一 skip。 |
...
| 设置一瞥第一句：“在这个只属于我们的小空间里……” | 先关闭 Agent 面板；settings 按钮 `#${p}-btn-settings` 作为圆形 retained spotlight；到 `openSettingsPanel` cue 时 action spotlight 指向齿轮。 | 台词开始后 Ghost Cursor 先平滑移动到齿轮并 wobble；等 cue 到达后 click，真实打开设置弹窗的 API 与 click 动画同步启动。 | 打开后等待设置弹窗稳定，不自动展开角色设置侧边栏。 |
...
3. `takeover_capture_cursor` 能真实打开 Agent 面板并演示总开关、键鼠控制；所有 Ghost Cursor 模拟点击都与对应 API/DOM 操作同步启动，不等待点击动画结束。
