...
| `TutorialHighlightController` | 语音按钮圆形 spotlight、猫爪/设置 retained extra、开关 virtual spotlight、设置侧边栏 union spotlight 全部通过 Director 包装方法调用。 | 不手写虚拟高亮 DOM，不手动残留 `data-yui-guide-spotlight-*` 属性。 |
...
| 语音入口：“这里有一个神奇的小按钮……” | `happy` | 可随机 happy motion；不得打断语音按钮 LookAt。 |
...
| 设置一瞥：“在这个只属于我们的空间里……” | `neutral` | 从 neutral 池随机；设置 tour 稳定说明。 |
| 主动搭话：“这个小按钮对我很重要哦……” | `happy` | 从 happy 池随机。 |
...
| 设置一瞥与主动搭话 | `takeover_settings_peek` | 打开设置弹窗，轻扫角色/API/记忆等长期入口，并高亮主动搭话入口。 |
| 归还控制权 | `takeover_return_control` | 剧本没有单独列出，但代码必须保留清理与完成态。 |
...
| --- | --- | --- | --- |
| 输入框激活提示：“点一下这里……” | 苏醒后先 `ensureChatVisible()`；普通首页模式把 persistent spotlight 放到聊天输入区；外置聊天窗模式改用外置窗口 spotlight。 | Cursor 出现在输入区中心并 wobble；等待用户点击完成浏览器音频激活；激活后再 wobble 一次。 | 激活提示气泡隐藏，overlay 进入 taking-over，才允许进入正式问候。 |
| 初见问候：“微风、阳光……” | 延续输入区/聊天区 spotlight；不打开业务面板。 | Cursor 停留在输入区附近，不移动到新 UI。 | 旁白和模型演出完成后调用 `clearIntroGreetingChatHighlight()`，后续主线不再把 spotlight 拉回聊天窗。 |
| 语音入口：“这里有一个神奇的小按钮……” | 台词进入聊天窗；action spotlight 放到 `#${p}-btn-mic` 的圆形按钮 shell。 | 旁白前段移动到语音按钮中心；移动时间按语音时长约 16% 估算；只指向，不点击。 | 停止 intro voice look-at handle，清理语音按钮方向的临时视线锁。 |
...
| 设置一瞥第一句：“在这个只属于我们的空间里……” | 先关闭 Agent 面板；settings 按钮 `#${p}-btn-settings` 作为圆形 retained spotlight；到 `openSettingsPanel` cue 时 action spotlight 指向齿轮。 | 等 cue 到达后移动到齿轮并 click，真实打开设置弹窗。 | 打开后等待角色设置入口可见。 |
| 设置一瞥第二句：“这个小按钮对我很重要哦……” | 展开角色设置侧边栏；先组合高亮 settings 按钮、角色设置入口、侧边栏/条目 union；随后把 action spotlight 切到主动搭话入口或 `interval-proactive-chat` 侧边栏入口。 | Cursor 先移动到角色设置入口，再移动到侧边栏/条目 union；若实现主动搭话展示，再移动到主动搭话入口并 wobble，不改开关。 | 细节旁白结束后清掉 scene extra、virtual、precise 和 action spotlight；收起侧边栏并关闭设置面板。 |
| 归还控制权：“好啦好啦……” | 清掉 persistent spotlight；目标容器是 `#${p}-container`；70% 语音 cue 触发每日花瓣转场时清掉所有高亮。 | Cursor 移到模型/容器中心并 wobble；关闭面板后移到视口中心再 wobble；70% cue 时隐藏 cursor。 | 花瓣转场期间恢复用户原模型；结束后关闭 taking-over，教程完成。 |

Day 1 的 `takeover_return_control` 是每日花瓣转场的基准实现。Day 2-7 的 wrap/收尾 scene 应复用同一视觉语义：收尾台词约 70% 触发，触发瞬间隐藏 Ghost Cursor、清理 spotlight，正常完成才播放；skip、angry exit 和 destroy 不播放正常收尾花瓣。
...
1. 先高亮齿轮按钮并打开设置弹窗。
2. 角色/API/记忆作为“长期入口”轻扫，不深讲。
3. 播放“这个小按钮对我很重要哦”时，把 spotlight 切到主动搭话入口。
4. 只展示，不实际改用户配置；收尾时恢复设置面板和侧边栏状态。
...
Day 1 新版总稿没有强制聊天窗支线。若后续恢复“聊天工具小提示”，应走聊天窗消息按钮，而不是插进首页主线：
...
5. `takeover_settings_peek` 能打开设置面板，展示角色/API/记忆等长期入口，并高亮主动搭话入口。
6. 教程演示不保存主动搭话或其他设置的临时改动。
7. `takeover_return_control` 正常清理高亮、面板、Ghost Cursor、接管态和临时模型。
8. `takeover_return_control` 的 70% cue 能触发花瓣转场，且转场期间不会残留高亮或 Ghost Cursor。
9. skip、轻微打断、生气退出在任意 takeover 场景中仍走通用模块语义。
