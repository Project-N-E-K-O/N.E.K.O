...
- 启动恢复遇到带授权文件的 `.deleting` 时，只有身份相符才继续清理；身份不符、授权文件无效或探测暂时失败时保留副本并保持待恢复，副本继续计入总配额，不覆盖同时出现的正式目录。只有 `.deleting` 确实不存在时才可回收孤立授权文件。没有授权文件的旧版或已经确认的 `.deleting` 残留仍按原有规则清理。
