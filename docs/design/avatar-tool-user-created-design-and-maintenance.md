...
- record 保存每个受管理资源的 SHA-256。逐字节核验发生在真正消费资源的地方——详情／修改、静态资源服务、互动落库前的权威读取——这些入口只接纳与摘要一致的资源，任何一处读到字节却对不上摘要，就地隔离该道具，其公开投影随即消失。公开目录列表本身只做轻量校验（记录形状、资源存在、摘要键集合、资源闭包），因为前端每次窗口聚焦都会拉取它，逐字节重算会让列表开销随总字节数增长；代价是无人访问过的损坏道具会在列表里短暂可见，直到有人消费它。轻量校验不得被当成核验的替代：任何入口都不能以大小、时间戳或其它文件元数据判定资源完好。读失败（如文件被占用）不等于损坏，不触发隔离；隔离是进程内状态，重启后重新评估。
