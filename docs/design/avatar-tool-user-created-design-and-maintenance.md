...
- record 保存每个受管理资源的 SHA-256；公开目录和修改详情只接纳与摘要一致的资源，单个资源损坏时隔离该道具；不能以大小、时间戳或其它文件元数据代替逐字节核验。
...
- 启动初始化只处理本模块严格命名的临时目录和可证明完整的备份；删除在发布删除前必须先清理同 ID 的旧 backup，使已删除道具不能在启动恢复中复活。普通 GET 不执行清理，也不扫描其它目录；唯一例外是启动初始化已因存储暂时不可用或写围栏而明确挂起时，首次重新确认目录可用且通过围栏的存储操作必须先完成同一初始化恢复。
