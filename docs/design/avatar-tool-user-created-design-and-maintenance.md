...
- record 保存每个受管理资源的 SHA-256；公开目录和修改详情只接纳与摘要一致的资源，单个资源损坏时隔离该道具。重复读取可复用进程内已核验摘要，但文件身份变化后必须重新逐字节核验，不能退回仅凭大小或时间戳发布资源。
...
| 刷新失败 | 保留上一份有效 snapshot，并在 Manager 打开、focus 或 surface 激活时重试。 |
