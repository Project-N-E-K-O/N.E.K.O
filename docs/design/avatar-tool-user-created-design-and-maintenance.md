...
- **能否用 backup 覆盖正式目录，判据是「有没有东西会被牺牲」，不是「正式目录是否有效」**：
  ...
  - 正式目录不存在 —— 可以回滚，没有内容会被牺牲（修改回滚时会先删 `.updating` 再把 backup 挪回，最后那步失败正是这个状态，不恢复就真丢了）；
...
- 这条同样适用于**目录是否存在的探测本身**。`Path.is_dir()` 会把任何 `OSError` 压成 `False`，而「正式目录不存在」正是上面放行回滚的那个答案 —— 于是网络盘上一次瞬时的元数据失败就足以让旧 `.backup` 顶掉盘上完好的正式目录。判定存在性必须用能区分「确实不在」与「这次没读到」的调用（`os.lstat` 分辨 `FileNotFoundError` 与其它 `OSError`），读不到就按待恢复处理。
