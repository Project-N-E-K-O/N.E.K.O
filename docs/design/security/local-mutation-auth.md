...
- `screenshot` / `screenshot/interactive` 走 `_is_loopback_request` 拒绝非环回请求（[`main_routers/system_router.py:194`](main_routers/system_router.py:194)）
- `proactive_chat` 用 `mgr.state.can_start_proactive(...)` / `try_start_proactive(...)` 做并发 / 状态机门控（[`main_routers/system_router.py:4580`](main_routers/system_router.py:4580) / `4605`），并发拒绝时返回 409
