...
它**挡不住**：
- 直接省略 Origin 的本地调用（curl、Node 脚本、Python `requests`、原生 native 进程）
- 任何能读到 token 的同机攻击者（包括 Electron 主进程通过 IPC 拿到 token 后伪造请求）

换句话说：「本地变更端点」**没有被这套合同变成「已鉴权」**——只是关掉了「浏览器中介的 drive-by CSRF」这一个具体威胁面。任何依赖端点防御能力的高敏感功能仍然要在端点内部自己做权限检查（例如 screenshot 端点检查 `_is_loopback_request`、proactive_chat 检查 `_can_trigger_proactively` 等）。
...
NEKO-PC 的渲染器（[N.E.K.O.-PC 仓库](https://github.com/Project-N-E-K-O/N.E.K.O.-PC) `BrowserWindow.loadURL(...)`）从 `http://localhost:<port>/` 加载页面，渲染器里的 `fetch()` **浏览器会自动带** `Origin: http://localhost:<port>`，命中「有 Origin」分支，**必须提供 `X-CSRF-Token`**。

也就是说：
- 同源 Electron 渲染器 → **不能** 靠「无 Origin → 放行」豁免，必须走 token 路径
- 「无 Origin」分支实际只覆盖 *非浏览器* 调用方：curl、Node 脚本、Electron 主进程直接发的 `net.request`，等等
