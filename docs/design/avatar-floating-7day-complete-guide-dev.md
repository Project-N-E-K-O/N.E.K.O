...
2. 首页模板必须加载 `tutorial/yui-guide/avatar-standin.js`，并保证它在 overlay/page-handoff/performance stage 之后、`tutorial/core/universal-manager.js` 之前。`main_routers/pages_router.py` 的 static asset version 输入必须包含 `static/tutorial/yui-guide/avatar-standin.js` 和 `static/app-interpage.js`，避免浏览器缓存旧 runtime。
