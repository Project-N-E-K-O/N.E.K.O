...
1. Day 1 拆分后 scene id 改为 `day1_intro_activation` 等命名；旧的 YUI-only prelude 只按 `intro_` 前缀过滤，导致 `getYuiGuidePreludeSceneIds()` 返回空数组。`director.startPrelude()` 遇到空 prelude 会直接返回，流程看似启动但没有可靠进入 Day 1 avatar floating guide round。
...
1. 首页 Day 1 必须优先走 avatar floating guide round。`UniversalTutorialManager.startTutorialWhenI18nReady()` 在 `shouldStartHomeAvatarFloatingGuideRound()` 为真时直接调用 `startAvatarFloatingGuideRound(1, { source })`；`startTutorialSteps()` 在 home YUI flow 且 prelude 为空时，也必须回落到 Day 1 floating round，不能让空 prelude 静默成功。
...
9. PC 端教程控件必须在 `overChatWindow` 分支前做坐标命中判断。`isPointWithinTutorialInputSurface()` 命中 `#neko-tutorial-skip-btn`、`[data-yui-skip-control]`、`[data-yui-emergency-exit]`、`.yui-guide-overlay`、`.yui-guide-stage`、`.driver-overlay` 或 `.driver-popover` 时，必须先 `setIgnoreState(false, true)` 并返回。这个判断要覆盖 `handleMousePosition()`、`updateDesktopAvatarToolCursorFromPoint()`、`syncMouseThroughStateWithCursor()` 和 `startMousePoller()`。
...
- 主项目：`static/tutorial/core/universal-manager.js`、`static/tutorial/avatar/reload-controller.js`、`static/tutorial/core/skip-controller.js`、`static/app-interpage.js`、`static/app-ui.js`、`static/live2d-init.js`、`templates/index.html`、`main_routers/pages_router.py`、`tests/test_agent_rewrite_regression.py::test_home_yui_guide_avatar_override_does_not_persist_tutorial_model`、`tests/unit/test_avatar_floating_day1_round_contracts.py`
...
| `day1_takeover_return_control`：“好啦好啦...” | `happy`，自然完成时挥手/花瓣自定义优先 | 收尾开始前关闭临时面板；T+0 primary 回到胶囊输入框，spotlight 使用 `plain-capsule`；T+220ms Ghost Cursor 从 `day1_takeover_capture_cursor` 记录的键鼠控制开关锚点平滑移动到 `chat-capsule-input` 中心，移动时长 900ms；自然完成时 T+70% 花瓣 cue 隐藏 cursor、清理所有高光并播放花瓣。本句只使用通用 `cleanup` operation，且 `shouldPreserveExternalizedChatCursor()` 必须保留 `day1_takeover_capture_cursor -> day1_takeover_return_control`，让外置聊天窗 settled anchor 能同步回首页内部 cursor 位置。 | 收尾期间不得保留设置/Agent/按钮高光；不得使用旧版 `day1-managed-scene:takeover_return_control`；不得在进入本句时先清空外置 cursor，否则对抗台词可触发但 Ghost Cursor 没有反向动画。skip 是硬结束：不得播放本句挥手、模型回归、模型淡出或花瓣。 |
