...
1. `day5_character_settings` 打开 `character-settings` 侧边栏，只高亮角色设置入口组；模型、声音、API 只认门，不跳转、不写配置。
2. `day5_character_panic` 是独立替换反应 scene，primary 只落到 `#${p}-sidepanel-live2d-manage`，secondary 只落到 `#${p}-sidepanel-voice-clone`，不再框整张角色设置面板。
...
| --- | --- | --- | --- |
| `day5_character_settings` | `settings-sidepanel:character-settings` | `tour` + `show-settings-sidepanel:character-settings` | 打开角色设置侧边栏，只做入口组认门。 |
| `day5_character_panic` | `#${p}-sidepanel-live2d-manage`，secondary `#${p}-sidepanel-voice-clone` | `tour` + `settings-peek-panic` | 只框模型管理和声音克隆入口，不框整张角色面板。 |
...
- 动作 1：`day5_character_settings` 在 scene 准备阶段打开设置弹窗并展开 `character-settings` 侧边面板；台词开始后高亮角色设置、模型管理、声音克隆与 API Key 等入口组。Ghost Cursor 在入口组附近短 tour，只认门，不强制跳转或修改配置；角色卡与云存档入口不在 Day 5 主线中高亮。
- 台词：“从今天起，我就真正成为只属于你的专属猫娘啦。你看，在这里可以为我穿上漂亮的新衣服，也可以帮我换一个更好听的声音……”
- 动作 2：`day5_character_panic` 不再高亮整张角色设置面板；primary 切到 `#${p}-sidepanel-live2d-manage`，secondary 切到 `#${p}-sidepanel-voice-clone`，Ghost Cursor 只在这两个真实入口附近 tour。该 scene 使用 `surprised`，优先播放 `settings-peek-panic` 自定义慌乱动作；只表达吃醋/慌张，不触发 angry exit。
