...
| 猜拳 | `rps` | `round-choice` | 无 | 第一步只做本地确认，不生成 Host action/intensity |
...
- 猜拳当前第一步在合法松开后只冻结用户已看到的手势并播放声明的确认声，不生成 Host/Python interaction payload。
...
3. 按下冻结屏幕上已经显示的手势，合法松开只生成一次本地确认和确认声；取消链不确认。
4. 图片切换只更新固定视觉节点的内容，不重建视觉组件；Web 和桌面端分别复用现有 session/runtime 清理。
5. 当前猫娘头顶手势、双方碰撞、胜负结果和临时对话尚不属于第一步实现。
