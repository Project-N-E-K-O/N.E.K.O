...
（外部スクリプト、または互換期間中の非推奨な既存 Extension module 等）から登録する場合など。
...
コードが同じ Python process で動く場合（組み込み機能、または互換期間中の
**非推奨な既存 Extension** など）は、HTTP をバイパスして `LLMSessionManager.register_tool(...)`
