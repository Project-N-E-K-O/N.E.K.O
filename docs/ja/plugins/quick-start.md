...
## インストール版で外部プラグインを開発する

既存のプラグインソースがあれば、N.E.K.O 本体のリポジトリをクローンする必要はありません。

開発ディレクトリ API はローカルのデスクトップ版とループバックアドレスのウェブページでのみ利用できます。LAN のリモートページやリバースプロキシ経由のアクセスにはまだ対応していません。

開発パッケージは別の場所に保存され、ローカルの開発ページからダウンロードします。通常のパッケージ一覧には表示されません。登録解除や開発モードの無効化後も、ダウンロードはローカル接続に制限されます。ダウンロードしたファイルは通常のインポート機能でインストールできます。

1. プラグイン管理の左サイドバーで「サーバーログ」の下にある「開発モード」を開き、スイッチを有効にして「未パッケージのプラグインを読み込む」を選択します。
2. `plugin.toml` のあるフォルダーを指定して検証します。ブラウザー版では、**バックエンドが動作するコンピューター**上の絶対パスを入力します。ブラウザーからのフォルダーアップロードではありません。
3. 名前、ID、バージョン、エントリーポイントを確認して読み込みます。起動に失敗しても登録とエラーは残り、修正後に再試行できます。
4. 自分のエディターで変更し、「再読み込み」をクリックします。依存関係を変更した場合も再起動してください。
5. 「パッケージ作成」で `.neko-plugin` をダウンロードし、別の N.E.K.O 環境に通常のインポート機能でインストールして確認します。停止中でもパッケージを作成できます。

フォルダー名はエントリーパッケージ名と一致させ、他のプラグインと同じ ID を使用しないでください。依存関係は既存の宣言と `vendor/` の規則を使用し、自動インストールは行いません。移動したソースは「フォルダーを変更」で関連付けを更新します。ID を変更した場合は登録し直してください。

開発モードを無効にするとプラグインを停止し、関連付けを保持します。アプリの再起動後は手動で起動します。「関連付けを解除」はソースと実行データを保持し、停止が失敗した場合は解除成功と表示しません。

作成中にソースを変更すると再試行が必要です。既存の除外規則はすべての秘密情報の除去を保証しません。パッケージ作成成功とは別に、機能テストとマーケット審査が必要です。

配布前にはソースフォルダー名をプラグイン ID とも一致させてください。既存のパッケージ形式は ID をインストール先のディレクトリ名として使用します。異なる場合はフォルダー名とエントリー／インポートを修正し、関連付けを更新してから再試行します。ソースの自動書き換えは行いません。

以下は CLI によるテンプレート作成、検査、リリース自動化の手順です。
