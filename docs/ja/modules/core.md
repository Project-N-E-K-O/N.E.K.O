...
共有の `TranslationService`（`utils/language_utils.py` 由来）を遅延初期化して返します。このサービスは `translate_text_robust()` / `translate_dict()` を用い、googletrans → translatepy → LLM のフォールバックチェーンで堅牢な翻訳を行います。注意: `LLMSessionManager` はこのアクセサーを公開しているだけで、`translate_if_needed(text)` というメソッドは存在しません。実際の「ユーザーの言語がキャラクターの言語と異なる場合に翻訳する」処理は、セッションマネージャーではなく REST 層（例: キャラクタープロフィールのフィールドを翻訳する `main_routers/characters_router.py`）にあります。
