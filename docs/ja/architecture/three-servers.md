...
## メインサーバー（`app/main_server/__init__.py`、ポート48911）
...
## メモリサーバー（`app/memory_server.py`、ポート48912）
...
## エージェントサーバー（`app/agent_server.py`、ポート48915および48916）

エージェントサーバーは、会話コンテキストによってトリガーされるバックグラウンドタスクの実行を処理します。同一プロセスがポート48915でツールサーバー（Tool Server）を、ポート48916で組み込みのユーザープラグインサーバー（user-plugin server）をホストします。
...
2. エージェントサーバーがタスクを受信し、プランを作成（`task_executor.py`。`DirectTaskExecutor` クラスが従来の Analyzer と Planner の役割を統合）
3. アダプターを通じてアクションを実行：
   - **MCP Client** — Model Context Protocolツール呼び出し
   - **Computer Use** — スクリーンショット分析、マウス/キーボード操作
   - **Browser Use** — Webブラウジング自動化
4. 結果は同じ `DirectTaskExecutor`（`task_executor.py`）で分析され、重複を排除（`deduper.py`）
