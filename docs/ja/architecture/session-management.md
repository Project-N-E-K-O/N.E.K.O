...
`main_logic/core/` パッケージの `LLMSessionManager` クラスは、各キャラクターの会話セッションの中央コーディネーターです。各キャラクターは独自のマネージャーインスタンスを持ちます。
...
自動翻訳（ユーザーの言語がキャラクターの設定言語と異なる場合）は、`TranslationService`（`utils/language_utils.py` 内）によって実行され、googletrans → translatepy → LLMベース翻訳の順にフォールバックします。これは `LLMSessionManager` のメソッドではなく、REST レイヤー（例：キャラクタープロファイルのフィールドを翻訳する `main_routers/characters_router.py`）で実行されます。
