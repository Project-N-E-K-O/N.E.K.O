...
| 変数 | デフォルト | 説明 |
|------|------------|------|
| `NEKO_MERGED` | ソース環境: `0`、凍結パッケージ: `1` | `1` は main、memory、agent の HTTP サービスを同一プロセスで実行しつつ各契約を維持します。`0` は 3 サービスを別プロセスで実行します。既存バックエンドの一部だけを再利用することはなく、競合する公開・内部ポートをフォールバックして、完全な新規構成を分離して起動します。 |
