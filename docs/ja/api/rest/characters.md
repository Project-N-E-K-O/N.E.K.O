...
テキストによる説明から再利用可能な音声を作成して保存します。対応プロバイダーは TTS provider registry で決まります。vLLM-Omni と CosyVoice International は対象外で、Alibaba の国際リージョンでは別の Qwen Voice Design API が必要です。
...
```json
{
  "provider": "cosyvoice",
  "prefix": "aria01",
  "voice_prompt": "温かく明瞭な若い成人の声",
  "ref_language": "en",
  "i18n_language": "ja"
}
```

**レスポンス:** `{ "voice_id": "<プロバイダーの音声 ID>", "provider": "cosyvoice", "source": "design" }`
