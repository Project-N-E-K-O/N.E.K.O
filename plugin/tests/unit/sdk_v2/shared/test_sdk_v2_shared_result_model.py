...
from plugin.sdk_v2.shared.models.result import (
    Err,
    Ok,
    ResultError,
    bind_result,
...
def test_unwrap_must_and_match_result() -> None:
    ...
    assert match_result(ok_result, on_ok=lambda value: f"ok:{value}", on_err=lambda value: f"err:{value}") == "ok:8"
    assert match_result(err_result, on_ok=lambda value: f"ok:{value}", on_err=lambda value: f"err:{value}") == "err:boom"
