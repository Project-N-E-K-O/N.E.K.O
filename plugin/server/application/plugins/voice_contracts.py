"""Shared voice-transcript contract types, helpers, and arbitration logic.

Both the host (app/agent_server) and plugins (e.g. study_companion)
import from this module.  Adding new action types should happen here,
not inside individual plugins, so that no host code ever needs to
import from ``plugin.plugins.*``.
"""
...
VOICE_TRANSCRIPT_ACTIONS = frozenset(
    {
...
# Higher rank = higher priority in arbitration.
VOICE_TRANSCRIPT_ACTION_RANK: dict[str, int] = {
...
def voice_transcript_noop(reason: str, **extra: object) -> dict[str, object]:
    """Produce a ``noop`` action result (plugin declined to act)."""
    payload = dict(extra)
    payload.update(
        {
...
def voice_transcript_cancel_response(
    ...
    **extra: object,
    ...
    """Produce a ``cancel_response`` action (stop the current LLM response)."""
...
def voice_transcript_prime_context(
    ...
    **extra: object,
    ...
    """Produce a ``prime_context`` action (pre-load context into the session)."""
    payload: dict[str, object] = dict(extra)
    payload.update(
        {
...
def _coerce_priority(value: object) -> float:
    """Safely cast a loosely-typed priority to a finite float."""
...
def arbitrate_voice_transcript_results(dispatch_results: object) -> dict[str, Any]:
    """Pick the highest-priority action from parallel plugin dispatch results.

    Tie-breaking: cancel > prime > noop.  Within the same action type,
    higher priority wins; equal priorities break toward the *first*
    handler that reported the action.
    """
