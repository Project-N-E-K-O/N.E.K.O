...
class MessageRecord(BusRecord):
    ...
    def from_raw(raw: Dict[str, Any]) -> "MessageRecord":
        """
        Create a MessageRecord from a raw payload or value.
        
        Normalizes a raw message dict (or wraps a non-dict value) into a MessageRecord by extracting a best-effort float timestamp, plugin_id, source, priority, content, metadata, message_id, message_type, and description. The record's type is derived from message_type, payload['type'], or defaults to "MESSAGE". The original normalized payload is stored on the record's `raw` field.
        
        Parameters:
            raw (Dict[str, Any] | Any): A raw message dictionary to normalize or any value to be wrapped as the message `content`.
        
        Returns:
            MessageRecord: A normalized, immutable message record populated from the provided payload.
        """
    ...
    def dump(self) -> Dict[str, Any]:
        """
        Return a dictionary representation of the record including message-specific fields.
        
        The returned dict contains the serialized fields from the base BusRecord dump augmented with the `message_id`, `message_type`, and `description` keys.
        
        Returns:
            dict: Serialized record with base fields plus `message_id`, `message_type`, and `description`.
        """
        base = super().dump()
...
class MessageList(BusList[MessageRecord]):
    def __init__(
        ...
        *,
        ...
        """
        Initialize a MessageList containing the given MessageRecord items and optional metadata.
        
        Parameters:
        	items (Sequence[MessageRecord]): Sequence of message records to include in the list.
        	plugin_id (Optional[str]): Identifier of the plugin that produced these messages. Use "*" to indicate messages from multiple plugins.
        	ctx (Optional[Any]): Optional plugin context associated with the list.
        	trace (Optional[Sequence[BusOp]]): Optional operation trace for the list.
        	plan (Optional[Any]): Optional execution plan associated with the list.
        	fast_mode (bool): When True, create the list in a lightweight mode that may omit or reduce additional metadata.
        """
    ...
    def merge(self, other: "BusList[MessageRecord]") -> "MessageList":
        """
        Merge this MessageList with another MessageList and return a new combined MessageList.
        
        Parameters:
            other (BusList[MessageRecord]): The other message list to merge into this one.
        
        Returns:
            MessageList: A new MessageList containing records from both lists. The returned list's
            `plugin_id` is `self.plugin_id` if it equals `other.plugin_id`, otherwise `"*"`. The
            returned instance preserves the merged trace, plan, context, and `fast_mode` flag.
        """
    ...
    def __add__(self, other: "BusList[MessageRecord]") -> "MessageList":
        """
        Return a new MessageList representing the merged contents of this list and another.
        
        Parameters:
            other (BusList[MessageRecord]): The message list to merge with.
        
        Returns:
            MessageList: A merged MessageList; if the two lists have the same plugin_id that id is preserved, otherwise the resulting plugin_id is "*". Trace, plan, and fast_mode are preserved or propagated from the inputs.
        """
...
class _LocalMessageCache:
    ...
    def __post_init__(self) -> None:
        """
        Initialize the cache's internal queue and optional thread lock.
        
        Attempts to create a bounded deque using self.maxlen; if that fails, falls back to a plain list.
        Then attempts to create a threading.Lock and assigns it to self._lock; if that fails, sets self._lock to None.
        """
        try:
    ...
    def on_delta(self, _bus: str, op: str, delta: Dict[str, Any]) -> None:
        """
        Handle a bus delta for messages by appending a compact record to the internal recent-message queue.
        
        Processes only "add" and "change" operations and, when `delta` contains a non-empty string `message_id`, builds a small dict with `message_id` and any of `rev`, `priority`, `source`, and `export` present in `delta`, then appends it to the instance queue. Invalid operations, missing/invalid `message_id`, or malformed `delta` are ignored. Appends are performed under the instance lock when available to ensure thread safety.
         
        Parameters:
            _bus (str): Originating bus name (unused by this handler).
            op (str): Operation name; only "add" and "change" are processed.
            delta (Dict[str, Any]): Delta payload expected to include `message_id` and optionally `rev`, `priority`, `source`, or `export`.
        """
    ...
    def tail(self, n: int) -> List[Dict[str, Any]]:
        """
        Return the last `n` cached message delta items from the local queue.
        
        Parameters:
            n (int): Number of items to return. If `n` is less than or equal to 0, an empty list is returned.
        
        Returns:
            List[Dict[str, Any]]: A list of up to `n` most-recent cache entries (each a dict). If `n` is greater than or equal to the queue length, the entire queue is returned. The call obtains the cache lock if one is configured to produce a consistent snapshot.
        """
        nn = int(n)
        if nn <= 0:
            return []
...
def _ensure_local_cache() -> _LocalMessageCache:
    """
    Ensure a global local message cache exists and return it.
    
    Creates a new _LocalMessageCache and attempts to register its on_delta handler as a bus change listener for "messages" if a global cache is not already present.
    
    Returns:
        _LocalMessageCache: The existing or newly created global local message cache.
    """
...
class MessageClient:
    ...
    def get(
        ...
        """
        Fetches messages from the bus that match the provided criteria.
        
        Builds and sends a MESSAGE_GET request to the bus (via ZeroMQ or the plugin communication queue), waits for a response, normalizes the returned payloads into MessageRecord objects, and returns them as a MessageList. When `raw` is True and `plugin_id` is None or "*" with no additional filters (priority_min, source, filter, since_ts), a local in-process cache is used to return recent messages without an IPC round-trip.
        
        Parameters:
            plugin_id (Optional[str]): Plugin ID to filter messages for; use "*" to query across all plugins. If omitted, defaults to the current plugin's ID in the returned MessageList.
            max_count (int): Maximum number of messages to retrieve.
            priority_min (Optional[int]): Minimum priority to include.
            source (Optional[str]): Source identifier to filter messages.
            filter (Optional[Dict[str, Any]]): Arbitrary metadata filter applied to messages.
            strict (bool): If True, apply filtering strictly.
            since_ts (Optional[float]): Only include messages with timestamp >= this POSIX timestamp.
            timeout (float): Seconds to wait for a bus response before raising a timeout.
            raw (bool): If True, perform minimal normalization and preserve original payloads in each record's `raw` field (enables local-cache fast path when applicable).
        
        Returns:
            MessageList: A MessageList of MessageRecord objects matching the request parameters. The MessageList.plugin_id reflects the effective plugin scope (resolved from `plugin_id` or the current context).
        
        Raises:
            TimeoutError: If no valid response is received within `timeout`.
            RuntimeError: On IPC failures, invalid responses, or if the plugin communication queue is unavailable.
        """
    ...
    def delete(self, message_id: str, timeout: float = 5.0) -> bool:
        """
        Delete a message from the bus by its identifier.
        
        Parameters:
            message_id (str): The identifier of the message to delete.
        ...
        Raises:
            ...
            RuntimeError: If the plugin communication queue is unavailable or an IPC/result is invalid.
