...
class StudyCompanionPlugin(NekoPluginBase):
    ...
    def _update_screen_classification(
        ...
        if self._event_bus is not None and new_type:
    ...
    async def _emit_review_due_if_needed(self) -> None:
        ...
        try:
            ...
            if due_count <= 0:
                return
            topics = await asyncio.to_thread(self._get_due_topics)
            bus.schedule_emit(
                StudyEvent(
                    name="review_due",
                    payload={
    ...
    def _get_due_topics(self) -> list[str]:
        topics: list[str] = []
        for item in self._memory_deck_store.due_reviews(limit=50):
