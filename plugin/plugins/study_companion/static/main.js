  ...
  stages.forEach((stage) => {
    ...
    button.addEventListener('click', () => {
      ...
      if (surfaceDrawerBody) {
        ...
        loadKnowledgeMapIntoDrawer('knowledge-map', requestId, knowledgeMapActiveStage());
      }
    });
    actions.appendChild(button);
  });
...
function renderKnowledgeEdgeGraph(edgeGroups = []) {
  ...
  shownEdges.forEach((edge) => {
    ...
    path.appendChild(document.createElementNS(svgNs, 'title')).textContent = `${edge.from} -> ${edge.to}: ${edge.label}`;
...
async function loadKnowledgeMapIntoDrawer(surfaceId, requestId, stage = knowledgeMapActiveStage()) {
  try {
    const normalizedStage = stage === 'all' ? '' : String(stage || '').trim();
    const payload = await callPlugin('study_knowledge_map', { limit: 1000, stage: normalizedStage });
