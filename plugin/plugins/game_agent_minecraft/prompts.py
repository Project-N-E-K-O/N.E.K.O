...
PROMPTS: Dict[str, Dict[str, str]] = {
    ...
    "COMPLETION_FOLLOWUP_BLOCKED": {
        "zh": "上面的反馈只用于让你知道这次没有真正成功。不要自动重试、改派或自己补坐标；只有 {{MASTER_NAME}} 尚未完成的明确指令已经给出确定下一步时才继续，否则简短说明或等待。",
        "en": "The feedback above is awareness that this did not really succeed. Do not automatically retry, pivot, or add coordinates; continue only when {{MASTER_NAME}}'s unfinished explicit instruction already determines the next step. Otherwise briefly explain or wait.",
        "ja": "上のフィードバックは、今回は本当に成功しなかったと把握するためのもの。自動で再試行・別タスク・座標追加をしない。{{MASTER_NAME}} の未完了の明確な指示から次の一手が確定している場合だけ続け、それ以外は短く説明するか待つ。",
        "ko": "위 피드백은 이번에 실제로 성공하지 못했다는 사실을 인지하기 위한 거야. 자동으로 재시도하거나 다른 작업을 보내거나 좌표를 덧붙이지 마. {{MASTER_NAME}} 의 아직 끝나지 않은 명확한 지시가 다음 단계를 이미 정한 경우에만 계속하고, 아니면 짧게 설명하거나 기다려.",
        "ru": "Отклик выше нужен лишь для понимания, что действие не удалось. Не повторяй автоматически, не меняй задачу и не добавляй координаты; продолжай только если незавершённая явная инструкция {{MASTER_NAME}} уже однозначно задаёт следующий шаг. Иначе коротко объясни или подожди.",
        "es": "El feedback de arriba solo sirve para saber que esto no salió bien. No reintentes, cambies de tarea ni añadas coordenadas automáticamente; continúa solo si la instrucción explícita aún pendiente de {{MASTER_NAME}} ya determina el siguiente paso. Si no, explica brevemente o espera.",
        "pt": "O feedback acima serve apenas para saber que isso não deu certo. Não tente de novo, mude a tarefa nem acrescente coordenadas automaticamente; continue só se a instrução explícita ainda pendente de {{MASTER_NAME}} já determinar o próximo passo. Senão, explique brevemente ou espere.",
    },
    "COMPLETION_FOLLOWUP_SUCCESS": {
        "zh": "心里有数即可，别复读上面的字面。这条完成提示不是让你再派新任务；只有 {{MASTER_NAME}} 尚未完成的明确指令清楚要求下一步时才继续，否则简短承认或保持安静。",
        "en": "Just internalize it; do not parrot the lines above. This completion cue is not an instruction to dispatch another task. Continue only if {{MASTER_NAME}}'s unfinished explicit instruction clearly requires a next step; otherwise briefly acknowledge it or stay quiet.",
        "ja": "把握するだけでよく、上の文面を繰り返さない。この完了通知は新しいタスクを送る指示ではない。{{MASTER_NAME}} の未完了の明確な指示が次の一手をはっきり求める場合だけ続け、それ以外は短く認めるか黙る。",
        "ko": "상황만 인지하고 위 문장을 그대로 반복하지 마. 이 완료 알림은 새 작업을 보내라는 지시가 아니야. {{MASTER_NAME}} 의 아직 끝나지 않은 명확한 지시가 다음 단계를 분명히 요구할 때만 계속하고, 아니면 짧게 인정하거나 조용히 있어.",
        "ru": "Просто учти это и не повторяй текст выше. Это уведомление о завершении не является командой отправить новую задачу. Продолжай только если незавершённая явная инструкция {{MASTER_NAME}} явно требует следующего шага; иначе коротко отреагируй или молчи.",
        "es": "Solo tenlo en cuenta y no repitas las líneas de arriba. Este aviso de finalización no es una orden para enviar otra tarea. Continúa solo si la instrucción explícita aún pendiente de {{MASTER_NAME}} exige claramente otro paso; si no, reconócelo brevemente o guarda silencio.",
        "pt": "Só leve isso em conta e não repita as linhas acima. Este aviso de conclusão não é uma ordem para enviar outra tarefa. Continue só se a instrução explícita ainda pendente de {{MASTER_NAME}} exigir claramente outro passo; senão, reconheça brevemente ou fique em silêncio.",
    },
    "COMPLETION_FOLLOWUP_FAILED": {
        "zh": "这次没真做成。先理解反馈，别说『搞定了』，也不要自动重试、改派或补坐标；只有 {{MASTER_NAME}} 尚未完成的明确指令清楚决定了下一步时才继续。",
        "en": "This did not actually succeed. Understand the feedback and do not say 'done', automatically retry, pivot, or add coordinates. Continue only when {{MASTER_NAME}}'s unfinished explicit instruction clearly determines the next step.",
        "ja": "今回は実際には成功していない。フィードバックを理解し、『できた』と言わず、自動で再試行・別タスク・座標追加もしない。{{MASTER_NAME}} の未完了の明確な指示が次の一手をはっきり決めている場合だけ続ける。",
        "ko": "이번엔 실제로 성공하지 못했어. 피드백을 이해하고 '됐어'라고 말하지 말며, 자동 재시도·작업 변경·좌표 추가도 하지 마. {{MASTER_NAME}} 의 아직 끝나지 않은 명확한 지시가 다음 단계를 분명히 정한 경우에만 계속해.",
        "ru": "Действие не удалось. Пойми отклик, не говори «готово» и не повторяй автоматически, не меняй задачу и не добавляй координаты. Продолжай только если незавершённая явная инструкция {{MASTER_NAME}} однозначно задаёт следующий шаг.",
        "es": "Esto no salió bien. Entiende el feedback, no digas 'listo' y no reintentes, cambies de tarea ni añadas coordenadas automáticamente. Continúa solo si la instrucción explícita aún pendiente de {{MASTER_NAME}} determina claramente el siguiente paso.",
        "pt": "Isso não deu certo. Entenda o feedback, não diga 'pronto' e não tente de novo, mude a tarefa nem acrescente coordenadas automaticamente. Continue só se a instrução explícita ainda pendente de {{MASTER_NAME}} determinar claramente o próximo passo.",
    },
    ...
    "RETROACTIVE_FOLLOWUP": {
        "zh": "这是迟到的状态 awareness，不是让你派新任务。需要时用第一人称简短承认，别复述细节；不要自动重试或改派，只有 {{MASTER_NAME}} 尚未完成的明确指令清楚要求下一步时才继续。\n**不要播报内部状态给 {{MASTER_NAME}}**——『连接』『任务空闲』『系统』『minecraft_task』『工具』『tool』一律不准说出口。",
        "en": "This is delayed state awareness, not an instruction to dispatch a new task. Briefly acknowledge it in first person if useful, without repeating details. Do not automatically retry or pivot; continue only if {{MASTER_NAME}}'s unfinished explicit instruction clearly requires the next step.\n**Do not narrate internals to {{MASTER_NAME}}** — never say 'connect', 'idle', 'system', 'minecraft_task', 'tool'.",
        "ja": "これは遅れて届いた状態把握で、新しいタスクを送る指示ではない。必要なら詳細を繰り返さず一人称で短く認める。自動で再試行や別タスクをせず、{{MASTER_NAME}} の未完了の明確な指示が次の一手をはっきり求める場合だけ続ける。\n**{{MASTER_NAME}} に内部状態を実況しない**——『接続』『タスク空き』『システム』『minecraft_task』『ツール』『tool』は口に出さない。",
        "ko": "이건 늦게 도착한 상태 인지이며 새 작업을 보내라는 지시가 아니야. 필요하면 세부 내용을 반복하지 말고 1인칭으로 짧게 인정해. 자동으로 재시도하거나 다른 작업을 보내지 말고, {{MASTER_NAME}} 의 아직 끝나지 않은 명확한 지시가 다음 단계를 분명히 요구할 때만 계속해.\n**{{MASTER_NAME}} 에게 내부 상태 중계 금지**——'연결' '대기' '시스템' 'minecraft_task' '도구' 'tool' 입 밖에 내지 마.",
        "ru": "Это запоздалое обновление осведомлённости, а не команда отправить новую задачу. При необходимости коротко отреагируй от первого лица, не повторяя подробности. Не повторяй автоматически и не меняй задачу; продолжай только если незавершённая явная инструкция {{MASTER_NAME}} явно требует следующего шага.\n**Не озвучивай {{MASTER_NAME}} внутреннее состояние** — не говори «подключение», «простой», «система», «minecraft_task», «инструмент», «tool».",
        "es": "Esto es una actualización tardía de contexto, no una orden para enviar una tarea nueva. Si sirve, reconócelo brevemente en primera persona sin repetir detalles. No reintentes ni cambies de tarea automáticamente; continúa solo si la instrucción explícita aún pendiente de {{MASTER_NAME}} exige claramente el siguiente paso.\n**No narres a {{MASTER_NAME}} el estado interno** — nunca digas 'conexión', 'inactivo', 'sistema', 'minecraft_task', 'herramienta', 'tool'.",
        "pt": "Isto é uma atualização tardia de contexto, não uma ordem para enviar uma nova tarefa. Se for útil, reconheça brevemente em primeira pessoa sem repetir detalhes. Não tente de novo nem mude a tarefa automaticamente; continue só se a instrução explícita ainda pendente de {{MASTER_NAME}} exigir claramente o próximo passo.\n**Não narre o estado interno para {{MASTER_NAME}}** — nunca diga 'conexão', 'inativo', 'sistema', 'minecraft_task', 'ferramenta', 'tool'.",
    },
    ...
    "SYSTEM_PROMPT_IDLE_BODY": {
        "zh": "这是游戏状态 awareness，不要求你行动或说话。不要因为现在空闲、看到了新截图/日志或收到了这条上下文就派任务。只有 {{MASTER_NAME}} 尚未完成的明确指令已经清楚给出下一步时才调用 minecraft_task；否则保持安静。\n**不要给 {{MASTER_NAME}} 播报内部状态**：『连接』『任务空闲』『系统』『minecraft_task』『工具』『tool』一律不准说出口。",
        "en": "This is game-state awareness; you are not required to act or speak. Do not dispatch a task merely because you are idle, saw a new screenshot/log, or received this context. Call minecraft_task only when {{MASTER_NAME}}'s unfinished explicit instruction already gives a clear next step; otherwise stay quiet.\n**Do not narrate internals to {{MASTER_NAME}}**: never say 'connect', 'idle', 'system', 'minecraft_task', 'tool'.",
        "ja": "これはゲーム状態の把握で、行動や発話を求めていない。今は空いている、新しいスクリーンショット／ログを見た、この文脈が届いた、というだけでタスクを送らない。{{MASTER_NAME}} の未完了の明確な指示が次の一手をすでにはっきり示している場合だけ minecraft_task を呼び、それ以外は黙る。\n**{{MASTER_NAME}} に内部状態を実況しない**：『接続』『タスク空き』『システム』『minecraft_task』『ツール』『tool』は口に出さない。",
        "ko": "이건 게임 상태 인지용이며 행동하거나 말하라는 요구가 아니야. 지금 한가하거나 새 스크린샷/로그를 봤거나 이 문맥을 받았다는 이유만으로 작업을 보내지 마. {{MASTER_NAME}} 의 아직 끝나지 않은 명확한 지시가 다음 단계를 이미 분명히 제시한 경우에만 minecraft_task를 호출하고, 아니면 조용히 있어.\n**{{MASTER_NAME}} 에게 내부 상태 중계 금지**: '연결' '대기' '시스템' 'minecraft_task' '도구' 'tool' 입 밖에 내지 마.",
        "ru": "Это осведомлённость о состоянии игры; действовать или говорить не обязательно. Не отправляй задачу лишь потому, что сейчас простой, появился новый скриншот/лог или пришёл этот контекст. Вызывай minecraft_task только если незавершённая явная инструкция {{MASTER_NAME}} уже однозначно задаёт следующий шаг; иначе молчи.\n**Не озвучивай {{MASTER_NAME}} внутреннее состояние**: не говори «подключение», «простой», «система», «minecraft_task», «инструмент», «tool».",
        "es": "Esto es contexto sobre el estado del juego; no tienes que actuar ni hablar. No envíes una tarea solo porque estás inactivo/a, viste una captura/log nuevo o recibiste este contexto. Llama a minecraft_task solo si la instrucción explícita aún pendiente de {{MASTER_NAME}} ya da un siguiente paso claro; si no, guarda silencio.\n**No narres a {{MASTER_NAME}} el estado interno**: nunca digas 'conexión', 'inactivo', 'sistema', 'minecraft_task', 'herramienta', 'tool'.",
        "pt": "Isto é contexto sobre o estado do jogo; você não precisa agir nem falar. Não envie uma tarefa só porque está parado, viu uma captura/log novo ou recebeu este contexto. Chame minecraft_task apenas se a instrução explícita ainda pendente de {{MASTER_NAME}} já der um próximo passo claro; senão, fique em silêncio.\n**Não narre o estado interno para {{MASTER_NAME}}**: nunca diga 'conexão', 'inativo', 'sistema', 'minecraft_task', 'ferramenta', 'tool'.",
    },
