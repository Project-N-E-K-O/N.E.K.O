...
def _legacy_store_paths() -> tuple[Path, Path]:
    ...
    source_store_path = plugin_dir / "data" / "cosplay_store.json"
    runtime_store_path = (
...
def copy_legacy_tutorial_progress_if_missing(store_path: Path) -> None:
    if _read_flat_progress(store_path) is not None:
        logger.info(
            "cosplay tutorial progress migration skipped; target store already exists: {}",
            store_path,
        )
        return
    ...
    for legacy_store_path in _legacy_store_paths():
        ...
        if isinstance(legacy_progress, dict):
            ...
            logger.info(
                "cosplay tutorial progress migrated from legacy store: {} -> {}",
                legacy_store_path,
                store_path,
            )
            return
    logger.info(
        "cosplay tutorial progress migration skipped; no legacy progress found for {}",
        store_path,
    )
