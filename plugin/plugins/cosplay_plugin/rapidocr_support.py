...
def _app_runtimes_root() -> Path:
    return get_config_manager().app_docs_dir / "runtimes" / "cosplay_plugin"
...
def _resolve_rapidocr_model_paths(
    *,
    ...
    test_cosplay_rapidocr_support fixtures that came in with PR #1194).
    """
...
def inspect_rapidocr_installation(
    *,
    ...
    # pyproject.toml [dependency-groups] cosplay). Treat either source as
    ...
    return {
        ...
        "installed": installed,
        # rapidocr-onnxruntime is now bundled into the main program (see
        # pyproject.toml [dependency-groups] cosplay). When it's not importable
        # the user is on a source install without `uv sync --group cosplay` —
            ...
            try:
                async with client.stream(
                    ...
                    headers={
                        ...
                        "User-Agent": "N.E.K.O/cosplay_plugin",
                    },
                ) as response:
