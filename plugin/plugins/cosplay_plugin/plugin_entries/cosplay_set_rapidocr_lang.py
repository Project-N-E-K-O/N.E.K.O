...
class _CosplaySetRapidocrLangMixin:
    @plugin_entry(
        id="cosplay_set_rapidocr_lang",
        name=tr("entries.cosplay_set_rapidocr_lang.name", default='切换 OCR 识别语言'),
        description=tr(
            "entries.cosplay_set_rapidocr_lang.description",
    ...
    async def cosplay_set_rapidocr_lang(
        self,
            ...
            if self._ocr_reader_manager is not None:
                try:
                    ...
                    _log_plugin_noncritical(
                        self.logger,
                        "warning",
                        "cosplay rapidocr lang rollback update_config failed: {}",
                        rollback_exc,
                    )
