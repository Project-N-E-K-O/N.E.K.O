...
class _GalgameSetOcrBackendMixin:
    @plugin_entry(
        id="cosplay_set_ocr_backend",
        name=tr("entries.cosplay_set_ocr_backend.name", default='设置 OCR / 截图后端'),
        description=tr("entries.cosplay_set_ocr_backend.description", default='切换 OCR 文本识别后端和截图后端。只影响 OCR 读取，不改变 Agent 点击安全策略。'),
        input_schema={
            "type": "object",
            "properties": {
                "backend_selection": {
    ...
    async def cosplay_set_ocr_backend(
        self,
            ...
            if self._ocr_reader_manager is not None:
                try:
                    ...
                    _log_plugin_noncritical(
                        self.logger,
                        "warning",
                        "cosplay OCR backend rollback update_config failed: {}",
                        rollback_exc,
                    )
            ...
            if callable(reset_capture_runtime):
                try:
                    ...
                    _log_plugin_noncritical(
                        self.logger,
                        "warning",
                        "cosplay OCR backend switch diagnostic reset failed: {}",
                        exc,
                    )
