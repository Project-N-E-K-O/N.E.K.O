...
class _GalgameBuildOcrScreenTemplateDraftMixin:
    @plugin_entry(
        id="cosplay_build_ocr_screen_template_draft",
        name=tr("entries.cosplay_build_ocr_screen_template_draft.name", default='生成 OCR 屏幕模板草稿'),
        description=tr("entries.cosplay_build_ocr_screen_template_draft.description", default='根据当前 OCR 运行时、窗口信息和最近识别文本生成可编辑的屏幕模板草稿。'),
        input_schema={
            "type": "object",
            "properties": {
                "stage": {
    ...
    async def cosplay_build_ocr_screen_template_draft(
        self,
        stage: str | None = None,
