...
class _GalgameSuggestChoiceMixin:
    @plugin_entry(
        id="cosplay_suggest_choice",
        name=tr("entries.cosplay_suggest_choice.name", default='建议当前选项'),
        description=tr("entries.cosplay_suggest_choice.description", default='对当前可见选项给出推荐顺位与理由。'),
        input_schema={"type": "object", "properties": {}},
        timeout=45.0,
        llm_result_fields=["choices", "diagnostic"],
    )
    async def cosplay_suggest_choice(self, **_):
        if self._llm_gateway is None:
            return Err(SdkError("cosplay_plugin llm_gateway is not initialized"))
