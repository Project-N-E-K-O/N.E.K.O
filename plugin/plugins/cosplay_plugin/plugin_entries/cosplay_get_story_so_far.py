...
class _CosplayGetStorySoFarMixin:
    @plugin_entry(
        id="cosplay_get_story_so_far",
        name=tr(
            "entries.cosplay_get_story_so_far.name",
            default="查询全局故事线",
        ),
        description=tr(
            "entries.cosplay_get_story_so_far.description",
    ...
    async def cosplay_get_story_so_far(self, **_):
        if self._cfg is None:
            return Err(SdkError(self._not_configured_message()))
        throttled = self._check_query_rate_limit("cosplay_get_story_so_far")
