...
class _GalgameSetModeMixin:
    @plugin_entry(
        id="cosplay_set_mode",
        name=tr("entries.cosplay_set_mode.name", default='设置 cosplay 模式'),
        description=tr("entries.cosplay_set_mode.description", default='设置 silent / companion / choice_advisor 模式，并可选更新通知开关。'),
        input_schema={
            "type": "object",
            "properties": {
    ...
    async def cosplay_set_mode(
        ...
        **_,
        ...
        if mode not in MODES:
            return Err(SdkError(f"invalid cosplay mode: {mode!r}"))
            ...
            for manager, label in (
                ...
                try:
                    ...
                    _log_plugin_noncritical(
                        self.logger,
                        "warning",
                        "cosplay {} mode rollback update_config failed: {}",
                        label,
                        rollback_exc,
                    )

        # cosplay_set_mode runs in the plugin's asyncio flow; simple config field
        ...
        try:
            ...
            try:
                ...
                _log_plugin_noncritical(
                    self.logger,
                    "warning",
                    "cosplay mode preference rollback failed: {}",
                    rollback_exc,
                )
            _restore_mode_runtime_state()
            return Err(SdkError(f"persist mode failed: {exc}"))
