...
class _CosplaySetOcrScreenTemplatesMixin:
    @plugin_entry(
        id="cosplay_set_ocr_screen_templates",
        name=tr("entries.cosplay_set_ocr_screen_templates.name", default='设置 OCR 屏幕模板'),
        description=tr("entries.cosplay_set_ocr_screen_templates.description", default='保存 OCR 屏幕分类模板；模板仅影响 OCR Reader，不影响 Bridge SDK / Memory Reader。'),
        input_schema={
            "type": "object",
            "properties": {
                "screen_templates": {
    ...
    async def cosplay_set_ocr_screen_templates(
        self,
            ...
            if self._ocr_reader_manager is not None:
                try:
                    ...
                    _log_plugin_noncritical(
                        self.logger,
                        "warning",
                        "cosplay OCR screen template rollback update_config failed: {}",
                        rollback_exc,
                    )
