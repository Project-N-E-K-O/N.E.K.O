...
class _CosplayRollbackOcrCaptureProfileMixin:
    @plugin_entry(
        id="cosplay_rollback_ocr_capture_profile",
        name=tr("entries.cosplay_rollback_ocr_capture_profile.name", default='回滚 OCR 推荐截图校准'),
        description=tr("entries.cosplay_rollback_ocr_capture_profile.description", default='回滚最近一次由推荐 profile 应用产生的 OCR 截图校准。'),
        input_schema={
            "type": "object",
            "properties": {
    ...
    async def cosplay_rollback_ocr_capture_profile(
        self,
        confirm: bool = False,
        **_,
    ):
