...
class _CosplayAgentCommandMixin:
    @plugin_entry(
        id="cosplay_agent_command",
        name=tr("entries.cosplay_agent_command.name", default='向 Game LLM Agent 发送指令'),
        description=tr("entries.cosplay_agent_command.description", default='查询 Agent 状态、上下文、发送消息或控制待机。'),
        input_schema={
            "type": "object",
            "properties": {
                "action": {
    ...
    async def cosplay_agent_command(
        self,
        action: str,
        message: str = "",
        context_query: str = "",
        ...
        **_,
        ...
        if self._game_agent is None:
            return Err(SdkError("cosplay_plugin game agent is not initialized"))
