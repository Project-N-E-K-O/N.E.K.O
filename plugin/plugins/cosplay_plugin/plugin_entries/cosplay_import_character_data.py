...
class _CosplayImportCharacterDataMixin:
    @plugin_entry(
        id="cosplay_import_character_data",
        name=tr(
            "entries.cosplay_import_character_data.name",
            default="导入角色档案 JSON",
        ),
        description=tr(
            "entries.cosplay_import_character_data.description",
    ...
    async def cosplay_import_character_data(
        self,
        file_path: str,
        game_id: str = "",
        **_,
        ...
        try:
            ...
            self.logger.warning(
                "cosplay character data import failed",
                exc_info=True,
            )
            return Err(
                SdkError(
