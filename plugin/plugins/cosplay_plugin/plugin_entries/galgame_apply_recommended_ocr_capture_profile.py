...
class _GalgameApplyRecommendedOcrCaptureProfileMixin:
    @plugin_entry(
        id="cosplay_apply_recommended_ocr_capture_profile",
        name=tr("entries.cosplay_apply_recommended_ocr_capture_profile.name", default='应用推荐 OCR 截图校准'),
        description=tr("entries.cosplay_apply_recommended_ocr_capture_profile.description", default='在用户确认后应用当前 OCR Reader 推荐的截图 profile，并记录回滚点。'),
        input_schema={
            "type": "object",
            "properties": {
    ...
    async def cosplay_apply_recommended_ocr_capture_profile(
        self,
        confirm: bool = False,
