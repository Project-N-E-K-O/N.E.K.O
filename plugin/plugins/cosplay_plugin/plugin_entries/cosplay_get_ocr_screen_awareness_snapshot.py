...
class _CosplayGetOcrScreenAwarenessSnapshotMixin:
    @plugin_entry(
        id="cosplay_get_ocr_screen_awareness_snapshot",
        name=tr("entries.cosplay_get_ocr_screen_awareness_snapshot.name", default='获取 OCR 屏幕感知截图'),
        description=tr("entries.cosplay_get_ocr_screen_awareness_snapshot.description", default='返回最近一次 OCR 屏幕感知截图；仅在 Vision 显式开启且短期缓存未过期时可用。'),
        input_schema={"type": "object", "properties": {}},
        llm_result_fields=["summary"],
    )
    async def cosplay_get_ocr_screen_awareness_snapshot(self, **_):
