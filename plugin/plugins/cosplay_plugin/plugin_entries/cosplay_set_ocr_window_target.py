...
class _CosplaySetOcrWindowTargetMixin:
    @plugin_entry(
        id="cosplay_set_ocr_window_target",
        name=tr("entries.cosplay_set_ocr_window_target.name", default='设置 OCR 目标窗口'),
        description=tr("entries.cosplay_set_ocr_window_target.description", default='锁定或清除 OCR Reader 的手动目标窗口。'),
        input_schema={
            "type": "object",
            "properties": {
    ...
    async def cosplay_set_ocr_window_target(
        self,
        window_key: str = "",
        clear: bool = False,
        **_,
        ...
        try:
            ...
            _log_plugin_noncritical(
                self.logger,
                "warning",
                "cosplay OCR window target previous store read failed: {}",
                exc,
            )
        ...
        try:
            ...
            try:
                ...
                _log_plugin_noncritical(
                    self.logger,
                    "warning",
                    "cosplay OCR window target persist rollback failed: {}",
                    rollback_exc,
                )
            with self._state_lock:
