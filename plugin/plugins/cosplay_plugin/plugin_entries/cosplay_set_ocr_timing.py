...
class _CosplaySetOcrTimingMixin:
    @plugin_entry(
        id="cosplay_set_ocr_timing",
        name=tr("entries.cosplay_set_ocr_timing.name", default='设置 OCR 识别时机'),
        description=tr("entries.cosplay_set_ocr_timing.description", default='设置 OCR Reader 触发模式与轮询间隔；DXcam 截图后端会随 OCR 触发。'),
        input_schema={
            "type": "object",
            "properties": {
    ...
    async def cosplay_set_ocr_timing(
        self,
            ...
            if self._ocr_reader_manager is not None:
                try:
                    ...
                    _log_plugin_noncritical(
                        self.logger,
                        "warning",
                        "cosplay OCR timing rollback update_config failed: {}",
                        rollback_exc,
                    )
            ...
            try:
                ...
                try:
                    ...
                    _log_plugin_noncritical(
                        self.logger,
                        "warning",
                        "cosplay OCR fast loop rollback persist failed: {}",
                        rollback_exc,
                    )
