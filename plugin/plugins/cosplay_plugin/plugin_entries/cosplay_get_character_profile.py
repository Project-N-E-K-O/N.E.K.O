...
class _CosplayGetCharacterProfileMixin:
    @plugin_entry(
        id="cosplay_get_character_profile",
        name=tr(
            "entries.cosplay_get_character_profile.name",
            default="查询角色完整档案",
        ),
        description=tr(
            "entries.cosplay_get_character_profile.description",
    ...
    async def cosplay_get_character_profile(self, name: str, **_):
            ...
            return Ok(
                {
                    "disabled": True,
                    "reason": (
                        "角色档案功能未开启，请先调用 cosplay_set_character_mode"
                        " 切换为 fixed"
                    ),
