...
class _CosplayListMemoryReaderProcessesMixin:
    @plugin_entry(
        id="cosplay_list_memory_reader_processes",
        name=tr("entries.cosplay_list_memory_reader_processes.name", default='列出 Memory Reader 候选进程'),
        description=tr("entries.cosplay_list_memory_reader_processes.description", default='返回 Memory Reader 可选进程，包含 exe 路径、检测到的引擎和识别原因。'),
        input_schema={
            "type": "object",
            "properties": {
    ...
    async def cosplay_list_memory_reader_processes(
        self,
