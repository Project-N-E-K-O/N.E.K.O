...
class _CosplayAutoRecalibrateOcrDialogueProfileMixin:
    @plugin_entry(
        id="cosplay_auto_recalibrate_ocr_dialogue_profile",
        name=tr("entries.cosplay_auto_recalibrate_ocr_dialogue_profile.name", default='自动重新校准 OCR 对白区'),
        description=tr("entries.cosplay_auto_recalibrate_ocr_dialogue_profile.description", default='对当前已附着 OCR 目标窗口自动重校准对白区，并保存到当前窗口分辨率。'),
        input_schema={"type": "object", "properties": {}},
        timeout=120.0,
        llm_result_fields=["summary", "sample_text"],
    )
    async def cosplay_auto_recalibrate_ocr_dialogue_profile(self, **_):
