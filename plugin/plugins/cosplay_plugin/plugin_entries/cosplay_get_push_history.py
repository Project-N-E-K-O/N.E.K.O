...
class _CosplayGetPushHistoryMixin:
    @plugin_entry(
        id="cosplay_get_push_history",
        name=tr(
            "entries.cosplay_get_push_history.name",
            default="查询推送历史",
        ),
        description=tr(
            "entries.cosplay_get_push_history.description",
    ...
    async def cosplay_get_push_history(self, limit: int = 10, **_):
        if self._cfg is None:
            return Err(SdkError(self._not_configured_message()))
        throttled = self._check_query_rate_limit("cosplay_get_push_history")
