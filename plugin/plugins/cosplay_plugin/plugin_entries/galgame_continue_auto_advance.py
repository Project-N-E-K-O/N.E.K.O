...
class _GalgameContinueAutoAdvanceMixin:
    @plugin_entry(
        id="cosplay_continue_auto_advance",
        name=tr("entries.cosplay_continue_auto_advance.name", default='继续自动推进 cosplay 剧情'),
        description=tr("entries.cosplay_continue_auto_advance.description", default='切换到自动推进模式，并向 Game LLM Agent 发送继续推进消息。'),
        input_schema={
            "type": "object",
            "properties": {
    ...
    async def cosplay_continue_auto_advance(
        self,
        message: str = "继续推进剧情",
        **_,
    ):
        normalized_message = str(message or "").strip() or "继续推进剧情"
        mode_res = await self.cosplay_set_mode(
            mode="choice_advisor",
        ...
        agent_res = await self.cosplay_agent_command(
            action="send_message",
