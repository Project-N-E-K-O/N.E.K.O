...
class _CosplaySetLlmVisionMixin:
    @plugin_entry(
        id="cosplay_set_llm_vision",
        name=tr("entries.cosplay_set_llm_vision.name", default='设置 LLM 视觉辅助'),
        description=tr("entries.cosplay_set_llm_vision.description", default='切换 OCR Agent 低置信度场景的截图直传，并设置图片最大边长。'),
        input_schema={
            "type": "object",
            "properties": {
    ...
    async def cosplay_set_llm_vision(
        self,
        vision_enabled: bool,
            ...
            if self._ocr_reader_manager is not None:
                try:
                    ...
                    _log_plugin_noncritical(
                        self.logger,
                        "warning",
                        "cosplay LLM vision rollback update_config failed: {}",
                        rollback_exc,
                    )
