...
class _CosplayGetRecentLinesMixin:
    @plugin_entry(
        id="cosplay_get_recent_lines",
        name=tr(
            "entries.cosplay_get_recent_lines.name",
            default="查询最近台词",
        ),
        description=tr(
            "entries.cosplay_get_recent_lines.description",
    ...
    async def cosplay_get_recent_lines(self, n: int = 10, **_):
        if self._cfg is None:
            return Err(SdkError(self._not_configured_message()))
        throttled = self._check_query_rate_limit("cosplay_get_recent_lines")
