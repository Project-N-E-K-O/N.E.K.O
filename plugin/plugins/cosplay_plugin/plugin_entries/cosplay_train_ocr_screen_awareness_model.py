...
class _CosplayTrainOcrScreenAwarenessModelMixin:
    @plugin_entry(
        id="cosplay_train_ocr_screen_awareness_model",
        name=tr("entries.cosplay_train_ocr_screen_awareness_model.name", default='训练 OCR 屏幕感知模型'),
        description=tr("entries.cosplay_train_ocr_screen_awareness_model.description", default='从已标注 JSONL 样本训练轻量原型分类器，并导出可部署 JSON 模型。'),
        input_schema={
            "type": "object",
            "properties": {
    ...
    async def cosplay_train_ocr_screen_awareness_model(
        self,
        sample_path: str = "",
