...
class CosplayPlugin(
    ...
    def __init__(self, ctx):
        ...
        self._query_rate_limits: dict[str, deque[float]] = {
            "cosplay_get_recent_lines": deque(maxlen=2),
            "cosplay_get_scene_context": deque(maxlen=3),
            "cosplay_get_story_so_far": deque(maxlen=1),
            "cosplay_get_push_history": deque(maxlen=10),
        }

    def _not_configured_message(self) -> str:
        return self.i18n.t(
            "errors.not_configured",
            default="cosplay_plugin 未配置",
        )
        ...
        *,
        ...
        if self._ocr_reader_manager is not None:
            ...
            try:
                ...
                self.logger.warning(
                    "cosplay_plugin failed to refresh OCR runtime after saving capture profile: {}",
                    exc,
                )
            else:
                with self._state_lock:
    ...
    def _commit_state(self, payload: dict[str, Any]) -> None:
        ...
        if cross_scene_memory_to_persist is not None:
            try:
                ...
                self.logger.warning(
                    "failed to persist cosplay strategy memory state key=%s",
                    STORE_CROSS_SCENE_MEMORY,
                    exc_info=True,
                )
        ...
        if loop is not None and not loop.is_closed():
            try:
                ...
                _log_plugin_noncritical(
                    self.logger,
                    "warning",
                    "cosplay bridge poll loop graceful stop failed: {}",
                    exc,
                )
                try:
        ...
        if thread is not None and thread.is_alive():
            ...
            if thread.is_alive():
                _log_plugin_noncritical(
                    self.logger,
                    "warning",
                    "cosplay bridge poll loop thread did not stop within timeout",
                )
    ...
    async def _cancel_ocr_fast_loop(self) -> None:
        ...
        try:
            ...
            _log_plugin_noncritical(
                self.logger,
                "warning",
                "cosplay OCR fast loop cancellation failed: {}",
                exc,
            )
        ...
        if not task.done():
            try:
                ...
                _log_plugin_noncritical(
                    self.logger,
                    "warning",
                    "cosplay bridge background poll cancellation failed: {}",
                    exc,
                )
    ...
    async def _cancel_ocr_foreground_advance_monitor(self) -> None:
        ...
        try:
            ...
            _log_plugin_noncritical(
                self.logger,
                "warning",
                "cosplay OCR foreground advance monitor cancellation failed: {}",
                exc,
            )
    ...
    def _persist_context_snapshot_from_summary(
        ...
        if not live_matches:
            self.logger.warning("cosplay saved snapshot persist skipped: stale summary payload")
            return
        with self._state_lock:
            if not self._context_snapshot_liveness_matches(
                ...
                self.logger.warning(
                    "cosplay context_snapshot persist skipped: stale summary context"
                )
                return
    ...
    def _on_rapidocr_auto_lang_changed(self, lang_type: str) -> None:
        ...
        if normalized not in {"ch", "japan", "korean", "en"}:
            _log_plugin_noncritical(
                self.logger,
                "debug",
                "cosplay rapidocr auto-lang ignored invalid lang_type: {}",
                lang_type,
            )
            return
        ...
        try:
            ...
            _log_plugin_noncritical(
                self.logger,
                "warning",
                "cosplay rapidocr auto-lang persist failed for {}: {}",
                normalized,
                exc,
            )
    ...
    def _refresh_dependency_status(self) -> None:
        """Recompute cosplay dependency status (rapidocr/dxcam inspection).
        ...
        try:
            ...
            _log_plugin_noncritical(
                self.logger,
                "warning",
                "cosplay rapidocr dependency inspection failed: {}",
                exc,
            )
            rapidocr = {
                "installed": False,
        ...
        try:
            ...
            _log_plugin_noncritical(
                self.logger,
                "warning",
                "cosplay dxcam dependency inspection failed: {}",
                exc,
            )
            dxcam = {
                "installed": False,
    ...
    async def startup(self, **_):
        try:
            ...
            return Err(SdkError(f"failed to load cosplay_plugin config: {exc}"))
        ...
        try:
            ...
            return Err(SdkError(f"failed to restore cosplay_plugin store: {exc}"))
        ...
        if self._memory_reader_manager is not None:
            try:
                ...
                _log_plugin_noncritical(
                    self.logger,
                    "warning",
                    "cosplay memory reader shutdown failed: {}",
                    exc,
                )
        if self._ocr_reader_manager is not None:
            try:
                ...
                _log_plugin_noncritical(
                    self.logger,
                    "warning",
                    "cosplay OCR reader shutdown failed: {}",
                    exc,
                )
        if self._game_agent is not None:
            try:
        ...
        if self._llm_gateway is not None:
            try:
                ...
                _log_plugin_noncritical(
                    self.logger,
                    "warning",
                    "cosplay LLM gateway shutdown failed: {}",
                    exc,
                )
        if self._host_agent_adapter is not None:
            try:
                ...
                _log_plugin_noncritical(
                    self.logger,
                    "warning",
                    "cosplay host agent adapter shutdown failed: {}",
                    exc,
                )
        try:
        ...
        *,
        ...
        try:
            ...
            _log_plugin_noncritical(
                self.logger,
                "warning",
                "cosplay persist runtime state after bridge scan failure failed: {}",
                persist_exc,
            )
    ...
    # [dependency-groups] cosplay). Run `uv sync --group cosplay` for source
    ...
    # ------------------------------------------------------------------
    # Query / debug entries (cosplay-host-play-mode plan, step 19 + G4)
    # ------------------------------------------------------------------
