...
def _crop_window_image(
    ...
    *,
    ...
    cropped.info["cosplay_bounds_coordinate_space"] = "capture"
    cropped.info["cosplay_source_size"] = {"width": float(crop_w), "height": float(crop_h)}
    cropped.info["cosplay_full_frame_image"] = image
    cropped.info["cosplay_source_background_hash"] = source_background_hash
    cropped.info["cosplay_capture_rect"] = {
        "left": float(window_rect[0] + left),
        "top": float(window_rect[1] + top),
        "right": float(window_rect[0] + right),
        "bottom": float(window_rect[1] + bottom),
    }
    cropped.info["cosplay_window_rect"] = {
        "left": float(window_rect[0]),
        "top": float(window_rect[1]),
        "right": float(window_rect[2]),
        "bottom": float(window_rect[3]),
    }
    cropped.info["cosplay_capture_backend_kind"] = backend_kind
    cropped.info["cosplay_capture_backend_detail"] = backend_detail
    return cropped
