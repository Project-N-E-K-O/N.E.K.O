    ...
    async def _push_agent_message(
        ...
        *,
        ...
        try:
            ...
            self._logger.warning("cosplay outbound message delivery failed (will retry): {}", exc)
            try:
                ...
                self._logger.warning("cosplay outbound message retry also failed: {}", retry_exc)
        self._recent_pushes = self._recent_push_records()
        # host-play-mode plan, step 19 / G4: record minimal push metadata for the
        # `cosplay_get_push_history` query entry. Never store original line text
