...
class GalgameStore:
    ...
    def _read_values_from_path(self, path: Path) -> dict[str, Any]:
        ...
        if not isinstance(raw, dict):
            raise ValueError("cosplay store json root is not an object")
        return {
            str(key): value
    ...
    def _load_values(self, *, force: bool = False, locked: bool = False) -> None:
        ...
        if self._store_path.exists():
            try:
                ...
                self._logger.warning("failed to read cosplay store json {}: {}", self._store_path, exc)
                ...
                try:
                    ...
                    self._logger.warning(
                        "failed to recover cosplay store json from backup {}: {}",
                        backup_path,
                        backup_exc,
                    )
                    raise
                self._logger.warning(
                    "recovered cosplay store values from backup {} after read failure",
                    backup_path,
                )
            self._values = values
        else:
    ...
    def _save_values(self, *, locked: bool = False) -> None:
        ...
        try:
            ...
            try:
                ...
                self._logger.warning(
                    "failed to refresh cosplay store backup before save; continuing write: {}",
                    exc,
                )
