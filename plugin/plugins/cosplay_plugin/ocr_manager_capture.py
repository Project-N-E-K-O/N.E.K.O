...
class CaptureMixin:
    ...
    def _submit_capture_worker(
        ...
        with self._capture_worker_lock:
            ...
            if timeout_error is None:
                executor = self._capture_executor
                if executor is None:
                    executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="cosplay-ocr-capture")
    ...
    def _capture_and_extract_text(
        ...
        embedded_background_hash = (
            str(frame_info.get("cosplay_source_background_hash") or "")
        ...
        vision_frame = frame
        if isinstance(frame_info, dict):
            source_frame = frame_info.get("cosplay_full_frame_image")
        ...
        if isinstance(frame_info, dict):
            extraction.capture_backend_kind = str(
                frame_info.get("cosplay_capture_backend_kind")
            ...
            extraction.capture_backend_detail = str(
                frame_info.get("cosplay_capture_backend_detail")
                or getattr(self._capture_backend, "last_backend_detail", "")
                or ""
            )
            extraction.bounds_coordinate_space = str(
                frame_info.get("cosplay_bounds_coordinate_space") or ""
            )
            source_size = frame_info.get("cosplay_source_size")
            if isinstance(source_size, dict):
                extraction.source_size = dict(source_size)
            capture_rect = frame_info.get("cosplay_capture_rect")
            if isinstance(capture_rect, dict):
                extraction.capture_rect = dict(capture_rect)
            window_rect = frame_info.get("cosplay_window_rect")
