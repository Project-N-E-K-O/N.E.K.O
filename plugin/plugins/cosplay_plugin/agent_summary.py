        ...
        *,
        ...
        def _finish(done: asyncio.Task[bool]) -> None:
            ...
            try:
                ...
                self._logger.warning("cosplay scene summary task failed: {}", exc)
                return
            if not delivered:
                self._restore_failed_summary_schedule(
                    scene_id=scene_id,
        ...
        *,
        ...
        try:
            ...
            self._logger.warning(
                "cosplay json_copy failed in scene context update: {}",
                exc,
            )
    ...
    async def _run_scene_summary_task(
        ...
        *,
        ...
        if lock is None:
            ...
            self._logger.warning("cosplay scene_summary drop: missing_summary_lock scene=%s", scene_id)
            return False
        async with lock:
            if generation != self._summary_generation:
                ...
                self._logger.info(
                    "cosplay scene_summary drop: generation_mismatch scene=%s gen=%d current=%d",
            ...
            if session_id != self._observed_session_id:
                ...
                if not allow_transient_delivery:
                    ...
                    self._logger.info(
                        "cosplay scene_summary drop: session_mismatch scene=%s session=%s current=%s",
            ...
            if scene_no_longer_current and trigger != "line_count":
                ...
                self._logger.info(
                    "cosplay scene_summary drop: scene_mismatch scene=%s current=%s trigger=%s",
            ...
            delivered = await self._push_agent_message(
                ...
                content=(
                    "======[游戏上下文提示]\n"
                    "以下内容来自 cosplay 插件对当前游戏画面和近期台词的理解。"
            ...
            if not delivered:
                ...
                self._logger.warning(
                    "cosplay scene_summary drop: push_not_delivered scene=%s status=%s",
                    scene_id,
                    "not_delivered",
                )
                return False
            self._logger.info(
                "cosplay scene_summary delivered: scene=%s key=%s trigger=%s",
            ...
            if callable(story_recorder):
                try:
                    ...
                    self._logger.warning(
                        "cosplay story_so_far update failed",
                        exc_info=True,
                    )
    ...
    async def _maybe_push_periodic_scene_summary(
        ...
        *,
        ...
        if not self._should_push_scene(shared):
            ...
            self._logger.info("cosplay scene_summary gate: push_notifications=%s mode=%s",
        ...
        if not ready_scene_ids:
            ...
            self._logger.info(
                "cosplay scene_summary gate: no ready scenes (total_lines=%d scenes=%d)",
                total_lines,
        ...
        if scheduled:
            ...
            self._logger.info(
                "cosplay scene_summary scheduled: count=%d scenes=%s",
                len(scheduled),
