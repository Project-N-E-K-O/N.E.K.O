        ...
        try:
            ...
            try:
                self._logger.warning(
                    "cosplay LLM client close failed during {}: {}",
                    reason,
                    exc,
                )
            except Exception:
        ...
        *,
        ...
        for attempt in range(_LLM_CALL_MAX_ATTEMPTS):
            try:
                ...
                try:
                    self._logger.warning(
                        "cosplay LLM {} call failed; retrying {}/{} after {:.2f}s: {}",
                        model_role,
                        attempt + 1,
                    ...
                    logging.getLogger(__name__).warning(
                        "cosplay logger.warning failed during LLM retry logging",
                        exc_info=True,
                    )
                await asyncio.sleep(delay)
        raise last_exc or RuntimeError("cosplay LLM call failed")
