...
router = APIRouter(tags=["cosplay-install"])
logger = get_logger("cosplay.install_routes")
run_service = RunService()

_INSTALL_PLUGIN_IDS = {"cosplay_plugin", "study_companion"}
...
async def get_cosplay_ui_locale(plugin_id: str) -> JSONResponse:
...
async def get_cosplay_ui_i18n(plugin_id: str, locale: str) -> Response:
    _ensure_has_install(plugin_id)
...
def _install_entry_id(plugin_id: str, cosplay_entry_id: str) -> str:
    if plugin_id == "study_companion":
        mapping = {
            "cosplay_install_tesseract": "study_install_tesseract",
            "cosplay_download_rapidocr_models": "study_download_rapidocr_models",
        }
        return mapping.get(cosplay_entry_id, cosplay_entry_id)
    return cosplay_entry_id


def _get_install_kind_spec(kind: str, *, plugin_id: str = "cosplay_plugin") -> dict[str, Any]:
    ...
    # cosplay). textractor + tesseract still need runtime install. rapidocr_models
    # is a model-pack download (not a package install) for non-bundled (lang,
    # version) combos like japan + PP-OCRv4.
    mapping = {
        "textractor": {
            "kind": "textractor",
            "entry_id": _install_entry_id(plugin_id, "cosplay_install_textractor"),
            "label": "Textractor",
        ...
        "tesseract": {
            "kind": "tesseract",
            "entry_id": _install_entry_id(plugin_id, "cosplay_install_tesseract"),
            "label": "Tesseract",
        ...
        "rapidocr_models": {
            ...
            "entry_id": _install_entry_id(plugin_id, "cosplay_download_rapidocr_models"),
...
async def cosplay_plugin_start_textractor_install(
    plugin_id: str,
    payload: InstallStartPayload,
    request: Request,
):
...
async def cosplay_plugin_start_tesseract_install(
    plugin_id: str,
    payload: InstallStartPayload,
    request: Request,
):
...
async def cosplay_plugin_latest_textractor_install(plugin_id: str):
...
async def cosplay_plugin_latest_tesseract_install(plugin_id: str):
...
async def cosplay_plugin_get_textractor_install(plugin_id: str, task_id: str):
...
async def cosplay_plugin_get_tesseract_install(plugin_id: str, task_id: str):
...
async def cosplay_plugin_stream_textractor_install(
    plugin_id: str,
    task_id: str,
    request: Request,
):
...
async def cosplay_plugin_stream_tesseract_install(
    plugin_id: str,
    task_id: str,
    request: Request,
):
...
async def cosplay_plugin_start_rapidocr_models_download(
    plugin_id: str,
    payload: InstallStartPayload,
    request: Request,
):
...
async def cosplay_plugin_latest_rapidocr_models_download(plugin_id: str):
...
async def cosplay_plugin_get_rapidocr_models_download(plugin_id: str, task_id: str):
...
async def cosplay_plugin_stream_rapidocr_models_download(
    plugin_id: str,
    task_id: str,
    request: Request,
):
