...
def _default_bridge_root_raw() -> str:
    if sys.platform.startswith("win"):
        return "%LOCALAPPDATA%/N.E.K.O/cosplay-bridge"
    if sys.platform == "darwin":
        return "~/Library/Application Support/N.E.K.O/cosplay-bridge"
    xdg_data_home = str(os.getenv("XDG_DATA_HOME") or "").strip()
    if xdg_data_home:
        return f"{xdg_data_home}/N.E.K.O/cosplay-bridge"
    return "~/.local/share/N.E.K.O/cosplay-bridge"
...
def build_config(raw_config: dict[str, Any]) -> GalgameConfig:
    ...
    cosplay_obj = cosplay if isinstance(cosplay, dict) else {}
    ...
    if rapidocr_lang_type_raw == "ch":
        _logger.warning(
            'cosplay_plugin RapidOCR is using lang_type = "ch"; if this came from the '
            'packaged default, set rapidocr.lang_type = "japan" explicitly for Japanese games.'
        )

    default_mode_obj = cosplay_obj.get("default_mode")
    default_mode = (
        default_mode_obj
        if isinstance(default_mode_obj, str) and default_mode_obj in MODES
        else MODE_COMPANION
    )
    bridge_root_value = cosplay_obj.get("bridge_root")
    ...
    return GalgameConfig(
        ...
        idle_poll_interval_seconds=_coerce_float(
            cosplay_obj.get("idle_poll_interval_seconds"), 3.0, minimum=0.1
        ),
        stale_after_seconds=_coerce_float(
            cosplay_obj.get("stale_after_seconds"), 15.0, minimum=0.1
        ),
        history_events_limit=_coerce_int(
            cosplay_obj.get("history_events_limit"), 500, minimum=1
        ),
        history_lines_limit=_coerce_int(
            cosplay_obj.get("history_lines_limit"), 200, minimum=1
        ),
        history_choices_limit=_coerce_int(
            cosplay_obj.get("history_choices_limit"), 50, minimum=1
        ),
        dedupe_window_limit=_coerce_int(
            cosplay_obj.get("dedupe_window_limit"), 64, minimum=1
        ),
        warmup_replay_bytes_limit=_coerce_int(
            cosplay_obj.get("warmup_replay_bytes_limit"), 65536, minimum=1
        ),
        warmup_replay_events_limit=_coerce_int(
            cosplay_obj.get("warmup_replay_events_limit"), 50, minimum=1
        ),
        default_mode=default_mode,
        push_notifications=bool(cosplay_obj.get("push_notifications", True)),
        scene_change_cooldown_seconds=_coerce_float(
            cosplay_obj.get("scene_change_cooldown_seconds"), 15.0, minimum=0.0
        ),
        scene_push_half_threshold=max(1, int(cosplay_obj.get("scene_push_half_threshold") or 4)),
        scene_push_time_fallback_seconds=_coerce_float(
            cosplay_obj.get("scene_push_time_fallback_seconds"), 120.0, minimum=10.0
        ),
        scene_merge_total_threshold=max(
            1, int(cosplay_obj.get("scene_merge_total_threshold") or 12)
        ),
        auto_open_ui=_coerce_bool(cosplay_obj.get("auto_open_ui"), False),
        ...
        reader_mode=_coerce_reader_mode(cosplay_obj.get("reader_mode")),
        ...
        vision_classifier_model_name=str(
            vision_classifier_obj.get("model_name") or "v1_cosplay"
        ).strip(),
...
def build_primary_diagnosis(local_state: dict[str, Any]) -> dict[str, Any]:
    ...
    # via [dependency-groups] cosplay; if it's missing the dev needs to run
    # `uv sync --group cosplay`, not click an in-app install button. We still
