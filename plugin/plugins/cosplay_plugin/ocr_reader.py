...
class OcrReaderManager(
    ...
    def __init__(
        ...
        *,
        ...
        self._capture_executor: ThreadPoolExecutor | None = ThreadPoolExecutor(
            max_workers=1,
            thread_name_prefix="cosplay-ocr-capture",
        )
    ...
    def _initialize_vision_classifier(self) -> None:
        ...
        try:
            ...
            model_name = str(
                getattr(self._config, "vision_classifier_model_name", "v1_cosplay")
                or "v1_cosplay"
            )
            if classifier.load(model_name):
                ...
                self._log_info(
                    "cosplay vision classifier loaded: model_dir={} model_name={}",
                    str(model_dir),
                    model_name,
                )
            else:
                ...
                self._log_warning(
                    "cosplay vision classifier unavailable: model_dir={} model_name={}",
                    str(model_dir),
                    model_name,
                )
        except ImportError as exc:
            self._vision_classifier_detail = "dependency_unavailable"
            self._log_warning("cosplay vision classifier dependency unavailable: {}", exc)
