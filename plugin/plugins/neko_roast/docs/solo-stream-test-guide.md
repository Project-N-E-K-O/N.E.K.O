...
这份给组长、内测主播或接手开发者快速体验 `solo_stream`。目标不是读完所有诊断字段，而是让 NEKO 真正接入一个直播间、看她能不能独立撑场。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\plugin\plugins\neko_roast\tools\monitor_live.ps1 -ExpectRealOutput -BackendLogPath .\.codex-backend-live-test.log
```

先看 `alerts`。没有告警时不用展开所有字段。
...
- `reply_quality_fallback_many`：最终输出层频繁救场，说明上游话题或 prompt 还需要收敛。
