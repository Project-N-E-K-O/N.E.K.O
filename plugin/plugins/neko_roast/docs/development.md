...
当前测试树没有单一的 `test_module_boundaries.py`。模块化和宿主核心冻结边界由现有契约、注册表、输出合约和 smoke 测试共同守门；它们不验证直播效果，而是验证架构边界：
...
当前测试树没有单一的 `test_distribution_boundaries.py`。插件可分发边界通过 Git index、ignore 规则和插件检查器共同验证，确认本地运行态产物不会进入插件包或 PR：
