...
| `core/live_reply_policy.py` | NEKO Live 最终回复策略兼容门面，保留旧 import 路径 | 不新增策略实现 |
| `core/live_reply_contracts.py` | 回复 metadata、route 字数上限、module 合约常量 | 不做文本质量判断 |
| `core/live_reply_fallbacks.py` | 怪话/空泛/漂移检测和质量 fallback 文案 | 不拼 prompt instruction |
| `core/live_reply_shape.py` | 最终输出句子裁剪、长度裁剪、dangling choice 修剪 | 不决定 route metadata |
| `core/live_reply_instructions.py` | NEKO Live 输出合约 prompt、最近回复 negative examples、callback metadata 合并 | 不改最终输出文本 |
...
| Dashboard/UI Owner | `ui/panel.tsx`、`ui/panel_components.tsx`、`ui/panel_helpers.ts`、`config_schema()` 渲染、i18n 文案、面板信息架构 | UI 外壳稳定、模块卡隔离、8 locale 同步、Hosted UI 约束 |
...
- Dashboard shell：`ui/panel.tsx` 的导航外壳、跨页面状态；`ui/panel_components.tsx` 的展示组件；`ui/panel_helpers.ts` 的标签/格式化 helper。
...
- `live_events`：直播事件中枢（P2.5）。经 `event_bus` **订阅 `"danmaku"` / `"gift"` / `"super_chat"` / `"guard"` 事件**，解包信封 `raw` 取富模型 `LiveDanmaku`，冷却期缓冲候选互动、按 `get_score()` 打分，冷却结束择优（舰长/总督/SC、礼物、粉丝牌、用户等级、长文本优先）取分最高者投 `pipeline`；空闲态首条仍即时锐评。Gift / SC / Guard 专属致谢或朗读模块完成前，`runtime.handle_live_payload()` 只把这些事件记录为 signal-only skipped result（`gift_signal` / `super_chat_signal`），不得误走 `avatar_roast` 或 `danmaku_response`。详见下文「直播事件中枢」。
...
礼物 / SC / Guard 在当前阶段仍只做信号观测，不代表专属行为完成。直播复盘时可用 `latest_signal` 判断最新事件是否为 `gift_signal` / `super_chat_signal`；`recent_signal_danmaku_signal` / `recent_signal_gift_signal` / `recent_signal_super_chat_signal` 仍只统计 pushed / dry_run 结果，不把普通 skipped / failed 尝试当作已观测到。signal-only skipped result 的作用是证明事件被看见且没有污染普通弹幕输出路径。
...
Danmaku Response Quality v2 进一步把“普通弹幕接话”从首评模板里拆清楚：`danmaku_response` 会在 prompt 中标记当前弹幕的轻量 `danmaku_profile`，包括 `viewer_to_viewer_mention`、`question`、`emoji_or_reaction`、`short_line`、`normal_line` 和 `empty`。短反应 / 表情 / 哈哈类弹幕只允许做很短的情绪镜像；问题弹幕必须先直接回答；`@其他观众` 只能作为公开内容的极短旁白，不能当成观众在喊 NEKO，也不能替观众之间调停；`@猫猫` 仍然视为当前弹幕目标。`danmaku_profile`、`danmaku_reply_target` 和 `danmaku_reply_shape` 会进入 request metadata、recent result 和 `monitor_live.ps1` 的 `latest_danmaku_profile` / `latest_danmaku_reply_shape`，便于直播复盘判断猫猫为什么这么接话。这个 profile 只影响回复形状，不新增事件类型，不绕过 viewer profile、safety guard、dispatcher、dry_run、cooldown 或 pacing。

所有直播开口 prompt 必须复用 `anti_repeat_rules()`：先对照 NEKO Live 的 recent-output 记忆，避免复用上一句的开头、句式、包袱、话题切法、奖励梗、计划、观众问卷或主持节拍，也不能把上一句换词改写成新回复。插件侧 `recent_interaction_context()` 只能作为“已用素材 / 已用主持节拍”的 blocklist，不能当成下一句的脚本前缀或继续话题；`viewer_session_context()` 同样只能用于同一观众的轻量连续感和防复读，不能默认续写该观众上一轮话题，除非当前弹幕明确要求继续；若当前草稿和 recent context 共享同一主题、开头或 joke shape，应换角度或只回答当前弹幕。recent context 里出现的 `topic_family`、`host_beat_family`、`fun_axis`、`shape`、`intent` 也必须按已用素材处理，不能只避开原句却继续复用同一类“主播力 / 小鱼干 / 暗号 / 小电台 / 二选一”主持手法。宿主 callback instruction 也会消费 `live_reply_contract=short_tts_line`，在最终生成层追加同样的 anti-repeat 约束，并把最近 12 条 NEKO Live 输出作为 negative examples 传入，让模型不要继续、复述或改写这些旧句子；这会覆盖文本 proactive、语音 proactive 和 passive drain 路径。文本 / 语音 proactive 直播回调会把 NEKO Live metadata 透传给最终输出 handler；异步输出回调若晚于注入函数返回，也会用 `speech_id` 兜底匹配。因此真实生成文本落到 `send_lanlan_response()` 时也会在输出 metadata 上标记 `neko_live_reply_repeat=true/false` 和 `neko_live_reply_repeat_window`，用于确认最终播出的文本是否与最近最多 24 条直播输出窗口中的某条 NEKO Live 文本高度相似，包含非相邻复读、短片段重排复读，以及“小鱼干奖励 + 弹幕接话”“惊喜 + 奖励”“特别企划 + 大家互动”“主播力自测”“一个字 / 一个词回调”“小电台 / 气氛”“安静冷场”等同一直播包袱 anchor 的换皮复读；窗口按 `turn_id` / `speech_id` 聚合，流式直播输出会先缓冲到对应 completion 回调再作为完整短句进入判重窗口，不会把半句话碎片当成多条历史，也不会让短前缀先漏到前端或 TTS；如果 provider 发送的是完整缓冲快照而不是 delta，窗口会用快照覆盖同一轮当前值，避免把“前半句 + 完整句”拼成脏历史。带 `live_reply_contract=short_tts_line` 的 NEKO Live 输出只进入直播自身的 recent reply / voice-echo 防护窗口，不再累加到宿主普通 AI turn 文本，避免下一轮普通生成把上一句直播短播报当成需要延续的对话上下文。若第一段完整直播输出已经判定为复读，宿主会 suppress 这次前端 / TTS 输出，而不是只做事后复盘。

同一条隔离规则也必须覆盖 `cross_server` 的普通 memory / analyzer 汇合点：带 `live_reply_contract=short_tts_line` 的 `gemini_response` 和对应 turn end 只能作为直播可见输出处理，不能被写成普通聊天历史。

热切新 session 期间的 `message_cache_for_new_session` 也必须遵守同一规则：NEKO Live 短播报不能作为新 session 的普通上下文预热材料，只能保留在直播自身的 recent-output / voice-echo 防护窗口。
...
`monitor_live.ps1` 会把这层内容族也暴露给直播复盘：`latest_topic_family` / `latest_host_beat_family` 用于看最近一次主动营业或冷场陪播用了哪类主持手法，`recent_topic_family_*` / `recent_host_beat_family_*` 用于看最近窗口内是否一直停在同一类；当最近主动营业或冷场陪播至少 3 次且同一内容族占比过高时，`alerts` 会出现 `topic_family_bias` 或 `host_beat_family_bias`。这类告警对应的是“听起来像在复读同一种主持套路”，应优先扩素材池或调整 family 轮换，而不是先提高开口频率。
...
Live Feel Pack v1.10 把这条规则补到宿主最终输出层：即使模型没有逐字复读，只要第一段真实输出命中了同一类高信号直播包袱，例如小鱼干 / 奖励、特别企划、主播力自测、一个字 / 一个词 / 暗号回调、安静冷场，或在二选一、房间氛围、桌面场景、轻吐槽、小挑战等内容族上又和近期输出有明显短片段重叠，也应视为换皮复读并触发 `neko_live_reply_repeat` / `neko_live_reply_suppressed`。下一场直播如果仍觉得猫猫“说法变了但意思又绕回来了”，优先看 `log_reply_repeat`、`log_reply_suppressed`、`topic_family_bias` 和 `host_beat_family_bias`，再决定是继续扩内容族还是调整触发频率。

Live Feel Pack v1.12 把 audience-prompt 复读作为单独的高风险主持套路处理。连续输出里如果都在召唤观众“想听 / 想看 / 聊点 / 发言 / 发弹幕 / 接话 / 来一句 / 扣 1 / 吱一声 / 冒个泡 / 给点反应 / 还在吗 / 有人吗 / 在不在 / 打个分”，即使句子不相似，也应视为 `audience_prompt` family 的换皮复读并进入宿主最终输出层检测。真实 pushed 输出中的这些表达也会被 `_spent_output_families()` 标成 `audience_prompt`，进入 `recent_interaction_context()`、`spent_output_family`、素材选择避让和 monitor 复盘口径。普通“弹幕”一词本身不能单独作为强复读信号，避免把正常接弹幕误判成模板营业；需要命中更明确的观众召唤动作。
...
Hosted UI 入口位于 `ui/panel.tsx`，展示组件与标签/格式化 helper 分别位于 `ui/panel_components.tsx` / `ui/panel_helpers.ts`。外壳 = **生命周期-域导航**（薄外壳 + 模块贡献），完整契约见 `docs/ui-architecture.md`。
