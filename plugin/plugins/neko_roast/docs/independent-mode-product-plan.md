...
- Active Engagement v0 is implemented as a solo-stream quiet-moment trigger with both automatic and manual paths. It is meant for controlled live-effect validation only: one small replyable topic, standard pacing around a 120-second minimum interval, active pacing around a 90-second minimum interval, and no Gift / SC / Guard coupling.
...
3. Reply Length Contract: the shared prompt contract is implemented offline for first-appearance roast, follow-up danmaku, warmup hosting, idle hosting, and active engagement. Each path now asks for one sentence, no paragraph, at most 14 Chinese characters or 8 English words; short danmaku should get even shorter replies, with no explanation, setup, comma-chained clauses, or second sentence. Live feel still needs the next run.
...
2. Active Engagement v1: implemented as a first pass. Active topics now carry lightweight topic material, rotate topic shape / title, and prefer meaningful recent live-room danmaku before falling back to filtered public Bilibili trending material or built-in topics. The prompt also expands each topic shape into a concrete example pattern, so NEKO gets a replyable structure without hard-coded lines. Active pacing now exposes separate minimum-interval and post-danmaku waits; `standard` pacing uses an approximately 120-second minimum interval, `active` uses approximately 90 seconds, and both wait after a danmaku reply to avoid talking over it while staying observable in the next live test. The fallback topic pool now uses at least ten concrete low-pressure hooks, natural per-topic shapes, and a longer recent-topic de-dup window, so low-danmaku tests are less likely to hear the same generic prompt loop.
...
- If profiles were cleared, confirm the monitor reports `profile_count=0` before the controlled run.
...
- `monitor_live.ps1` should be captured at least once during quiet / idle windows; record `director_action`, `director_reason`, `director_eligible`, `director_wait`, `viewer_age`, `output_age`, `entrance_pacing_window`, `active_min_interval`, `active_min_wait`, `active_danmaku_wait`, `active_idle_wait`, `latest_status`, `latest_route`, `latest_uid`, `latest_source`, `latest_text`, `latest_reason`, `latest_age`, `latest_age_status`, `latest_output_len`, `latest_output_length_status`, `recent_total`, `recent_avatar_roast`, `recent_danmaku_response`, `recent_idle_hosting`, `recent_active_engagement`, `recent_pushed`, `recent_dry_run`, `recent_skipped`, `recent_failed`, `latest_topic_source`, `latest_topic_shape`, `latest_topic_key`, `latest_topic_hook`, `latest_topic_pattern`, `latest_topic_repeat`, `latest_host_beat_key`, `latest_host_beat_shape`, `latest_host_beat_title`, `latest_host_beat_repeat`, `avatar_repeat_uid`, and `avatar_repeat_count` so the review can tell whether pacing, routing, topic quality, latest input, repeated topic material, repeated avatar roast, overlong replies, missing idle hosting, missing active engagement, or a stalled result stream failed. Use the recent status counts to distinguish actual output from skipped / failed attempts. `alerts` should flag `test_isolation`, `topic_repeat`, `topic_filter_direct_request`, `topic_filter_reaction`, `topic_filter_runtime_feedback`, `avatar_repeat`, `avatar_bias`, `long_reply`, `generic_host_prompt`, `recent_failed`, `host_beat_repeat`, `idle_missing`, or `active_missing` when those risks appear.
