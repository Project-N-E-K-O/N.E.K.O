...
1. Larger Content Pool: Idle Hosting now has at least 24 host beats and Active Engagement has at least 36 fallback topics, so a long quiet room has more material before it loops.
...
- For Live Feel Pack v1.14, also verify cross-server and hot-swap memory isolation: `live_reply_contract=short_tts_line` outputs and turn ends should not enter ordinary chat history through `cross_server`, and should not enter `message_cache_for_new_session` while a new session is being prepared. If replies still feel like they continue the previous live line, inspect the live recent-output window, ordinary memory/analyzer logs, and new-session cache.
