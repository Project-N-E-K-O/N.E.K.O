...
3. No Generic Host Prompt: continue rejecting "everyone interact", "send danmaku", "what should we talk about", and similar customer-service-like prompts.
...
- For Live Feel Pack v1.6, also verify Gift / Guard / SC stay signal-only until dedicated modules exist, `profile_count` follows solo readiness even when `recent_profiles` is empty, and host routes (`warmup_hosting`, `idle_hosting`, `active_engagement`) warn on 60+ character outputs even if the global long-reply threshold is higher.
