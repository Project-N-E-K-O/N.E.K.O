...
4. **后续再做旧插件能力迁移矩阵与退役评估**：当前只确认登录与直播只读主链路已独立，不能直接删除 `bilibili_danmaku`。本项暂不阻塞当前离线收尾；恢复后再逐项核对其 47 个入口及内部存储/分析能力，记录“已由 NEKO Live 替代 / 应吸收 / 应拆独立插件 / 明确废弃”，并先完成所有获批迁移。最终删除仍须从最新 `main` 建独立分支，不得与功能迁移 PR 堆叠。
5. ⏸ **完成新版面板下的真实 30 分钟独播验证**：发布前仍须按 `independent-mode-product-plan.md` 的 `Release Validation Acceptance Checklist` 验证登录、直播间确认、普通弹幕、Gift / SC / Guard 短句致谢、冷场、主动营业、结束直播和恢复路径。维护者已于 2026-07-15 明确暂缓真机测试；恢复前不得为了补结论自行启动前后端或连接真实直播间。
6. ⏸ **UI 性能与可访问性验收**：只读区块已开始从 `panel.tsx` 拆出，面板状态刷新契约也已离线收口为仅可见时 3 秒低频刷新、隐藏即停、恢复可见立即补刷、单请求串行并清理 `visibilitychange` 监听。进一步拆分只在能降低实际维护或渲染成本时进行；打包版 Hosted UI 的窄宽、键盘、焦点、状态播报、对比度、空闲 CPU 和监听器泄漏实测随真机验收一并暂缓，不再重做导航结构。
7. ~~**确认观众记忆产品契约后再加普通用户控制**~~ ✅ **已完成（2026-07-15）**：个性化记忆默认开启，固定保留 90 天并惰性清理；关闭只停止安全派生偏好的学习与 prompt 使用，不影响基础档案、首次出场记录和本场防复读；普通用户可清空全部档案，或对单 UID 重置印象 / 删除档案。当前明确不加入 `watch_time`、`contribution_rank`、raw 弹幕历史、后台清理任务或新依赖。
