...
This document is the canonical source for NEKO Live runtime observability language. It defines what code, reviews, monitor views, and dashboard surfaces must be able to explain. Monitor implementation details live in `tools/monitor_live.ps1`; Gift / SC / Guard behavior is currently implemented by `live_support_events`.
...
Monitor Signal is a stable operational event name or snapshot field that monitor code may emit or derive. The current emitter is `tools/monitor_live.ps1`; it reads hosted-ui context and must stay privacy-safe.
...
Current implementation status: backend projection implemented in `core/runtime_dashboard.py`. `runtime.dashboard_state()` exposes the initial rows as `health_rows`, Dashboard consumes the same facts through `live_explain.chain`, and `tools/monitor_live.ps1` now emits the latest `trace_id` plus compact timeline stage/status/route/reason fields from the same read-only projection.
