...
9. 在“设置 → 节奏与安全”保持“自动急停”开启，按需调整冷却秒数和队列上限。冷却秒数即最小锐评间隔，爆量房间靠它控制猫猫不连珠炮，`0` 关闭限流；爆量时同一冷却窗口内会按价值（舰长 / SC / 粉丝牌 / 等级 / 长文本）择优，只评分最高的一条。
10. 观众档案当前固定保存在本机插件默认 AppData 目录；“自定义存储位置”在 2026-06-19 真机测试中确认重启后不稳定，入口已暂时屏蔽，留到下一阶段修复。
11. 到“观众”页查看本场概况：直播总结（本场锐评粗报 + 最近锐评摘要）和观众档案（UID、昵称、锐评次数、最近出现时间）。
12. 需要离线测试时，开启开发者模式后到“开发者沙盒”输入 UID 或 B 站空间链接。
13. 点击“查询资料”只抓取 UID、昵称和头像状态；“发射模拟弹幕”走统一 pipeline 让当前界面猫猫按人设输出；“运行内置案例”用内置演示观众 + 测试头像、不访问 B 站；“清空沙盒记录”遗忘沙盒临时记录和头像预览缓存，不影响观众档案、直播总结或真实直播记录。
