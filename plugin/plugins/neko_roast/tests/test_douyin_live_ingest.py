...
@pytest.mark.parametrize("uid", ["ttwid", "token", "sessionid", "douyin:webcast_sign"])
...
def test_douyin_live_event_re_sanitizes_direct_provider_event_fields():
    ...
    assert isinstance(live_event.raw, DouyinLiveProviderEvent)
    assert asdict(live_event.raw) == {
        "event_type": "unknown",
        "uid": "",
        "nickname": "",
        "text": "",
    ...
    assert "must-not-leak" not in json.dumps(live_event.to_dict(), ensure_ascii=False)
    assert "must-not-leak" not in json.dumps(asdict(live_event.raw), ensure_ascii=False)
