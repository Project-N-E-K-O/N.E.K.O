...
def test_live_content_catalog_aggregates_preserve_original_order():
    assert len(ACTIVE_ENGAGEMENT_FALLBACK_TOPIC_CANDIDATES) == 33
    assert [
...
def test_idle_hosting_json_loader_skips_malformed_entries(tmp_path):
    path = tmp_path / "idle_hosting_beats.json"
    path.write_text(
        dumps(
            {
    ...
    assert [beat["key"] for beat in beats] == ["idle:ok"]
