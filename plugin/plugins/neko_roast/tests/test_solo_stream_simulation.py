...
async def test_solo_stream_realistic_simulation_keeps_routes_and_quality_stable(tmp_path: Path) -> None:
    ...
    assert statuses == {"pushed": 26, "skipped": 2}
    assert routes["warmup_hosting"] == 1
    assert routes["avatar_roast"] == 9
    assert routes["danmaku_response"] == 6
    assert routes["idle_hosting"] >= 6
    assert routes["active_engagement"] >= 3
    assert routes["gift_signal"] == 1
    assert routes["super_chat_signal"] == 1
    ...
    assert len(runtime.dispatcher.outputs) == 26
