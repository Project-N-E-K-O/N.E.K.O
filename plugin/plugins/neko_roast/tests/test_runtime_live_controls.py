...
async def test_live_state_moves_from_warmup_after_any_hosting_output(runtime: RoastRuntime) -> None:
    ...
    runtime.record_result(
        InteractionResult(
            ...
            status="dry_run",
            event=ViewerEvent(uid="__neko_active__", nickname="NEKO", source="active_engagement", live_mode="solo_stream"),
            steps=[PipelineStep("active_engagement", "ok"), PipelineStep("neko_dispatcher", "dry_run")],
            created_at=_created_at_age(240),
        )
    )
    ...
    for index, source in enumerate(("warmup_hosting", "active_engagement"), start=1):
        runtime.record_result(
            InteractionResult(
                ...
                steps=[PipelineStep(source, "ok"), PipelineStep("neko_dispatcher", "dry_run")],
...
async def test_active_engagement_avoids_recent_spent_output_family(runtime: RoastRuntime) -> None:
    ...
    assert topic["key"] == "fallback:serious-cat"
...
async def test_active_engagement_fallback_topics_use_their_natural_shapes(runtime: RoastRuntime) -> None:
    ...
    assert second["key"] == "fallback:serious-cat"
    assert second["shape"] == "small_challenge"
    assert second["live_column"] == "NEKO three-second challenge"
...
@pytest.mark.asyncio
async def test_auto_active_engagement_stops_after_repeated_idle_hosting_without_viewer_response(runtime: RoastRuntime) -> None:
    runtime.config.live_room_id = 123
    runtime.config.live_enabled = True
    runtime.config.dry_run = True
    runtime.config.live_mode = "solo_stream"
    ...
    assert before["live_director_status"]["next_auto_action"] == "none"
    assert before["live_director_status"]["reason"] == "no_viewer_response"
...
@pytest.mark.asyncio
async def test_auto_active_engagement_does_not_take_over_after_two_idle_beats(runtime: RoastRuntime) -> None:
    runtime.config.live_room_id = 123
    runtime.config.live_enabled = True
    runtime.config.dry_run = True
    runtime.config.live_mode = "solo_stream"
    ...
    assert state["live_director_status"]["reason"] == "no_viewer_response"
