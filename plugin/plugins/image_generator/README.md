...
- 提供商错误正文不会回显或写入日志。提示词、修订提示和历史摘要有长度上限并会对
  密钥形态做脱敏。
- OpenAI-compatible 响应只接受 Base64，拒绝 URL 图片，不会为该路径发起二次下载。
  DashScope 原生异步路径会下载任务结果中的 HTTPS OSS URL，限制为批准的 OSS 主机名，
  并执行地址、重定向和下载字节数检查；这是上述 Base64 策略的受限例外。
...
生成文件优先写入插件安装目录的 `static/generated/`，以兼容冻结版宿主的静态路由。
安装目录不可写时，仅在探测确认宿主支持目录覆盖后，才回退到插件数据目录的
`static_ui/generated/`；两者都不可用时禁用图片缓存。升级或重装可能移除安装目录中的
图片，需保留的结果请提前导出或备份。缓存通过 `/plugin/image_generator/ui/generated/...`
提供；管理面板只预览同源且匹配该路径的图片，不加载任意远程 URL。
