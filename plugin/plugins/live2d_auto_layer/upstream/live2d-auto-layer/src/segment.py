...
def _segment_with_anime_face(
    ...
    from src.anime_face import AnimePartSegmenter

    # 如果提供了 API key，创建 GPT 检测器
    gpt_detector = None
    if gpt_api_key and gpt_api_key.strip():
        from src.vision_landmarks import GPTLandmarkDetector
...
def _segment_with_gsam(
    ...
    if gsam_instance is None:
        from src.grounded_sam import get_gsam
...
def _segment_with_color(image: Image.Image) -> dict[str, Image.Image]:
    """颜色聚类分割 (兜底方案)"""
    from src.segment import ColorSegmenter
