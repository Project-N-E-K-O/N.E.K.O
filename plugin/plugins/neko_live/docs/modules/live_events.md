...
It also owns two deliberately separate forms of short-lived room context. `room_topic.py` keeps advisory prompt context for theme grouping. `recent_chat.py` keeps a bounded factual window so the current NEKO role can use the `get_recent_live_chat` LLM tool when a user explicitly asks who just said what. The same role-scoped tool may perform one local relevance lookup for a concrete topic during ordinary live conversation, and an unselected remark may become one low-pressure `active_engagement` topic candidate. The factual window is never injected into every prompt.
...
- Recent-chat contract: `recent_chat_snapshot(limit=1) -> list[dict]` returns up to three newest public, sanitized facts from a fixed session tail. The tail is size-bounded rather than time-expired and labels whether each row is still inside the 30-second fresh window. The role-scoped tool accepts `position=1|2|3` and returns only that exact row, so the model never has to index a list itself. `relevant_chat_snapshot(query, limit=1) -> list[dict]` locally ranks the separate time-bounded candidate view and returns at most one unselected, unused remark under the ambient pressure gate. The dynamically registered `get_recent_live_chat` tool is visible only to the resolved NEKO role while the listener is connected.
...
`NekoLivePlugin` dynamically registers `get_recent_live_chat(limit=1, position=null, query="")` after a listener connects. Registration is scoped to the role returned by `resolve_plugin_target_lanlan`; unresolved roles do not fall back to a global tool. Disconnect and failed reconnect unregister the tool. With no `query`, the handler performs an exact read from the three-entry session tail and remains read-only. `position=1`, `2`, or `3` means latest, previous, or third latest, and the backend returns only the requested row; an unavailable position returns `status=position_unavailable`. `limit` remains a bounded compatibility/read-ahead control for callers that need a newest-first list, but live-scene instructions use `position` for positional questions. With one concrete, sanitized `query`, the position is ignored and the handler performs deterministic local token/substring relevance ranking over only the 120-second candidate view, returns at most one low-pressure unselected remark, and reserves that transient record against repeat pickup. Query text is never stored or exposed in status. The handler returns structured facts and never writes a character line itself.

Live-scene instructions require the model to call the no-query form for factual recent-danmaku questions, request the position needed by the wording, preserve speaker and meaning, and say that no session-tail fact was observed when the tool returns no entry rather than reconstructing text from conversation memory. An entry with `within_fresh_window=false` must be described as the latest retained fact from this session, not as something said just now. During ordinary live conversation the model may call the query form once only when the current turn has one concrete topic and a matching viewer remark would materially improve the reply. It must not poll, call on every turn, override the current speaker, or mention an empty relevance result. Personality may affect framing, not the underlying fact.
...
The exact recent-chat read and ambient candidate view do not create output by themselves, so neither bypasses pipeline or safety guard. A relevant tool read and an accepted active-topic candidate only mutate transient `ambient_used` flags to prevent repeats. The model may use tool results only inside the ordinary NEKO conversation/output path; ambient candidates can only enter an already eligible `active_engagement` event, which still traverses pipeline, safety, output contracts, and dispatcher. The feature adds no external network request, polling loop, timer, disk write, dependency, or background model call. A model-chosen query-form tool call can add one ordinary tool round-trip to that conversation turn.
...
- Unselected danmaku are not passively injected into ordinary chat. For at most 120 seconds and only under low pressure, one may be returned by a model-chosen concrete-topic relevance lookup or used as a compact topic for an existing solo-stream active-engagement turn.
...
| Prompt exposure | One role-scoped tool: no-query exact read by `position=1|2|3` (one returned row), compatibility newest-first `limit`, or concrete-query relevant read (exactly one) | Tool schema is present only during a connected live session; no per-danmaku prompt injection. A query call may add one tool round-trip on the chosen ordinary turn. |
...
| Ambient awareness | Existing active-engagement turns plus an optional concrete-topic tool lookup | At most one relevant record is returned; active hosting still gets at most one 40-character candidate. Both are disabled under queue, viewer-burst, danmaku-burst, safety, or non-solo pressure and reserve used duplicates. |
