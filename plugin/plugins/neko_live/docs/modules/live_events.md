...
It also owns two deliberately separate stores of short-lived room context. `room_topic.py` keeps advisory prompt context for theme grouping. `recent_chat.py` keeps a bounded factual window for internal selection, local relevance, and the three-row co-stream projection; the legacy `get_recent_live_chat` handler remains available for compatibility but is not registered while the listener is running. An unselected remark may become one low-pressure `active_engagement` topic candidate. In `co_stream`, the newest three session rows may additionally be projected as one replaceable `ai_behavior="read"` snapshot. A selected row keeps its true position but is marked replied and limited to explicit positional fact questions. Separately, an eligible selected danmaku may enter the normal bounded active response path when `co_stream_output_policy=auto_low_interrupt`. The complete buffer is never injected.
...
- Support-event boundary: `gift`, `super_chat`, and `guard` remain owned by `live_support_events`. In solo stream they keep the existing active scheduler and immediate acknowledgement. In co-stream, light/medium support is passive context only, while verified high/milestone support keeps the priority/aggregation scheduler and requests one bounded active acknowledgement. The same event is retained as a passive shadow.
...
- Co-stream passive contract: at most three recent session-tail danmaku, one deterministic callback candidate, and two verified support facts form one hidden `read` message. Every danmaku row is authoritative for its fixed position; selected rows are marked `已回复` and remain available only for explicit positional fact questions, so active selection never shifts “上一条” into “最新” or causes ordinary conversational repetition. The callback candidate is selected only from the newest five current-session records by `ambient_hook.py`; transport redelivery, same-viewer or anonymous repeats, one-viewer floods, pure reactions, contextless fragments, and selected rows cannot win. A substantive exact line repeated by distinct identified viewers may win once as room chorus. A fixed per-role/per-live-session `coalesce_key` lets the host keep only the newest pending snapshot. The snapshot never requests a response and never triggers or advances a realtime hot swap; it is consumed only inside the existing natural-user-turn/passive delivery contract or an already-occurring safe session swap. There is no freshness timer or mid-session instruction overlay. Stable positional labels keep delayed snapshots truthful, and session reset clears the old session key. Viewer text is explicitly marked as untrusted data and is capped before dispatch.
...
`NekoLivePlugin` still exposes the `get_recent_live_chat(query="")` compatibility handler for older callers and tests, but `runtime_live_listener` keeps its model tool registration disabled before, during, and after a live listener run. The handler remains role-scoped if another controlled caller explicitly enables it. With no `query`, it reads the three-entry session tail; with one sanitized `query`, it performs deterministic local token/substring relevance ranking over the 120-second candidate view. Query text is never stored or exposed in status.
...
| Prompt exposure | One coalesced passive three-position tail plus at most one callback hint in co-stream; the legacy role-scoped read tool remains disabled | No extra model/tool round-trip. A positional row is capped at 36 characters and an older callback excerpt at 32; truncation is marked, so wording is never reconstructed beyond visible current-session facts. |
