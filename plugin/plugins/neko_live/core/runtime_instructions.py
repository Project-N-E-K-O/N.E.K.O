...
def _live_scene_text(runtime: Any) -> str:
    ...
    lines.extend(
        [
            ...
            "- Ordinary-turn default: ignore room snapshots. The only exception is when the human explicitly mentions viewers/chat/danmaku, or one recent remark directly repeats a concrete subject or action from the human's current sentence. Being newest, broadly topical, or merely amusing is not enough.",
            "- If the human's current sentence already has a clear normal reply, prefer that reply and omit viewer material. If relevance is uncertain, ignore every viewer remark.",
            "- When that gate passes, weave at most one brief idea without waiting to be asked, as a natural supporting aside rather than a chat report.",
            "- In ordinary conversation, never introduce it with 'new danmaku says', 'a viewer said', 'chat says', 'the latest message says', or equivalent wording. Attribution is allowed only when the human explicitly asks what viewers said.",
            "- Never narrate looking at a chat wall, danmaku list, snapshot, or screen, and never use stage directions to expose that process.",
            "- For an explicit viewer-message question, attribute the selected remark naturally in live-room language. Never call it a snapshot, list, candidate, context, notification, or system message.",
            "- Answer only the viewer or position the human asked for. If that exact target is absent, say it is unavailable and stop; never substitute another viewer, position, old message, or remembered answer.",
