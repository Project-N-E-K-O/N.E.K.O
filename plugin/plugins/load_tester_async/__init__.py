...
class LoadTestPlugin(NekoPluginBase):
    ...
    def _await_sync(self, coro):
        """Run async SDK calls from sync plugin code safely."""
        try:
        ...
        t = threading.Thread(target=_runner, daemon=True, name="load_tester-await-sync")
        t.start()
        done.wait()
        if "error" in box:
            raise box["error"]
    ...
    def _get_load_test_section(self, section: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            return self._await_sync(self.config.get_section(path))
        except Exception:
    ...
    def op_bus_messages_get(
        ...
        **_: Any,
        ...
        res = self._await_sync(self.ctx.bus.messages.get_async(
            plugin_id=pid_norm,
    ...
    def op_buslist_reload(
        ...
        **_: Any,
        ...
        base_list = self._await_sync(self.ctx.bus.messages.get_async(
            plugin_id=None,
            max_count=int(max_count),
            timeout=float(timeout),
            raw=True,
    ...
    def bench_bus_messages_get(
        ...
        **_: Any,
        ...
        def _op() -> None:
            _ = self._await_sync(self.ctx.bus.messages.get_async(
                plugin_id=pid_norm,
                max_count=int(max_count),
                timeout=float(timeout),
                raw=True,
    ...
    def bench_bus_events_get(
        ...
        **_: Any,
        ...
        def _op() -> None:
            _ = self._await_sync(self.ctx.bus.events.get_async(
                plugin_id=pid_norm,
                max_count=int(max_count),
                timeout=float(timeout),
            ))
    ...
    def bench_bus_lifecycle_get(
        ...
        **_: Any,
        ...
        def _op() -> None:
            _ = self._await_sync(self.ctx.bus.lifecycle.get_async(
                plugin_id=pid_norm,
                max_count=int(max_count),
                timeout=float(timeout),
            ))
