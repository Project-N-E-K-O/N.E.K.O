...
from plugin.sdk import ok, fail, SystemInfo,hook
...
class HelloPlugin(NekoPluginBase):
    ...
    def _start_debug_timer(self) -> None:
        ...
        try:
            ...
            asyncio.run(self.plugins.call_entry_async(
    ...
    def _startup_memory_debug(self) -> None:
        ...
        try:
            ...
            result = asyncio.run(self.memory_debug(query=query, timeout=timeout, **kwargs))
            self.ctx.push_message(
    ...
    def _startup_messages_debug(self) -> None:
        ...
        try:
            ...
            msg_list = asyncio.run(self.ctx.bus.messages.get_async(
            ...
            mp_list = None
            try:
                mp_list = asyncio.run(self.ctx.bus.messages.get_async(
                    plugin_id=plugin_id or None,
                    max_count=max_count,
                    priority_min=pri_opt,
            ...
            watch_list = asyncio.run(self.ctx.bus.messages.get_async(
                plugin_id=plugin_id or None,
                max_count=max_count,
                priority_min=pri_opt,
                timeout=timeout,
            )).filter(source=watch_source, strict=True)
            watcher = asyncio.run(watch_list.watch_async(self.ctx))
    ...
    async def shutdown(self, **_) -> Any:
        ...
        if timer_id:
            try:
                await self.plugins.call_entry_async(
    ...
    def _startup_events_debug(self) -> None:
        ...
        try:
            ...
            ev_list = asyncio.run(self.ctx.bus.events.get_async(
                plugin_id=plugin_id or None,
                max_count=max_count,
                timeout=timeout,
            ))
    ...
    def _startup_lifecycle_debug(self) -> None:
        ...
        try:
            ...
            lc_list = asyncio.run(self.ctx.bus.lifecycle.get_async(
                plugin_id=plugin_id or None,
                max_count=max_count,
                timeout=timeout,
            ))
    ...
    class OnDebugTickParams(BaseModel):
        model_config = ConfigDict(extra="forbid", str_strip_whitespace=True)
        timer_id: str = Field(..., min_length=1, description="Timer ID")
        burst_size: int = Field(5, ge=0, le=1000, description="Messages per tick")
        current_count: int = Field(0, ge=0, description="Current tick count")
        max_count: int = Field(0, ge=0, description="Max tick count (0 means unlimited)")

    class ConfigDebugParams(BaseModel):
        model_config = ConfigDict(extra="forbid")
        include_values: bool = Field(False, description="Include full config values")

    class MemoryDebugParams(BaseModel):
        model_config = ConfigDict(extra="forbid", str_strip_whitespace=True)
        query: str = Field("hello", min_length=1, max_length=2000, description="Query text")
        lanlan_name: str = Field("", max_length=128, description="Override lanlan_name")
        timeout: float = Field(5.0, gt=0, le=60.0, description="Timeout seconds")

    class ConversationDebugParams(BaseModel):
        model_config = ConfigDict(extra="forbid")
        max_count: int = Field(10, ge=1, le=200, description="Max conversations to retrieve")

    class HelloRunParams(BaseModel):
        model_config = ConfigDict(extra="forbid", str_strip_whitespace=True)
        name: str = Field("world", min_length=1, max_length=64, description="User name")
        sleep_seconds: float = Field(0.6, ge=0.0, le=10.0, description="Sleep duration per step")
    ...
    @plugin_entry(id="on_debug_tick", params=OnDebugTickParams)
    def on_debug_tick(
        self,
        timer_id: str,
        burst_size: int = 5,
    ...
    async def config_debug(self, include_values: bool = False, **_):
        if include_values:
            ...
            if not allow_sensitive:
                return fail(
                    "FORBIDDEN",
                    "include_values requires debug.config.allow_sensitive=true",
                )
    ...
    @plugin_entry(
        ...
        params=MemoryDebugParams,
    )
    async def memory_debug(self, query: str = "hello", lanlan_name: str = "", timeout: float = 5.0, **kwargs):
        ...
        if not ln:
            return fail(
                "VALIDATION_ERROR",
                "lanlan_name is missing (expected in args._ctx.lanlan_name or explicit lanlan_name)",
            )

        memory_query_result: Any = None
        try:
            memory_query_result = await MemoryClient(self.ctx).query_async(ln, query, timeout=float(timeout))
        except Exception as e:
            memory_query_result = {"ok": False, "error": str(e)}

        memory_list = await self.ctx.bus.memory.get_async(bucket_id=ln, limit=20, timeout=timeout)
    ...
    @plugin_entry(
        ...
        params=ConversationDebugParams,
    )
    async def conversation_debug(self, max_count: int = 10, **kwargs):
        ...
        if conversation_id:
            try:
                conversations = await self.ctx.bus.conversations.get_by_id_async(
                    conversation_id,
                    max_count=max_count,
                )
        ...
        # 2. 获取最近的对话（不过滤）
        try:
            recent = await self.ctx.bus.conversations.get_async(max_count=max_count)
