...
class MemoReminderPlugin(NekoPluginBase):
    ...
    async def _load_reminders_unlocked_async(self) -> List[Dict[str, Any]]:
        data = await self.store.get_async(_STORE_KEY, [])
        return data if isinstance(data, list) else []

    async def _save_reminders_unlocked_async(self, reminders: List[Dict[str, Any]]) -> None:
        await self.store.set_async(_STORE_KEY, reminders)

    def _load_reminders_unlocked(self) -> List[Dict[str, Any]]:
        return asyncio.run(self._load_reminders_unlocked_async())

    def _save_reminders_unlocked(self, reminders: List[Dict[str, Any]]) -> None:
        asyncio.run(self._save_reminders_unlocked_async(reminders))

    async def _load_reminders_async(self) -> List[Dict[str, Any]]:
        with self._reminders_lock:
            return await self._load_reminders_unlocked_async()

    async def _save_reminders_async(self, reminders: List[Dict[str, Any]]) -> None:
        with self._reminders_lock:
            await self._save_reminders_unlocked_async(reminders)
    ...
    async def startup(self, **_):
        ...
        count = len(await self._load_reminders_async())
    ...
    async def add_reminder(self, time: str, message: str, repeat: str = "once", **kwargs):
        ...
        with self._reminders_lock:
            reminders = await self._load_reminders_unlocked_async()
            cfg = await self.store.get_async("_memo_cfg", {})
    ...
    async def list_reminders(self, **_):
        reminders = await self._load_reminders_async()
    ...
    async def clear_reminders(self, **_):
        with self._reminders_lock:
            count = len(await self._load_reminders_unlocked_async())
            await self._save_reminders_unlocked_async([])
        self._notify_change()
