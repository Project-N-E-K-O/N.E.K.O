...
> SDK v2 完整开发教程。新开发使用 Plugin / Adapter；Extension 仅保留已有包的弃用兼容。
...
- [第六章：完整示例](#第六章完整示例)
- [第七章：Extension 弃用兼容](#第七章extension-弃用兼容)
...
N.E.K.O 插件系统是一个基于 Python 的插件框架，允许开发者创建可扩展的功能模块。普通 Plugin 与 Adapter 运行在独立进程中，通过 ZMQ IPC 与主系统交互；弃用兼容的 Extension 会注入宿主插件进程。
...
| **Extension（已弃用）** | `plugin.sdk.extension` | 仅兼容已存在的 Extension | 注入宿主插件进程 |
...
- **进程隔离**：普通 Plugin 与 Adapter 独立运行；兼容 Extension 与宿主同进程，其崩溃或副作用可能影响宿主
- **异步支持**：支持同步和异步入口函数
...
├── extension/      ← 已弃用兼容入口（仅维护已有 Extension）
...
> 使用 `adapter`。下文仅用于识别、维护和迁移已经存在的 Extension，不能作为
> 新建模板。完整替代方案见 [v0.9 迁移指南](../docs/zh-CN/plugins/migration-v0.9.md)。

### 7.1 什么是 Extension？

兼容加载器可把已有 `PluginRouter` 注入宿主插件进程（不是独立进程）。这条路径只用于迁移旧包。

### 7.2 新功能应放在哪里？

- 独立功能：普通 `plugin`
- 你维护原宿主且只需拆分大型代码：宿主内使用 `PluginRouter`
- 外部协议桥接：`adapter`
...
对应 manifest 使用 `type = "extension"`、声明 `[plugin.host]`，并让
`plugin.entry` 指向这个 `PluginRouter` 子类。历史 `NekoExtensionBase`、
`extension`、`extension_entry`、`extension_hook` 只保留导入兼容；加载器不读取
其 Extension 专用 metadata。入口应改成 `@plugin_entry`，钩子逻辑应迁入宿主。
...
### 7.5 兼容期 Extension SDK 导出

`plugin.sdk.extension` 的历史符号仍可导入，以便诊断旧代码；普通 Plugin 新代码
不得使用该 facade。注意这些符号的专用 metadata 不在当前加载执行路径上：

- `NekoExtensionBase`, `extension`, `extension_entry`, `extension_hook` — 历史迁移标记，不是当前 loader contract
...
普通 Plugin 与 Adapter 的崩溃通常不会影响主系统或其他插件，因为它们独立运行。弃用兼容的 Extension 注入宿主插件进程，其崩溃或副作用可能影响宿主；应尽快迁移。
...
- **Extension**：已弃用；仅维护已有包，并尽快迁入普通 Plugin 或原宿主
- **Adapter**：桥接外部协议（MCP、NoneBot 等）

### Q: `shared` 包是什么？我需要用它吗？

`shared` 是 SDK 的内部实现细节。**你不应该直接导入它。** 新代码始终从 `plugin.sdk.plugin` 或 `plugin.sdk.adapter` 导入；`plugin.sdk.extension` 只允许已有 Extension 在兼容迁移期使用。
...
### Extension 兼容 SDK（已弃用，`plugin.sdk.extension`）

以下历史接口仅用于识别旧代码，不得用于新开发；当前可加载的兼容入口必须是
`PluginRouter` + `@plugin_entry`：
