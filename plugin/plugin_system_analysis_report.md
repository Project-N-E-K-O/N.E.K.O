...
N.E.K.O Plugin 系统是一个基于 Python 的高性能插件框架,采用进程隔离架构,支持异步编程和类型安全。该系统通过 FastAPI 提供 HTTP API 接口,使用 ZeroMQ 实现高效的进程间通信,并集成了 Rust 编写的消息平面组件以提升性能。
...
- **runs.py**: 运行实例总线客户端 (通过 server/runs.py 管理)
...
#### 4.1 主服务器 (user_plugin_server.py)
- **技术栈**: FastAPI
- **核心功能**:
  - HTTP API 端点
  - WebSocket 支持
...
**Rust 实现** (`plugin/rust/neko-message-plane/`):
- **语言**: Rust
- **架构**:
  - 单进程多线程架构
  - Worker Pool 模式 (基于 CPU 核心数)
  - 使用 crossbeam channel 进行线程间通信
  - ZeroMQ ROUTER/PULL/PUB 模式
- **存储**:
  - `Store`: 使用 DashMap (并发哈希表) + VecDeque
  - 读缓存优化 (read_cache): 无锁快速访问最近事件
  - 原子计数器 (AtomicU64) 追踪指标
- **内存分配器**: jemalloc (非 MSVC 平台)
- **序列化**: 
  - rmp-serde (MessagePack)
  - serde_json (JSON)
  - 双格式缓存 (payload_json + payload_mp)
- **优点**:
  - 零拷贝消息传递
  - 真正的并发处理 (无 GIL)
  - 内存安全保证
...
**协议兼容性**:
- 两种实现使用相同的 ZeroMQ 端点和协议
- 支持相同的 RPC 操作 (bus.get_recent, bus.query, bus.publish 等)
- 可以互换使用,无需修改客户端代码

**使用场景建议**:
- **Python 实现**: 开发测试环境、小规模部署、快速原型
- **Rust 实现**: 生产环境、高并发场景、大规模部署
...
#### 8.2 Python 绑定 (neko_message_plane_wheel/)
- **功能**: Rust 消息平面的 Python 接口
- **技术**: PyO3

#### 8.3 CLI 工具 (neko_plugin_cli/)
- **功能**: 命令行工具
- **用途**: 插件管理和调试
...
N.E.K.O Plugin 系统是一个设计精良、功能完善的插件框架。它通过进程隔离保证稳定性,通过 ZeroMQ 和 Rust 优化性能,通过完善的 SDK 降低开发门槛。系统支持复杂的插件间通信、事件驱动编程、定时任务等高级特性,同时提供了完整的生命周期管理、配置管理、日志系统和监控能力。
