...
/**
 * Manage a WebSocket-based live log stream for a plugin.
 *
 * Establishes and maintains a WebSocket connection for the given plugin ID, handles incoming
 * log messages (initial and appended logs), automatic reconnection with a capped retry count,
 * and integrates with the logs and auth stores.
 *
 * @param pluginIdInput - A plugin ID or a ref to a plugin ID; changing the value will reconnect to the new plugin's stream
 * @returns An object with:
 *  - `isConnected`: a reactive boolean indicating whether the WebSocket is currently connected
 *  - `connect`: a function to (re)establish the WebSocket connection
 *  - `disconnect`: a function to close the connection and cancel pending reconnection attempts
 */
