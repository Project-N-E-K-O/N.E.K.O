...
def _normalize_quick_lines(value: Any) -> Dict[str, list[str]]:
    """Validate and trim quick-path lines; failed keys fall back to the frontend's built-in lines."""
    if not isinstance(value, dict):
        return {}
...
def _parse_control_instructions(reply: str) -> Dict[str, Any]:
    """Parse structured control instructions (the mood/difficulty JSON line) from the reply."""
...
async def game_route_any_active(lanlan_name: str = ""):
    """Bootstrap reconciliation endpoint: returns whether the given lanlan_name has
    any active game_route, for game_window_state_change event subscribers such as
    chat.html / pet.

    Scenario: the edge-triggered game_window_state_change WS event is only pushed
    at the activate / finalize instant; if a subscriber loads for the first time
    (or the WS reconnects) while a route is already active, it never hears the
    historical opened event and the UI stays in the normal state forever.
    Subscribers call this endpoint on their init path and, on seeing active,
    dispatch an opened DOM event themselves to realign the UI with the backend
    state. codex P1.

    Single GET, lanlan_name as the only parameter, no side effects — no CSRF
    protection needed."""
    resolved = _resolve_lanlan_name(lanlan_name)
...
async def game_end(game_type: str, request: Request):
    """End a game round and clean up its LLM session."""
    try:
        data = await request.json()
    except Exception:
        data = {}
...
async def game_quick_lines(game_type: str, request: Request):
    """Generate a set of quick-path lines specific to the current catgirl when entering a game.

    Product semantics: this is part of "in-game context initialization". The code
    tells the LLM: the current catgirl is about to play soccer with the player;
    generate backup short lines matching the current persona. On success the
    frontend replaces its built-in quick-path lines with these; on failure the
    frontend's built-in lines are still used.

    quick-lines is the first endpoint in the soccer flow to hit the LLM (before
    /route/start), so it must absorb ``i18n_language`` from the request body to
    proactively heal mgr.user_language — otherwise the first batch of quick lines
    would come out in the English imposed by the global cache at ``start_session``.
    """
    if game_type != "soccer":
...
async def game_character(game_type: str, request: Request):
    """Get current character info (requirement 2: for character replacement).

    Returns the character's model type and path. The soccer game AI side supports
    Live2D / VRM; for MMD the frontend still falls back to the default Live2D model.
    """
    try:
        config_manager = get_config_manager()
