...
_CHAT_FULL_REWRITE_RE = re.compile(
    ...
    #
    # ⚠️⚠️ 口子的判据是**全量限定词**（全部/所有/每个/一切/整体/内容），不是
    # 「的」后面那个名词叫什么。上一版只白名单了「内容」，于是
    # `把整個角色卡的全部設定重寫一遍` 被挡掉——整卡补全通路不触发，只落库半张卡。
    # 同族还有 的全部資料 / 的全部人設 / 的全部描述。「整卡级名词」是开集，
    # 枚举不完；「全量限定词」是闭集，就括号里这几个。
    # `{0,12}?` 的有界窗口顺带覆盖语序倒置的「…的設定全部重寫」，
    # `[^。！？!?]` 保证不跨句吸附。
    r"整个卡片|整個卡片|整个卡|整個卡|全卡)"
    r"(?:(?=的[^。！？!?]{0,12}?(?:全部|所有|每个|每個|一切|整体|整體|内容|內容))"
    r"|(?![的片]))"
