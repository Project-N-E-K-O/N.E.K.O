...
_CHAT_FULL_REWRITE_RE = re.compile(
    ...
    #   整个卡          → 命中                          → True
    #   整个卡的名字     → 被「的」挡                     → False
    # ⚠️ `(?![的片])` 之外再开一个口子：「整卡目标 + 的 + 全量限定词」仍然是整卡
    # 重写，只有「…的名字」这类单字段定语才该被挡。
    #
    # ⚠️⚠️ 口子的判据是**全量限定词**，不是「的」后面那个名词叫什么。上一版只
    # 白名单了「内容」，于是 `把整個角色卡的全部設定重寫一遍` 被挡掉——整卡补全
    # 通路不触发，只落库半张卡。「整卡级名词」（設定/資料/人設/描述）是开集，
    # 枚举不完；「全量限定词」是闭集。
    #
    # ⚠️⚠️ 但闭集还要**按语法分布再切一刀**，不能一视同仁地丢进 12 字窗口：
    #   · 全部 / 所有 / 每个 / 每個 / 一切 是真·全称限定词，可以后置浮动
    #     （「…的設定全部重寫」），给它们有界窗口。
    #   · 整体 / 整體 是**副词**（整体重写＝holistically 地重写），
    #     内容 / 內容 是能挂在任何单字段后的**普通名词**（簡介內容）。
    #     这两类只有紧贴「的」当中心语时才代表整卡（「…的整體設定」「…卡片的內容」）。
    #     放进窗口的话，`把整個卡的名字整體重寫一下` 会被判成整卡重写，
    #     _complete_full_rewrite_actions 就把其余所有字段一起覆盖掉——
    #     跟前两轮修的 P1 是同一个破坏面（Codex P1 第四轮）。
    r"整个卡片|整個卡片|整个卡|整個卡|全卡)"
    r"(?:(?=的(?:[^。！？!?]{0,12}?(?:全部|所有|每个|每個|一切)"
    r"|整体|整體|内容|內容))"
    r"|(?![的片]))"
