...
_CHAT_FULL_REWRITE_RE = re.compile(
    ...
    # ⚠️ 全称限定词要**列全**。上一版只有 每个/每個，漏掉 每一个/每一個/每項/
    # 每项/各項/各项——`把整個卡的每一個欄位都重寫` 是明确的整卡请求，被挡掉后
    # 整卡补全通路不触发，只落库部分字段（Codex P2）。
    # 这一侧是闭集（汉语的全称限定词就这些），跟「整卡级名词」那个开集不一样。
    r"(?:(?=的(?:全部|所有|每一个|每一個|每个|每個|每项|每項|各项|各項"
    r"|一切|整体|整體|内容|內容))"
    r"|(?![的片]))"
...
_CHAT_NEGATED_REWRITE_RE = re.compile(
    ...
    r"(?:不要|不用|不需要|不必|不想|不准|不許|不许|不得|不可|不能"
