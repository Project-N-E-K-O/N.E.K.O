...
async def save_conversation_settings(request: Request):
    ...
    try:
        ...
        if 'localTurnDetectionEnabled' in data:
            await _restart_active_voice_sessions_for_local_turn(bool(data['localTurnDetectionEnabled']))

        if 'smartTurnEnabled' in data:
            _apply_smart_turn_to_active_sessions(bool(data['smartTurnEnabled']))
