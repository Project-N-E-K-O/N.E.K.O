...
from config.prompts_sys import _loc, INNER_THOUGHTS_HEADER
...
async def new_dialog(lanlan_name: str):
    ...
    result = _loc(INNER_THOUGHTS_HEADER, _lang).format(name=lanlan_name) + f"{lanlan_name}的脑海里经常想着自己和{master_name}的事情，她记得{json.dumps(settings_manager.get_settings(lanlan_name), ensure_ascii=False)}\n\n"
