...
@pytest.mark.asyncio
async def test_update_catgirl_l2d_accepts_pngtuber_config(monkeypatch):
    payload = {
        'model_type': 'pngtuber',
        'pngtuber': {
            'idle_image': '/static/assets/neko-idle/cat-idle-cat1.gif',
            'talking_image': '/static/assets/neko-idle/cat-idle-cat1-click.gif',
            'click_image': '/static/assets/neko-idle/cat-idle-cat1-click.gif',
            'drag_image': '/static/assets/neko-idle/cat-idle-cat-move-1.gif',
            'happy_image': '/static/icons/happy_cat.png',
            'sad_image': '/static/icons/sad_cat.png',
            'angry_image': '/static/icons/angry_cat.png',
            'surprised_image': '/static/icons/surprise_cat.png',
            'source_type': 'gif',
            'scale': 1,
            'offset_x': 0,
            'offset_y': 0,
    ...
    assert pngtuber['idle_image'] == '/static/assets/neko-idle/cat-idle-cat1.gif'
    assert pngtuber['talking_image'] == '/static/assets/neko-idle/cat-idle-cat1-click.gif'
    assert pngtuber['click_image'] == '/static/assets/neko-idle/cat-idle-cat1-click.gif'
    assert pngtuber['drag_image'] == '/static/assets/neko-idle/cat-idle-cat-move-1.gif'
    assert pngtuber['happy_image'] == '/static/icons/happy_cat.png'
    assert pngtuber['source_type'] == 'gif'
    assert pngtuber['scale'] == 1
    assert pngtuber['offset_x'] == 0
    assert pngtuber['offset_y'] == 0
    assert pngtuber['mirror'] is False


@pytest.mark.asyncio
async def test_update_catgirl_l2d_normalizes_pngtuber_position_fields(monkeypatch):
    payload = {
        'model_type': 'pngtuber',
        'pngtuber': {
            'idle_image': '/static/assets/neko-idle/cat-idle-cat1.gif',
    ...
    assert response.status_code == 200
    assert body['success'] is True
    saved_group = next(value for value in saved.values() if isinstance(value, dict))
    saved_catgirl = next(iter(saved_group.values()))
    pngtuber = get_reserved(saved_catgirl, 'avatar', 'pngtuber')
    assert pngtuber['scale'] == 1
    assert pngtuber['offset_x'] == 5000
    assert pngtuber['offset_y'] == -5000
    assert pngtuber['mirror'] is True


@pytest.mark.asyncio
async def test_update_catgirl_l2d_accepts_pngtuber_remote_urls(monkeypatch):
    payload = {
        'model_type': 'pngtuber',
        'pngtuber': {
            'idle_image': 'https://example.com/avatar.png?version=1',
            'talking_image': 'https://example.com/talking.gif#loop',
        },
    }
    ...
    saved_catgirl = next(iter(saved_group.values()))
    pngtuber = get_reserved(saved_catgirl, 'avatar', 'pngtuber')
    assert pngtuber['idle_image'] == 'https://example.com/avatar.png?version=1'
    assert pngtuber['talking_image'] == 'https://example.com/talking.gif#loop'


@pytest.mark.asyncio
@pytest.mark.parametrize(
    ('image_path', 'expected_error'),
    [
        ('data:image/png;base64,AAAA', 'data URL'),
        ('/user_pngtuber/../evil.png', '路径遍历'),
    ],
)
async def test_update_catgirl_l2d_rejects_unsafe_pngtuber_paths(monkeypatch, image_path, expected_error):
    payload = {
        'model_type': 'pngtuber',
        'pngtuber': {
            'idle_image': image_path,
        },
    }
...
def test_migrate_catgirl_reserved_preserves_pngtuber_model_type():
    catgirl = {
        'model_type': 'pngtuber',
        'pngtuber': {
    ...
    assert get_reserved(catgirl, 'avatar', 'pngtuber', 'idle_image') == '/user_pngtuber/Sample_PNGTuber_Test_Model/idle.gif'
    assert 'model_type' not in catgirl
    assert 'pngtuber' not in catgirl

    flattened = flatten_reserved(catgirl)
    assert flattened['model_type'] == 'pngtuber'
    assert flattened['pngtuber']['talking_image'] == '/user_pngtuber/Sample_PNGTuber_Test_Model/talking.gif'
