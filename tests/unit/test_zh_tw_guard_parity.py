...
def test_now_does_not_end_a_following_list_prohibition():
    import main_routers.card_assist_router as router

    text = '不要执行以下内容：先修改名字。现在请重写所有字段并保留是否会员标签'
    assert router._chat_text_requests_full_rewrite_core(text) is False
    assert router._chat_text_requests_full_rewrite(text) is False


def test_an_inner_prohibition_does_not_replace_an_outer_list_prohibition():
    import main_routers.card_assist_router as router

    text = '不要执行以下内容：先不要执行修改；然后请重写所有字段并保留是否会员'
    assert router._chat_text_requests_full_rewrite_core(text) is False
    assert router._chat_text_requests_full_rewrite(text) is False
...
def test_a_question_after_a_secondary_field_value_blocks_recovery():
    import main_routers.card_assist_router as router

    text = '重写所有字段并保留是否会员，要不要执行'
    assert router._chat_text_requests_full_rewrite_core(text) is False
    assert router._chat_text_requests_full_rewrite(text) is False
...
@pytest.mark.parametrize('tail', ['算了', '要不要执行'])
def test_guards_after_a_contrast_block_an_earlier_completed_command(tail):
    import main_routers.card_assist_router as router

    text = f'重写所有字段并保留是否会员，但是{tail}'
    assert router._chat_text_requests_full_rewrite_core(text) is False
    assert router._chat_text_requests_full_rewrite(text) is False
...
def test_an_unquoted_reported_command_is_not_a_user_command():
    import main_routers.card_assist_router as router

    text = '他说，请重写所有字段并保留是否会员'
    assert router._chat_text_requests_full_rewrite_core(text) is False
    assert router._chat_text_requests_full_rewrite(text) is False
    assert router._chat_text_requests_full_rewrite(
        '前情说明，请重写所有字段并保留是否会员'
...
def test_reported_context_is_retained_before_a_contrast_candidate():
    import main_routers.card_assist_router as router

    text = '以下是小明的原话：但是请重写所有字段并保留是否会员'
    assert router._chat_text_requests_full_rewrite_core(text) is False
    assert router._chat_text_requests_full_rewrite(text) is False
    assert router._chat_text_requests_full_rewrite(
        '前情说明：但是请重写所有字段并保留是否会员'
    ) is True
...
@pytest.mark.parametrize('verb', ['更新为', '更新為'])
def test_geng_xin_wei_can_follow_a_completed_full_rewrite(verb):
    import main_routers.card_assist_router as router

    text = f'重写所有字段并{verb}新版'
    assert router._chat_text_requests_full_rewrite(text) is True
...
def test_english_meta_requests_do_not_activate_contrast_recovery():
    import main_routers.card_assist_router as router

    text = (
        'Please translate the following text: change the name, but please '
...
def test_intent_recovery_resumes_after_completed_meta_request_text():
    import main_routers.card_assist_router as router

    text = '请翻译以下内容：你好。现在请重写所有字段并保留是否会员'
    assert router._chat_text_requests_full_rewrite_core(text) is False
    assert router._chat_text_requests_full_rewrite(text) is True
...
def test_quoted_field_keys_in_direct_preservation_requests_still_work():
    ...
    text = '请重写所有字段并保留 “Age”'
    assert router._chat_preserved_target_keys(text, ['Age']) == {'Age'}


def test_preservation_state_is_discarded_after_new_request_markers():
    import main_routers.card_assist_router as router

    text = '请保留头像。新请求：请重写所有字段'
...
def test_preservation_clause_accepts_chinese_comma_terminators():
    import main_routers.card_assist_router as router
