...
class TestGetModelApiConfig:
    ...
    @pytest.mark.unit
    def test_agent_respects_custom_toggle_off(self, config_manager):
        """Agent follows enableCustomApi like other model types:
        when toggle is off, agent falls back to assist profile defaults."""
        _write_core_config(config_manager, {
            'coreApiKey': 'sk-core',
            'coreApi': 'qwen',
            'assistApi': 'qwen',
        ...
        assert result['is_custom'] is False, \
            'Agent should respect enableCustomApi=false'
