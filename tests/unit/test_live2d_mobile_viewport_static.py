...
def test_mobile_renderer_initializes_from_current_web_viewport():
    ...
    init_block = source[
        source.index("const useViewportSize = isMobileWidth();"):
        source.index("this.pixi_app = new PIXI.Application", source.index("const useViewportSize = isMobileWidth();"))
    ]
...
def test_mobile_default_and_reset_positions_use_viewport_not_renderer_screen():
    ...
    reset_block = core_source[reset_start:core_source.index("console.log('模型位置已复位到初始状态');", reset_start)]

    for block in (apply_block, reset_block):
