...
def test_generic_opening_angles_rotate_before_reuse():
    ...
    assert (
        _select_startup_greeting_variant(recent, has_followup=False) == "time_greeting"
    )

    exhausted = [
        _record("simple_presence", ts=5.0),
        _record("light_question", ts=4.0),
        _record("personal_share", ts=3.0),
        _record("recent_continuity", ts=2.0),
        _record("time_greeting", ts=1.0),
    ]
    assert (
...
def test_fresh_startup_prefers_time_of_day_greeting_variant():
    assert _select_startup_greeting_variant([], has_followup=False) == "time_greeting"
