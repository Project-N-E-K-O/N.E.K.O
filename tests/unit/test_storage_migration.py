...
def test_load_storage_migration_accepts_complete_legacy_v1_checkpoint(tmp_path):
    ...
    assert upgraded["migration_mode"] == "copy"
    assert isinstance(upgraded["target_baseline"], dict)
...
def test_run_pending_storage_migration_marks_failure_and_recovers_to_source_root(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("utils.storage_migration._merge_runtime_entry", _boom)
...
@pytest.mark.unit
def test_first_custom_selection_combines_legacy_and_current_data_without_moving_cloudsave(tmp_path):
    config_manager = _make_config_manager(tmp_path)
    current_root = config_manager.app_docs_dir
    legacy_root = tmp_path / "legacy-documents" / "N.E.K.O"
    target_root = tmp_path / "chosen-parent" / "N.E.K.O"

    (current_root / "config").mkdir(parents=True, exist_ok=True)
    (current_root / "config" / "characters.json").write_text("CURRENT", encoding="utf-8")
    legacy_root.mkdir(parents=True)
    (legacy_root / "community_auth.json").write_text("CREDENTIAL", encoding="utf-8")
    (legacy_root / "pngtuber").mkdir()
    (legacy_root / "pngtuber" / "avatar.png").write_bytes(b"avatar")
    (legacy_root / "cloudsave").mkdir()
    (legacy_root / "cloudsave" / "manifest.json").write_text("CLOUD", encoding="utf-8")

    create_pending_storage_migration(
        config_manager,
        source_root=current_root,
        additional_source_roots=[legacy_root],
        target_root=target_root,
        selection_source="custom",
    )
    ...
    assert result["completed"] is True
    assert (target_root / "config" / "characters.json").read_text(encoding="utf-8") == "CURRENT"
    assert (target_root / "community_auth.json").read_text(encoding="utf-8") == "CREDENTIAL"
    assert (target_root / "pngtuber" / "avatar.png").read_bytes() == b"avatar"
    assert not (target_root / "cloudsave").exists()
    assert (legacy_root / "cloudsave" / "manifest.json").is_file()
    completed = load_storage_migration(config_manager)
    assert completed["retained_source_root"] == str(legacy_root.resolve())
    assert completed["backup_root"] == str(legacy_root.resolve())
    root_state = config_manager.load_root_state()
    assert root_state["last_migration_backup"] == str(legacy_root.resolve())
...
def test_interrupted_publish_is_rolled_back_before_retry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(storage_migration_module, "_merge_runtime_entry", fail_retry_copy)
    result = run_pending_storage_migration(config_manager)
