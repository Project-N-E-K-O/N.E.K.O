...
def test_home_template_loads_avatar_performance_adapter_before_director_without_old_wakeup():
    source = Path("templates/index.html").read_text(encoding="utf-8")

    assert "yui-guide-wakeup.js" not in source
    positions = [
        _script_tag_position(source, name)
        for name in (
            "yui-guide-overlay.js",
...
def test_yui_guide_static_asset_version_tracks_avatar_performance_runtime_files():
    source = Path("main_routers/pages_router.py").read_text(encoding="utf-8")
    paths_block = source[
        source.index("_YUI_GUIDE_ASSET_VERSION_PATHS = ("):
        source.index("_STATIC_ASSET_CACHE_TTL", source.index("_YUI_GUIDE_ASSET_VERSION_PATHS = ("))
    ]

    assert "static/avatar-performance-stage.js" in paths_block
    assert "static/yui-guide-avatar-stage.js" in paths_block
    assert "static/yui-guide-director.js" in paths_block
    assert "static/yui-guide-wakeup.js" not in paths_block


def test_avatar_performance_exports_p0_contract_facade():
    source = Path("static/avatar-performance-stage.js").read_text(encoding="utf-8")

    assert "if (window.AvatarPerformanceStage && window.AvatarPerformance)" in source
    assert "const CONTRACTS = Object.freeze({" in source
    assert "window.AvatarPerformance = {" in source
    assert "createStage: legacyApi.create" in source
    assert "createCoordinator: function (options)" in source
    assert "createLive2DDriver: legacyApi.createLive2DDriver" in source
    assert "createLive2DPerformance: function (options)" in source
    assert "createNoopDriver: createNoopDriver" in source
    assert "contracts: CONTRACTS" in source
...
def test_avatar_performance_p0_driver_and_sequence_contracts_are_declared():
    source = Path("static/avatar-performance-stage.js").read_text(encoding="utf-8")

    for driver_method in (
        "'resolveAvatar'",
        "'capture'",
        "'restore'",
        "'stopMotion'",
        ...
        assert driver_method in source
    ...
    assert "resolveAvatar: function () { return null; }" in source
    assert "captureExpression: function () { return {}; }" in source
    assert "applyExpression: function () { return false; }" in source
    assert "applyEmotion: function () { return Promise.resolve(false); }" in source
    assert "clearExpression: function () { return false; }" in source
    assert "if (type === 'emotion' || type === 'setEmotion' || type === 'applyEmotion')" in source
    assert "if (type === 'speechCue')" in source
    assert "sequenceOptions.onSpeechCue" in source
    assert "capabilities: Array.isArray(normalized.capabilities)" in source
    assert "capabilities: normalizeCapabilities(normalized.capabilities)" in source
    assert "sessionHasCapability(session, capability)" in source
    assert "this.sessionHasCapability(session, 'motion')" in source
    assert "this.sessionHasCapability(session, 'frame')" in source
    assert "captureParamIds: Array.isArray(normalized.captureParamIds)" in source
    assert "if (!this.isSessionActive(sessionId))" in source


def test_avatar_performance_module_is_generic_not_home_yui_specific():
    source = Path("static/avatar-performance-stage.js").read_text(encoding="utf-8")
...
def test_avatar_performance_p0_owned_sequence_keeps_session_contract_runtime():
    node = shutil.which("node")
    if not node:
        pytest.skip("node is required for avatar performance browser-script smoke")
    ...
    script = r"""
        ...
        const source = fs.readFileSync('static/avatar-performance-stage.js', 'utf8');
        const captures = [];
        const cues = [];
        let clearExpressionCalls = 0;
        const driver = {
            kind: 'contract-test',
            isAvailable: () => true,
            capture: (_session, options) => {
            ...
            applyFrame: () => {},
            hasMotion: () => false,
            playMotion: () => Promise.resolve(false),
            clearExpression: () => {
                clearExpressionCalls += 1;
                return true;
            },
            ...
            applyExpression: () => Promise.resolve(true),
            resolveParamId: (id) => id,
            setParam: () => true,
            captureParams: () => ({}),
            restoreParams: () => {},
            lookAt: () => true,
        ...
        sandbox.window.performance = { now: () => 0 };
        ...
        const stage = sandbox.window.AvatarPerformanceStage.create({
            driver,
            sequences: {
                owned: [
                    { type: 'speechCue', cue: 'line-start' },
        ...
        (async () => {
            const result = await stage.runOwnedSequence('owned-owner', 'owned', {
                priority: 2,
                force: true,
                capabilities: ['expression', 'params'],
                paramIds: ['ParamAngleX'],
                captureParamIds: ['ParamSmile'],
                releaseOnComplete: true,
                onSpeechCue: (payload) => {
                    cues.push(payload.cue + ':' + payload.sessionId);
                    return true;
                }
            });

            assert.strictEqual(result.completed, true);
            assert.strictEqual(stage.activeSession, null, 'owned sequence did not release its session');
            assert.strictEqual(clearExpressionCalls, 1, 'clearExpression did not run inside the active session');
            assert(cues.includes('emotion:happy'), 'emotion step did not delegate to the driver');
            assert.strictEqual(JSON.stringify(captures[0].capabilities), JSON.stringify(['expression', 'params']));
            assert.strictEqual(JSON.stringify(captures[0].paramIds), JSON.stringify(['ParamAngleX']));
            assert.strictEqual(JSON.stringify(captures[0].captureParamIds), JSON.stringify(['ParamSmile']));
            assert(cues.some((cue) => cue.startsWith('line-start:avatar-performance-')));
            assert.strictEqual(await stage.clearExpression({ sessionId: 'missing-session' }), false);
        })().catch((error) => {
...
def test_avatar_performance_p0_coordinator_is_safe_placeholder():
    source = Path("static/avatar-performance-stage.js").read_text(encoding="utf-8")

    assert "function createNoopCoordinator()" in source
    assert "kind: 'noop-coordinator'" in source
    assert "acquire: function () { return null; }" in source
    assert "isCapabilityLocked: function () { return false; }" in source
    assert "getLockedCapabilities: function () { return []; }" in source
    assert "getActiveSession: function () { return null; }" in source
    assert "window.AvatarPerformanceStage = legacyApi;" in source


def test_avatar_performance_p3_coordinator_contract_is_exposed():
    source = Path("static/avatar-performance-stage.js").read_text(encoding="utf-8")

    assert "class AvatarPerformanceCoordinator" in source
    assert "findLockHolders(avatarId, capabilities)" in source
    assert "getLockedCapabilities(avatarId)" in source
    assert "AvatarPerformanceCoordinator: AvatarPerformanceCoordinator" in source
    assert "const defaultCoordinator = new AvatarPerformanceCoordinator();" in source
    assert "createCoordinator: function (options)" in source
    assert "getDefaultCoordinator: function ()" in source
    assert "isCapabilityLocked: function (avatarId, capability)" in source
    assert "'getLockedCapabilities'" in source


def test_avatar_performance_p4_live2d_resource_resolver_contract_is_declared():
    source = Path("static/avatar-performance-stage.js").read_text(encoding="utf-8")
    live2d_source = Path("static/live2d-model.js").read_text(encoding="utf-8")
    emotion_source = Path("static/live2d-emotion.js").read_text(encoding="utf-8")

    for marker in (
        "collectMotionCandidates(motion, options)",
        "collectExpressionCandidates(expression, options)",
        "captureExpressionParamsFromFile(file)",
        "hasExpressionName(name)",
        "hasExpressionFile(file)",
        "expressionParamSnapshot",
        "withPerformanceBypass(callback)",
    ):
        assert marker in source

    assert "if (type === 'expression' || type === 'applyExpression')" in source
    assert "if (type === 'clearExpression')" in source
    assert "priority <= this.activeSession.priority && !normalized.force" in source
    assert "Live2DManager.prototype.isAvatarPerformanceCapabilityLocked" in live2d_source
    assert "this.isAvatarPerformanceCapabilityLocked('motion')" in live2d_source
    assert "this.isAvatarPerformanceCapabilityLocked('lookAt')" in live2d_source
    assert "this.isAvatarPerformanceCapabilityLocked('expression')" in emotion_source


def test_avatar_performance_p1_builtin_presets_are_registered():
    source = Path("static/avatar-performance-stage.js").read_text(encoding="utf-8")

    for preset_name, method_name in (
        ("hop", "runHop"),
        ("shake", "runShake"),
        ("settle", "runSettle"),
        ("breathe", "startBreathe"),
    ):
        assert f"if (name === '{preset_name}')" in source
        assert f"stage.{method_name}(options || {{}})" in source
        assert f"{method_name}(options)" in source

    assert "this.cancelPresetTweens(sessionId, 'breathe');" in source
    assert "sessionId + ':preset:breathe:'" in source
    assert "this.reducedMotion ? 0" in source


def test_avatar_performance_p1_driver_temporarily_disables_transition_and_restores_it():
    source = Path("static/avatar-performance-stage.js").read_text(encoding="utf-8")

    assert "transition: container.style.transition || ''" in source
    assert "container.style.transition = 'none';" in source
    assert "restoreContainerStyle(styleSnapshot)" in source
    assert "container.style.transition = styleSnapshot.transition || '';" in source


def test_avatar_performance_p1_presets_and_release_work_in_runtime():
    node = shutil.which("node")
    if not node:
        pytest.skip("node is required for avatar performance browser-script smoke")
    ...
    script = r"""
        ...
        const manager = {
            ...
            playMotion: () => Promise.resolve(true),
            suspendTemporaryMotions: () => {
                suspendCalls += 1;
                return true;
            },
            resumeTemporaryMotions: () => {
                resumeCalls += 1;
                return true;
            }
        };
        const driver = {
            ...
            acquireSession: () => {},
            releaseSession: () => {},
            applyFrame: (frame) => frames.push(Object.assign({}, frame)),
            hasMotion: () => true,
            playMotion: () => Promise.resolve(true),
            resolveParamId: (id) => id,
            setParam: () => true,
        ...
        sandbox.window.performance = { now: () => now };
        sandbox.window.matchMedia = () => ({ matches: false });
        ...
        sandbox.window.cancelAnimationFrame = (id) => cancelledRaf.add(id);
        sandbox.window.setTimeout = (callback) => {
            callback();
            return 1;
        };
        sandbox.window.clearTimeout = () => {};
        sandbox.window.live2dManager = manager;

        async function flushRaf(limit = 80) {
            let count = 0;
            while (rafQueue.length && count < limit) {
                const item = rafQueue.shift();
                if (!cancelledRaf.has(item.id)) {
                    now += 1000;
                    item.callback(now);
                }
                count += 1;
            ...
            assert(count < limit, 'requestAnimationFrame queue did not settle');
        }

        (async () => {
            vm.runInNewContext(source, sandbox);
            const api = sandbox.window.AvatarPerformanceStage;
            assert(api, 'legacy AvatarPerformanceStage export is missing');
            assert(sandbox.window.AvatarPerformance, 'P0 AvatarPerformance facade is missing');

            const stage = api.create({ driver });
            const session = stage.acquire('preset-smoke', { priority: 1, force: true });
            assert(session && session.id, 'stage did not acquire a session');

            let promise = stage.runPreset('hop', { sessionId: session.id, durationMs: 180 });
            await flushRaf();
            assert.strictEqual(await promise, true, 'hop preset did not complete');

            promise = stage.runPreset('shake', { sessionId: session.id, durationMs: 140, cycles: 1 });
            await flushRaf();
            assert.strictEqual(await promise, true, 'shake preset did not complete');

            assert.strictEqual(
                await stage.runPreset('settle', { sessionId: session.id, durationMs: 0 }),
                true,
                'settle preset did not complete'
            );

            assert.strictEqual(stage.runPreset('breathe', { sessionId: session.id }), true, 'breathe did not start');
            await flushRaf(1);
            assert(stage.tweens.size > 0, 'breathe did not register a cancellable tween');
            assert(frames.length > 0, 'presets did not apply any frames');
            assert.strictEqual(stage.release(session.id, 'smoke-release'), true, 'release failed');
            assert.strictEqual(stage.tweens.size, 0, 'release did not clear preset tweens');

            const liveDriver = api.createLive2DDriver({
                managerResolver: () => manager,
                containerResolver: () => container
            });
            const liveStage = api.create({ driver: liveDriver });
            const liveSession = liveStage.acquire('transition-smoke', { priority: 1, force: true });
            assert(liveSession && liveSession.id, 'live stage did not acquire a session');
            assert.strictEqual(suspendCalls, 1, 'default full-capability session did not suspend motion');
            assert.strictEqual(container.style.transition, 'none', 'transition was not disabled during performance');
            assert.strictEqual(container.style.willChange, 'transform, opacity');
            assert.strictEqual(liveStage.release(liveSession.id, 'transition-release'), true);
            assert.strictEqual(resumeCalls, 1, 'default full-capability session did not resume motion');
            assert.strictEqual(container.style.transition, 'opacity 2s ease', 'transition was not restored');
            assert.strictEqual(container.style.transform, 'translateX(3px)', 'transform was not restored');
            assert.strictEqual(container.style.transformOrigin, 'left top', 'transform origin was not restored');
            assert.strictEqual(container.style.opacity, '0.75', 'opacity was not restored');
            assert.strictEqual(container.style.willChange, 'auto', 'willChange was not restored');

            const scopedSession = liveStage.acquire('look-at-only-smoke', {
                priority: 1,
                force: true,
                capabilities: ['params', 'lookAt']
            });
            assert(scopedSession && scopedSession.id, 'scoped session did not acquire');
            assert.strictEqual(JSON.stringify(scopedSession.capabilities), JSON.stringify(['params', 'lookAt']));
            assert.strictEqual(suspendCalls, 1, 'params/lookAt session should not suspend motion');
            assert.strictEqual(container.style.transition, 'opacity 2s ease', 'params/lookAt session should not disable transition');
            assert.strictEqual(container.style.willChange, 'auto', 'params/lookAt session should not change willChange');
            assert.strictEqual(liveStage.release(scopedSession.id, 'scoped-release'), true);
            assert.strictEqual(resumeCalls, 1, 'params/lookAt session should not resume unsuspended motion');
        })().catch((error) => {
            console.error(error && error.stack ? error.stack : error);
            process.exit(1);
        });
...
def test_avatar_performance_p2_live2d_snapshot_restore_contract_works_in_runtime():
    node = shutil.which("node")
    if not node:
        pytest.skip("node is required for avatar performance browser-script smoke")
    ...
    script = r"""
        ...
        const source = fs.readFileSync('static/avatar-performance-stage.js', 'utf8');
        const paramIds = ['ParamAngleX', 'ParamAngleY', 'ParamEyeBallX', 'ParamEyeBallY', 'ParamCustom'];
        const values = {
            ParamAngleX: 1,
            ParamAngleY: 2,
            ParamEyeBallX: 0.1,
        ...
        const coreModel = {
            ...
            getParameterValueByIndex: (index) => values[paramIds[index]],
            setParameterValueById: (id, value) => { values[id] = value; },
            setParameterValueByIndex: (index, value) => { values[paramIds[index]] = value; }
        };
        const container = {
            style: {
        ...
        const manager = {
            ...
            savedModelParameters: { ParamSaved: 0.25 },
            _shouldApplySavedParams: true,
            persistentExpressionParamsByName: {
                smile: [{ Id: 'ParamSmile', Value: 1 }]
            },
            _activeExpressionParamIds: ['ParamSmile'],
            playMotion: () => Promise.resolve(true),
            setTemporaryPoseOverride: (sourceName) => {
                manager.activePoseSource = sourceName;
            },
            clearTemporaryPoseOverride: (sourceName) => {
                clearedPoseSources.push(sourceName);
                if (manager.activePoseSource === sourceName) {
                    manager.activePoseSource = '';
                }
            }
        };
        const sandbox = {
        ...
        (async () => {
            vm.runInNewContext(source, sandbox);
            const api = sandbox.window.AvatarPerformanceStage;
            const driver = api.createLive2DDriver({
                managerResolver: () => manager,
                containerResolver: () => container
            });
            const stage = api.create({ driver });
            const session = stage.acquire('p2-smoke', {
                priority: 1,
                force: true,
                captureParamIds: ['ParamCustom']
            });
            assert(session && session.snapshot, 'stage did not capture a driver snapshot');
            assert.strictEqual(session.snapshot.params.ParamCustom, 42);
            assert.strictEqual(JSON.stringify(session.snapshot.expression.savedModelParameters), JSON.stringify({ ParamSaved: 0.25 }));
            assert.strictEqual(JSON.stringify(session.snapshot.expression.activeExpressionParamIds), JSON.stringify(['ParamSmile']));

            stage.lookAt({ x: 1000, y: 200 }, { sessionId: session.id });
            container.style.transform = 'scale(2)';
            container.style.transition = 'opacity 2s ease';
            container.style.transformOrigin = 'center center';
            container.style.opacity = '0.1';
            container.style.willChange = 'transform';
            values.ParamAngleX = 30;
            values.ParamAngleY = 20;
            values.ParamEyeBallX = 0.7;
            values.ParamEyeBallY = 0.6;
            values.ParamCustom = 99;
            manager.savedModelParameters = { ParamSaved: 0.9 };
            manager._shouldApplySavedParams = false;
            manager.persistentExpressionParamsByName = { angry: [{ Id: 'ParamAngry', Value: 1 }] };
            manager._activeExpressionParamIds = ['ParamAngry'];

            assert.strictEqual(stage.release(session.id, 'p2-release'), true);
            assert.strictEqual(container.style.transform, 'translateX(8px)');
            assert.strictEqual(container.style.transition, 'transform 1s ease');
            assert.strictEqual(container.style.transformOrigin, 'right bottom');
            assert.strictEqual(container.style.opacity, '0.8');
            assert.strictEqual(container.style.willChange, 'auto');
            assert.strictEqual(values.ParamAngleX, 1);
            assert.strictEqual(values.ParamAngleY, 2);
            assert.strictEqual(values.ParamEyeBallX, 0.1);
            assert.strictEqual(values.ParamEyeBallY, -0.1);
            assert.strictEqual(values.ParamCustom, 42);
            assert.strictEqual(JSON.stringify(manager.savedModelParameters), JSON.stringify({ ParamSaved: 0.25 }));
            assert.strictEqual(manager._shouldApplySavedParams, true);
            assert.strictEqual(JSON.stringify(manager.persistentExpressionParamsByName), JSON.stringify({
                smile: [{ Id: 'ParamSmile', Value: 1 }]
            }));
            assert.strictEqual(JSON.stringify(manager._activeExpressionParamIds), JSON.stringify(['ParamSmile']));
            assert(clearedPoseSources.length > 0, 'temporary pose override was not cleared');
            assert.strictEqual(stage.activeSession, null);
            assert.strictEqual(stage.release(session.id, 'p2-release-again'), false, 'second release should be a safe no-op');

            const noopDriver = sandbox.window.AvatarPerformance.createNoopDriver();
            const noopStage = api.create({
                driver: noopDriver,
                sequences: {
                    smoke: [
                        { type: 'frame', frame: { x: 5 }, durationMs: 0 },
                        { type: 'param', key: 'ParamMissing', value: 1 },
                        { type: 'lookAt', target: { x: 1, y: 1 }, durationMs: 0 },
                        { type: 'wait', durationMs: 0 }
                    ]
                }
            });
            const noopSession = noopStage.acquire('noop-smoke', { priority: 1, force: true });
            assert(noopSession && noopSession.id, 'noop stage did not acquire a session');
            assert.strictEqual(await noopStage.runSequence('smoke', { sessionId: noopSession.id }), true);
            assert.strictEqual(noopStage.release(noopSession.id, 'noop-release'), true);
            assert.strictEqual(noopStage.release(noopSession.id, 'noop-release-again'), false);
        })().catch((error) => {
            console.error(error && error.stack ? error.stack : error);
            process.exit(1);
        });
...
def test_avatar_performance_p3_coordinator_locking_works_in_runtime():
    node = shutil.which("node")
    if not node:
        pytest.skip("node is required for avatar performance browser-script smoke")
    ...
    script = r"""
        ...
        vm.runInNewContext(source, sandbox);
        const api = sandbox.window.AvatarPerformance;
        const coordinator = api.createCoordinator();
        const released = [];

        const low = coordinator.acquire({
            owner: 'idle',
            avatarId: 'main-live2d',
            priority: 10,
            capabilities: ['frame', 'motion', 'expression'],
            onRelease: (session, reason) => released.push([session.owner, reason])
        });
        assert(low && low.id, 'low priority session was not acquired');
        assert.strictEqual(coordinator.isCapabilityLocked('main-live2d', 'frame'), true);
        assert.strictEqual(coordinator.isCapabilityLocked('main-live2d', 'motion'), true);
        assert.strictEqual(coordinator.isCapabilityLocked('main-live2d', 'params'), false);
        assert.strictEqual(
            JSON.stringify(coordinator.getLockedCapabilities('main-live2d').sort()),
            JSON.stringify(['expression', 'frame', 'motion'])
        );

        let idleWrites = 0;
        function ordinaryIdleWrite() {
            if (coordinator.isCapabilityLocked('main-live2d', 'motion')) {
                return false;
            }
            idleWrites += 1;
            return true;
        }
        assert.strictEqual(ordinaryIdleWrite(), false, 'ordinary idle ignored the coordinator lock');
        assert.strictEqual(idleWrites, 0);
        ...
        assert.strictEqual(blocked, null, 'same priority session should not preempt without force');

        const high = coordinator.acquire({
            owner: 'guide',
            avatarId: 'main-live2d',
            priority: 80,
            capabilities: ['motion']
        });
        assert(high && high.id, 'high priority session did not preempt low priority motion lock');
        assert.strictEqual(released.length, 1);
        assert.strictEqual(JSON.stringify(released[0]), JSON.stringify(['idle', 'preempted']));
        assert.strictEqual(coordinator.isCapabilityLocked('main-live2d', 'motion'), true);
        assert.strictEqual(coordinator.isCapabilityLocked('main-live2d', 'frame'), false);
        assert.strictEqual(coordinator.getActiveSession({ avatarId: 'main-live2d', capability: 'motion' }).id, high.id);

        assert.strictEqual(coordinator.release(high.id, 'sequence-complete'), true);
        assert.strictEqual(coordinator.release(high.id, 'sequence-complete-again'), false);
        assert.strictEqual(coordinator.isCapabilityLocked('main-live2d', 'motion'), false);
        assert.strictEqual(ordinaryIdleWrite(), true, 'ordinary idle did not resume after release');
        assert.strictEqual(idleWrites, 1);

        const forcedA = coordinator.acquire({
            owner: 'a',
            avatarId: 'main-live2d',
            priority: 40,
            capabilities: ['params']
        });
        const forcedB = coordinator.acquire({
            owner: 'b',
            avatarId: 'main-live2d',
            priority: 40,
            force: true,
        ...
        assert(forcedA && forcedB && forcedA.id !== forcedB.id, 'force should preempt same priority');
        assert.strictEqual(coordinator.getActiveSession({ avatarId: 'main-live2d', capability: 'params' }).id, forcedB.id);
        coordinator.destroy('test-destroy');
        assert.strictEqual(coordinator.isCapabilityLocked('main-live2d', 'params'), false);

        const reentrant = api.createCoordinator();
        let replacement = null;
        const reentrantLow = reentrant.acquire({
            owner: 'reentrant-low',
            avatarId: 'reentrant-live2d',
            priority: 1,
            capabilities: ['motion'],
            onRelease: () => {
                replacement = reentrant.acquire({
                    owner: 'replacement',
                    avatarId: 'reentrant-live2d',
                    priority: 200,
                    capabilities: ['motion']
        ...
        assert(reentrantLow && reentrantLow.id, 'reentrant low session was not acquired');
        const reentrantBlocked = reentrant.acquire({
            owner: 'guide',
            avatarId: 'reentrant-live2d',
            priority: 80,
            capabilities: ['motion']
        });
        assert.strictEqual(reentrantBlocked, null, 'reentrant replacement lock should not be overwritten');
        assert(replacement && replacement.id, 'release callback did not acquire replacement lock');
        assert.strictEqual(
            reentrant.getActiveSession({ avatarId: 'reentrant-live2d', capability: 'motion' }).id,
            replacement.id
        );

        const defaultCoordinator = api.getDefaultCoordinator();
        const defaultSession = defaultCoordinator.acquire({
            owner: 'default',
            avatarId: 'default-live2d',
            priority: 1,
            capabilities: ['frame']
        });
        assert(defaultSession && defaultSession.id, 'default coordinator did not acquire a lock');
        assert.strictEqual(api.isCapabilityLocked('default-live2d', 'frame'), true);
        assert.strictEqual(JSON.stringify(api.getLockedCapabilities('default-live2d')), JSON.stringify(['frame']));
        assert.strictEqual(defaultCoordinator.release(defaultSession.id, 'default-release'), true);
        assert.strictEqual(api.isCapabilityLocked('default-live2d', 'frame'), false);
    """
...
def test_avatar_performance_p4_live2d_expression_and_resource_mapping_work_in_runtime():
    node = shutil.which("node")
    if not node:
        pytest.skip("node is required for avatar performance browser-script smoke")
    ...
    script = r"""
        ...
        const source = fs.readFileSync('static/avatar-performance-stage.js', 'utf8');
        const paramIds = ['ParamSmile', 'ParamThink'];
        const values = { ParamSmile: 0, ParamThink: 0 };
        const motionCalls = [];
        const emotionCalls = [];
        const coreModel = {
            getParameterIndex: (id) => paramIds.indexOf(id),
        ...
        const manager = {
            ...
            currentModel: {
                ...
                internalModel: {
                    ...
                    motionManager: {
                        definitions: {
                            Guide: [{ File: 'motions/guide.motion3.json' }]
                        },
                        stopAllMotions: () => {}
                    }
                },
                motion: async (group, index) => {
            ...
            emotionMapping: {
                expressions: {
                    happy: ['expressions/joy.exp3.json']
                },
                motions: {}
            },
            fileReferences: {
                Expressions: [
                    { Name: 'joy', File: 'expressions/joy.exp3.json' }
                ],
                Motions: {
                    Guide: [{ File: 'motions/guide.motion3.json' }]
                }
            ...
            resolveExpressionReferenceByFile: (file) => {
                const base = String(file || '').split('/').pop();
                if (base === 'joy.exp3.json') {
                    return { name: 'joy', file: 'expressions/joy.exp3.json' };
                }
                return null;
            },
            isExpressionFileMissing: () => false,
            playExpression: async (name, file) => {
                if (String(file || '').includes('joy.exp3.json') || name === 'happy') {
                    values.ParamSmile = 1;
                    manager._activeExpressionParamIds = ['ParamSmile'];
                    return true;
                }
                return false;
            },
            setEmotion: async (emotion) => {
                emotionCalls.push([emotion, manager._avatarPerformanceBypassLocks === true]);
                values.ParamSmile = 1;
                motionCalls.push([emotion, 'mapped']);
                return true;
            },
            resetTransientMotionAndExpressionState: async () => true,
            applyPersistentExpressionsNative: async () => true,
            suspendTemporaryMotions: () => true,
            resumeTemporaryMotions: () => true
        };
        const sandbox = {
            console,
            document: {
            ...
            fetch: async (url) => ({
                ok: String(url).includes('joy.exp3.json'),
                json: async () => ({
                    Parameters: [{ Id: 'ParamSmile', Value: 1 }]
                })
            }),
            window: {}
        };
        ...
        (async () => {
            ...
            const profile = {
                expressions: {
                    smile: [{ name: 'happy', file: 'expressions/joy.exp3.json' }],
                    thinking: [{ params: { ParamThink: 0.8 } }],
                    missing: ['not-real']
                },
                motions: {
                    guidePoint: [{ group: 'Guide', index: 0 }],
                    missingMotion: ['Ghost']
                },
                params: {
                    blush: ['ParamMissing', 'ParamThink']
                }
            };
            const stage = api.createLive2DStage({
                profile,
                driverOptions: {
                    managerResolver: () => manager,
                    containerResolver: () => container
                ...
                sequences: {
                    ...
                    missingOptional: [
                        { type: 'expression', name: 'missing' }
                    ],
                    missingRequired: [
                        { type: 'expression', name: 'missing', required: true }
                    ],
                    fallbackMotion: [
                        { type: 'motionWithFallback', motion: 'missingMotion', fallback: 'fallbackParam' }
                    ],
                    fallbackParam: [
                        { type: 'param', key: 'blush', value: 0.4 }
                    ],
                    emotionSmoke: [
                        { type: 'emotion', emotion: 'happy' }
                    ]
            ...
            const first = stage.acquire('p4-first', { priority: 5 });
            assert(first && first.id, 'first session did not acquire');
            assert.strictEqual(stage.acquire('p4-same-priority', { priority: 5 }), null, 'same priority should not preempt without force');
            assert.strictEqual(stage.release(first.id, 'same-priority-check'), true);

            const session = stage.acquire('p4-smoke', { priority: 10, force: true });
            assert(session && session.id, 'stage did not acquire a P4 session');
            assert.strictEqual(await stage.runSequence('expressionSmoke', { sessionId: session.id }), true);
            assert.strictEqual(values.ParamSmile, 0, 'clearExpression did not restore expression file params');
            assert.strictEqual(values.ParamThink, 0, 'clearExpression did not restore param fallback');
            assert.strictEqual(await stage.runSequence('missingOptional', { sessionId: session.id }), true);
            assert.strictEqual(await stage.runSequence('missingRequired', { sessionId: session.id }), false);
            assert.strictEqual(await stage.runSequence('fallbackMotion', { sessionId: session.id }), true);
            assert.strictEqual(values.ParamThink, 0.4, 'motion fallback sequence did not run');
            assert.strictEqual(await stage.runSequence('emotionSmoke', { sessionId: session.id }), true);
            assert.deepStrictEqual(emotionCalls[0], ['happy', true], 'emotion step did not use setEmotion with performance bypass');
            assert.strictEqual(stage.release(session.id, 'p4-release'), true);
            assert.strictEqual(values.ParamThink, 0, 'release did not restore param fallback');

            const expressionSession = stage.acquire('p4-release-expression', { priority: 10, force: true });
            assert(expressionSession && expressionSession.id, 'expression release session did not acquire');
            assert.strictEqual(await stage.runSequence([{ type: 'expression', name: 'smile' }], { sessionId: expressionSession.id }), true);
            assert.strictEqual(values.ParamSmile, 1, 'semantic expression did not apply');
            assert.strictEqual(stage.release(expressionSession.id, 'p4-expression-release'), true);
            assert.strictEqual(values.ParamSmile, 0, 'release did not restore expression params');

            motionCalls.length = 0;
            const motionSession = stage.acquire('p4-motion', { priority: 10, force: true });
            assert(motionSession && motionSession.id, 'motion session did not acquire');
            assert.strictEqual(await stage.runSequence([{ type: 'motion', name: 'guidePoint' }], { sessionId: motionSession.id }), true);
            assert.strictEqual(JSON.stringify(motionCalls[0]), JSON.stringify(['Guide', 0]));
            assert.strictEqual(stage.release(motionSession.id, 'p4-motion-release'), true);
        })().catch((error) => {
            console.error(error && error.stack ? error.stack : error);
            process.exit(1);
        });
...
def test_avatar_performance_p4_live2d_normal_paths_respect_locks_with_bypass():
    node = shutil.which("node")
    if not node:
        pytest.skip("node is required for avatar performance browser-script smoke")
    ...
    script = r"""
        ...
        const stageSource = fs.readFileSync('static/avatar-performance-stage.js', 'utf8');
        const modelSource = fs.readFileSync('static/live2d-model.js', 'utf8');
        const emotionSource = fs.readFileSync('static/live2d-emotion.js', 'utf8');
        const sandbox = {
            console,
            window: {},
            document: { getElementById: () => null },
        ...
        sandbox.window.clearTimeout = () => {};
        sandbox.window.LIPSYNC_PARAMS = [];
        sandbox.LIPSYNC_PARAMS = [];
        sandbox.LIVE2D_MOTION_PRIORITY = { IDLE: 1 };
        sandbox.fetch = async () => ({
            ok: true,
            json: async () => ({ Parameters: [{ Id: 'ParamSmile', Value: 1 }] })
        });
        sandbox.window.fetch = sandbox.fetch;
        ...
        vm.runInNewContext(stageSource, sandbox);
        vm.runInNewContext(modelSource, sandbox);
        vm.runInNewContext(emotionSource, sandbox);

        const api = sandbox.window.AvatarPerformance;
        const coordinator = api.getDefaultCoordinator();
        const manager = new sandbox.Live2DManager();
        let normalMotionCalls = 0;
        let normalExpressionCalls = 0;
        let idleMotionCalls = 0;
        manager.modelName = 'lock-test-model';
        ...
        manager.fileReferences = {
            Expressions: [{ Name: 'happy', File: 'happy.exp3.json' }],
            Motions: { happy: [{ File: 'happy.motion3.json' }] }
        };
        manager.emotionMapping = {
            expressions: { happy: ['happy.exp3.json'] },
            motions: { happy: ['happy.motion3.json'] }
        };
        manager.resolveAssetPath = (file) => file;
        manager.resolveExpressionReferenceByFile = () => ({ name: 'happy', file: 'happy.exp3.json' });
        manager.isExpressionFileMissing = () => false;
        manager.getRandomElement = (items) => items[0];
        manager.resetTransientMotionAndExpressionState = async () => true;
        manager.applyPersistentExpressionsNative = async () => true;
        manager._nextMotionTimerGeneration = () => 1;
        manager._isCurrentMotionTimerGeneration = () => true;
        manager._clearActiveMotionParamIds = () => {};
        manager._trackActiveMotionParametersFromData = () => {};
        manager.playSimpleMotion = () => {};
        ...
        (async () => {
            const lock = coordinator.acquire({
                ...
                avatarId: 'main-live2d',
                priority: 80,
                capabilities: ['motion', 'expression']
            });
            assert(lock && lock.id, 'default coordinator did not acquire performance lock');
            assert.strictEqual(await manager.playExpression('happy'), false, 'normal playExpression ignored expression lock');
            assert.strictEqual(await manager.playMotion('happy'), false, 'normal playMotion ignored motion lock');
            await manager._playIdleMotion(manager.currentModel.internalModel.motionManager);
            assert.strictEqual(normalExpressionCalls, 0);
            assert.strictEqual(normalMotionCalls, 0);
            assert.strictEqual(idleMotionCalls, 0, 'idle motion ignored coordinator lock');

            manager._avatarPerformanceBypassLocks = true;
            assert.strictEqual(await manager.playExpression('happy'), true, 'performance bypass did not allow expression');
            assert.strictEqual(await manager.playMotion('happy'), true, 'performance bypass did not allow motion');
            assert.strictEqual(normalExpressionCalls, 1);
            assert.strictEqual(normalMotionCalls, 1);
            manager._avatarPerformanceBypassLocks = false;
            assert.strictEqual(coordinator.release(lock.id, 'lock-test-release'), true);
        })().catch((error) => {
            console.error(error && error.stack ? error.stack : error);
            process.exit(1);
        });
...
def test_avatar_performance_p5_home_yui_adapter_uses_coordinator_and_step_lifecycle():
    adapter_source = Path("static/yui-guide-avatar-stage.js").read_text(encoding="utf-8")
    director_source = Path("static/yui-guide-director.js").read_text(encoding="utf-8")

    assert "DEFAULT_AVATAR_ID = 'main-live2d'" in adapter_source
    assert "DEFAULT_CHARACTER_ID = 'yui'" in adapter_source
    assert "window.AvatarPerformance.getDefaultCoordinator()" in adapter_source
    assert "avatarId: this.avatarId" in adapter_source
    assert "characterId: this.characterId" in adapter_source
    assert "capabilities: lockCapabilities" in adapter_source
    assert "resolveSequenceCapabilities(sequenceOrName)" in adapter_source
    assert "type === 'emotion' || type === 'setEmotion' || type === 'applyEmotion'" in adapter_source
    assert "this.ensureSession('wakeup', WAKEUP_CAPABILITIES)" in adapter_source
    assert "buildWakeupSequence(options, resultRef)" in adapter_source
    assert "type: 'poseTimeline'" in adapter_source
    assert "computePose: computeYuiWakeupPose" in adapter_source
    assert "ParamEyeLOpen" in adapter_source
    assert "Param95" in adapter_source
    assert "onRelease: (_session, releaseReason)" in adapter_source
    assert "enterStep(stepId, context)" in adapter_source
    assert "onSpeechStart(stepId, context)" in adapter_source
    assert "onSpeechEnd(stepId, context)" in adapter_source
    assert "onTimelineAction(stepId, action, context)" in adapter_source
    assert "buildTimelineActionSequence(action, context)" in adapter_source
    assert "normalizedAction === 'returnControl' || normalizedAction === 'handoffPluginDashboard'" in adapter_source
    assert "exitStep(stepId)" in adapter_source
    assert "stepEnter: Object.freeze([" in adapter_source
    assert "speechStart: Object.freeze([" in adapter_source
    assert "speechEnd: Object.freeze([])" in adapter_source
    assert "stepExit: Object.freeze([" in adapter_source
    assert "wakeupPolish" not in adapter_source
    assert "afterWakeup" not in adapter_source
    assert "polishWakeup()" not in adapter_source
    assert "resumeAfterWakeup()" not in adapter_source
    assert "Object.freeze({ type: 'lookAt', target: 'spotlight', durationMs: 120 })" in adapter_source
    assert "Object.freeze({ type: 'clearLookAt' })" in adapter_source
    assert "window.innerHeight * 0.42" not in adapter_source
    assert "wakeupSoftSettleStart" not in adapter_source
    assert "wakeupSoftSettleEnd" not in adapter_source
    assert "resolveContextEmotion" not in adapter_source
    assert "hasEmotion" not in adapter_source
    assert "buildEmotionSequence" not in adapter_source
    assert "const DEFAULT_CAPABILITIES = Object.freeze([\n        'params',\n        'lookAt'\n    ]);" in adapter_source
    assert "guideStepFocus" not in adapter_source
    assert "guideSpeechFocus" not in adapter_source
    assert "guideSettle" not in adapter_source
    assert "type: 'optionalMotion'" not in adapter_source
    assert "type: 'expression'" not in adapter_source
    assert "name: 'breathe'" not in adapter_source
    for guessed_name in ("neutral_yyy", "neutral_z1", "neutral_sbx", "yyy", "z1", "sbx"):
        assert guessed_name not in adapter_source

    assert "createAvatarStageContext(stepId, step, performance, meta)" in director_source
    assert "const emotion = typeof normalizedPerformance.emotion === 'string'" in director_source
    assert "this.callAvatarStage('enterStep', stepId" in director_source
    assert "this.callAvatarStage('applyEmotion', normalizedEmotion" in director_source
    assert "callAvatarTimelineAction(stepId, action, target, meta)" in director_source
    assert "this.callAvatarStage('onTimelineAction', stepId, action, context)" in director_source
    assert "this.callAvatarStage('runWakeup'" in director_source
    assert "this.wakeup.run()" not in director_source
    assert "window.YuiGuideWakeup" not in director_source
    assert "source: 'intro-prelude'" in director_source
    assert "this.callAvatarStage('onSpeechStart', stepId, avatarContext)" in director_source
    assert "await this.callAvatarStage('onSpeechEnd', stepId, avatarContext)" in director_source
    assert "this.callAvatarStage('exitStep', stepId)" in director_source

    speak_start = director_source.index("async speakGuideLine(text, options)")
    speak_end = director_source.index("resolvePerformanceBubbleText(performance)", speak_start)
    speak_block = director_source[speak_start:speak_end]
    assert "const avatarSpeechStartPromise = this.callAvatarStage('onSpeechStart', stepId, avatarContext)" in speak_block
    assert "finally" in speak_block
    assert speak_block.index("await avatarSpeechStartPromise") < speak_block.index(
        "await this.callAvatarStage('onSpeechEnd', stepId, avatarContext)"
    )
    assert "await this.callAvatarStage('onSpeechEnd', stepId, avatarContext)" in speak_block

    managed_start = director_source.index("async playManagedScene(stepId, meta)")
    managed_end = director_source.index("disableInterrupts()", managed_start)
    managed_block = director_source[managed_start:managed_end]
    assert "const avatarEnterPromise = this.callAvatarStage('enterStep', stepId" in managed_block
    assert "finally" in managed_block
    assert managed_block.index("await avatarEnterPromise") < managed_block.index(
        "await this.callAvatarStage('exitStep', stepId)"
    )
    assert "await this.callAvatarStage('exitStep', stepId)" in managed_block

    intro_start = director_source.index("async runChatIntroPrelude()")
    intro_end = director_source.index("async runChatIntroPreludeExternalized", intro_start)
    intro_block = director_source[intro_start:intro_end]
    assert intro_block.index("await this.callAvatarStage('exitStep', 'intro_basic');") < intro_block.index(
        "const introScenesCompleted = await this.playRemainingIntroPreludeScenes('intro_basic');"
    )

    externalized_start = director_source.index("async runChatIntroPreludeExternalized")
    externalized_end = director_source.index("async startPrelude()", externalized_start)
    externalized_block = director_source[externalized_start:externalized_end]
    assert externalized_block.index("await this.callAvatarStage('exitStep', 'intro_basic');") < externalized_block.index(
        "const introScenesCompleted = await this.playRemainingIntroPreludeScenes('intro_basic');"
    )

    takeover_start = director_source.index("async runTakeoverMainFlow()")
    takeover_end = director_source.index("async ensureChatVisible()", takeover_start)
    takeover_block = director_source[takeover_start:takeover_end]
    for scene_id in (
        "takeover_capture_cursor",
        "takeover_plugin_preview",
        "takeover_settings_peek",
        ...
        assert f"await this.playManagedScene('{scene_id}'" in takeover_block
    ...
    for action_name in (
        ...
        assert f"'{action_name}'" in director_source


def test_avatar_performance_p5_home_yui_adapter_runs_step_and_speech_protocol():
    node = shutil.which("node")
    if not node:
        pytest.skip("node is required for yui avatar stage browser-script smoke")
    ...
    script = r"""
        ...
        const source = fs.readFileSync('static/yui-guide-avatar-stage.js', 'utf8');
        const calls = [];
        ...
        const coordinator = {
            acquire: (request) => {
                calls.push(['coordinator.acquire', request.avatarId, request.characterId, request.capabilities.join(',')]);
                coordinatorSession = Object.assign({ id: 'coordinator-1' }, request);
                return coordinatorSession;
            },
            release: (sessionId, reason) => {
                calls.push(['coordinator.release', sessionId, reason]);
                const released = coordinatorSession;
                coordinatorSession = null;
                if (released && typeof released.onRelease === 'function') {
                    released.onRelease(released, reason);
                }
                return true;
            },
            getActiveSession: (query) => {
                return coordinatorSession && query && query.sessionId === coordinatorSession.id
                    ? coordinatorSession
                    : null;
            }
        };
        ...
        const fakeStage = {
            ...
            acquire: (owner, options) => {
                calls.push(['stage.acquire', owner, options.priority, options.capabilities.join(',')]);
                stageActive = true;
                return { id: 'stage-1' };
            },
            isSessionActive: (sessionId) => stageActive && sessionId === 'stage-1',
            resolveSequence: (sequenceOrName) => {
                if (Array.isArray(sequenceOrName)) return sequenceOrName;
                return [];
            },
            runSequence: async (sequenceOrName, options) => {
                const steps = fakeStage.resolveSequence(sequenceOrName);
                calls.push([
                    'stage.runSequence',
                    typeof sequenceOrName === 'string' ? sequenceOrName : 'inline',
                    steps.map((step) => step.type + ':' + (step.name || step.target || step.mode || step.emotion || '')).join('|'),
                    options.sessionId,
                    options.targets && options.targets.spotlight ? 'targeted' : 'untargeted'
                ]);
                return true;
            },
            release: (sessionId, reason) => {
                calls.push(['stage.release', sessionId, reason]);
                stageActive = false;
                return true;
            },
            destroy: (reason) => calls.push(['stage.destroy', reason])
        };

        const target = {
            getBoundingClientRect: () => ({ left: 100, top: 50, width: 80, height: 40 })
        };
        let guideOverlayRemoved = false;
        const guideOverlay = {
            parentNode: {
                removeChild: () => {
                    guideOverlayRemoved = true;
                }
            }
        };
        const sandbox = {
            ...
            window: {
                ...
                innerHeight: 720,
                AvatarPerformance: {
                    getDefaultCoordinator: () => coordinator
                },
                AvatarPerformanceStage: {
                    createLive2DDriver: () => ({}),
                    createLive2DStage: () => fakeStage,
                    create: () => fakeStage
                },
                setTimeout: (callback) => {
                    callback();
            ...
            document: {
                ...
                querySelector: (selector) => selector === '#storage-location-overlay' ? null : null,
                querySelectorAll: (selector) => selector === '#yui-guide-overlay' ? [guideOverlay] : [],
                body: {
                    classList: {
                        remove: () => {
                            guideOverlayRemoved = true;
        ...
        vm.runInNewContext(source, sandbox);
        const controller = sandbox.window.YuiGuideAvatarStage.create({ logger: console });
        (async () => {
            assert.strictEqual(controller.isAvailable(), true);
            await controller.enterStep('no_target_step', {});
            assert.strictEqual(calls.length, 0, 'adapter should not invent a lookAt target when the guide has no target');

            await controller.enterStep('intro_basic', {
                targets: { spotlight: target }
            });
            await controller.onSpeechStart('intro_basic', {
                speechId: 'speech-1',
                targets: { spotlight: target }
            });
            await controller.onSpeechEnd('intro_basic', {
                speechId: 'old-speech'
            });
            await controller.onSpeechEnd('intro_basic', {
                speechId: 'speech-1'
            });
            await controller.exitStep('intro_basic');

            const stepCalls = calls.slice();
            assert.deepStrictEqual(stepCalls[0], [
                'coordinator.acquire',
                'main-live2d',
                'yui',
                'params,lookAt'
            ]);
            assert.strictEqual(stepCalls.filter((call) => call[0] === 'stage.acquire').length, 1);
            assert(stepCalls.some((call) => call[0] === 'stage.runSequence' && call[2].includes('lookAt:spotlight')));
            assert(stepCalls.some((call) => call[0] === 'stage.runSequence' && call[2].includes('clearLookAt:')));
            assert(stepCalls.every((call) => {
                return call[0] !== 'stage.runSequence'
                    || (
                        !call[2].includes('expression:')
                        && !call[2].includes('optionalMotion:')
                        && !call[2].includes('motion:')
                        && !call[2].includes('emotion:')
                        && !call[2].includes('preset:breathe')
                        && !call[2].includes('frame:guide')
                    );
            }), 'step lifecycle should not invent yui expressions, motions, frame movement, or breathe');
            assert(stepCalls.some((call) => call[0] === 'stage.release' && call[2] === 'step-exit'));
            assert(stepCalls.some((call) => call[0] === 'coordinator.release' && call[2] === 'step-exit'));
            calls.length = 0;

            await controller.onTimelineAction('takeover_capture_cursor', 'enableAgentMaster', {
                targets: { actionTarget: target }
            });
            ...
            assert(calls.some((call) => {
                return call[0] === 'stage.runSequence'
                    && call[2].includes('lookAt:spotlight')
                    && call[4] === 'targeted';
            }), 'timeline action should translate existing guide targets into a lookAt sequence');
            calls.length = 0;

            await controller.onTimelineAction('takeover_plugin_preview', 'handoffPluginDashboard', {});
            assert(calls.some((call) => {
                return call[0] === 'stage.runSequence'
                    && call[2].includes('clearLookAt:');
            }), 'handoff timeline action should clear guide lookAt without inventing a motion');
            controller.release('timeline-test-done');
            calls.length = 0;

            await controller.applyEmotion('emotion-from-existing-step', {
                targets: { spotlight: target }
            });
            assert.deepStrictEqual(calls[0], [
                'coordinator.acquire',
            ...
            assert(calls.some((call) => {
                return call[0] === 'stage.runSequence'
                    && call[2].includes('emotion:emotion-from-existing-step');
            }), 'explicit guide emotion should be delegated without adapter-side classification');
            assert(calls.every((call) => {
                return call[0] !== 'stage.runSequence' || !call[2].includes('lookAt:');
            }), 'applyEmotion should not append guide lookAt choreography');
            controller.release('emotion-test-done');
            calls.length = 0;

            await controller.runWakeup({
                durationMs: 0
            });
            ...
            assert.strictEqual(
                guideOverlayRemoved,
                false,
                'wakeup adapter must not remove or mutate the homepage guide overlay/cursor state'
            );
            assert.deepStrictEqual(calls[0], [
                'coordinator.acquire',
                'main-live2d',
                'yui',
                'params,motion,lookAt,expression'
            ]);
            assert(calls.some((call) => {
                return call[0] === 'stage.runSequence'
                    && call[1] === 'inline'
                    && call[2].includes('poseTimeline:yuiWakeup');
            }), 'wakeup should be driven through the AvatarPerformance pose timeline');
            assert(calls.some((call) => call[0] === 'stage.release' && call[2] === 'wakeup-complete'));
            assert(calls.some((call) => call[0] === 'coordinator.release' && call[2] === 'wakeup-complete'));
        })().catch((error) => {
            console.error(error && error.stack ? error.stack : error);
            process.exit(1);
        });
...
def test_yui_wakeup_takeover_lives_in_avatar_performance_adapter_and_keeps_m2_boundaries():
    adapter_source = Path("static/yui-guide-avatar-stage.js").read_text(encoding="utf-8")
    ...
    assert not Path("static/yui-guide-wakeup.js").exists()
    assert "DEFAULT_WAKEUP_DURATION_MS = 4000" in adapter_source
    assert "LIVE2D_HANDOFF_MS = 620" in adapter_source
    assert "LIVE2D_HANDOFF_MS" in adapter_source
    assert "removeBlockingGuideOverlay" not in adapter_source
    ...
    assert "computePose: computeYuiWakeupPose" in adapter_source
    assert "suspendEyeBlink: true" in adapter_source
    assert "restoreOnComplete: true" in adapter_source
    assert "onInitialPose: () =>" in adapter_source
    assert "onResult: (result) =>" in adapter_source
    assert "runPoseTimeline(step, options)" in performance_source
    assert "_suspendEyeBlinkOverride" in performance_source
    assert "manager.setTemporaryPoseOverride(source" in performance_source
    assert "manager.clearTemporaryPoseOverride(source)" in performance_source
    ...
    for param_id in (
        ...
        assert param_id in adapter_source
    ...
    assert "class Live2DWakeupSession" not in adapter_source
    assert "coreModel.update =" not in adapter_source
    assert "motionManager.update =" not in adapter_source
    assert "model.focus(" not in adapter_source
    assert "document.createElement" not in adapter_source
    assert "appendChild" not in adapter_source
    assert "yui-guide-wakeup-stage" not in adapter_source
    assert "yui-guide-wakeup-backdrop" not in adapter_source
    assert "yui-guide-wakeup-particles" not in adapter_source
