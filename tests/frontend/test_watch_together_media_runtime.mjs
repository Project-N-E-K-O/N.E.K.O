...
const liveLine={audio:'/api/watch-together/live-audio/aaaaaaaaaaaaaaaaaaaaaaaa',text:'Thanks for the gift'};
const speechVideo=new Media(),speechCues=[];
globalThis.fetch=async()=>new Response('speech');
const speaking=await mount({video:speechVideo,timeline:{status:'ready',id:'job',version:'version',video:'/video',events:[cue]},onCue:value=>speechCues.push(value)});
await assert.rejects(speaking.say(liveLine),{name:'AudioOwnershipError'},'live speech needs the scene audio lock');
await speaking.play();
const soundtrackGain=gains.at(-2),speechAudio=audios.at(-1);
await assert.rejects(speaking.say({...liveLine,audio:'/api/watch-together/media/job/version/comment.mp3'}),/Unregistered live speech/);
speechVideo.currentTime=1;nextFrame();
const finished=speaking.say(liveLine);
await new Promise(resolve=>setTimeout(resolve,0));
assert.equal(speechAudio.paused,false,'live line plays through the reaction element');
assert.equal(speechCues.at(-1)?.text,'Thanks for the gift');
assert.equal(soundtrackGain.gain.value,.25,'live line ducks the soundtrack');
speechVideo.currentTime=1.1;nextFrame();
assert.equal(speechAudio.paused,false,'frames between reactions keep the live line playing');
assert.equal(await speaking.say(liveLine),false,'a second line waits for the first');
speechAudio.dispatchEvent(new Event('ended'));
assert.equal(await finished,true);
assert.equal(speechCues.at(-1),null);
assert.equal(soundtrackGain.gain.value,1);
const preempted=speaking.say(liveLine);
await new Promise(resolve=>setTimeout(resolve,0));
speechVideo.currentTime=3.95;nextFrame();speechVideo.currentTime=4.05;nextFrame();
assert.equal(await preempted,false,'a due reaction cuts the live line');
assert.equal(speechAudio.paused,false,'the reaction itself plays');
speechVideo.dispatchEvent(new Event('seeking'));speechVideo.currentTime=20;speechVideo.dispatchEvent(new Event('seeked'));
assert.equal(speechAudio.paused,true,'seeking away stops the reaction');
const seekedLine=speaking.say(liveLine);
speechVideo.dispatchEvent(new Event('seeking'));speechVideo.currentTime=30;speechVideo.dispatchEvent(new Event('seeked'));
assert.equal(await seekedLine,false,'a seek during fetch discards the live line');
assert.equal(speechAudio.paused,true);
const disposedLine=speaking.say(liveLine);
await new Promise(resolve=>setTimeout(resolve,0));
speaking.dispose();
assert.equal(await disposedLine,false,'disposal settles a playing live line');
