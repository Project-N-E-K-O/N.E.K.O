...
await speaking.play();
const pausedDuringFetch=speaking.say(liveLine);
speechVideo.pause();
assert.equal(await pausedDuringFetch,'skipped','a pause during fetch means the live line never started');
assert.equal(speechAudio.paused,true,'a paused video never starts the live line');
