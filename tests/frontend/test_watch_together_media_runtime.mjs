...
globalThis.fetch=async()=>new Response('speech');
const textVideo=new Media();
const textHost=await mount({video:textVideo,timeline:{status:'ready',id:'job',version:'version',video:'/video',events:[{at:0,audio:null,text:'silent reaction'}]}});
...
await new Promise(resolve=>setTimeout(resolve,0));
textHost.dispose();
assert.equal(await pausedOverText,'interrupted','a paused text reaction lets a live line start');
