...
25. **"跨边界的 shape / role / 字段名必须 rg 实际消费方不按蓝图草稿拼"** ⚠⚠⚠ (2026-04-23 P25 Day 2 + Day 2 polish 第二轮 + Day 2 polish 第三轮, 72 小时内**四次**同族). **场景**: 跨"生产方 vs 消费方"边界的数据形状 / 字段名 / 角色 / 结构契约, 默认按蓝图草稿或"第一眼语义直觉"拼, 而**不实际 rg 消费方代码**. 消费方既可以是前端 JS 面板 (消费后端 API response), 也可以是下游 LLM / 序列化消费者 (消费 wire 消息). **四次同族案例**: (1) 字段名漂移 — 前端按 BLUEPRINT §2.7 抄 `dedupe_info.remaining_ms`, 后端实际 shape 是 `{hit, cache_size, dedupe_key, dedupe_rank}`; (2) envelope vs flat 漂移 — router 返 `{"kind":..., "result":{...}}` 而 UI 期望扁平 `{"kind":..., ...result}`, UI 读 `undefined` 字段全报"假失败"; (3) LLM wire role 漂移 (代码写死) / prompt_ephemeral 语义契约违反 — `external_events.py` 三个 simulator 里 `base_wire.append({"role":"system", "content":instruction})` **违反**主程序 `prompt_ephemeral` 契约 (主程序是 `HumanMessage(content=instruction)` 即 `role=user`), 空 session 触发 → Gemini 400 INVALID_ARGUMENT "Model input cannot be empty"; 非空 session → Gemini 偶尔返空字符串 + 200 OK, 空 reply 被持久化, 下一轮 LLM 读"上一轮 user + 空 assistant + 新 instruction" 的残缺 wire, 基于**上一轮**事件生成 reply — tester 观察到"再次触发才拿到上次的 reply"; (4) **LLM wire role 漂移 (UI 允许 tester 手选) / 主程序 SystemMessage 语义契约违反** — testbench composer 有 Role=User/System 下拉 + [发送] 按钮, tester 选 role=system 发送后 chat_runner.stream_send 以 role=system 把消息 append 进 session.messages, 然后 build_prompt_bundle **原样透传**到 wire, 于是 wire 尾出现 role=system 消息. 主程序 `omni_offline_client.py` 的契约是 `SystemMessage` **只存在于** `_conversation_history[0]` (初始化阶段), 运行期所有输入路径 (send_text_message / create_response / prompt_ephemeral) 统一以 `HumanMessage` (role=user) 注入 — 运行期 wire 里出现 role=system 消息是主程序从不存在的 shape. 同样撞上 Gemini shape 过敏: 空 session 400 / 非空 session 空 reply + 200 → stale reply 时序错位 (和第 3 次完全同型). Smoke 漏的原因: 没给**主 chat /send 路径**的 wire 组装环节做契约断言 (第 3 次补的是 external_events 的 smoke 断言, 管不到这条). **失败模式 (三级严重度)**: (a) 浅 — 字段 undefined → UI 显示 `N/A`, 易定位; (b) 中 — 整块功能"假失败" (后端做了前端以为没做), 诱导重跑 / 回滚 / 改配置白费精力; (c) 深 — 消费方偶尔成功偶尔失败 (Gemini 空 reply 是概率行为), fake-LLM smoke 覆盖不到, 上线后手测才暴, **更坏的是级联**: 空 reply 被持久化, 下一轮 wire 残缺, 错位一次 reply — "回答变味但不报错" 是**静默语义毒化**, 事后追根只能靠完整 wire log + 逐字节对比. **归纳为五层防御规则** (第 5 层由第 4 次同族加入):
    ...
    5. **chokepoint 防守优于单点堵入口** (第 4 次同族加入的关键教训): 第 3 次只修了 `external_events.py` 那一处代码的 `role="system"` → `role="user"`, 属于"堵入口". 第 4 次发现 composer UI 让 tester 手选 role=system 是**完全另一条入口**, 上次的修复管不到. 彻底方案是把守门点下沉到**所有路径必经的 wire 组装 chokepoint** — 本次在 `prompt_builder.build_prompt_bundle` 里统一把 `session.messages` 里的 role=system 消息重写成 role=user + `[system note] ` 前缀. 这样**未来**任何新路径 (手动 POST /messages / 脚本回放 / 导入历史 / 未知入口) 写的 role=system 消息都会在 wire 层被守住, 不需要再逐个入口堵. 对应 skill: `single-writer-choke-point` (已存在) + `audit-chokepoint-invariant` (已存在). **识别信号**: 同一失败模式在两个独立入口重现 = 应该立即考虑 chokepoint 下沉, 不是再修第二个入口. 对 chokepoint 补 unit-level smoke (本次 `p25_wire_role_chokepoint_smoke.py` 五 case: C1 空 / C2 单 system / C3 混 / C4 多模态 / C5 post-chokepoint invariant "wire[1:] 永无 role=system"), 彻底闭环.

    **元归纳**: §7.6 "多源写入是纸面原则成败分水岭" 讲的是"同一进程多入口", §7.25 是它在**跨边界契约**维度的扩展 — "生产方 N 种实装 vs 消费方 M 种解析" 的笛卡尔积里, 任何一对错配都静默毒化, 不 rg 消费方代码就等着被笛卡尔积里的某个格子咬. 第 4 次同族的新元归纳: **"按错误出现的边界堵入口"和"按正确消费契约守 chokepoint"是两种不同粒度**, 前者适合第 1 次发现, 后者是**第 ≥ 2 次同族**时的应然选择; 当你发现自己在修"第二个相同类型的入口"就应该停下来想 chokepoint. 同族延伸: 任何 SDK / adapter / plugin 对接主系统时都适用 — 不是只为 testbench, 也为 OSS fork / 第三方 client / 分布式系统 message contract / protobuf schema migration. 对应 Cursor skill: `ui-wire-field-rg-backend-first` (升级版, 覆盖 response shape / envelope 决策 / wire role 决策 / chokepoint 下沉四类场景).
