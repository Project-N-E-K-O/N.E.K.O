...
> 元教训 (L28-L39, 登记于 §7.A 候选区, 未计入主编号 25 条).
>
> **§7.25 特别说明**: 一周内已连续 5 次同族实锤 (字段名漂移 / envelope 漂移 /
> LLM wire role 三次漂移 / **Prompt Preview "重建视图 ≠ 真实 stream" 架构级
> 分叉**), 从最初的"四层防御"升级为当前的"**五层防御**" (第 5 层含 5a
> chokepoint 下沉 + 5b preview 消费 ground-truth snapshot 两子条).
>
