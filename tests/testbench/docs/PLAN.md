...
todos:
  ...
  - id: p_memory_code_inference
    content: "pXX 记忆分析→主程序记忆代码反推 (未来受控 phase, 未排期占位): 由 P30 可行性裁决 (MEMORY_CODE_INFERENCE_FEASIBILITY.md) 引出. 只有在**主程序侧愿意补结构化不变量事件钩子** (写入 chokepoint 落 谁/何时/写了什么/引用哪些 id) 的前提下才推进; 首版只做机械不变量类 (悬空引用/断裂晋升/多向量空间/游标非单调等) 的**导航级线索** (指向可疑写入操作/模块 + 弱证据分级 + 人工确认闸门), **明文排除**内容质量类 (矛盾/冗余/归因/晋升漂移). 延续 testbench 只读边界, 绝不自动改主程序. 是否推进由记忆系统相关人员按投入产出决定."
    status: pending
isProject: false
---
