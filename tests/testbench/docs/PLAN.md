...
**未完成阶段 (P26 Commit 3, 1 条)**: **P26 文档阶段 Commit 3 = `testbench_USER_MANUAL.md` 中文简体测试员手册** (估 ~0.5-1 天). P25 已全量 sign-off (Day 1-3 + 6 轮 polish + r7 2nd pass), P26 Commit 1 (版本号 + CHANGELOG + `/docs/` 端点 + About 接线) + Commit 2 (ARCHITECTURE_OVERVIEW + LESSONS 升级 + 2 skill + smoke) 已交付, 剩 USER_MANUAL + 8 张配图占位. 推荐执行顺序 **P25 → P26** (价值与依赖分析见 `PROGRESS.md` 中后期回顾与展望 §三). P00-P24 完整实际路径 **P19 → P18 → P20 → P21 → P21.1 → P21.2 → P21.3 → P22 → P22.1 → P23 → P24 (12 天, Day 10-12 v1.0 sign-off)** 已全量交付, 其中 P21.1/.2/.3/P22.1 是跨阶段"加固 pass"而非主线编号. **P24 = 第一个完善版本分水岭**: 之后所有开发均视为"版本更新", 基线能力 (会话/沙盒/时钟/Chat 四模式/Memory ops/Stage Coach/Evaluation 四 Judger + Run/Results/Aggregate/Export/快照/Diagnostics 六子页/持久化/自动保存/导出 11 组合) 已冻结.
...
**下一个 Agent 的第一件事 (2026-04-24 更新, P26 Commit 2 完结, Commit 3 开工前)**:
1. **先读 P26 Commit 1/2 交付档**: `AGENT_NOTES.md` 顶部两条 P26 banner (Commit 2 = ARCH + LESSONS L45-L49 + §7.26/§7.27 升级 + 2 skill + p26_docs_endpoint_smoke; Commit 1 = 版本号常量化 + CHANGELOG + `/docs/{doc_name}` 端点 + About 接线) + `§4.27 #118` (Commit 2 完整 spec) + `PROGRESS.md` 对应两条 P26 日志 + `testbench_ARCHITECTURE_OVERVIEW.md` + `LESSONS_LEARNED.md §7.26/§7.27` 两条新主编号原文.
2. **P26 Commit 3 开工重点** (估 ~0.5-1 天):
   - **主交付 `tests/testbench/docs/testbench_USER_MANUAL.md`** 中文简体, 面向**测试员/中级用户**非开发者. 章节建议 8 节: 准备事项 (启动命令 + 端口 + 数据目录) / Workspace 导航 (六 workspace + 顶栏折叠控件) / Chat 对话区 (四模式: 手动 / SimUser / Scripted / Dual-AI Auto + 外部事件模拟) / Memory 记忆编辑 (Recent + Facts + Reflection + Persona 四子页 + 5 ops + preview prompt) / Evaluation 评分 (Schemas + Run + Results + Aggregate + Export) / Session 管理 (保存 + 加载 + 自动保存 + 快照 + Rewind + 导出 11 组合) / Diagnostics 诊断 (Errors + Logs + Paths + Snapshots + Reset + Security 六子页) / FAQ + 限制声明 + 扩展点.
   - **配图占位**: 8 张截图位 (顶栏 / Chat 四模式切换 / Memory 四子页 / Eval Run + Results / Session Save 对话框 / Diagnostics Errors + Logs / 外部事件面板 / About 页), 每张加 `<!-- IMG: description -->` placeholder + 拍摄指引 (分辨率 / 高亮区域 / 数据脱敏). 图实际由用户拍.
   - **白名单登记**: `_PUBLIC_DOCS` 已登记 `testbench_USER_MANUAL`, Commit 3 md 落地后 `/docs/testbench_USER_MANUAL` 端点从 404 file_missing 转为 200. p26_docs_endpoint_smoke D9 会自动绿 (至少 1 条白名单 md 在盘).
   - **Commit 3 结束**: 三 commit 全齐 + 全量 smoke (18/18 + Commit 3 可能再加 1-2 测 USER_MANUAL 结构的 smoke) 全绿后一次 push.
3. **读经验层基线**: `AGENT_NOTES.md §3A` 57 条设计原则 → `LESSONS_LEARNED §7` **27 条主编号** (§7.26 Subagent 三段式 / §7.27 Preview 面板按消费域分区 均 P26 Commit 2 升级) + **`§7.A` 候选 L28-L49** (22 条, P26 Commit 2 新增 L45-L49 5 条); §7.25 "跨边界 shape / role / 字段名必须 rg 消费方" 已升到第 6 次同族 + 五层防御 + §7.27 从 L44 升为两次同族实锤.
4. **Commit 2 基线**: 全量 smoke baseline **18/18 全绿** ~35s, 含新 `p26_docs_endpoint_smoke.py` (10 静态契约). P26 Commit 3 新改动要保持这个绿线.
5. **未合入的本地残留**: git status 开场显示 `tests/testbench/static/testbench.css` + `static/ui/setup/memory_trigger_panel.js` 有未 commit 的 working-copy 改动, 属于 #117 r7 2nd pass 的用户本地调整, **Commit 2 未合并也未覆盖**, 下个 agent 进来先 git diff 这两份文件确认后再决定是否随 Commit 3 一起打包.
6. **git 状态**: P25 Day 1-3 若干 commit 本地已存但**未 push**. P26 Commit 1 本地 commit 未 push. P26 Commit 2 本地 commit 未 push. 三 commit 全齐 + 用户手测通过后一次 push NEKO-dev/main.
