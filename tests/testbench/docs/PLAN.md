...
4. **Day 2 开工时按 §3A E3 原则**: 先跑 **Full-Repo Pre-Sweep** (`rg "external-event|simulate_avatar|simulate_agent_callback|simulate_proactive" tests/testbench/static/ui/` 确认前端现零消费) + **全量 smoke baseline** (当前 **11/11 全绿**: p21_* 3 / p22_* 2 / p23_* 1 / p24_* 3 / p25_* 2), 确认基线干净再动工;
...
6. **git 待办**: 本批 P25 §A 八轮审查修改 + P25 Day 1 骨架 (`P25_BLUEPRINT.md` + `LESSONS_LEARNED.md` + `AGENT_NOTES.md` + `PROGRESS.md` + `PLAN.md` + `pipeline/avatar_dedupe.py` + `pipeline/diagnostics_ops.py` + `pipeline/external_events.py` + `routers/external_event_router.py` + `server.py` + `session_store.py` + `smoke/p25_*`) 按用户指示**未 commit**, 可在 Day 2 交付时合并 commit (`feat(testbench): P25 Day 1-2 external event injection (backend + frontend)`) 或 Day 1 单独 commit.
