...
## 当前快照 (2026-04-20, P17 完成 → 2026-04-21 P22/P22.1 补交付 → 2026-04-21 新增 P24 联调/代码审查/延期加固阶段 → 2026-04-21 P23 交付 → 2026-04-22 Day 12 P24 收尾 **v1.0 "第一个完善版本" sign-off** → 2026-04-23 P25 开工前 §A 八轮设计审查完成 → 2026-04-23 **P25 Day 1 后端骨架交付 + Subagent 并行开发模式首次应用**)

> 本节为后期追加, 帮助新 Agent 或调研者在不通读全文的情况下快速定位现状. 核心 `todos` 的状态仍以**条目内 `status` 字段**为准; 本节仅作总览.

**进度 (2026-04-23 更新)**: 已完成 24 / 26 阶段 (约 **92%**) + **P25 Day 1 已交付 (后端骨架 + Subagent 并行开发模式首次应用)**. **P25 Day 1** (2026-04-23) 主 agent + 4 subagent 并行落地: Subagent A 抄 `cross_server._should_persist_avatar_interaction_memory` + `_AvatarDedupeCache` → `pipeline/avatar_dedupe.py` / Subagent B 扩 `pipeline/diagnostics_ops.py` +4 ops / 主 agent 写 `pipeline/external_events.py` (三 handler + shared LLM + `SimulationResult` dataclass + dedupe 注册表) + `routers/external_event_router.py` (3 端点, BUSY 短锁, 禁 AbortController) / Subagent C 写 `smoke/p25_external_events_smoke.py` (TestClient 7 组 40+ 断言) **并自诊出主 agent 写的 bug** (meta 键名 `dedupe_key` → 应为 `memory_dedupe_key`, 主 agent 5 分钟修 + B2 从 record-and-continue 升级为 strict assert) / Subagent D 写 `smoke/p25_avatar_dedupe_drift_smoke.py` (6 规则 R1-R6 byte-hash sha256). **全量回归 11/11 smoke 全绿** (9 历史 + P25 新 2 个). **Day 1 核心收获**: 产出 L33 候选 "Subagent 并行开发 + 主 agent 三段式 review" 范式 (主 agent 拆粒度任务书 → subagent 并行交付 + Observation 字段列自诊疑点 → 主 agent 先读 Observation 再 code review + 集成 smoke). 历史: P24 联调联测 + 代码审查 + 延期加固 12 天交付 + Day 12 欠账清返 + v1.0 "第一个完善版本" sign-off. 2026-04-23 追做 P25 开工前 §A 八轮设计层回顾 (六轮元审核 + 第七轮 self-audit + 第八轮漂移诊断), 净产出 8 条有效矫正 + 5 条候选元教训 (L28-L32). **剩余路径**: **P25 Day 2 前端 Chat 外部事件模拟面板 → P25 Day 3 联调 + smoke + 文档 → P26 用户 README**.
...
**下一个 Agent 的第一件事 (2026-04-23 更新, P25 Day 2 启动前)**:
1. **先读 Day 1 交付总表**: `AGENT_NOTES.md §4.27 #109` (P25 Day 1 后端骨架交付完整档案, 含 5 新 + 2 改文件 + Subagent 并行开发模式 + 主 agent 写的 bug 被 subagent 自诊出 + L33 候选) + `PROGRESS.md 变更日志 2026-04-23` 条目;
2. **按 §A.8 回写正文**: `§1.2` 触发源精确化 / `§1.3` OOS 补 `merge_unsynced_tail_assistants` 一行 / `§2.4` 三类分别说明 session_only vs mirror / `§3` Day 1 必改 6 处 (Day 1 交付时实际按 §A.8 落地, 复核蓝图正文是否仍和 §A.8 匹配) / `§4` 新决策行 / `§5` 验收点微调 (~1h, 无新决策);
3. **读经验层基线**: `AGENT_NOTES.md §3A` 57 条设计原则 (含 P24 新增 10 条 A12-A18/B14/B15/E3) → `LESSONS_LEARNED §7` 24 条主编号 + **`§7.A` 候选 L28-L33** (L33 为 P25 Day 1 派生的 "Subagent 并行开发 + 三段式 review" 范式, Day 2 前端开发可继续按此范式拆分);
4. **Day 2 开工时按 §3A E3 原则**: 先跑 **Full-Repo Pre-Sweep** (`rg "external-event|simulate_avatar|simulate_agent_callback|simulate_proactive" tests/testbench/static/ui/` 确认前端现零消费) + **全量 smoke baseline** 用**新统一 runner** `tests\testbench\smoke\_run_all.cmd` (2026-04-23 新增, 详见 `AGENT_NOTES §2A`, 当前 **11/11 全绿**: p21_* 3 / p22_* 2 / p23_* 1 / p24_* 3 / p25_* 2, 总耗时 ≈ 19s), 确认基线干净再动工. **不要再用** `Invoke-RestMethod` / `python -c` / 逐个手敲 `.venv\Scripts\python.exe tests\testbench\smoke\pxx.py` 之类老招式 (都已被新 runner 替代);
5. **硬刷浏览器 + 重启服务器**: Day 1 后端新增了 `external_event_router` 3 端点 + `session_store` 延迟 import 调 `discard_session_caches()`, 前端暂无消费入口, 必须重启 + 硬刷才能进 Day 2 前端开发;
6. **git 状态**: P25 Day 1 主交付已 commit `3d6c525` (2026-04-23, 未 push) + commit hash 回填 `395b2ac`; Day 2 交付可单独 commit (`feat(testbench): P25 Day 2 external event injection frontend panel`) 或与 Day 3 verification 合并.
