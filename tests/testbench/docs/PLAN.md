...
**下一个 Agent 的第一件事 (2026-04-23 更新, P25 Day 3 启动前)**:
1. **先读 Day 1 + fixup + Day 2 三条交付档**: `AGENT_NOTES.md §4.27 #109` (Day 1 后端骨架) + **`#110` (Day 1 fixup: mirror_to_recent shape mismatch 修复 + L34 候选)** + **`#111` (Day 2 前端面板 + Logs op 说明, L35/L36 候选)** + `PROGRESS.md` 对应 3 条变更日志;
2. **Day 3 开工重点** (P25_BLUEPRINT §3 Day 3, 估 ~0.7 天):
   - **额外 smoke 断言**: reason 复现表 OOS 守护 (任何 reason 字段含 "cooldown"/"voice"/"websocket"/"not_text_session" 应 FAIL) + persona.language=es/pt 回退断言 (agent_callback→英文 via `_loc._SILENT_FALLBACK`, proactive→英文 via `_normalize_prompt_language`);
   - **tester 手册**: `tests/testbench/docs/external_events_guide.md` (~220 行) 三类事件使用场景 + payload 字段 + dedupe 策略 + 三层影响 + OOS 边界 + SimulationResult.reason 表;
   - **用户手测全链路影响评估**: 创 session → avatar 10 次 fist burst head + agent callback "已完成播放列表整理" + proactive trigger_idle → 手动 trigger recent.compress/facts.extract/reflect 观察三层影响, 记 `tests/testbench/docs/p25_events_report.md`;
