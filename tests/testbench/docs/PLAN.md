...
**下一个 Agent 的第一件事 (2026-04-23 更新, P25 Day 3 完结, P26 README 收尾前)**:
1. **先读 Day 1-3 + 6 轮 polish 交付档**: `AGENT_NOTES.md §4.27 #109-#118` (Day 1 后端骨架 / Day 1 fixup mirror shape / Day 2 前端面板 / polish r1-r6 / **#118 Day 3 last_llm_wire 补 stamp + AST 覆盖率 smoke + tester 手册**) + `PROGRESS.md` 对应 10+ 条变更日志; 最新顶部 banner 是 Day 3 完结.
2. **P26 README 收尾开工重点** (P25_BLUEPRINT §3 Day 4+, 估 ~0.3 天):
   - **顶层 README**: 把 P25 交付 (后端外部事件 router + 前端 Chat 外部事件面板 + external_events_guide.md) 汇到 testbench 顶层用户门面; 链接 `docs/external_events_guide.md` 入口; 更新全量 smoke 数到 16/16.
   - **git push**: P25 从 Day 1 到 Day 3 若干 commit (本地 `3d6c525` / `25135d7` / `5c03446` / Day 2 polish r2-r6 / Day 3 交付) 尚未 push, 收尾时 `git push NEKO-dev main` 落盘.
   - **P26 立项门槛**: 用户手测 tester 手册 `external_events_guide.md` 走一遍确认内容准确; 如发现新 bug / 不清楚的字段, 补 r7 polish; 否则 P25 sign-off 进 P26.
3. **读经验层基线**: `AGENT_NOTES.md §3A` 57 条设计原则 → `LESSONS_LEARNED §7` 25 条主编号 + **`§7.A` 候选 L28-L43** (L43 最新: "LLM 调用点契约用 AST 静态扫 + NOSTAMP sentinel escape-hatch", P25 Day 3 派生); §7.25 "跨边界 shape / role / 字段名必须 rg 消费方" 一周内累积到**第 6 次同族实锤**, 五层防御齐整.
4. **Day 3 基线**: 全量 smoke baseline `tests\testbench\smoke\_run_all.cmd` 当前 **16/16 全绿 (33.85s)**, 含新 `p25_llm_call_site_stamp_coverage_smoke.py` + 扩展后的 `p25_external_events_smoke.py` (A-I 9 组). P26 新改动要保持这个绿线.
5. **Day 3 手测门槛**: Day 3 纯测试 + 文档 + backend stamp 无 UI 改动, 无重启需求 (smoke 已覆盖). tester 手册验证 = `open tests/testbench/docs/external_events_guide.md` 从头读一遍确认章节齐全 + `SimulationResult.reason` 快速查表 8 个值都能对上后端实际行为.
6. **git 状态**: P25 Day 1 主交付 commit `3d6c525`, Day 1 fixup (mirror_to_recent shape) commit `25135d7`, Day 2 前端面板 + 7 项 polish (含 docs 回写) commit `5c03446`, Day 2 polish r2-r6 多 commit, Day 3 交付 **working tree 有未 commit 的 16 个文件 + 3 个未跟踪的新文件** (见 `git status`). 下一个 agent 进来先帮用户做 Day 3 commit.
