...
* **纯静态推荐**: 推荐 op 是 stage → op 一对一常量映射, 不依赖 messages
  ...
  "永远只建议 + 预览 + 等确认, 绝不自动跑".
* **/stage/preview 在 P14 统一不支持**: P14 范围内没有 op 可以 dry-run —
  memory op 的 dry-run 已经由 P10 的 ``/api/memory/trigger/{op}`` + modal
  承担 (测试人员直接在 Setup → Memory 子页点 Trigger 即可), 我们不在
  stage 层重新包一层. evaluation 阶段的 dry-run 是 "跑一次评分看分数但
  不存", 要等 P15/P16 上线. 本期 ``preview_suggested_op`` 统一返回
  ``PreviewUnsupported`` + 指向性文案 (告诉测试人员去哪个子页预览).
...
_OP_EVALUATION_PENDING: Final = SuggestedOp(
    op_id="evaluation.pending",
    stage="evaluation",
    ui_action="evaluation_pending",
    label_i18n_key="stage.op.evaluation_pending.label",
    description_i18n_key="stage.op.evaluation_pending.description",
    when_to_run_i18n_key="stage.op.evaluation_pending.when_to_run",
    when_to_skip_i18n_key="stage.op.evaluation_pending.when_to_skip",
)
...
STAGE_SUGGESTIONS: Final[dict[Stage, SuggestedOp]] = {
    ...
    "evaluation": _OP_EVALUATION_PENDING,
}
...
def preview_suggested_op(
    ...
    """P14 范围内 preview 一律不提供 dry-run.

    理由 (详见模块 docstring):
      - memory op 的预览用 ``/api/memory/trigger/{op}`` (P10).
      - evaluation op 的预览要等 P15 ScoringSchema + P16 Judger.
      - persona / chat / prompt 这三类 op 本身没"不执行只预览"语义
        (编辑人设永远安全, chat 发送不该有假装版, prompt preview 已经
        是 Chat 右栏的常规视图).

    端点会 raise PreviewUnsupported(412) + ``reason_i18n_key`` + 指向性
    的 ``redirect_hint_i18n_key``, 让前端 toast 提示 "请去 X 子页预览".
    """
    ...
    raise StageError(
        "PreviewUnsupported",
        (
            f"P14 阶段 {stage!r} 的推荐 op {target_op!r} 不提供内置 dry-run;"
            " memory op 请去 Setup → Memory 子页的 Trigger 按钮预览,"
            " evaluation 需等 P15/P16 上线"
        ),
        status=412,
    )
