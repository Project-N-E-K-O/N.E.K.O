...
{{
  ...
  "analysis": "2到4句中文简要分析"
    ...
    def judge_single_response(
        ...
        Used by the prompt-test framework to judge one reply at a time
        against real conversation data.  Scoring dimensions and anchors
        are identical to ``judge_human_like_conversation``.

        Args:
            system_prompt: The assembled system prompt (session_init + character_prompt + ...).
            conversation_context: Prior turns as ``[{"role": "system"|"user"|"assistant", "content": ...}]``.
            ...
            master_name: User/master name for display.
        ...
        prompt = f"""你是一名专门评估角色扮演 AI 的中文测评员，重点评估其是否自然、有人味、贴近角色设定，并能让用户感到被理解与陪伴。

你的任务是：根据提供的角色设定和对话上下文，评价 AI 对用户最新一句话的**单条回复**。
...
- persona_warmth：角色一致性，是否符合角色设定的语气和性格
...
{{
  ...
  "analysis": "2到4句中文简要分析"
        ...
        try:
            ...
            logger.info(
                "Single-response judgement [%s]: %s (overall score: %.1f/100)",
                result_entry["test_name"],
                verdict_str,
                computed_overall,
            )
        except (json.JSONDecodeError, KeyError, TypeError, ValueError) as e:
            logger.warning("Failed to parse single-response judgement JSON: %s. Raw: %s", e, response_text[:300])
            result_entry["error"] = f"JSON parse failed: {e}"
            result_entry["analysis"] = response_text[:1000]
            result_entry["verdict"] = "NO"
            result_entry["passed"] = False
